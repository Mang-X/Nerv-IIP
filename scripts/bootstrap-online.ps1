...
function Ensure-Pnpm {
    ...
    if (Test-CommandAvailable -Name 'corepack') {
        ...
        Invoke-NativeCommandWithTimeout -Command $corepack -Arguments @('prepare', 'pnpm@11.22.0', '--activate') -WorkingDirectory $root -TimeoutSeconds 300 -Name 'bootstrap-corepack-pnpm' | Out-Null
        Update-ProcessPath
        return
    }
    ...
    if (Test-CommandAvailable -Name 'npm') {
        ...
        Invoke-NativeCommandWithTimeout -Command $npm -Arguments @('install', '-g', 'pnpm@11.22.0') -WorkingDirectory $root -TimeoutSeconds 300 -Name 'bootstrap-npm-pnpm' | Out-Null
        Update-ProcessPath
        return
    }
