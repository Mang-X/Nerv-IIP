...
#   Category: verify
#   SideEffects:
#     - Runs .NET restore and focused ERP Sales MVP test commands only
#   Writes:
...
if (-not $SkipRestore) {
    Invoke-DotNet -Name "business-erp-sales-restore" -WorkingDirectory $root -Arguments @(
        "restore",
        "backend/Nerv.IIP.sln"
    ) | Out-Null
}

Invoke-DotNet -Name "business-erp-sales-domain-tests" -WorkingDirectory $root -Arguments @(
    "test",
...
Invoke-DotNet -Name "business-erp-sales-web-tests" -WorkingDirectory $root -Arguments @(
    "test",
...
Write-Host "Business ERP Sales MVP verified."
