...
if ($PSCmdlet.ParameterSetName -eq 'GitHubConsole') {
    ...
    $metadata = @{
        ...
        runnerOs = 'Linux'; runnerImage = 'ubuntu-latest'; dotnetSdk = '10.0.x'; selectedLanes = @('backend')
    ...
    $summaries = @($summaryPaths | ForEach-Object {
        $summary = Get-Content -LiteralPath $_.FullName -Raw | ConvertFrom-Json
        [pscustomobject]@{ schemaVersion = 1; granularity = 'test'; lane = $summary.lane; assemblies = @($summary.assemblies) }
    })
    $metadata = @{
        sourceKind = 'trx-evidence'; repository = ''; workflowRunId = ''; runAttempt = 1; jobId = ''
        commitSha = ''; sourceUrl = ''; event = ''; headBranch = ''; conclusion = 'success'; jobConclusion = 'success'
        runnerOs = ''; runnerImage = ''; dotnetSdk = ''; selectedLanes = @($summaries.lane | Sort-Object -Unique)
