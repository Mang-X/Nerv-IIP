...
## 矩阵
  ...
  * 枚举面下界 —— 需求数不得低于实测基线。它守的是本票那条承重主张（需求集从消费方代码枚举、
    是个闭集）**没有静默塌缩**；没有它时，扫描面塌到 1/10 仍然全绿。
  * 扫描面失明 —— 注释里、字符串里的调用不算数；插值洞里的引号、字符字面量 `'"'`、
    放行名单内的预处理指令之后的**真**调用仍然算数。一个不处理这些形态的扫描器会在这里整段
    丢失需求，而丢失的方向是静默变绿。
  * 拒绝面 —— 逐字字符串、原始字符串、放行名单外的预处理指令（含条件编译）必须 **throw**。
    它们**不在**实现面内；这一段断言的是「扫描器遇到没实现的形态会喊出来」，不是「它认得它们」。
  * fail-closed —— 读不懂的写法（非字面量键、`using static`、归属不到资源的注入、重复注入、
    AddProject 没绑到 var、赋值左值与链接收者指向不同资源、提到 `__BaseUrl` 却读不出字面量键）
    必须 throw，不是跳过。
#>
...
try {
    ...
    $prefixCases = @(
        ...
        # `'"'` 独占判别力，**实测**：把整个字符字面量分支缴械（`if ($false -and …)`）后，
        # 318 条断言里**恰好这一条红**，`','` 那格绿。原因是内含的 `"` 会被当成字符串起点、
        # 把其后的扫描相位带偏，而 `','` 的内含字符不改变相位。两格并存，不是二选一。
        #
        # ⚠️ 这里原本写的理由（「能杀掉『把分支简化成跳 3 个字符』的变异」）是**错的**，已实测证伪：
        # skip-3 与 skip-2 变异都**全绿 318**——`'X'` 对任意单字符都是 3 个字符，skip-3 在构造上
        # 就不可能区分这两格；throw 哨兵另证该分支确实被执行，所以那个绿是真存活、不是变异没生效。
        # 结论对、理由错，是本仓记过的形态；留这段是为了让下一个人不必重走一遍。
