...
try {
    ...
    # MES：替代料快照 provider 证明登记后，九个类中有 22 条受治理的真实 PostgreSQL 证明；CAP 的原生存储表落在独立 cap schema，
    ...
    Assert-Contract (@($mesMember.expectedTestIdentities).Count -eq 22) 'The MES member must freeze exactly its twenty-two governed PostgreSQL identities.'
