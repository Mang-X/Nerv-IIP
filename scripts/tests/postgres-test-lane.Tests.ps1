...
try {
    ...
    # MES：既有受治理证明加上 OperationActualTimeSettlement 的四条原子性/并发证明，共有 32 条真实 PostgreSQL 证明；CAP 的原生存储表落在独立 cap schema，
    ...
    Assert-Contract (@($mesMember.expectedTestIdentities).Count -eq 32) 'The MES member must freeze exactly its thirty-two governed PostgreSQL identities.'
