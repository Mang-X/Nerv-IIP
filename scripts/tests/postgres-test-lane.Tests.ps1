...
try {
    ...
    # MES：既有类、WorkOrderTransformation 与替代料快照类共有 27 条受治理的真实 PostgreSQL 证明；CAP 的原生存储表落在独立 cap schema，
    ...
    Assert-Contract (@($mesMember.expectedTestIdentities).Count -eq 27) 'The MES member must freeze exactly its twenty-seven governed PostgreSQL identities.'
