...
try {
    ...
    # MES：八个既有类与 WorkOrderTransformation 类共有 28 条受治理的真实 PostgreSQL 证明；CAP 的原生存储表落在独立 cap schema，
    ...
    Assert-Contract (@($mesMember.expectedTestIdentities).Count -eq 28) 'The MES member must freeze exactly its twenty-eight governed PostgreSQL identities.'
