...
try {
    ...
    # 以及停机读面 3 条（列表行投影、按原因聚合的时长结算与名次、按原因过滤与汇总面），共有 39 条真实 PostgreSQL 证明；
    ...
    Assert-Contract (@($mesMember.expectedTestIdentities).Count -eq 39) 'The MES member must freeze exactly its thirty-nine governed PostgreSQL identities.'
