...
try {
    ...
    $masterDataIdentities = @(
        ...
        'Nerv.IIP.Business.MasterData.Web.Tests.MasterDataPostgresProfileTests.Postgres_migration_backfills_stations_from_legacy_device_station_codes',
        'Nerv.IIP.Business.MasterData.Web.Tests.MasterDataPostgresProfileTests.Postgres_migration_normalizes_legacy_sku_tracking_policy_synonyms_and_leaves_valid_rows',
        'Nerv.IIP.Business.MasterData.Web.Tests.MasterDataPostgresProfileTests.Postgres_station_backfill_aborts_and_lists_codes_attached_to_different_lines',
    ...
    Assert-Contract ([string]::Equals((@($masterDataMember.expectedTestIdentities) -join "`n"), ($masterDataIdentities -join "`n"), [StringComparison]::Ordinal)) 'The MasterData member must freeze exactly the fourteen governed profile identities and exclude the world-bible seed test.'
