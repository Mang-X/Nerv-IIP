...
'@
    $workflowWithoutAcceptanceRuntimeContract = $workflow.Replace($acceptanceRuntimeContractStep, '').Replace(
        'step 预算合计 148m（30 个 step：3m checkout',
        'step 预算合计 143m（29 个 step：3m checkout').Replace(
        '+ 29 × 5m；',
        '+ 28 × 5m；')
    ...
    Assert-Contract ($workflowWithoutAcceptanceRuntimeContract.Contains('step 预算合计 143m（29 个 step：3m checkout', [StringComparison]::Ordinal) -and $workflowWithoutAcceptanceRuntimeContract.Contains('+ 28 × 5m；', [StringComparison]::Ordinal)) 'Acceptance runtime workflow mutation must keep its budget comment truthful at 29 steps and 143m.'
...
'@
    ...
    $workflowWithIncorrectBudgetComment = $workflow.Replace(
        ...
        '+ 29 × 5m；',
        '+ 28 × 5m；')
    Assert-Contract (-not [string]::Equals($workflowWithIncorrectBudgetComment, $workflow, [StringComparison]::Ordinal)) 'Script Governance budget-comment mutation must alter the canonical 30-step/148m comment.'
    ...
    $expectedBudgetCommentDiagnostic = 'Script Governance budget comment must match its actual 30-step/148m structure.'
