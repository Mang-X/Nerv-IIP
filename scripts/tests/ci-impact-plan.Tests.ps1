...
Assert-Contract ($dotNetSdkFindings.Count -eq 0) "Every managed CI job must contain exactly one isolated setup-dotnet step for SDK $expectedDotNetSdkVersion. Findings=[$($dotNetSdkFindings -join ', ')]"
