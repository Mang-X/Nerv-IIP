...
if (-not $content.Contains('business-scheduling-gateway-openapi-tests', [StringComparison]::Ordinal)) {
    throw 'BusinessScheduling APS lite verify script must run the BusinessGateway OpenAPI enum regression tests.'
}

if (-not $content.Contains('FullyQualifiedName~BusinessGatewayOpenApiTests', [StringComparison]::Ordinal)) {
