...
- `backend/test-determinism-baseline.json`（schema 2）每一行必须声明 `classification`：`expiring-debt` 是已承认的债务而不是豁免——owner 必须是一个**在登记它的变更之外仍然存在**的 issue（`MAN-\d+` 或 `#\d+`），到期日按类别错开，`reason` 按行而非按文件书写；用当前 PR 自己的票做 owner 属于门禁失效。`permanent` 只给「被扫到的构造就是受审计原语本身」的位点（原语实现与它自己的自测），必填 `rationale`、禁带 `ownerIssue`/`exitCondition`/`expiresOn`，且只在 checker 自己持有的 `路径=pattern` 白名单内生效——白名单锁到 pattern 一级，同一文件里后来新增的别的 pattern 不会被旧理由顺带放行；baseline 不能把自己写进白名单，新增常设例外必须改 `scripts/check-backend-test-determinism.ps1` 并走脚本治理。`reason` 按行书写，`rationale` 按「同一条常设理由」书写（scope 外前置/teardown、scope 内被测变异、原语实现、轮询原语各一条）。到期债务当前为 0 行。
