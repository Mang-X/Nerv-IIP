...
3. 从 `docs/README.md` 按任务类型选择必要文档；不要默认读取历史状态快照或兼容入口。若旧 `docs/architecture/*.md` 路径只剩兼容 shim，必须跟随其 canonical 指针后再引用或修改，不把 shim 当当前事实源。
...
- 人工文档或协作文本：读取 `docs/AGENTS.md` 与 `docs/governance/docs/language.md`。新增文档或链接时使用 canonical owner，不新增指向 Architecture 根目录兼容 shim 的权威入链。
