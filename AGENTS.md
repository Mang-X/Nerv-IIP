...
- 推进假时钟前必须先等到被测计时器**已注册**的显式边沿信号；`await Task.Yield()` 与「`BackgroundService.StartAsync` 已返回」都不是屏障。晚注册的计时器会以推进后的 now 重新定期，tick 永久丢失、等待方永不返回（MAN-799 与 MAN-663 各踩一次）。
- 网络测试显式区分 connection/request budget、caller cancellation 与业务 HTTP response；不得输出 headers/body/凭据。
- FluentValidation、culture、`TZ`/env 等可变全局值使用 scoped capture/restore 并序列化 mutator；FastEndpoints 静态变异只允许三种手段：collection serialization、sacrificial process isolation、或对「写该静态状态的动作」与「读它的动作」做显式互斥门（MAN-663 的 `BusinessGatewayTestHostGate`：宿主构建独占全部 permit，请求各持一个 permit 且必须**服务端**持有，覆盖整条服务端管线）。三者都绝不声称 restore；互斥门必须有一个在门被削弱时会失败的回归测试，否则退回前两种。
...
- 测试体里不要手写 `Environment.SetEnvironmentVariable` / `CultureInfo.Current*` / `ValidatorOptions.Global.*` 赋值；用 `GlobalTestStateScope` 的 mutator（`UseCulture`、`SetEnvironmentVariable`…）。它串行化全部 mutator、按行捕获旧值、精确恢复「不存在/空串/有值」三态，并在 dispose 后拒绝再写。自己写的 `PreserveEnvironment`/`RestoreEnvironment` 只恢复不串行化，是弱化复制品。
- `backend/test-determinism-baseline.json`（schema 3）每一行必须声明 `classification`：`expiring-debt` 是已承认的债务而不是豁免——除 `ownerIssue`/`exitCondition`/`expiresOn` 外，还必须用 `registeredByIssue` 记录登记变更、用 `registeredOn` 记录登记日期；owner 必须是一个**在登记它的变更之外仍然存在**的 issue。`ownerIssue` 与 `registeredByIssue` 均只接受 `MAN-\d+` 或 `#\d+`，按命名空间及去前导零后的数字比较 canonical identity，两者相同即属自担保并失败。`registeredOn`/`expiresOn` 都按 UTC、invariant `yyyy-MM-dd` 解析：登记日不得晚于 UTC 今日，expiry 必须落在登记日到其后 45 天的**含边界**区间，且早于 UTC 今日仍会失败；到期日按类别错开，`reason` 按行而非按文件书写。`permanent` 只给「被扫到的构造就是受审计原语本身」的位点（原语实现与它自己的自测），必填 `rationale`、禁带 `ownerIssue`/`registeredByIssue`/`exitCondition`/`registeredOn`/`expiresOn`，且只在 checker 自己持有的 `路径=pattern=maxRows` 白名单容量内生效；当前三条容量为 `GlobalTestStateScopeTests.cs=StaticSetter=12`、`GlobalTestStateScope.cs=StaticSetter=9`、`BoundedObservationWindow.cs=Task.Delay=1`。白名单按 path/pattern ordinal 精确匹配，容量统计通过行级校验的 permanent baseline 行数而非 source occurrence 或 `occurrenceCount`；同一文件后来新增的别的 pattern 不会被旧理由顺带放行，baseline 也不能自我扩容。新增常设例外或提高容量必须改 `scripts/check-backend-test-determinism.ps1` 并走脚本治理。`rationale` 按「同一条常设理由」书写（scope 外前置/teardown、scope 内被测变异、原语实现、轮询原语各一条）。到期债务当前为 0 行。
- 扫描范围含 `backend/common/Testing/**`：静态写入被集中到 `GlobalTestStateScope` 之后，这条边界是被依赖的设计前提而不是顺带事实，因此共享测试基建目录必须在门禁里（checker 找不到该目录下的项目会直接失败）。
