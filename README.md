...
7. 将现有 `verify`/`generate` 脚本迁移到脚本治理 helper 与 fast gate，并保持 `scripts/verify-business-wave1-foundation.ps1` 覆盖 BusinessMasterData、BusinessProductEngineering、BusinessInventory、BusinessQuality 和 BusinessMES。
8. Business Wave 1 后续进入 DemandPlanning、WMS、ERP、IndustrialTelemetry、Maintenance 和业务前端入口前，应先通过 `scripts/verify-business-wave1-foundation.ps1` 保持五个底座业务服务稳定。
