...
Nerv-IIP 是一个从 0 到 1 规划的原生 AI 工业互联网平台，核心目标不是先做复杂业务系统，而是先建立一个稳定的控制面与应用管理底座，使平台能够接入、发现、观测、控制并治理真实运行中的应用实例。

当前仓库以文档优先方式启动。第一阶段先冻结不可轻易反转的架构决策、服务边界、前端范式、AI 边界与基础设施基线，再进入工程骨架和最短纵切链路实现。

## 项目目标

1. 建立统一的平台控制面，覆盖身份、组织、环境、应用目录、实例状态、运维动作与审计闭环。
...
5. 通过一个可被平台完整管理的示例应用验证平台底座，而不是在首期堆叠复杂业务功能。
...
4. docs/architecture/api-contract-and-codegen.md
5. docs/architecture/ai-boundaries.md
6. docs/architecture/backend-bootstrap-plan.md
7. docs/architecture/core-domain-model-v1.md
8. docs/architecture/agent-platform-protocol-v1.md
9. docs/architecture/implementation-readiness.md

## 里程碑

1. M0：完成 ADR、单仓结构、前后端基础工程与文档冻结。
2. M1：完成 IAM、组织环境上下文和控制台基础登录能力。
...
当前仓库尚未创建业务代码，已完成第一轮架构决策冻结，并将关键设计沉淀为 ADR 与架构文档。当前已经补齐首批实现所需的环境前置、平台与 Agent 的 v1 协议边界，以及共享契约放置规则。下一阶段可以按以下顺序直接进入工程实现：
