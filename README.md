...
- shadcn-vue 2.7.3，使用 `reka-nova` style、Tailwind CSS v4 和 `@nerv-iip/ui` 稳定导出边界。
...
1. 在 IAM 持久化登录、Gateway 权限门禁和 Console Auth 基线之上补齐完整用户/角色/会话管理、OAuth/OIDC、SSO、MFA、ABAC 和外部应用授权演进。
...
5. 基于 Console Auth + shadcn-vue Baseline 继续收敛前端 Design System，后续控制台页面、视觉组件和组件库迁移必须沿用已选 registry、preset、token 和导出边界。
