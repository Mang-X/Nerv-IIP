...
当前仓库以文档优先方式启动，并已经从首批架构冻结推进到第一、第二、第三阶段纵切实现、第四阶段真实基础设施门禁和第五阶段迁移发布底座：第一阶段验证 Connector Host 接入、AppHub 状态沉淀与 Gateway 查询，第二阶段验证低风险运维动作创建、派发、执行、结果回传与审计记录，第三阶段验证 Gateway OpenAPI、类型安全前端 API client 与 Vue 控制台工作区，第四阶段把 AppHub/Ops 迁移到 netcorepal/CleanDDD、PostgreSQL profile、结构化日志和平台级 Aspire AppHost，第五阶段把 AppHub/Ops 的 PostgreSQL 路径从 `EnsureCreated()` 推进到 migration-based 验证。
...
- Node.js >= 22.18.0；仓库根 `.node-version` 固定为 22.22.3，本机第三阶段验证使用 OpenJS.NodeJS.22 v22.22.3。
...
28. docs/architecture/observability-baseline.md
...
1. 在迁移基线稳定后补齐 IAM 完整授权链路、FileStorage 真实上传下载闭环和控制台登录能力。
2. 补齐 PostgreSQL profile 的备份、恢复、seed、初始化脚本和后续安装包迁移入口。
3. 在 IAM、FileStorage 等新增持久化服务首个 migration 前复用 AppHub/Ops 的 schema convention tests、catalog 草案和 service-schema migrations history 配置。
4. 扩展 Ops 的审批、权限、通知和持久化 outbox，逐步覆盖更高风险动作。
5. 前端 Design System 单独规划，未冻结前不推进新的控制台页面、视觉组件或组件库迁移。
