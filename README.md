...
当前仓库以文档优先方式启动，并已经从首批架构冻结推进到第一、第二、第三阶段纵切实现：第一阶段验证 Connector Host 接入、AppHub 状态沉淀与 Gateway 查询，第二阶段验证低风险运维动作创建、派发、执行、结果回传与审计记录，第三阶段验证 Gateway OpenAPI、类型安全前端 API client 与 Vue 控制台工作区。
...
- 当前第三阶段分支：`codex/third-vertical-slice-console`
...
20. docs/architecture/deployment-baseline.md
21. docs/architecture/technology-stack-references.md
...
当前仓库已经在首批架构文档基础上落地第一、第二、第三阶段纵切，并将关键设计沉淀为 ADR 与架构文档。平台 HTTP 接口统一采用 FastEndpoints；Program.cs 只负责服务注册、中间件和 `UseFastEndpoints()` 接线，具体接口放在各 Web 项目的 `Endpoints/` 目录。
