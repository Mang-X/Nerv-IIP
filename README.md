...
平台 HTTP 服务固定为 `5100-5111`：Gateway `5100`、AppHub `5101`、IAM `5102`、Ops `5103`、FileStorage `5104`、Console `5105`、Notification `5106`、BusinessMasterData `5107`、BusinessProductEngineering `5108`、BusinessInventory `5109`、BusinessQuality `5110`、BusinessMES `5111`。Console 避开 Vite 默认 `5173`，降低与其他前端项目冲突的概率。
...
业务平台当前已完成 Wave 1、Wave 2、Equipment Reliability 和 Wave 3 ERP 接入：BusinessMasterData、ProductEngineering、Inventory、Quality、MES、DemandPlanning、BarcodeLabel、BusinessApproval、WMS、IndustrialTelemetry、Maintenance 和 ERP 均已有服务代码、AppHost 注册和对应 verify 脚本。#77 Full-chain acceptance 已从 harness baseline 升级到 live public-surface 验收：WMS 通过 HTTP Inventory posting 与 WCS 诊断/完成查询提供可观测事实，MES 暴露生产报工、完工入库请求和产能影响查询，ERP Finance 支持 AP/AR/Cost 按 source document drill-down；验收脚本会覆盖这些支撑 surface 与七条链路的公开 API / IntegrationEvent 可见事实。当前不启动 Docker/PostgreSQL 或外部设备。
...
7. 将现有 `verify`/`generate` 脚本迁移到脚本治理 helper 与 fast gate，并保持 `scripts/verify-business-wave1-foundation.ps1`、`scripts/verify-business-wave2-execution.ps1` 和 `scripts/verify-business-equipment-reliability.ps1` 作为业务平台回归入口。
8. 业务平台 ERP Wave 3 已完成 Procurement、Sales、Finance 和注册/验证收口；#77 已具备 live public-surface full-chain acceptance，本地验证入口为 `pwsh scripts/verify-business-full-chain-acceptance.ps1`。后续再接入 Docker/PostgreSQL 或 RabbitMQ profile 时，链路断言仍以公开 API 和 IntegrationEvent 可见事实为准。
