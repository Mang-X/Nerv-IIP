...
业务平台当前已完成 Wave 1、Wave 2、Equipment Reliability 和 Wave 3 ERP 接入：BusinessMasterData、ProductEngineering、Inventory、Quality、MES、DemandPlanning、BarcodeLabel、BusinessApproval、WMS、IndustrialTelemetry、Maintenance 和 ERP 均已有服务代码、AppHost 注册和对应 verify 脚本。#77 Full-chain acceptance 已具备验收 harness 基础：公开业务 API contract surface、统一 org/env 与 internal bearer token fixture、correlation/event recorder 和 response envelope HTTP helper；当前不启动 Docker/PostgreSQL 或真实服务 host。
...
8. 业务平台 ERP Wave 3 已完成 Procurement、Sales、Finance 和注册/验证收口；下一主线是基于 #77 harness 编写真正的公开 API + IntegrationEvent full-chain 测试。规划入口见 `docs/superpowers/specs/2026-05-23-business-full-chain-acceptance-design.md`，本地 harness 验证入口为 `pwsh scripts/verify-business-full-chain-acceptance.ps1`。
