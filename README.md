...
user-secrets，执行 `dotnet restore`、`pnpm install` 和 AppHost build。Docker
...
5. 继续推进完整 OAuth/OIDC 授权码服务器、WebAuthn/企业 IdP 深化、复杂 ABAC 策略语言、Ops 审批 Console 管理入口、更细权限 scope 和通知外部通道；#77 的 P0 验收脚本继续作为业务链路回归入口。
