    ...
    >
      ...
      >
        <ChevronRightIcon
          ...
          :class="{ 'nerv-leg-chevron-open': !collapsed.category }"
          aria-hidden="true"
        />
      ...
      >
        <component
          :is="hidden.category ? EyeOffIcon : EyeIcon"
          class="size-3.5"
          aria-hidden="true"
        />
      </button>
      <div v-show="!collapsed.category" class="nerv-leg-items">
        <span v-for="c in categories" :key="c.key" class="inline-flex items-center gap-1.5">
          <span
            ...
            :style="{ background: `var(--nv-scheduling-category-${c.key})` }"
          ></span>
          {{ c.label }}
        </span>
      </div>
    ...
    <template v-if="view === 'order'">
      ...
      <div class="nerv-leg-row" :class="{ 'nerv-leg-row-hidden': hidden.gantt }">
        <button
          type="button"
          class="nerv-leg-head"
          :aria-expanded="!collapsed.gantt"
          @click="toggleCollapse('gantt')"
        >
          <ChevronRightIcon
            class="nerv-leg-chevron"
            :class="{ 'nerv-leg-chevron-open': !collapsed.gantt }"
            aria-hidden="true"
          />
        ...
        >
          <component
            :is="hidden.gantt ? EyeOffIcon : EyeIcon"
            class="size-3.5"
            aria-hidden="true"
          />
        </button>
        <div v-show="!collapsed.gantt" class="nerv-leg-items">
          ...
          <!-- TODO(MAN-675 / #1242,关键路径):模型无 critical-path 字段、引擎不渲染,故图例暂不列该项;
      ...
      <div class="nerv-leg-row" :class="{ 'nerv-leg-row-hidden': hidden.status }">
        ...
        >
          <ChevronRightIcon
            class="nerv-leg-chevron"
            :class="{ 'nerv-leg-chevron-open': !collapsed.status }"
            aria-hidden="true"
          />
        ...
        >
          <component
            :is="hidden.status ? EyeOffIcon : EyeIcon"
            class="size-3.5"
            aria-hidden="true"
          />
        </button>
        <div v-show="!collapsed.status" class="nerv-leg-items">
          <span class="inline-flex items-center gap-1.5">
            <span
              class="h-2.5 w-6 rounded-[3px] border-2 border-destructive bg-destructive/25"
            ></span
            >冲突
          </span>
          <span class="inline-flex items-center gap-1.5">
      ...
      <div class="nerv-leg-row" :class="{ 'nerv-leg-row-hidden': hidden.calendar }">
        <button
          type="button"
          class="nerv-leg-head"
          :aria-expanded="!collapsed.calendar"
          @click="toggleCollapse('calendar')"
        >
          <ChevronRightIcon
            class="nerv-leg-chevron"
            :class="{ 'nerv-leg-chevron-open': !collapsed.calendar }"
            aria-hidden="true"
          />
        ...
        >
          <component
            :is="hidden.calendar ? EyeOffIcon : EyeIcon"
            class="size-3.5"
            aria-hidden="true"
          />
        </button>
    ...
    <template v-else>
      ...
      <div class="nerv-leg-row" :class="{ 'nerv-leg-row-hidden': hidden.card }">
        <button
          type="button"
          class="nerv-leg-head"
          :aria-expanded="!collapsed.card"
          @click="toggleCollapse('card')"
        >
          <ChevronRightIcon
            class="nerv-leg-chevron"
            :class="{ 'nerv-leg-chevron-open': !collapsed.card }"
            aria-hidden="true"
          />
        ...
        <div v-show="!collapsed.card" class="nerv-leg-items">
          ...
          <span class="inline-flex items-center gap-1.5">
            <span class="rounded bg-foreground/10 px-1.5 py-px text-[0.58rem] font-semibold"
              >换型</span
            >换型耗时
          </span>
          <span class="inline-flex items-center gap-1.5">
      ...
      <div class="nerv-leg-row" :class="{ 'nerv-leg-row-hidden': hidden.status }">
        ...
        <div v-show="!collapsed.status" class="nerv-leg-items">
          <span class="inline-flex items-center gap-1.5">
            <span
              class="h-2.5 w-6 rounded-[3px] border-2 border-destructive bg-destructive/15"
            ></span
            >冲突
          </span>
          <span class="inline-flex items-center gap-1.5">
            <span
              class="h-2.5 w-6 rounded-[3px] border border-dashed border-muted-foreground/70"
            ></span
            >锁定(虚线框)
          </span>
        </div>
      ...
      <div class="nerv-leg-row" :class="{ 'nerv-leg-row-hidden': hidden.block }">
        <button
          type="button"
          class="nerv-leg-head"
          :aria-expanded="!collapsed.block"
          @click="toggleCollapse('block')"
        >
          <ChevronRightIcon
            class="nerv-leg-chevron"
            :class="{ 'nerv-leg-chevron-open': !collapsed.block }"
            aria-hidden="true"
          />
        ...
        >
          <component
            :is="hidden.block ? EyeOffIcon : EyeIcon"
            class="size-3.5"
            aria-hidden="true"
          />
        </button>
        <div v-show="!collapsed.block" class="nerv-leg-items">
          <span class="inline-flex items-center gap-1.5">
            <span
              ...
              style="--h: var(--nv-scheduling-block-maintenance)"
            ></span
            >设备维护
          </span>
          <span class="inline-flex items-center gap-1.5">
            <span
              class="nerv-leg-hatch h-2.5 w-6 rounded-[3px]"
              style="--h: var(--nv-scheduling-block-downtime)"
            ></span
            >计划停机
          </span>
          <span class="inline-flex items-center gap-1.5">
            <span
              class="nerv-leg-hatch h-2.5 w-6 rounded-[3px]"
              style="--h: var(--nv-scheduling-block-linechange)"
            ></span
            >换线窗口
          </span>
          <span class="inline-flex items-center gap-1.5">
            <span
              class="nerv-leg-hatch h-2.5 w-6 rounded-[3px]"
              style="--h: var(--nv-scheduling-block-changeover)"
            ></span
            >换型窗口
          </span>
        </div>
