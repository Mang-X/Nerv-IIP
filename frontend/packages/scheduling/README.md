...
> **许可与 vendor 移出计划(TODO #1270)**:owner 已确认持有 DHTMLX **商业许可**。下面的试用包流程
> 是过渡态——正式授权包接入后,`vendor/dhtmlx/` 目录与本节安装步骤一并移除,适配层接口不变
> (可替换性由 `engine/conformance.ts` 背书)。进度跟踪见 issue #1270。
