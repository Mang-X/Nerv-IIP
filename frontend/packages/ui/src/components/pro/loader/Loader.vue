...
.ds-loader-ring {
  ...
  background: conic-gradient(
    from 0deg,
    transparent 0deg,
    color-mix(in oklch, var(--brand) 90%, transparent) 300deg,
    var(--brand) 360deg
  );
...
.ds-loader-dots i {
  ...
  animation: ds-dot 1s var(--ease-in-out-quart, ease-in-out) infinite;
}
...
.ds-loader-bars i {
  ...
  animation: ds-bar 0.9s var(--ease-in-out-quart, ease-in-out) infinite;
}
...
.ds-loader-pulse i {
  ...
  background: currentColor;
  opacity: 0.5;
  animation: ds-pulse 1.4s var(--ease-out-expo, ease-out) infinite;
}
