...
import CarouselPro from './CarouselPro.vue'
...
describe('CarouselPro', () => {
  it('moves the track when index is driven externally (v-model:index)', async () => {
    const wrapper = mount(CarouselPro, { props: { index: 0 }, slots: { default: THREE } })
    await nextTick()
    await wrapper.setProps({ index: 2 })
    await nextTick()
    // offset = -index * width = -2 * 300
    expect(wrapper.find('.ds-carousel-track').attributes('style')).toContain('-600px')
  })

  it('clamps a mount-time out-of-range index to the last slide', async () => {
    const wrapper = mount(CarouselPro, { props: { index: 9 }, slots: { default: THREE } })
    await nextTick()
  ...
  it('non-loop autoplay stops at the last slide (does not wrap)', async () => {
    fakeIntervals()
    const wrapper = mount(CarouselPro, {
  ...
  it('starts autoplay after items load asynchronously ([] → many)', async () => {
    fakeIntervals()
    const wrapper = mount(CarouselPro, { props: { items: [], autoplay: 1000 } })
    await nextTick()
