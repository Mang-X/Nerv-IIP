...
/**
 * Pro — time picker (HH:mm). A NvButton trigger opens a popover with scrollable
 * hour / minute columns; the current value auto-scrolls into view. Brand-tinted
 * selection. Composes Popover + NvButton; never edits原版 primitives.
 */
  ...
  <Popover v-model:open="open">
    ...
    <PopoverContent class="w-auto p-0" align="start">
      <div class="flex divide-x divide-border">
        <div ref="hourCol" class="nv-tp-col">
          <button
            v-for="h in hours"
            :key="h"
            type="button"
            :data-active="parts.h === h"
            :class="cn('nv-tp-cell', parts.h === h && 'nv-tp-cell-active')"
            @click="selectHour(h)"
          >
            {{ pad(h) }}
          </button>
        </div>
        <div ref="minuteCol" class="nv-tp-col">
          <button
            v-for="m in minutes"
            :key="m"
            type="button"
            :data-active="parts.m === m"
            :class="cn('nv-tp-cell', parts.m === m && 'nv-tp-cell-active')"
      ...
      <div class="border-t border-border p-1.5">
        <NvButton variant="ghost" size="sm" class="w-full" @click="now">此刻</NvButton>
      </div>
    </PopoverContent>
  </Popover>
</template>
