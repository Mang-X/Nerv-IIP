...
把 PC 业务页从 base `ui/` 迁到 `pc/` 时的接口差异、变体映射、缺口清单。
...
下列 NvUI PC 名称是 reka 原语的再导出（已在各 `index.ts` 注释标明）。base 对应物也只是纯转发包装（仅多一个 `data-slot` 属性，无样式/行为），故功能等价，可放心使用：
