...
<template>
  <ol :class="cn('nv-tl', props.class)">
    <li v-for="(item, i) in orderedItems" :key="item.key ?? i" class="nv-tl-item">
      <div class="nv-tl-rail">
        <span
          class="nv-tl-dot"
          :data-hollow="item.dotType === 'hollow' || undefined"
          :data-icon="item.icon ? '' : undefined"
          :style="{ '--nv-tl-dot': dotColor(item) }"
        >
          <component :is="item.icon" v-if="item.icon" class="size-3" aria-hidden="true" />
        </span>
        <span v-if="i < orderedItems.length - 1 || pending" class="nv-tl-line" />
      </div>
      <div class="nv-tl-body">
        <div class="nv-tl-head">
          <span v-if="item.title" class="nv-tl-title">{{ item.title }}</span>
          <span v-if="item.label" class="nv-tl-label">{{ item.label }}</span>
        </div>
        <p v-if="item.description" class="nv-tl-desc">{{ item.description }}</p>
        <div v-if="item.key && $slots[item.key]" class="nv-tl-slot">
          <slot :name="item.key" :item="item" />
        </div>
      </div>
    </li>

    <li v-if="pending" class="nv-tl-item">
      <div class="nv-tl-rail">
        <span class="nv-tl-dot nv-tl-dot-pending" style="--nv-tl-dot: var(--nv-brand)">
          <span class="nv-tl-pulse" aria-hidden="true" />
        </span>
      </div>
      <div class="nv-tl-body">
        <span class="nv-tl-title text-muted-foreground">{{ pendingText }}</span>
      </div>
    </li>
  </ol>
</template>
...
@layer nv-components {
  ...
  .nv-tl-dot {
    ...
    background-color: var(--nv-tl-dot);
  ...
  .nv-tl-dot[data-hollow] {
    background-color: var(--card);
    box-shadow:
      0 0 0 3px var(--card),
      inset 0 0 0 2px var(--nv-tl-dot);
  }
  ...
  .nv-tl-item:last-child .nv-tl-body {
    padding-bottom: 0;
  }
  .nv-tl-head {
    display: flex;
  ...
  .nv-tl-pulse {
    ...
    border-radius: 9999px;
    background-color: var(--nv-brand);
    animation: nv-tl-ping 1.6s var(--nv-ease-out-quart, ease-out) infinite;
  }
  @keyframes nv-tl-ping {
    0% {
      transform: scale(1);
      opacity: 0.55;
    }
