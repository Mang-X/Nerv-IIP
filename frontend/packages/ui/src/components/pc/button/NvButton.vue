...
import NvLoader from '../loader/NvLoader.vue'
...
type NvButtonVariants = VariantProps<typeof nvButtonVariants>

const props = withDefaults(
  defineProps<
    PrimitiveProps & {
      variant?: NvButtonVariants['variant']
      size?: NvButtonVariants['size']
      loading?: boolean
...
<template>
  <Primitive
    ...
    :class="cn(nvButtonVariants({ variant, size }), props.class)"
  >
    <NvLoader v-if="loading" variant="ring" size="sm" class="text-current" />
    <slot v-else name="leading" />
    <slot />
    <slot v-if="!loading" name="trailing" />
    <span v-if="isSolid" class="nv-btn-sheen" aria-hidden="true" />
  </Primitive>
</template>
...
@layer nv-components {
  ...
  .nv-btn-solid:hover:not(:disabled) {
    box-shadow:
  ...
  .nv-btn-solid:hover .nv-btn-sheen {
    opacity: 1;
