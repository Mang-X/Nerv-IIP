...
import type { NvDataTableDensity } from './types'
 ...
 * NvDataTable. `surface="plain"` drops the card chrome for embedding.
 */
...
const density = defineModel<NvDataTableDensity>('density', { default: 'comfortable' })
...
<template>
  <div
    :class="
      cn(
        'nv-tb flex flex-col gap-3 px-3 py-3 sm:px-4',
  ...
  >
    ...
    <div class="flex flex-wrap items-center gap-2">
      <div v-if="title || description || count != null" class="mr-auto flex min-w-0 flex-col">
        <div v-if="title || count != null" class="flex items-center gap-2">
          ...
          <span v-if="count != null" class="nv-tb-count">{{ count }}</span>
        </div>
      ...
      >
        ...
        <template v-if="search" #trailing>
          <button type="button" class="nv-tb-clear" aria-label="清除搜索" @click="search = ''">
      ...
      <DropdownMenu v-if="showMore || slots.menu">
        <DropdownMenuTrigger as-child>
          <NvButton variant="outline" size="icon-sm" aria-label="更多操作">
    ...
    <div v-if="tabs && tabs.length" class="flex flex-wrap items-center gap-2">
      <div ref="segEl" class="nv-tb-seg" role="tablist">
        <span
          v-show="segInd.ready"
          class="nv-tb-seg-ind"
        ...
        >
          {{ t.label }}
          <span v-if="t.count != null" class="nv-tb-seg-count">{{ t.count }}</span>
        </button>
      </div>
      <slot name="tabs-end" />
    </div>
...
@layer nv-components {
  ...
  .nv-tb-seg-btn:hover:not([data-active]) {
    color: var(--foreground);
  }
  .nv-tb-seg-btn:focus-visible {
    box-shadow: 0 0 0 3px color-mix(in oklch, var(--ring) 45%, transparent);
  }
  .nv-tb-seg-btn[data-active] {
    color: var(--foreground);
  }
  ...
  .nv-tb-seg-btn[data-active] .nv-tb-seg-count {
