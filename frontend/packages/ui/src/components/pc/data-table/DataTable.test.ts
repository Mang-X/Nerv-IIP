...
describe('NvDataTable 服务端分页 + 受控排序（公共契约回归）', () => {
  ...
  it('manual：页脚总数取外部 `totalItems`，而非当前页行数', async () => {
    ...
    const wrapper = mount(NvDataTable, { props: { ...base, rows, manual: true, totalItems: 95, pageSize: 10 } })
    await nextTick()
  ...
  it('clientSort=false：点表头发 update:sort 且不内部重排', async () => {
    ...
    const wrapper = mount(NvDataTable, { props: { ...base, rows, pagination: false, clientSort: false, sort: null } })
    await nextTick()
