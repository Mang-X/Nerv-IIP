...
<template>
  <SliderRoot
    ...
    :class="
      cn(
        'nv-slider relative flex w-full touch-none items-center select-none data-[orientation=vertical]:h-44 data-[orientation=vertical]:w-auto data-[orientation=vertical]:flex-col data-[disabled]:pointer-events-none data-[disabled]:opacity-50',
        props.class,
      )
    "
  >
    <SliderTrack
      class="nv-slider-track relative h-1.5 w-full grow overflow-hidden rounded-full bg-muted data-[orientation=vertical]:h-full data-[orientation=vertical]:w-1.5"
    >
      <SliderRange
        class="nv-slider-range absolute h-full rounded-full bg-brand data-[orientation=vertical]:w-full"
      />
    </SliderTrack>
    <SliderThumb
      v-for="i in thumbCount"
      :key="i"
      class="nv-slider-thumb block size-4 shrink-0 rounded-full border-2 border-brand bg-background shadow-sm outline-none focus-visible:ring-[3px] focus-visible:ring-brand/30"
    />
  </SliderRoot>
</template>
...
@layer nv-components {
  ...
  .nv-slider-thumb:hover {
    transform: scale(1.08);
  }
  .nv-slider-thumb:active {
