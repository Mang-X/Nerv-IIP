...
// Watch the resolved-key PROJECTION (as an array, never join-serialised: a
// key containing the separator could collide two different memberships into
// the same string and swallow the change). The mapping getter reads every
// element, so in-place splice/sort fires it; a `() => props.segments`
// reference getter never would (probe-verified: 0 hits on splice).
watch(
