...
<template>
  <div class="flex items-center gap-2.5">
    <RadioGroupItem
      :id="id"
      data-slot="nv-radio-group-item"
      v-bind="forwarded"
      :class="
        cn(
          'nv-radio flex size-[18px] shrink-0 items-center justify-center rounded-full border border-input bg-card outline-none focus-visible:ring-[3px] focus-visible:ring-brand/30 focus-visible:border-brand data-[state=checked]:border-brand disabled:cursor-not-allowed disabled:opacity-50 dark:bg-input/30',
          props.class,
        )
      "
    >
      <RadioGroupIndicator class="nv-radio-ind flex items-center justify-center">
...
@layer nv-components {
  ...
  .nv-radio:active:not([data-disabled]) {
    transform: scale(0.9);
  }
  .nv-radio-ind {
    animation: nv-radio-in 0.16s var(--nv-ease-out-quart, ease-out);
  }
  @keyframes nv-radio-in {
    from {
      transform: scale(0);
    }
    to {
