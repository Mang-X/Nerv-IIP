...
<template>
  <Teleport to="body">
    ...
    <div class="nv-msg-region pointer-events-none">
      <TransitionGroup name="nv-msg">
        <div
          v-for="item in messages"
          :key="item.id"
          class="nv-msg pointer-events-auto flex max-w-[min(86vw,30rem)] items-center gap-2 rounded-full border border-border bg-popover py-1.5 pr-3.5 pl-3 text-sm whitespace-nowrap text-popover-foreground shadow-md"
        >
          <component
    ...
    <div class="nv-note-region pointer-events-none">
      <TransitionGroup name="nv-note">
        <div
          v-for="item in notifications"
          :key="item.id"
          class="nv-note pointer-events-auto flex w-80 gap-3 rounded-xl border border-border bg-popover p-3.5 text-popover-foreground shadow-lg"
        >
          <component
            :is="iconFor[item.kind]"
...
@layer nv-components {
  ...
  .nv-msg-enter-active,
  .nv-msg-leave-active,
  .nv-note-enter-active,
  .nv-note-leave-active {
    transition:
