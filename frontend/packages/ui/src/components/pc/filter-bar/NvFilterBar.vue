...
import NvDatePicker from '../date-picker/NvDatePicker.vue'
 ...
 * Composes NvInput / NvSelect / NvDatePicker / NvBadge / NvButton — no
...
<template>
  <div data-slot="nv-filter-bar" :class="cn('flex flex-col gap-3', props.class)">
    ...
    <div v-if="activeChips.length" class="flex flex-wrap items-center gap-1.5">
      <NvBadge v-for="chip in activeChips" :key="chip.key" variant="brand" class="gap-1 pr-1">
