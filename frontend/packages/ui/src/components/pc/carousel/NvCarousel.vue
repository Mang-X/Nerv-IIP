...
<template>
  <div class="nv-carousel group/carousel w-full" data-slot="nv-carousel">
    <div
      ref="viewportEl"
      :class="
        cn(
          'nv-carousel-viewport relative w-full overflow-hidden',
    ...
    >
      <div
        ref="trackEl"
        class="nv-carousel-track flex h-full items-stretch"
        :class="!dragging && 'nv-carousel-snap'"
        :style="trackStyle"
        @pointerdown="onDown"
        @pointermove="onMove"
        @pointerup="onUp"
      ...
      >
        <template v-if="items">
          <div v-for="(item, i) in items" :key="i" class="nv-carousel-item w-full shrink-0">
      ...
      <template v-if="arrows && count > 1">
        <button
          type="button"
          class="nv-carousel-arrow nv-carousel-arrow-prev"
          :disabled="!canPrev"
          aria-label="上一张"
          @click="goTo(active - 1)"
        >
        ...
        <button
          type="button"
          class="nv-carousel-arrow nv-carousel-arrow-next"
          :disabled="!canNext"
          aria-label="下一张"
          @click="goTo(active + 1)"
        >
      ...
      <div
        v-if="dots && count > 1"
        class="nv-carousel-dots pointer-events-none absolute inset-x-0 bottom-3 flex justify-center gap-1.5"
        aria-hidden="true"
      >
        <span
          v-for="i in count"
          :key="i"
          class="nv-carousel-dot"
          :class="i - 1 === active && 'nv-carousel-dot-active'"
        />
      </div>
    </div>
  </div>
...
@layer nv-components {
  ...
  .group\/carousel:hover .nv-carousel-arrow,
  .nv-carousel-arrow:focus-visible {
    opacity: 1;
  }
  ...
  .nv-carousel-arrow:hover:not(:disabled) {
    background: var(--background);
  }
  .nv-carousel-arrow:disabled {
    opacity: 0 !important;
    pointer-events: none;
  }
  .nv-carousel-dot {
