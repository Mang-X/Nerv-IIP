      ...
      >
        ...
        <div class="max-h-[min(60vh,22rem)] overflow-y-auto p-1.5">
          <template v-for="group in filteredGroups" :key="group.label">
            ...
            <button
              ...
              class="nv-cmd-item flex w-full items-center gap-2.5 rounded-md px-2.5 py-2 text-left text-sm outline-none"
              @click="select(item)"
        ...
        >
          <span class="flex items-center gap-1"
            ><kbd class="nv-cmd-kbd">↑</kbd><kbd class="nv-cmd-kbd">↓</kbd>选择</span
          >
          <span class="flex items-center gap-1"
            ><kbd class="nv-cmd-kbd"><CornerDownLeftIcon class="size-3" /></kbd>执行</span
          >
          <span class="flex items-center gap-1"><kbd class="nv-cmd-kbd">esc</kbd>关闭</span>
        </div>
      </DialogContent>
    </DialogPortal>
  </DialogRoot>
