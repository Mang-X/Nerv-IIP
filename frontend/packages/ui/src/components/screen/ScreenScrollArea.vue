...
onMounted(() => {
  ...
  vpEl =
    ...
    (inst?.$el instanceof HTMLElement && inst.$el.classList.contains('nv-scr-sa-vp')
      ? inst.$el
      : (inst?.$el?.parentElement?.querySelector(
          ':scope > .nv-scr-sa-vp',
        ) as HTMLElement | null)) ??
    null
  if (!vpEl) return
...
<template>
  <ScrollAreaRoot class="nv-scr-sa" :type="type" :scroll-hide-delay="450">
    <ScrollAreaViewport ref="vpRef" class="nv-scr-sa-vp">
      <slot />
    </ScrollAreaViewport>
    <ScrollAreaScrollbar class="nv-scr-sa-bar" orientation="vertical">
      <ScrollAreaThumb class="nv-scr-sa-thumb" />
    </ScrollAreaScrollbar>
    <div v-show="canUp" class="nv-scr-sa-hint up" aria-hidden="true"><i /></div>
    <div v-show="canDown" class="nv-scr-sa-hint down" aria-hidden="true"><i /></div>
  </ScrollAreaRoot>
</template>
...
@layer nv-components {
  ...
  .nv-scr-sa-bar {
    ...
    transition: opacity 0.22s var(--nv-scr-ease);
    z-index: 2;
  }
  .nv-scr-sa-thumb {
    position: relative;
  ...
  .nv-scr-sa-bar:hover .nv-scr-sa-thumb,
  .nv-scr-sa-bar[data-state='visible']:active .nv-scr-sa-thumb {
  ...
  .nv-scr-sa-hint i {
    ...
    animation: nv-scr-sa-breath 2.8s ease-in-out infinite;
  }
  .nv-scr-sa-hint.up i {
    transform: rotate(-135deg);
  }
  .nv-scr-sa-hint.down i {
    transform: rotate(45deg);
  }
  @keyframes nv-scr-sa-breath {
    50% {
      opacity: 0.3;
    }
  }
