...
describe('wms PDA step flows', () => {
  ...
  it('outbound: order → packReviewNo → complete', () => {
    ...
    expect(
      outboundReviewFlow.isComplete({ orderId: 'OB1', packReviewNo: 'PR1', completed: true }),
    ).toBe(true)
  })
  it('count: execution selected → count entered → complete', () => {
    ...
    expect(countExecutionFlow.currentStep({ countExecutionId: 'CE1', countEntered: true }).id).toBe(
      'complete',
    )
    expect(
      countExecutionFlow.isComplete({
        countExecutionId: 'CE1',
        countEntered: true,
        completed: true,
      }),
    ).toBe(true)
    expect(countExecutionFlow.progress({ countExecutionId: 'CE1' })).toEqual({
      completed: 1,
      total: 3,
    })
  })
