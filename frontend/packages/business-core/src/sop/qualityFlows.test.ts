...
describe('qualityInspectionTaskFlow', () => {
  it('advances selectTask → recordResults → submit as the context fills', () => {
    ...
    expect(qualityInspectionTaskFlow.currentStep({ taskId: 'T1', hasResults: true }).id).toBe(
      'submit',
    )
  })
  ...
  it('reports completeness and progress', () => {
    expect(
      qualityInspectionTaskFlow.isComplete({ taskId: 'T1', hasResults: true, submitted: true }),
    ).toBe(true)
