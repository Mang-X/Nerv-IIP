...
import type { ConsoleAuthApi } from './api'
...
export interface AuthStoreMessages {
...
export function createAuthStore(options: CreateAuthStoreOptions) {
  ...
  return defineStore(storeId, () => {
    ...
    const displayName = computed(() => principal.value?.loginName ?? messages.unknownUser)

    async function login(loginName: string, password: string) {
      authError.value = undefined
      try {
        await applySession(await api.loginConsole({ loginName, password }))
      } catch (error) {
        clearSession('login-failed')
        authError.value = error instanceof Error ? error.message : messages.loginFailed
        throw error
      }
    }
    ...
    async function restoreSession() {
      ...
      try {
        const restoredSession = await api.refreshConsole({ refreshToken: stored.refreshToken })
    ...
    async function loadPrincipal() {
      ...
      const loadedPrincipal = await api.getConsoleMe(currentAccessToken)
    ...
    async function refreshSessionOnce(currentRefreshToken: string) {
      ...
      try {
        const refreshedSession = await api.refreshConsole({ refreshToken: currentRefreshToken })
    ...
    async function applySession(session: ConsoleAuthResponse) {
      const completeSession = assertValidSession(session, messages.invalidSession)
...
function assertValidSession(
  session: ConsoleAuthResponse,
  invalidSessionMessage: string,
): ValidConsoleAuthResponse {
  if (
