...
# 适用于 @nerv-iip/ui (pc/blocks/layout + touch) 与 @nerv-iip/ui-mobile。
# 目标：高质感、高一致性。任何新组件 / 改动都应过一遍本清单。
---

## 1. 缓动与时长令牌（唯一来源，勿手写 cubic-bezier）

令牌定义在 `packages/ui/src/styles/theme.css`（`@layer nv-tokens`，`--nv-*` 命名空间，
ADR 0020 §3），通过 `@theme inline` 暴露为 Tailwind `ease-*` 工具类，也可在 scoped CSS
里 `var(--…)` 直接引用。**新代码一律写 `--nv-*` 规范名**；无前缀旧名（`--ease-out-quart`
等）仅作 var 链别名兼容在途代码，收口批会删除。

| 令牌                                               | 值                                                                                   | 用途                                   |
| -------------------------------------------------- | ------------------------------------------------------------------------------------ | -------------------------------------- |
| `--nv-ease-out-quart`                              | `cubic-bezier(0.25,1,0.5,1)`                                                         | 微交互：hover / focus / 颜色 / 小位移  |
| `--nv-ease-out-expo`                               | `cubic-bezier(0.16,1,0.3,1)`                                                         | 入场揭示、覆盖层、滑动指示器、进度填充 |
| `--nv-ease-in-out-quart`                           | `cubic-bezier(0.76,0,0.24,1)`                                                        | 双向状态切换 / 往返位移                |
| `--nv-ease-out-back`（回弹，含轻微 overshoot 1.4） | 仅用于"出现"型强调：勾选/图标 pop、激活图标放大。**禁用于** 列表/布局位移            |
| 时长                                               | `--nv-duration-fast 150ms` / `--nv-duration-base 220ms` / `--nv-duration-slow 320ms` | 微交互→入场。整体落在 150–320ms        |

scoped CSS 里务必带 fallback：`var(--nv-ease-out-expo, cubic-bezier(0.16,1,0.3,1))`（组件库可能被无令牌环境消费）。
screen 层不用本表——大屏动效令牌是 `--nv-scr-ease` / `--nv-scr-ease-emphasized`（var 链
指回本表），契约见 `packages/ui/src/components/screen/product.md`。
...
- **令牌而非裸值**：颜色走语义令牌（`--nv-brand`/`--nv-success`…，Tailwind 工具类
  `bg-brand`/`text-success` 等），透明度用 `/10 /25` 或 `color-mix`，不写裸 hex / 调色板类名。
...
- 复制重建、**绝不改原版 shadcn-vue 组件**（见 foundation.md）。新件命名走 ADR 0020
  §1.2 R1–R5（移动层素名冲突 → `NvMobile*`，如 `NvMobileBadge` ≠ ui 的 `NvBadge`）。
