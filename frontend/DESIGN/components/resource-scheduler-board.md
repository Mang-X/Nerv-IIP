...
- **应当**与 `GanttChart` 共享同一 `ScheduleModel` 与 `SchedulingEngine`，按页面任务独立使用或组合切换。
- **不得**为资源视图复制一套数据契约；两视图只是同一模型的不同编排。
