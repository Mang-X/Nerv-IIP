...
**不得**用它做资源/机台负载视角（用 `ResourceSchedulerBoard`）。
...
- 亮/暗 + 运行时动态色由 `useColorMode` / `--brand` 驱动；DHTMLX 皮肤经 `engine/dhtmlx/skin.ts` 绑定 token。
...
- **应当**按页面任务独立组合工具栏与详情面板，保持甘特组件只负责工单/工序时间线。
- **不得**直接导入引擎实现（DHTMLX）；**不得**在模板出现 reasonCode/operationId 等工程语言。
