...
- Horizontal scroll state updates are animation-frame throttled, but Leafer overlays must update through the Leafer scene tree and redraw in the same frame as scroll, vertical virtualization, or drag-preview changes so dependencies, baselines, row lines, capacity bands, and today lines never leave old-frame artifacts or blank-frame flashes. Timeline tick DOM is bounded to the visible viewport plus overscan for large time ranges.
...
- Drag movement preserves the whole time window duration and may preview beyond the current fixture range; the host or future backend contract decides whether a preview can be committed.
