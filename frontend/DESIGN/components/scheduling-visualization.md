...
- Dependency links use orthogonal source/target rectangle routing. They must leave from the correct start/finish edge, stay inside the timeline area, and enter the target from outside the bar instead of drawing through the source or target block. When a finish-to-start target is clearly to the right of the source with enough horizontal space, use a compact top-edge route instead of sending the link through a distant lane.
...
- Horizontal scroll state updates are animation-frame throttled, but Leafer overlays must be redrawn in the same frame as scroll state changes so dependencies, baselines, capacity bands, and today lines stay synchronized with DOM bars. Timeline tick DOM is bounded to the visible viewport plus overscan for large time ranges.
