...
- Bars and highlight windows expose brief hover tooltips for scan-level details; the detail sheet is for deeper inspection, not the only way to read a fact.
...
- Do: keep large data rendering bounded by visible row/time ranges; tests should cover 1,000+ rows and multi-year timelines.
