...
| 承载           | 适用                                                                                                                                                 | 组件                                   |
| -------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------- |
| **Dialog**     | **≤3 个字段**的快捷操作：轻量登记、重命名、改期、单字段修正。纯确认（0~1 字段）用 `AlertDialogPro`（见 `flows/confirm-destroy.md` 与 §2 破坏性条款） | `DialogPro`                            |
| **Sheet 侧滑** | **4~8 个字段**，或**含动态行**（可增删的 `v-for` 行组），或填写时需对照列表/上文                                                                     | `SheetPro`（`sm:max-w-xl` 起）         |
| **独立页**     | **复杂多段/多步**：≥2 个分段标题或多个 `<form>`、向导步骤、含审批链/附件/子表、跨多聚合                                                              | 路由页（独立表单页或落在对象详情页内） |

### 判定

1. **数字段**：表单内可见输入控件数（`InputPro` / `SelectPro` / `DatePicker` / `Textarea` / `CheckboxPro` / 上传，**含只读回显字段**——只读字段同样占空间与认知）。≤3 → Dialog 可；4~8 → 必须 Sheet；>8 → 独立页。
2. **看结构**：出现动态行 → 至少 Sheet；出现 ≥2 个分段/多个 `<form>`/审批链面板/步骤态 → 独立页。
3. **反问**：「填这个表单需要对照列表或上文吗？」需要 → Sheet（不遮列表），不用居中 Dialog。

### 反例（现状证据，评审按此口径打回）

❌ **6 字段塞 Dialog** — 完工入库登记，`apps/business-console/src/pages/mes/receipts.vue:290`。工单号/成品/入库数量/单位成本/单位/登记时间共 6 个 `FieldPro`（300–325 行），开关变量名就叫 `receiptSheetOpen`——命名都承认该是 Sheet：
...
❌ **动态行塞 Dialog** — 发布工程变更，`apps/business-console/src/pages/engineering/eco.vue:312`：`sm:max-w-2xl` 的 `DialogPro` 里放受影响版本**动态行组**（`affectedVersions` 可增删，178–182 行）。按本节至少 Sheet；ECO 还带审批引用与多段信息，应独立页（见 §3 反例）。
...
现网容器选型正确的锚点：NCR 处置表单落在 `SheetPro`（`quality/ncrs.vue:267`）——**Sheet 选型对**；但该抽屉同时塞下详情 + 两段表单，属 §3 反例。

### 目标骨架（≤3 字段 Dialog；现网暂无同规模真例）

```vue
<DialogPro v-model:open="rescheduleOpen">
  <DialogProContent class="sm:max-w-md">
    <DialogProHeader><DialogProTitle>调整交期 · {{ row.workOrderId }}</DialogProTitle></DialogProHeader>
    <form class="grid gap-4" @submit.prevent="submitReschedule">
      <FieldPro><FieldProLabel>新交期</FieldProLabel><DatePicker v-model="form.dueDate" /></FieldPro>
      <FieldPro><FieldProLabel>调整原因</FieldProLabel><Textarea v-model="form.reason" required /></FieldPro>
      <DialogProFooter>…取消 / 保存…</DialogProFooter>
    </form>
  </DialogProContent>
</DialogPro>
```
...
1. 每行**最多 1~2 个高频动作**提为行内按钮（`ButtonPro size="sm"`，主动作在前）；其余一律收进 `RowActions`（下拉菜单）。
...
5. **破坏性动作**（删除/停用/关闭/作废/回滚）：必须 `AlertDialogPro` 二次确认，且**原因必填**——确认框内含原因输入（`Textarea`，或原因码 `SelectPro` + 备注），为空时确认按钮 `disabled`；确认按钮用 `variant="destructive"`；原因随请求提交、进审计。
...
```vue
<AlertDialogPro v-model:open="disableOpen">
  <AlertDialogProContent>
    <AlertDialogProHeader>
      <AlertDialogProTitle>确认停用该物料？</AlertDialogProTitle>
      <AlertDialogProDescription>停用后将不能用于新建/计划，已有记录不受影响。</AlertDialogProDescription>
    </AlertDialogProHeader>
    <FieldPro>
      <FieldProLabel for="disable-reason">停用原因 <span class="text-destructive">*</span></FieldProLabel>
    ...
    <AlertDialogProFooter>
      ...
      <AlertDialogProAction variant="destructive" :disabled="!disableReason.trim() || pending" @click="confirmDisable">
        确认停用
...
| 详情重量   | 判定特征                                                                              | 承载                                                          |
| ---------- | ------------------------------------------------------------------------------------- | ------------------------------------------------------------- |
| **轻详情** | 只读、看完即走；无状态机、≤1 类写操作                                                 | 速览 `DialogPro`（只读 `dl`）或 `SheetPro` 抽屉——保列表上下文 |
...
✓ **工单 = 独立页 + 速览分层**：独立页 `pages/mes/work-orders/[workOrderId].vue`；列表与关联页用 `WorkOrderQuickView` 速览（只读 `DialogPro`），底部按钮跳完整页（`components/mes/WorkOrderQuickView.vue:41`）：
...
❌ **重对象塞抽屉** — NCR（状态机 + 处置 + 关闭 + 审批链 + 附件）整个塞进 `sm:max-w-xl` 的 `SheetPro`（`quality/ncrs.vue:267–364`）：详情、「提交处置」、「关闭不合格品」三合一挤在一条侧滑里，无 URL、不可分享、不可从通知直达。应为独立页 `/quality/ncrs/[ncrId]`（处置/关闭作为页内动作区）。
...
**现状与过渡下限**：`DataTablePro` 只有 `emptyMessage` 文案位、无 `#empty` 插槽（`packages/ui/src/components/pro/data-table/DataTablePro.vue:48`），按钮进不了空态 → §8 能力项（补 `#empty` 插槽，接 `@nerv-iip/ui` 已有的 `Empty` 组件族）。缺口补齐前，**文案式出路是过渡下限**——如 `master-data/units.vue:540`：
...
**规则**：批量 = `DataTablePro` 内建选择系统，**禁止自造勾选列/悬浮动作条**：

- `selectable` 开勾选列（含全选/半选），`v-model:selected` 持有选中键，**`#bulk-actions` 插槽**放批量动作（上下文动作栏随选中数出现）。组件已实现——`DataTablePro.vue:53`（"Leading checkbox column + bulk bar"）、动作栏 `:568`、插槽 `:583`。
...
```vue
<DataTablePro :rows="rows" row-key="workOrderId" selectable v-model:selected="selectedIds">
  <template #bulk-actions="{ selected }">
    <ButtonPro size="sm" variant="outline" @click="bulkRelease(selected)">批量下达</ButtonPro>
  </template>
</DataTablePro>
```
...
- [ ] 破坏性动作有 `AlertDialogPro` 二次确认，且**原因必填**、随请求提交。
...
| 缺口                                                            | 现状出处                                                        | 目标                                                    |
| --------------------------------------------------------------- | --------------------------------------------------------------- | ------------------------------------------------------- |
| `DataTablePro` 空态只有 `emptyMessage` 文案位，无 `#empty` 插槽 | `packages/ui/src/components/pro/data-table/DataTablePro.vue:48` | 增 `#empty` 插槽，接 `Empty` 组件族渲染按钮 CTA（§5.1） |
