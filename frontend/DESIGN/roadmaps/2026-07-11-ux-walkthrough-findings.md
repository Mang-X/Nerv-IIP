...
> 证据：关键问题配运行时截图（DESIGN 资产在 `assets/2026-07-11-ux-walkthrough/`，逐条内联链接/图片，实机截图非占位）。少数条目仅以源码/检索为证并已逐条标注原因：**P1-2**（报工弹窗——种子工单全为"排队中"，报工入口在未开工态不出现，运行时不可达）、**P1-7**（批量为能力缺席，无可截之物，以 `grep` 零命中为证）。每条问题给复现路径 + 违反条款 + 证据 + 建议 + 严重度（阻断作业 / 低效 / 观感）。
> 关联：追踪 bug #868（P0-1）、修复中 PR #867（**门禁已通过**：head `a390f1f` 5 项必需检查全绿；分支 behind main、**未合并**，尚未进入基线）、本走查 PR #865。文中 P0-1 标"修复中"而非"已修复"——待 #867 同步 main 并合并后再改。
...
| **质量**    | ⚠️ Sheet容器对但超载  | ❌ 唯一动作收菜单+关闭无原因 | ❌ NCR重对象塞Sheet | ⚠️                   | ⚠️ 筛选进URL/无批量          | §3.1/3.2/3.3；[quality-ncrs-empty-no-cta.png](assets/2026-07-11-ux-walkthrough/quality-ncrs-empty-no-cta.png)、`quality/ncrs.vue:310/316/404`                                                                                |
...
| **工程**    | ❌ ECO动态行塞Dialog  | ⚠️                           | ❌ ECO无详情载体    | ⚠️                   | ✅ bom-analysis 进URL(正例)  | §3.2；[engineering-eco.png](assets/2026-07-11-ux-walkthrough/engineering-eco.png)、`engineering/eco.vue:329/396`                                                                                                             |
...
- **P0-2 破坏性动作无原因、原因不入审计**（运行时确认）。复现：`/master-data/units` 行 `...` → 停用 → 确认框只有说明文案"确认停用该计量单位？停用后将不能用于新建/计划…"和 取消/确认停用，**无原因输入**（运行时抓取 reasonInputs=0，可直接点"确认停用"）。质量"关闭不合格品"同（源码为准）。违反 **§2 破坏性条款**（`AlertDialogPro` + 原因必填 + 随请求进审计）。证据：[masterdata-disable-confirm-no-reason.png](assets/2026-07-11-ux-walkthrough/masterdata-disable-confirm-no-reason.png)；`components/masterData/MasterDataRowActions.vue`（`toggleOpen` 触发的 `NvAlertDialog` 停用确认，内无原因输入字段）、`quality/ncrs.vue:404`（关闭不合格品 `NvAlertDialog`；head 行号，下同）。建议：按 §2 目标写法补原因输入 + `disabled` 门禁 + 提交入审计。

### 3.2 低效 / 中优

- **P1-1 完工入库 6 字段塞居中 Dialog**（运行时确认弹窗 6 个输入）。复现：`/mes/receipts?workOrderId=…` → "登记完工入库"。违反 **§1**（4~8 字段应 Sheet 侧滑）。证据：[mes-receipts-dialog-6fields.png](assets/2026-07-11-ux-walkthrough/mes-receipts-dialog-6fields.png)（inputCount=6）、`mes/receipts.vue:325`（`<NvDialog v-model:open="receiptSheetOpen">` 登记完工入库，标题 328；开关变量名已叫 `receiptSheetOpen`）。建议：改 `NvSheet`。
- **P1-2 工单报工 7 字段塞 Dialog**。复现：`/mes/work-orders` → 报工。违反 §1。**证据以源码为证**：`mes/work-orders/index.vue:694`（`<NvDialog v-model:open="reportSheetOpen">` 生产报工，标题 697；`reportSheetOpen` 定义在 94）——旧引用 `:558` 实为"创建急单"弹窗说明文本，已更正。本次种子工单/工序均为"排队中"，报工按钮在未开工态不出现，运行时未能进入该弹窗截图（与 P1-1 完工入库同型，可参照 [mes-receipts-dialog-6fields.png](assets/2026-07-11-ux-walkthrough/mes-receipts-dialog-6fields.png)）。建议：改 `NvSheet`。
- **P1-3 工程变更 ECO：动态行塞 Dialog，且重对象无详情载体**。复现：`/engineering/eco` → 发布工程变更（受影响版本动态行组）。违反 **§1**（动态行至少 Sheet）+ **§3**（ECO 有状态机/审批链/多段，应独立页 `/engineering/eco/[id]`）。证据：[engineering-eco.png](assets/2026-07-11-ux-walkthrough/engineering-eco.png)、`engineering/eco.vue:329`（`<NvDialog v-model:open="formOpen">` 发布工程变更，标题 338）+ `:396`（`v-for (row, index) in form.affectedVersions` 受影响版本动态行）。建议：创建/编辑改独立页或 `NvSheet`；补 ECO 详情独立页。
- **P1-4 NCR 重对象塞 Sheet、唯一动作收菜单**。复现：`/quality/ncrs`（当前种子为空，源码为准）。违反 **§3**（NCR 有状态机+处置+关闭+审批链+附件，应独立页 `/quality/ncrs/[id]`）+ **§2**（整行唯一"打开处置"仍收 `RowActions`）。证据：[quality-ncrs-empty-no-cta.png](assets/2026-07-11-ux-walkthrough/quality-ncrs-empty-no-cta.png)、`quality/ncrs.vue:310`（`<NvRowActions>` 内唯一 `NvDropdownMenuItem` "打开处置" 311）+ `:316`（`<NvSheet v-model:open="detailOpen">` NCR 详情/处置/关闭三合一塞抽屉）。建议：NCR 独立页；"打开处置"提为行内按钮或由 ID 列承担。
- **P1-5 完工入库成功态是"死文案"，跨域失效漏库存**。违反 **§4.1**（创建成功须给"继续/查看/返回"至少两项）+ **§4.2**（入库改库存，`onSuccess` 未失效库存键）。证据：[mes-receipts-list.png](assets/2026-07-11-ux-walkthrough/mes-receipts-list.png)、`mes/receipts.vue:337`（`<p v-if="successMessage" ... role="status">` 常驻死文案）；`useBusinessMes.ts:856`（`useMesFinishedGoodsReceipts` create 的 `onSuccess`，失效键从 859 起只列 MES、缺库存键）。建议：容器内成功态 + 出路按钮；失效链补 `getBusinessConsoleInventoryAvailability` 等库存 operationId。
- **P1-6 MES 工序执行筛选状态不进 URL**（运行时确认）。复现：`/mes/operation-tasks` → 选"状态=待开工" → URL 仍为 `/mes/operation-tasks`（无 `?status`）；进详情返回即丢。违反 **§5.3**。证据：运行时 `URL_CHANGED=false`；[mes-operation-tasks-goldstandard.png](assets/2026-07-11-ux-walkthrough/mes-operation-tasks-goldstandard.png)（同页 §2 正例）；`mes/operation-tasks.vue:84`（`// Filters (live)` + `keyword/statusFilter = ref(...)` 85/86，全存本地 `ref`）。建议：筛选/搜索/页码 `router.replace({query})` 双向同步（库存/质量/条码域已有先例可抄）。
- **P1-7 批量能力在业务页零使用**（**能力缺席，无可截之物**——以源码检索为证）。`DataTablePro` 已内建 `selectable` + `#bulk-actions`，但 `apps/business-console/src` 与 `apps/business-pda/src` 业务页面**零接入**（`grep -rl selectable` 命中 0；设计系统展厅 `design-system.vue:1188/1198` 有演示用法，非业务页）。违反 **§5.2**。需批量的场景（工单批量下达、报警批量确认、主数据批量停用）现均逐行操作。建议：按 §5.2 形态接入业务页，破坏性批量带条数复述 + 原因。

### 3.3 观感 / 低优

- **P2-1 主数据 5–6 字段建档普遍塞 Dialog**。`master-data/units.vue:535`（`<NvDialog v-model:open="createOpen">` 新建计量单位 5 字段）、`:706`（`<NvDialog v-model:open="conversionOpen">` 新建换算关系 6 字段）、`quality/reason-codes.vue:247`（`<NvDialog v-model:open="formOpen">` 新建原因 5 字段）。违反 §1（主数据域整改以 A1 §1 取代 `master-data-templates.md` §5 的"Dialog 默认"）。证据：[masterdata-units.png](assets/2026-07-11-ux-walkthrough/masterdata-units.png)、[quality-reason-codes.png](assets/2026-07-11-ux-walkthrough/quality-reason-codes.png)。
- **P2-2 主数据行操作三动作全收下拉、行内零按钮**（运行时确认）。复现：`/master-data/units` 行 `...` 展开，菜单项 = 查看详情 / 编辑 / 停用（运行时抓取），行内零按钮。违反 §2（查看详情由 ID 列承担，编辑/停用按频率提行内）。证据：[masterdata-rowactions-dropdown.png](assets/2026-07-11-ux-walkthrough/masterdata-rowactions-dropdown.png)；`components/masterData/MasterDataRowActions.vue:79/83/87`（三个 `NvDropdownMenuItem`：查看详情/编辑/停用，行内零按钮）。
...
- **P2-4 PDA 报工步骤裸文案"第 1/4 步"**（运行时确认）。违反 **§6.3**（应挂 `Steps` 组件，全程可见步骤名 + 当前位置）。证据：[pda-mes-report-step-text.png](assets/2026-07-11-ux-walkthrough/pda-mes-report-step-text.png)；`business-pda mes/report.vue:257`（"第 {{…}}/{{ progress.total }} 步"裸文案）、`mes/receipt.vue:327`（同）。
- **P2-5 PDA 数量/测量值用原生 `<input type=number>`**。违反 **§6.1**（应用 `NumberKeyboard`，已导出零接入）。证据：[pda-wms-count.png](assets/2026-07-11-ux-walkthrough/pda-wms-count.png)；`business-pda wms/count.vue:222`（`<input type="number">` data-testid=counted-quantity）、`mes/report.vue:386`（良品数 `<input type="number">`）。
...
### 3.5 PDA 逐页矩阵（§6 · #815「PDA 全页」验收）

首页 + 12 个作业页逐页实机打开并按 §6 记录（§6.1 数字键盘 / §6.2 单 Sheet≤3 输入 / §6.3 步骤条 / §6.4 触点≥44px）。"结果"列 ✅ 合规 / ⚠️ 轻问题或运行时空 / ❌ 明确违反。截图均在 `assets/2026-07-11-ux-walkthrough/`。

| 页面     | 路由                 | 运行时数据               | 适用 §6   | 结果 | 说明 + 证据                                                                                                                                                                                                                      |
| -------- | -------------------- | ------------------------ | --------- | ---- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 首页     | `/`                  | 12 应用格 + 我的任务空态 | §6.4      | ✅   | 扫描栏 + 12 快捷应用；"暂无分配给你的任务"诚实空态；触点足。[pda-home.png](assets/2026-07-11-ux-walkthrough/pda-home.png)                                                                                                        |
| 收货入库 | `/wms/inbound`       | 5 ASN 真实               | §6.4      | ✅   | 扫码优先 + 列表渲染（`ASN-SCR-*`/`IN-2026-*`）。[pda-wms-inbound.png](assets/2026-07-11-ux-walkthrough/pda-wms-inbound.png)                                                                                                      |
| 上架     | `/wms/putaway`       | 5 上架任务真实           | §6.4      | ✅   | `PT-SCR-*`/`PUT-2026-*` + 库位路径。[pda-wms-putaway.png](assets/2026-07-11-ux-walkthrough/pda-wms-putaway.png)                                                                                                                  |
| 拣货     | `/wms/pick`          | 1 拣货任务真实           | §6.4      | ✅   | `PICK-2026-0001` + SKU/库位。[pda-wms-pick.png](assets/2026-07-11-ux-walkthrough/pda-wms-pick.png)                                                                                                                               |
| 复核发货 | `/wms/review`        | 5 出库单真实             | §6.4      | ✅   | `SO-SCR-*`/`OUT-2026-*` 待发货。[pda-wms-review.png](assets/2026-07-11-ux-walkthrough/pda-wms-review.png)                                                                                                                        |
| 盘点     | `/wms/count`         | 6 盘点行真实             | §6.1 §6.4 | ⚠️   | 列表渲染；进详情录数量用原生 `<input type=number>`（§6.1，见 P2-5）。[pda-wms-count.png](assets/2026-07-11-ux-walkthrough/pda-wms-count.png)                                                                                     |
| 领料     | `/mes/issue`         | 0 领料（空态）           | §6.2 §6.3 | ⚠️   | "共 0 条领料申请"诚实空态；新建领料多步流程运行时无数据可进，结论以源码为准。[pda-mes-issue.png](assets/2026-07-11-ux-walkthrough/pda-mes-issue.png)                                                                             |
| 工序执行 | `/mes/operation`     | 17 工序任务真实          | §6.4      | ⚠️   | 列表渲染；工作中心 `WC-ASSY/WC-TEST`；**状态全显"未知状态"**（映射缺口，同报工，见 §4.2）。[pda-mes-operation.png](assets/2026-07-11-ux-walkthrough/pda-mes-operation.png)                                                       |
| 报工     | `/mes/report`        | 17 工单真实              | §6.1 §6.3 | ❌   | "第 1/4 步"裸步骤文案（§6.3，P2-4）+ 数量原生 input（§6.1，P2-5）+ "未知状态"。[pda-mes-report-step-text.png](assets/2026-07-11-ux-walkthrough/pda-mes-report-step-text.png)                                                     |
| 完工入库 | `/mes/receipt`       | 1 入库申请真实           | §6.2 §6.3 | ❌   | "新建完工入库"多步：第 2 步一屏 4 输入（SKU/数量/单位成本/单位）+ 裸步骤文案（§6.2/§6.3 A1 反例 `receipt.vue:361` 步骤 2 录入 + `:327` 裸步骤文案）。[pda-mes-receipt.png](assets/2026-07-11-ux-walkthrough/pda-mes-receipt.png) |
| 设备报警 | `/equipment/alarms`  | 2 报警真实               | §6.4      | ⚠️   | "去报修"行内直达 ✓；但设备号显示原始 GUID `019f40de-…`（违反 `ui-no-manual-text` 人读编号，见 P2-8）。[pda-equipment-alarms.png](assets/2026-07-11-ux-walkthrough/pda-equipment-alarms.png)                                      |
| 点检     | `/equipment/inspect` | 4 保养计划真实           | §6.4      | ✅   | 选计划 + 提交门禁；设备号人读 `DEV-ASSY-01`；"暂无点检记录"空态。[pda-equipment-inspect.png](assets/2026-07-11-ux-walkthrough/pda-equipment-inspect.png)                                                                         |
| 报修     | `/equipment/repair`  | 表单 + 4 维修工单真实    | §6.2 §6.4 | ✅   | 新建报修 3 输入（设备号/优先级/描述，≤3）+ 提交门禁；`DEV-*` 人读。[pda-equipment-repair.png](assets/2026-07-11-ux-walkthrough/pda-equipment-repair.png)                                                                         |

矩阵新增（并入 §6 去向）：

- **P2-8 设备报警 PDA 显示原始设备 GUID**。`/equipment/alarms` 报警行设备标识为 `019f40de-9ec0-…`（原始 GUID），而点检/报修页同域显示人读 `DEV-ASSY-01`。违反 `ui-no-manual-text`（展示真实人读编号）。建议：报警读面取设备 `code` 而非内部 id。证据：[pda-equipment-alarms.png](assets/2026-07-11-ux-walkthrough/pda-equipment-alarms.png)。
- **触点（§6.4）** 全 13 页目测达标（`Cell`/`ListRow`/按钮均 ≥44px，与 `min-h-touch=48px` 一致），未见明显小于 44px 的主操作；`Stepper` 32px 属组件级缺口（A1 §8），本次数量录入未走 `Stepper` 故未触发。

## 4. 门禁绿 ≠ 真机无 bug（运行时专有发现）

教训复盘：单测桩掉了真实 HTTP，导致以下问题不被门禁拦截，只有实机能暴露——

1. **人员目录分页 0-based/1-based 契约错位（P0-1）**：前端发 `pageIndex=0`，网关 `PageIndex > 0` 校验器拒为 400。三处消费页（组织与班组、人员技能、派工看板）的人员选择器静默空。前端单测 mock 了 `workers` 响应且未校验取值，故 `pnpm test` 全绿。**修复中（PR #867，门禁已通过但未合并）**：前端改 1-based + 前端回归用例（断言发 1）+ 网关契约边界测试（钉住 `PageIndex > 0`）+ 实机 `/master-data/organization` workers 请求 200 复验；追踪 bug #868。head `a390f1f` 5 项必需检查全绿，分支 behind main、尚未合并进基线，待同步 main 合并后再改标"已修复"。
2. **PDA 工单状态显示"未知状态"**：`business-pda` 报工页（`mes/report.vue`）与工序执行页（`mes/operation.vue`）工单列表 17 条全部渲染"未知状态"，疑似状态枚举未映射/字段缺失（真机数据是 `released` 等）。观感级但影响一线判断。证据：[pda-mes-report-step-text.png](assets/2026-07-11-ux-walkthrough/pda-mes-report-step-text.png)、[pda-mes-operation.png](assets/2026-07-11-ux-walkthrough/pda-mes-operation.png)。→ 建议核对 PDA 侧状态映射与 facade 字段（去向见 §6）。
...
- **MES 工序执行 = 行操作金标准**：高频"报工"按状态条件行内直达，其余收 `NvRowActions`（`mes/operation-tasks.vue:240` `showReportButton` + `:422` 行内报工按钮）。§2 正例，勿动。证据 [mes-operation-tasks-goldstandard.png](assets/2026-07-11-ux-walkthrough/mes-operation-tasks-goldstandard.png)。
...
- **PDA 全 13 路由已逐页记录**（§3.5 矩阵，每页配截图）。仍有边界：数字键盘弹出、多步 Sheet 内字段数属**进入作业详情态**后的深层交互，本次以列表/表单态截图 + 源码（`count.vue:222`、`report.vue:386`、`receipt.vue:361/327`）判定，§6.3 步骤裸文案已获运行时截图；领料页运行时 0 数据，其新建多步流程未演示。
- **ERP/条码**不在 #815 十域清单内，仅顺带确认可打开，未逐项按六标准打分。
- **截图**：已提交 31 张实机截图至 `assets/2026-07-11-ux-walkthrough/`（console 关键问题 + PDA 全 13 页 + P0-2/P2-2 交互态）；未逐一提交全部 74 console 页截图，需要补某页可再截。
