...
- **UX**：信息层级清晰、空状态给"去新建/去维护"出路、术语说人话（工程术语→业务语言）、操作有反馈闭环；复用 `@nerv-iip/ui` 区块组件（见 §3）。
  ...
  - **视觉层次必须有**：表单分段标题 ≠ 字段标题（曾踩坑：两者都 `text-sm font-medium`，整片无引导）。多分段表单用 `components/masterData/FormSectionTitle.vue`（主色短条 + 16px 加粗 + 下边线）；字段标题保持 14px。长表单要分组，不要平铺一片。
  - **做完必须把页面拉起来看**（截图 / `preview_*` / 浏览器）再说"完成"——typecheck/test 绿 ≠ 好用。**不许盲交 UI**：自己先问"这真的好用、看得清吗"。
