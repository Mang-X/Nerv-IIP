...
const principal = {
  ...
  permissionCodes: [
    ...
    'business.mes.work-orders.manage', // 工单拆分/合并 HTTP lane
...
test('工单拆分与合并：Chromium mock HTTP lane 覆盖状态、回读重试与零写请求', async ({
  page,
}, testInfo) => {
  ...
  // 合并正例通过真实 Chromium fetch 命中同一 mock Gateway 路由，补足 merge wire 请求/响应。
  const mergeWireResponse = await page.evaluate(async () => {
    const response = await fetch(
      '/api/business-console/v1/mes/work-orders/merge?organizationId=org-001&environmentId=env-dev&scopeKind=organization&scopeId=org-001',
      {
        method: 'POST',
        ...
        body: JSON.stringify({
          ...
          reason: '同 SKU 小单合并',
          idempotencyKey: 'merge-e2e-1',
        }),
      },
    )
    return { status: response.status, body: await response.json() }
  })
  expect(mergeWireResponse.status).toBe(200)
  expect(mergeWireResponse.body.data.accepted).toBe(true)
  expect(httpRequests.find((item) => item.pathname.endsWith('/work-orders/merge'))).toMatchObject({
    method: 'POST',
    body: {
      sourceWorkOrderIds: ['WO-MERGE-1', 'WO-MERGE-2'],
      targetWorkOrderId: 'WO-MERGE-TARGET',
  ...
  await testInfo.attach('work-order-transformation-http.json', {
    body: JSON.stringify({ requests: httpRequests, responses: httpResponses }, null, 2),
    contentType: 'application/json',
  })
  expect(
    httpResponses
      .filter((item) => item.pathname.endsWith('/work-orders/WO-001/split'))
      .map((item) => item.status),
  ).toEqual([200, 500, 409])
  expect(
    httpResponses
      .filter((item) => item.pathname.includes('/work-order-transformations/TX-SPLIT'))
      .map((item) => item.status),
  ).toEqual([503, 200])
})

test('工单合并：列表契约缺少单位时 fail-closed 且不发送写请求', async ({ page }, testInfo) => {
  // 证据拓扑：真实 Chromium 页面 + Playwright route mock Gateway；列表响应刻意遵循
  // PR-C 当前契约（无 uomCode），验证不同单位/未知单位不能被前端当作相同单位提交。
