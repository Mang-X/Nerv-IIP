...
test('第五轮收尾取证', async ({ page }) => {
  ...
  const drawers: DrawerCase[] = [
    ...
    // 设备报警**没有行详情抽屉**：页上只有「批量确认」「搁置」两个处置弹框，
    // 而且 `v-if="canShelve(row)"` 还受行状态限制（首行已确认时连菜单项都不出现）。
    // 处置动作会落库，取证脚本不点——所以如实标 none，只取整页证据。
    //
    // 上一版试图点「抑制」，更早一版的正则甚至带了「确认」，直接把演示库里一条
    // 报警确认掉了（右下角弹「报警已确认。」）。**取证脚本的底线是只看不动。**
    { id: 'F-09', title: '设备报警', route: '/equipment/alarms', via: 'none' },
    {
      id: 'F-10',
      title: '维护工单',
