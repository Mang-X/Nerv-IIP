...
test('生产日报呈现跨源上下文、服务端第二页并导出当前筛选全量 CSV', async ({ page }, testInfo) => {
      ...
      return fulfill(
        ...
        envelope({
          ...
          buckets: [
            {
              dimension,
              dimensionValue: dimension === 'workCenter' ? 'WC-CNC-01' : '2026-08-31',
              businessDate: dimension === 'day' ? '2026-08-31' : null,
  ...
  await page.getByRole('option', { name: '按工作中心' }).click()
  await expect(page.getByText('22 个工作中心聚合', { exact: true })).toBeVisible()
  await expect(page.getByText('WC-CNC-01', { exact: true }).first()).toBeVisible()

  await page.getByRole('button', { name: '下一页' }).click()
  await expect(page.getByText('WC-CNC-21', { exact: true }).first()).toBeVisible()
  await expect(page.getByText('WC-CNC-22', { exact: true }).first()).toBeVisible()

  const downloadPromise = page.waitForEvent('download')
  await page.getByRole('button', { name: '导出 CSV' }).click()
  const download = await downloadPromise
  expect(download.suggestedFilename()).toMatch(/^生产日报_工作中心_.*\.csv$/)
...
function productionRows(url: URL) {
  if (url.searchParams.get('dimension') === 'workCenter') {
    return Array.from({ length: 22 }, (_, index) => {
      const workCenterId = `WC-CNC-${String(index + 1).padStart(2, '0')}`
      return {
        dimension: 'workCenter',
        dimensionValue: workCenterId,
        businessDate: null,
        shiftCode: null,
        workCenterId,
        skuId: null,
  ...
  const windowEnd = new Date(url.searchParams.get('windowEndUtc')!)
  const dates: string[] = []
  for (
