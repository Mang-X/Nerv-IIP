...
/**
 * 读错误只归列表区域。创建上架任务的失败一律走 toast，不并进这一条：
 ...
 * 表格骨架还在转就断言「0 个上架任务」，等于把加载中说成没有任务。
 */
...
<template>
  <BusinessLayout>
    <NvPageHeader title="上架任务" :breadcrumbs="[{ label: '仓储作业' }]" :count="headerCount">
      <template #actions>
        <NvButton
          size="sm"
          type="button"
    ...
    <!-- 读失败 / 未选组织环境都由表格自己的三态呈现，绝不退化成「暂无上架任务」。 -->
    <NvDataTable
      manual
      :page="page"
      :page-size="pageSize"
