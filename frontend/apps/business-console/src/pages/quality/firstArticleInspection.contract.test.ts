...
const planDialogSource = readFileSync(
  resolve(process.cwd(), 'src/components/quality/FirstArticlePlanDialog.vue'),
  'utf8',
)
...
describe('首件检验工作台合同', () => {
  it('在既有检验页提供首件方案与首件记录两个平级入口', () => {
    ...
    expect(pageSource).toContain('FirstArticlePlanDialog')
  ...
  it('首件方案固定业务分类并要求 SKU、工序工作中心和至少一个检验项', () => {
    expect(planDialogSource).toContain("category: 'first-article'")
    expect(pageSource).toContain("'first-article': '首件检验'")
    expect(planDialogSource).toContain('请选择适用物料。')
    expect(planDialogSource).toContain('请选择工序工作中心。')
    expect(planDialogSource).toContain('请至少添加一个检验项。')
    expect(planDialogSource).toContain('方案已创建但未启用')
  })
