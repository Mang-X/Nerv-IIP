...
vi.mock('@nerv-iip/ui', () => ({
  ...
  PageHeader: { props: ['title', 'count'], template: '<header><h1>{{ title }}</h1><span>{{ count }}</span><slot name="actions" /></header>' },
  RowActions: { template: '<div><slot /></div>' },
  SectionCard: { props: ['description', 'value', 'hint'], template: '<div>{{ description }} {{ value }} {{ hint }}</div>' },
...
vi.mock('@/composables/useBusinessTelemetry', () => ({
  ...
  formatOeeRate: (value?: number) => value === undefined ? '无数据' : `${(value * 100).toFixed(1)}%`,
