...
definePage({ meta: { requiresAuth: true, title: '采集标签', requiredPermissions: ['business.iiot.telemetry.read'] } })
...
const columns: DataTableProColumn<BusinessConsoleTelemetryTagItem>[] = [
  { key: 'tagKey', header: '采集标签', cellClass: 'font-medium', accessor: (r) => r.tagKey ?? '无标签' },
  { key: 'deviceAssetId', header: '设备', accessor: (r) => r.deviceAssetId ?? '无设备' },
  { key: 'valueType', header: '值类型', width: 'w-24', accessor: (r) => valueTypeLabel(r.valueType) },
...
function valueTypeLabel(value?: string | null) {
  const labels: Record<string, string> = { bool: '布尔', boolean: '布尔', number: '数值', numeric: '数值', text: '文本' }
...
<template>
  <BusinessLayout>
    <PageHeader title="采集标签" :breadcrumbs="[{ label: '设备监控（IoT）' }]" :count="`${tagsTotal} 个采集标签`">
      <template #actions>
        ...
        <ButtonPro size="sm" type="button" variant="outline" as-child>
          <RouterLink to="/equipment/telemetry/history"><LineChartIcon aria-hidden="true" />历史趋势</RouterLink>
        </ButtonPro>
        <ButtonPro size="sm" type="button" variant="outline" :disabled="tagsPending" @click="refreshTags">
    ...
    <Toolbar :show-search="false">
      <template #filters>
        <InputPro v-model="filters.deviceAssetId" class="h-9 w-72" placeholder="按设备编号筛选" aria-label="设备编号" />
      </template>
    </Toolbar>
    ...
    >
      ...
      <template #cell-actions="{ row }">
        <RowActions :label="`采集标签操作 ${row.tagKey ?? ''}`">
