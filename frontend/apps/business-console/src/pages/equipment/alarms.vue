...
import { BellOffIcon, CheckCircle2Icon, EyeIcon, LineChartIcon, RefreshCwIcon, Settings2Icon, Undo2Icon, WrenchIcon } from 'lucide-vue-next'
...
const currentActor = computed(() => auth.principal?.loginName ?? auth.principal?.principalId ?? 'business-console')
...
function statusLabel(value?: string | null) {
  const labels: Record<string, string> = { acknowledged: '已确认', cleared: '已解除', raised: '已触发', shelved: '已搁置' }
  return value ? (labels[value.toLowerCase()] ?? value) : '未知'
}
...
function canAcknowledge(row: Alarm) {
  return canManageAlarms.value && Boolean(row.alarmEventId) && !row.acknowledgedAtUtc && (row.status ?? '').toLowerCase() !== 'cleared'
}
function canShelve(row: Alarm) {
  return canManageAlarms.value && Boolean(row.alarmEventId) && !isShelved(row) && (row.status ?? '').toLowerCase() !== 'cleared'
}
...
<template>
  <BusinessLayout>
    ...
    <SectionCards :columns="4">
      ...
      <SectionCard description="升级 / 搁置" :value="`${escalatedCount} / ${shelvedCount}`" hint="当前处置状态" />
    </SectionCards>
    ...
    >
      ...
      <template #cell-status="{ row }">
        <BadgePro class="rounded-sm" :variant="statusVariant(row.status)">{{ statusLabel(row.status) }}</BadgePro>
      </template>
      ...
      <template #cell-actions="{ row }">
        <RowActions :label="`报警操作 ${row.alarmEventId ?? ''}`">
          <DropdownMenuProItem v-if="canAcknowledge(row)" @click="handleAcknowledge(row)">
            <CheckCircle2Icon aria-hidden="true" />
            确认报警
          </DropdownMenuProItem>
          <DropdownMenuProItem v-if="canShelve(row)" @click="handleShelve(row)">
            <BellOffIcon aria-hidden="true" />
            搁置 30 分钟
          </DropdownMenuProItem>
          <DropdownMenuProItem v-if="canUnshelve(row)" @click="handleUnshelve(row)">
