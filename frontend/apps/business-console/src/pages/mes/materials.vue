...
definePage({
  meta: {
    requiresAuth: true,
    title: '领料与齐套',
    requiredPermissions: ['business.mes.materials.read'],
  },
})
...
const columns: NvDataTableColumn<RequestRow>[] = [
  {
    key: 'requestId',
    header: '申请号',
    cellClass: 'font-medium',
    accessor: (r) => r.requestId ?? '无',
  },
  { key: 'workOrderId', header: '工单', accessor: (r) => r.workOrderNo ?? r.workOrderId ?? '无' },
  {
    key: 'materialId',
    header: '物料',
    accessor: (r) => resolveSku(r.materialCode ?? r.materialId) ?? '无',
  },
...
<template>
  <BusinessLayout>
    <NvPageHeader
      ...
      :count="`${materialIssueRequestsTotal} 条领料申请`"
    >
      <template #actions>
        <NvButton
          size="sm"
    ...
    <NvToolbar :show-search="false">
      <template #filters>
        <NvSelect v-model="statusFilter">
          <NvSelectTrigger class="h-9 w-32" aria-label="领料状态"
            ><NvSelectValue
          /></NvSelectTrigger>
          <NvSelectContent>
            <NvSelectItem
              v-for="option in mesMaterialIssueStatusOptions"
              :key="option.value"
              :value="option.value"
      ...
      <template #cell-receivedQuantity="{ row }">
        <div class="flex flex-col gap-1">
          <span class="text-sm tabular-nums">
            已收 {{ formatQuantity(row.receivedQuantity) }} / 应领
