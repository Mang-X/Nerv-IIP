  ...
  <div class="flex min-w-0 flex-col gap-4 border-b pb-5 sm:flex-row sm:items-end sm:justify-between">
    <div class="min-w-0">
      <div class="flex flex-wrap items-center gap-2">
        ...
        <span
          v-if="kicker"
          class="max-w-full truncate rounded-sm bg-muted px-2 py-0.5 text-xs text-muted-foreground"
        >
          {{ kicker }}
        </span>
      </div>
      <h1 class="mt-2 break-words text-2xl font-semibold tracking-normal text-foreground">
        {{ title }}
      </h1>
      <p class="mt-2 max-w-3xl text-sm leading-6 text-muted-foreground">
        {{ summary }}
      </p>
      <p v-if="meta" class="mt-1 max-w-3xl truncate text-xs text-muted-foreground">
        {{ meta }}
      </p>
    </div>
