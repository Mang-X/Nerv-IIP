...
const props = withDefaults(
  ...
  }>(),
  {
    ...
    context: undefined,
    gapMessage:
      '本页暂不显示逐行可用量与批次、序列号、冻结预留明细，请到库存可用量或批次与预留页查看。',
    locationCode: '',
    lotNo: '',
    serialNo: '',
    siteCode: '',
...
const hasQuantityFacts = computed(
  () =>
    !!props.context &&
    (contextStatus.value === '' ||
      contextStatus.value === 'ok' ||
      contextStatus.value === 'available'),
)
...
<template>
  <section
    :class="
      compact
        ? 'grid gap-2 rounded-md border bg-muted/20 p-2 text-xs'
        : 'grid gap-3 rounded-md border bg-muted/20 p-3 text-sm'
    "
  >
    <div class="flex flex-wrap items-center justify-between gap-2">
      <div>
        <h2
          :class="compact ? 'font-medium text-foreground' : 'text-sm font-semibold text-foreground'"
        >
          {{ title }}
        </h2>
    ...
    <div v-if="hasQuantityFacts" class="grid gap-2 sm:grid-cols-4">
      <div>
        <span class="text-muted-foreground">现存量：</span
        ><span class="tabular-nums">{{ formatQuantity(onHandQuantity) }}</span>
      </div>
      <div>
        <span class="text-muted-foreground">可用量：</span
        ><span class="tabular-nums">{{ formatQuantity(availableQuantity) }}</span>
      </div>
      <div>
        <span class="text-muted-foreground">预留量：</span
        ><span class="tabular-nums">{{ formatQuantity(reservedQuantity) }}</span>
      </div>
      <div>
        <span class="text-muted-foreground">冻结/其他：</span
        ><span class="tabular-nums">{{ formatQuantity(frozenQuantity) }}</span>
      </div>
    </div>
