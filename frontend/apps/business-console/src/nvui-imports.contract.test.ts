 ...
 *
 * App-wide companion to `pages/goldStandardPages.contract.test.ts`: that test
 * enforces block usage on the migrated page list, this one blankets EVERY source
 * file so the component library is consumed ONLY through its stable package
 * boundary — the bare `@nerv-iip/ui` and `@nerv-iip/ui-mobile` specifiers. The
 * concrete reason: collaborating agents kept reaching past the boundary
 * (deep-importing shadcn 原版 / reka-ui) and mistaking 原版 primitives for the
 * brand layer. `Nv*` names + this guard remove that ambiguity.
 *
