...
PC 与 PDA 在执行工序、报工完工、收发盘点、质量处置、报警和维修等生命周期动作前，会按当前组织与环境重新读取目标对象的权威状态。对象已被其他人处理时，页面会阻止过期动作、清理本次操作上下文并刷新列表或详情；网络结果不确定时，同一业务意图沿用原幂等键重试，明确被拒或用户修改输入后才生成新的业务意图。

PDA 的 WMS 作业、质检、工序执行、维修工单和设备报警任务列表统一按 20 条分页读取，页面同时显示业务范围、来源、总数、已加载数和最近成功更新时间。状态与关键字等筛选由服务端执行；下拉刷新会回到第一页，上拉加载会去重追加。若下一页失败，已加载任务仍保留并提供局部重试。离开列表再返回时，当前筛选与滚动位置会从本次会话恢复；这不是离线缓存，也不会在后台预取未查看的数据。
...
flowchart LR
  subgraph Plan["计划 / 排程"]
    ...
    P2["角色: 排程员<br/>入口: /scheduling<br/>对象/状态: SchedulePlan Draft -> Released<br/>当前: 表格 + 只读资源甘特<br/>缺口: 高级优化器和交互式重排"]
  end
  subgraph MES["制造执行"]
