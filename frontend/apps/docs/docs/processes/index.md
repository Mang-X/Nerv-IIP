...
PDA 的 WMS 作业、质检、工序执行、维修工单和设备报警任务列表统一按 20 条分页读取。状态与关键字等筛选由服务端执行；下拉刷新会回到第一页，上拉加载会去重追加。若下一页失败，已加载任务仍保留并提供局部重试。离开列表再返回时，当前筛选与滚动位置会从本次会话恢复；这不是离线缓存，也不会在后台预取未查看的数据。
