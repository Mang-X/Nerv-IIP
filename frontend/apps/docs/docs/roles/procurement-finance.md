...
| 6   | 在 `/erp/sales/quotations` 报价，在 `/erp/sales/orders` 成单（含信用检查）并对已下达订单「释放发货」，再到 `/erp/sales/deliveries` 跟进出库；已转出的报价再次转订单不会新建，系统幂等返回既有订单号 | 销售报价到发货成链       | ✅ 可用                                                                                                       |
...
- 发货单从销售订单释放生成，因此「释放发货」的入口在 `/erp/sales/orders` 的已下达订单行上，不在发货单列表里。释放按该订单当前全部未发行与剩余数量整单成单；订单已无未发数量时释放会被拒绝。
