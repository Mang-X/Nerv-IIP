...
- 在 `/erp/finance/machine-overhead` 输入工单编号核对机器实绩工时与固定、变动预定分配额，展开“结算追溯”查看费率期间和版本、结算编号和版本。预定分配不是已核定实际机器成本。
- 按会计期间查询工作中心月度实际池与已分配额。差异为实际池减已分配额：正数是未分配，负数是多分配，零是无差异；按行核对币种。
- “不适用”表示没有适用的机器制造费用；“不可用”需先按原因补齐事实或重新结算／对账。明确零会显示数值零。冲销后应刷新复核有效结算，不沿用旧金额；被替代的月度记录仅供追溯。
