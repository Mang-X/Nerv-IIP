...
每项规则都展示当前值、判定阈值、单位、判定证据、来源事实名称和发生时间，不显示内部数据库标识。历史样本尚不足时会明确标为“历史数据积累中”，仍展示已有证据且不把未形成结论误报为正常或风险。真实现场场景验证依赖 [#1086](https://github.com/Mang-X/Nerv-IIP/issues/1086) 提供的数据与验收条件；本文只说明已交付的页面行为，不代表该真实场景已经运行。
