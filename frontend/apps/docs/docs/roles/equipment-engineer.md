...
报警确认使用 first-write-wins：重复请求不会覆盖首次确认人和确认时间。报警搁置使用意图键去重；网络超时后应先按报警 ID 回读生命周期，确认状态和搁置截止时间后再决定是否重试。报修建单和维修完工在同一次失败或超时后会复用同一意图键，服务端返回同一工单结果；只有权威回执确认成功后，下一次操作才使用新键。页面显示“请求已受理”不等于业务状态已经确认。
