...
| 2   | 在 `/equipment/telemetry/history` 按设备、采集标签和本地时间窗核对趋势、统计、报警/状态事件，再到 `/equipment/telemetry/oee` 按业务日看趋势，或按工作中心、产线、车间、班次横比 OEE 与 A/P/Q | 设备运行事实与效率可解释   | ✅ 已可用：数值采样以趋势图和原始明细双重呈现，非数值采样保留原值并降级为事件/明细；OEE 报表明确显示 UTC 查询窗口、历史站点时区/班次口径和服务端分页，缺失率值保留为缺失并逐桶说明降级原因，不画成 0% 或 100%。 |
