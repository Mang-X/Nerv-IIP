...
| 2   | 在 `/equipment/telemetry/history` 按设备、采集标签和本地时间窗核对趋势、统计、报警/状态事件，再到 `/equipment/telemetry/oee` 看 OEE | 设备运行事实与效率可解释   | ✅ 已可用：数值采样以趋势图和原始明细双重呈现，非数值采样保留原值并降级为事件/明细；OEE 使用设备生产运行时长与 MES 报工投影计算性能率、质量率和乘积，数据不足时会明确说明。 |
