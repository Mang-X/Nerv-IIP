...
8. 在 `/mes/dispatch` 和 `/mes/operation-tasks` 派工、开工、暂停、恢复或完工。PDA 的“工序执行”入口按当前账号真实授权范围读取任务；详情内部同时核对工单与工序任务两个标识，界面只展示工单号、工序任务号和设备名称/编码，业务显示字段缺失时会明确提示未提供。页面同时展示 SOP、门禁评估时间以及前序、物料齐套、设备和质量阻塞。可执行按钮以服务端最新返回为准，页面不会根据状态自行补出“开始”；冲突后会刷新，结果尚未核实时也不会显示成功。
