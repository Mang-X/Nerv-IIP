...
8. 在 `/mes/dispatch` 和 `/mes/operation-tasks` 派工、开工、暂停、恢复或完工。PDA 的“工序执行”入口按当前账号真实授权范围读取任务；详情内部同时核对工单与工序任务两个标识，界面只展示工单号、工序任务号和设备名称/编码，业务显示字段缺失时会明确提示未提供。页面同时展示 SOP、门禁评估时间以及前序、物料齐套、设备和质量阻塞；readiness 与完成拒绝中的前序原因均使用“工序 N / 等 N 道”，不显示内部任务 ID。可执行按钮以服务端最新返回为准，页面不会根据状态自行补出“开始”；冲突后会刷新，结果尚未核实时也不会显示成功。未知结果重试前若账号、组织/环境或作业范围已变化，页面不会发送旧操作，而是保留原结果与重试凭据并提示上下文已变化；恢复原身份后仍可安全重试，也可返回后改选其它任务。完整工序链接会等当前写作业范围核验完成后再打开详情；操作等待期间切换账号、组织/环境、作业范围、工序链接，或直接从列表改选另一道工序时，旧成功/失败结果会被丢弃并刷新当前列表，不会覆盖或关闭新选择。
