...
1. 在 `/planning` 查看需求；销售订单需求会显示真实订单号和有效/取消状态，点击订单号可钻取到 ERP 销售订单筛选页。运行 MRP，并检查 pegging 和计划建议仍沿用该订单号追溯。
   预测数量属于包含起止日的完整期间总量。系统先统一到计划单位并按配置消耗窗口扣除有效销售需求和已发布 MPS，再把剩余量以六位小数按日守恒分摊；当前 MRP 窗口只接收与它重叠日期的份额，不会把跨月或跨季度预测的全部余量集中到窗口末日。相邻窗口看到的是同一份稳定剩余预测，合并各日后仍精确等于期间剩余量。
