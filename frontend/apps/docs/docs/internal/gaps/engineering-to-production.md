...
- 完整 PLM/PDM、图纸审批、复杂变更影响分析和 `effectiveDate` 的未来生效延迟切换未形成当前可上手闭环。
...
- 文档仍需要用 SKU 编码、`itemCode`、MBOM/Routing 编号说明对象关系；页面侧需要更多从列表选择对象的入口来减少手工录入。

### 术语不清

- EngineeringItem 的 `ItemCode` 与 MasterData SKU 编码的关系容易被误读为两套编码，需要在页面文案和帮助文本中继续澄清。
