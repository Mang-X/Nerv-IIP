...
test('live 写路径：真实登录 → 选待检任务 → 录合格结果提交 → 幂等语义捕获 → 后端状态回读', async ({
  ...
  const charResponse = await page.waitForResponse(
  ...
  for (let i = 0; i < characteristics.length; i += 1) {
    ...
    const rowName = (await row.getByTestId('char-name').innerText()).trim()
    const item = characteristics.find((c) => (c.name ?? c.characteristicCode ?? '') === rowName)
    if (!item) {
      throw new Error(`无法把特性行「${rowName}」对应回计划特性响应（items 中无同名项）。`)
    }
