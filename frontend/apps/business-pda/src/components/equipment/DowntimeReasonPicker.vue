...
// "输入框空了、列表还按旧关键字过滤"的错位不只是界面矛盾：空结果会显示"没有匹配的停机原因"，
// 工人据此以为本组织没配这个码，转而选"不登记设备不可用"，本该记录的停机原因就丢了——正是本页
// 要防的事。所以关键字一空就重查全量。
//
// 关键字被**清空成空串**（点清除按钮、退格删到空）已由 `NvSearchBar` 自己 emit `search('')`
// 收口（#3128）。这条 watch 仍然保留，因为它多覆盖一种上游不管的输入：只剩空白字符
// （`"   "`）——对本目录同样是"没有关键字"，而 `NvSearchBar` 不 trim、也就不会补发。
