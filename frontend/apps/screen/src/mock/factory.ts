// 工厂总览大屏的 mock 数据源。
// 后端聚合 API 就绪后，只需把 fetchFactoryOverviewMock 换成真实 fetcher（同形状），
// 页面与 useScreenData 无需改动。当前后端缺车间维度聚合（见 #570），故先 mock。
...
function spark(): number[] {
  return Array.from({ length: 11 }, (_, i) => jitter(58 + i * 2.5, 14))
}

export function createFactoryOverview(): FactoryOverview {
...
export async function fetchFactoryOverviewMock(): Promise<FactoryOverview> {
  await new Promise((resolve) => setTimeout(resolve, 280))
  return createFactoryOverview()
}
