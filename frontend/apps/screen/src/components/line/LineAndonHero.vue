...
.lah.run .lah-core {
  background: radial-gradient(circle at 38% 32%, #7dedb2, var(--sb-green) 55%, #0b7a44);
  box-shadow: 0 0 44px rgba(69, 208, 137, 0.5);
  animation: lah-breathe 4s ease-in-out infinite;
}
.lah.attention .lah-core {
  background: radial-gradient(circle at 38% 32%, #ffe29a, var(--sb-amber) 55%, #8f6c14);
  box-shadow: 0 0 44px rgba(242, 193, 78, 0.55);
  animation: lah-breathe 2.4s ease-in-out infinite;
}
.lah.alarm .lah-core {
  background: radial-gradient(circle at 38% 32%, #ff9aa1, var(--sb-red) 55%, #8f1b22);
