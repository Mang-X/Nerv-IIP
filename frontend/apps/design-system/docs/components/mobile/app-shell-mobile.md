...
<script setup>
import { ref } from 'vue'
import { AppShellMobile, NavBar, TabBar, CellGroup, Cell, Tag } from '@nerv-iip/ui-mobile'
...
<template #phone>
  <AppShellMobile class="h-full">
    ...
    <div class="space-y-4 p-4">
      ...
      <CellGroup title="我的任务">
        <Cell v-for="t in tasks" :key="t.code" :title="t.product" :note="`${t.code} · ${t.station}`" arrow>
          <template #value>
            <Tag :variant="t.variant" size="sm">{{ t.label }}</Tag>
          </template>
        </Cell>
      </CellGroup>
      <CellGroup title="快捷入口">
        <Cell title="扫码报工" arrow />
        <Cell title="设备点检" arrow />
        <Cell title="物料申请" arrow />
        <Cell title="异常上报" arrow />
      </CellGroup>
    </div>
    <template #footer>
