...
<script setup>
import { MobileImage } from '@nerv-iip/ui-mobile'
...
<template #phone>
  <section>
    ...
    <div class="grid grid-cols-2 gap-3">
      <div>
        <MobileImage :src="photo" fit="cover" ratio="1" radius="lg" alt="设备照片" />
        <p class="mt-1 text-center text-xs text-muted-foreground">cover 裁切填满</p>
      </div>
      <div>
        <MobileImage :src="tall" fit="contain" ratio="1" radius="lg" alt="产线照片" class="bg-muted" />
  ...
  <section>
    ...
    <div class="flex items-center gap-3">
      <MobileImage :src="photo" radius="full" class="size-16" alt="操作员头像" />
      <MobileImage :src="photo" ratio="16/9" radius="md" alt="工位看板" class="flex-1" />
    </div>
  </section>
  <section>
    <p class="ds-mdoc-label">加载失败回退</p>
    <MobileImage :src="broken" ratio="16/9" radius="lg" alt="缺图示例" />
  </section>
</template>
...
```vue
<MobileImage :src="photo" fit="cover" ratio="1" radius="lg" />
<MobileImage :src="photo" fit="contain" ratio="16/9" />
```
...
```vue
<MobileImage :src="avatar" radius="full" class="size-16" />
```
