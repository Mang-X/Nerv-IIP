...
帕累托 TOP-N——缺陷 / 停机原因 / 不良代码等「**少数关键项**」分析的正确形态:名称 + 发丝级条 + 数量/占比 + **累计占比 Σ**(没有累计不算真帕累托,80/20 集中度要能直接读出)。条长按 TOP1 归一便于对比;轨道即 100% 全量口径,竖刻度标累计位置;TOP1 红 / TOP2 橙强调、其余青色渐弱;填充语义色渐隐(左实右消),不发光、不描边。底部汇总 = TOP 合计 vs 长尾。基于独立的 `--nv-scr-*` 令牌。
