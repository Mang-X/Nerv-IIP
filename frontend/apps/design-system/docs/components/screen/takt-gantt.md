...
<script setup>
import { ref } from 'vue'
import { TaktGantt } from '@nerv-iip/ui'
...
::: tip 容器
`TaktGantt` **自带** [`ScreenPanel`](./screen-panel) 容器,直接使用即可。宽组件用 `<ScreenDemo wide>` 占满整列。
:::
...
<TaktGantt title="节拍 Takt 45s" :rows="rows" :axis="axis" />
```
