...
<script setup>
import { OeeHero, ScreenPanel } from '@nerv-iip/ui'
</script>
...
::: tip 容器
`OeeHero` 是「裸内容」组件,本身不带边框背景,通常放进 [`ScreenPanel`](./screen-panel) 容器里使用。
:::
...
```vue
<OeeHero label="制程不良率 PPM" :value="318" delta="较昨日 -42" :spark="series" />
```
