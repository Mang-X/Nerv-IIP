...
<script setup>
import { nvMessage, nvNotification, NvNotifierHost, NvButton } from '@nerv-iip/ui'
...
function fireBurst() {
  ...
  setTimeout(() => nvMessage.info('已同步 MES 网关'), 140)
  setTimeout(() => nvMessage.warning('库存接近下限'), 280)
}
function fireNotification(kind) {
  const map = {
    info: () => nvNotification.info('排产已更新', { description: '今日计划重排，受影响工单 6 张。' }),
    success: () => nvNotification.success('保养工单已创建', { description: 'CNC-07 · 今晚 22:00 执行。' }),
    warning: () => nvNotification.warning('B 线物料不足', { description: '液压阀体 V3 缺口 452 件，已转采购申请。' }),
    error: () => nvNotification.error('派工失败', { description: '工作中心 WC-ASM-04 处于阻塞状态。' }),
  }
  map[kind]()
}
</script>
...
`nvMessage.{info|success|warning|error}(标题)`，适合无需详述的即时结果。
...
```vue
<script setup>
import { nvMessage } from '@nerv-iip/ui'
</script>

<template>
  <NvButton @click="nvMessage.success('保存成功')">成功</NvButton>
  <NvButton @click="nvMessage.error('网络连接中断')">错误</NvButton>
</template>
```

## 通知 Notification

`nvNotification.{info|success|warning|error}(标题, { description })`，右上角卡片，适合带上下文的异步结果。
...
```vue
<script setup>
import { nvNotification } from '@nerv-iip/ui'
</script>
