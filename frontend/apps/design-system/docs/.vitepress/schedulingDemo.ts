...
const WC: Record<
  string,
  { color: string; device: [string, string]; team: [string, string]; line: [string, string] }
> = {
  '激光切割-01': {
    color: 'cut',
...
export function makeModel(): ScheduleModel {
  ...
  for (const t of m.tasks) {
    ...
    t.dimensions = {
      workCenter: t.dimensions?.workCenter ?? {
        id: t.workCenterId ?? '',
        label: t.workCenterId ?? '',
      },
  ...
  const lineChange: ScheduleTask = {
    ...
    resourceId: '激光切割-01',
    workCenterId: '激光切割-01',
    dimensions: blockDims('激光切割-01'),
    startUtc: iso(7),
    endUtc: iso(9),
    blockKind: 'lineChange',
    locked: true,
    hasConflict: false,
    conflictReason: null,
  }
...
const blockDemoPlan = {
  ...
  resourceLoads: [
    {
      resourceId: '激光切割-01',
      windowStartUtc: blkIso(0),
      windowEndUtc: blkIso(6),
      assignedMinutes: 120,
...
const calPlan = {
  ...
  resourceLoads: [
    {
      resourceId: '激光切割-01',
      windowStartUtc: calIso(0),
      windowEndUtc: calIso(76),
      assignedMinutes: 180,
      availableMinutes: 480,
      utilization: 0.38,
    },
