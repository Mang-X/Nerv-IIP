...
1. **单一 src 目录承载全部前端职责。** 背景记录「项目将同时包含控制台应用、接口生成包、通用 UI 包与平台层能力包，单一 src 无法稳定承载这些职责」，因此改为 pnpm 工作区的 apps 与 packages 分层。
2. **采用 Nuxt 式运行时约束。** 背景记录「团队接受 Nuxt 风格的目录语义，但不接受 Nuxt 式运行时约束，也不希望引入过强的隐式魔法」：目录语义被保留，运行时约束与隐式魔法被排除。
3. **在 Vite+ 之上默认叠加 ESLint、Prettier、Turborepo。** 背景记录「Vite+ 已覆盖格式化、检查与任务编排的默认需求，前端不再默认叠加 ESLint、Prettier、Turborepo」。
4. **默认创建 src/app 作为启动引导层。** 决策第 5 条只在「启动引导的拆分复杂度明显升高时」才引入可选的 src/app；理由第 4 条记录「不强制 src/app 可以降低样板复杂度，同时保留在入口层变重时再抽离的空间」。

## 后果

1. 前端仓库会比单应用模板多出 apps 与 packages 分层，初始化成本略高。
2. 团队需要明确哪些能力属于应用内共享，哪些必须上提到共享包，否则边界会重新模糊。
