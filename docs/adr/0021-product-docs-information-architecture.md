...
- **PR 约定（轻量检查清单，非硬门禁）**：新增页面、修改业务流程或改变用户可见行为的
  PR，在描述中回答"是否影响产品文档（frontend/apps/docs）"；影响则同 PR 更新文档或引用
  后续补文档的 issue，不影响写"文档：无影响"。该约定由本 ADR 规定并由评审习惯维持，
  不接 CI 硬门禁。
...
1. **维持按交付批次组织的文档导航**：背景记录了落选理由——
   「现有导航按交付批次组织（"上手路径""流程图"），没有回答"我是谁、我该看什么"」，
   且 `internal/gaps/product-docs-overview.md` 已把"按角色入口"记录为缺口。
2. **把既有页面迁入象限目录、改变已发布 URL**：
   第 1 节选择「已发布页面 URL 不迁移（`processes/` 留在原地）」，
   后果记录了理由——「`getting-started/` 与 `processes/` 的 URL 保持不变，外部链接不断」。
3. **把文档影响检查做成 CI 硬门禁**：
   第 3 节记录了该选择——PR 约定「由本 ADR 规定并由评审习惯维持，不接 CI 硬门禁」，
   后果补充「执行力度依赖评审习惯，季度回收作为兜底」。
   原文只记录了这一选择与兜底手段，未保留硬门禁落选的理由。
...
- `product-docs.contract.test.ts` 扩展角色地图与象限结构守护；后续文档 PR 触碰结构时
  必须让契约测试与本 ADR 保持一致。
- 本 ADR 规定 PR 文档影响检查清单约定；执行力度依赖评审习惯，
  季度回收作为兜底。
