...
项目希望保留文件路由带来的低样板与类型化路由能力，同时避免过强的框架魔法。团队明确不采用自造中间件运行时，也不再使用 unplugin-vue-router 作为默认方案，而是转向 Vue Router 官方文件路由能力。
...
3. 路由访问控制、布局切换、组织上下文校验统一通过路由守卫 + route meta 实现，不引入独立中间件运行时概念。
...
2. 保留 router/index.ts 可以让守卫、history、错误处理与布局元信息保持显式控制点。
...
1. **继续以 unplugin-vue-router 作为默认文件路由方案。** 背景记录「不再使用 unplugin-vue-router 作为默认方案，而是转向 Vue Router 官方文件路由能力」；理由第 1 条记录官方文件路由插件「可以减少生态分叉，降低未来升级风险」。
2. **自造中间件运行时承担访问控制与布局切换。** 背景记录「团队明确不采用自造中间件运行时」，落选依据是同段的「避免过强的框架魔法」；决策第 3 条改为路由守卫加 route meta，「不引入独立中间件运行时概念」。原文未进一步展开中间件运行时的具体代价。
3. **把页面私有组件全部提升到全局 components 目录。** 背景记录「如果页面私有组件全部提升到全局 components，目录会迅速扁平化并产生命名膨胀」。
4. **重写 filePatterns 以排除页面私有 .vue 目录。** 决策第 6 条选择 exclude 规则并「不默认重写 filePatterns」；理由第 4 条记录「exclude 方案比重写 filePatterns 更稳，能保留官方默认命名语义，降低团队心智负担」。

## 后果

1. 团队必须遵守页面入口文件与私有目录的命名约定，否则容易出现误扫路由或目录风格漂移。
2. 页面目录树会比传统单文件页面更深，需要通过约定维持可读性。
3. 如果页面私有组件被过早上提，会削弱本 ADR 的收益；如果长期不上提，也可能形成重复实现，需要在代码评审中把握边界。
