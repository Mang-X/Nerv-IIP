# ADR 0002：应用接入契约与 Connector Host/Connector 协议

- 状态：已接受
- 日期：2026-05-13

## 背景

Nerv-IIP 的首要价值不是承载业务逻辑，而是稳定接入、发现、控制与观测真实应用实例。受管目标可能运行在 Docker、Windows Service 与自定义 HTTP 进程环境中，平台不能把接入方式绑定到单一部署形态。

平台需要区分应用目录事实、实例状态事实、执行动作编排与本地资源探测，不能把这些职责全部塞进同一个服务或脚本。任何启停、备份、恢复与日志拉取动作都需要审计、幂等与失败原因回传，否则平台无法形成可信运维闭环。契约如果不版本化，平台与 Connector Host 将在字段和能力演进过程中快速失配。

## 决策

1. 平台统一通过 Connector Host 接入受管环境，本地资源适配由 Connector 实现。
2. 首批路线图至少规划 Docker Connector、Windows Service Connector、HTTP Connector；第一迭代只实现 Docker Connector，用于验证注册、心跳与状态同步链路。
3. 应用接入契约固定包含五类基础能力：注册、心跳、能力声明、健康检查、实例状态同步。
4. 平台首批动作路线图覆盖启动、停止、重启、拉取日志、触发备份、触发恢复；第一迭代验证注册、心跳与状态同步，第二迭代先落地低风险重启闭环，日志拉取和更高风险动作继续后置。
5. 应用接入元数据至少包含应用标识、版本、部署目标、能力清单、节点信息、环境信息。
6. 心跳与状态同步必须分离：心跳只证明 Connector Host 或实例仍存活，状态同步负责更新实例事实。
7. 所有执行动作请求必须带幂等键，并形成审计记录、执行结果与失败原因。
8. 高风险动作后续再叠加审批与人工确认；在审批闭环完成前，不开放恢复、批量停止、跨环境批量执行等高风险动作。
9. 能力声明与动作契约必须版本化，平台按能力版本协商可执行动作与参数结构。
10. 主平台必须把 Connector Host 视为外部接入客户端；双方只通过 Platform SDK、版本化 Connector Protocol、公开 HTTP API 和 IAM 授权协作，不共享内部实现。
11. Connector Host 主版本必须与主平台主版本对齐；同一主版本内小版本可以低于主平台小版本，并由主平台尽量保持兼容。

## 理由

1. 用 Connector Host 统一接入层，可以把平台核心服务与目标环境的异构性隔离开。
2. Connector 模式能在不改变平台核心协议的前提下扩展新的宿主环境。
3. 将心跳与状态同步拆分，可以避免把存活证明和业务状态事实混为一个脆弱接口。
4. 把幂等、审计、失败回传作为协议硬约束，可以让运维动作具备可追溯性与可恢复性。
5. 对能力声明和协议做版本化，可以支持主平台与 Connector Host 在同一主版本内独立升级，降低小版本不同步时的破坏性。

## 已考虑的替代方案

**平台绕过 Connector Host、直接远程调用受管环境。** 后果第 3 条同时记录了这条备选与它的落选理由：「某些简单场景下，直接远程调用会比 Connector Host 协议看起来更省事，但长期会牺牲可观测性、一致性与审计闭环」。

**把应用目录事实、实例状态事实、动作编排与本地资源探测塞进同一个服务或脚本。** 背景明确排除该形态（「不能把这些职责全部塞进同一个服务或脚本」），并给出理由：启停、备份、恢复与日志拉取动作都需要审计、幂等与失败原因回传，否则「平台无法形成可信运维闭环」；实施说明第 2 条据此把 AppHub 与 Ops 的职责在接口设计中严格分开。

**把接入方式绑定到单一部署形态。** 背景写明受管目标可能运行在 Docker、Windows Service 与自定义 HTTP 进程环境中，「平台不能把接入方式绑定到单一部署形态」；理由第 1、2 条记录 Connector Host 统一接入层能隔离目标环境异构性，Connector 模式能在不改变平台核心协议的前提下扩展新的宿主环境。

**把心跳与实例状态同步合并为同一个接口。** 决策第 6 条要求两者必须分离；理由第 3 条给出的落选理由是合并会「把存活证明和业务状态事实混为一个脆弱接口」。

**不做版本化的能力声明与动作契约。** 背景记录的落选理由是「契约如果不版本化，平台与 Connector Host 将在字段和能力演进过程中快速失配」；理由第 5 条补充版本化能支持双方在同一主版本内独立升级，降低小版本不同步时的破坏性。

**主平台与 Connector Host 共享内部实现、要求按源码或小版本同步升级。** 决策第 10 条要求把 Connector Host 视为外部接入客户端，只通过 Platform SDK、版本化 Connector Protocol、公开 HTTP API 和 IAM 授权协作，「不共享内部实现」；实施说明第 4 条允许首批在单仓中用项目引用加快开发，但发布边界必须由契约表达，「不能要求 Connector Host 跟随主平台源码或小版本同步升级」。

## 后果

1. Connector Host 侧实现成本会上升，因为每类 Connector 都要遵守统一协议并回报结构化结果。
2. 平台需要维护能力版本兼容策略，而不是只靠最新版本覆盖所有节点。
3. 某些简单场景下，直接远程调用会比 Connector Host 协议看起来更省事，但长期会牺牲可观测性、一致性与审计闭环。
4. 日后扩展更多高风险动作时，协议层需要继续演进审批、授权与回滚信息。

## 实施说明

1. 第一迭代脚手架需要创建 Connector Host 最小宿主、Docker Connector 原型以及注册、心跳、状态同步三条基本链路的契约定义。
2. AppHub 负责应用目录与实例事实，Ops 负责执行动作、任务记录与审计闭环，二者职责需要在接口设计中严格分开。
3. v1 协议基线、共享契约落点与首批公开接口以 docs/architecture/connector-platform-protocol-v1.md 为准；若后续调整传输机制，应在该文档中增量演进而不是回退到口头约定。
4. 首批可以在单仓中用项目引用加快开发，但发布边界必须按 Platform SDK、版本化协议包、OpenAPI 或等价契约表达，不能要求 Connector Host 跟随主平台源码或小版本同步升级。
5. 第二阶段低风险动作闭环的当前落地范围以 docs/architecture/second-vertical-slice-ops.md 为准。

## 范围之外

1. 不在本 ADR 中决定 Connector Host 与平台之间最终采用 HTTP 长连接、轮询还是消息驱动。
2. 不在本 ADR 中定义每个动作的完整参数模型与错误码表。
3. 不在本 ADR 中覆盖审批流、人工确认 UI 与回滚策略的最终实现细节。
