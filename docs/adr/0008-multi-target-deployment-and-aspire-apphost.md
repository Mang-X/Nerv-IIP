...
Nerv-IIP 面向私有化、混合部署和工程联调场景。目标环境可能包含开发者本机、小规模 PoC、客户内网服务器、仅支持 Windows 的环境、Linux 服务器、具备容器运行时的环境，以及不能使用容器的受限环境。
...
7. Connector Host 必须保持可独立安装和独立升级；它既可以被纳入 Aspire/Compose 联调，也可以作为 Windows Service、systemd 服务或压缩包部署在受管节点上。
...
2. Aspire AppHost 首批至少覆盖 PostgreSQL、Redis、MinIO、Qdrant、OpenTelemetry Collector 和平台服务之间的引用关系；RabbitMQ 作为 `Messaging:Provider=RabbitMQ` 时启用的可选资源。
...
5. Windows 整合安装脚本建议负责 .NET 运行时/SDK 检查、配置目录创建、服务注册、端口检查、连接串写入和健康检查。
