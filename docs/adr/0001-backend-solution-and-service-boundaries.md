...
Nerv-IIP 后端需要同时承载身份与权限、对外授权、应用目录与实例发现、运维编排、AI 工具治理、知识检索以及前端聚合接口。0 到 1 阶段既不能把所有平台能力压成单体，也不能过早兑现成完全独立部署的高运维成本微服务群。
...
2. 逻辑边界固定为 IAM、AppHub、Ops、AI Integration、Knowledge、Gateway/BFF、Agent Host 七类能力；其中 IAM 统一拥有平台身份、权限和对外授权事实。
3. Agent Host 与 Connector 物理上保持在独立的 agents 根目录中，不并入 backend solution。
...
8. common 只允许沉淀 Contracts、Hosting、Observability、Testing 等窄共享库，禁止创建 SharedKernel、Common、Utils 一类无边界聚合库。
...
5. 平台 HTTP 服务的默认命名需要统一调整为 .Web、.Domain、.Infrastructure 主线；.Host 只保留给 Agent 宿主或后台进程类工程。

## Implementation Notes

1. 首批脚手架至少落下 PlatformGateway、Iam、AppHub、Ops 四个最小服务骨架，以及 common/Observability、common/Contracts、common/Testing 三个窄共享库；Iam 骨架需要预留用户、角色、权限、外部客户端和授权授予的基础边界。
2. Agent Host 与 Connector 需在 agents 根目录下建立独立 solution，但沿用同一套构建与版本治理约定。
3. 平台 HTTP 服务的默认命名应采用点分 PascalCase，例如 Nerv.IIP.AppHub.Web、Nerv.IIP.AppHub.Domain、Nerv.IIP.AppHub.Infrastructure；Agent 宿主仍可保留 Nerv.IIP.AgentHost.Host 这类命名。
