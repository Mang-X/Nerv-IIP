...
项目已决定以 netcorepal-cloud-framework 作为后端主框架，但必须保证领域边界优先于框架能力堆叠。团队需要一种既能支持后续服务拆分，又能在早期维持开发效率的解决方案组织方式。过宽的共享库会快速侵蚀服务边界，导致项目重新退化成逻辑耦合的大单体。

## 决策

1. 后端平台服务采用单一解决方案管理多服务代码库，根目录固定为 backend/Nerv.IIP.sln、Directory.Build.props、Directory.Packages.props、services、gateway、common、tests。
...
3. Connector Host 与 Connector 物理上保持在独立的 connector-hosts 根目录中，不并入 backend 解决方案。
...
1. 单一解决方案能降低 0 到 1 阶段的协作与调试成本，同时保留服务级边界。
...
**把全部平台能力压成单体。** 背景把它与相反的极端一并排除：「0 到 1 阶段既不能把所有平台能力压成单体，也不能过早兑现成完全独立部署的高运维成本微服务群」；原文只记录了该排除结论，未单独保留单体一侧的落选理由。

**早期即兑现为完全独立部署的微服务群。** 同一句把它的落选理由记为「高运维成本」；决策第 10 条据此改为物理部署允许在早期合并，但代码边界、数据库边界与契约边界不因部署合并而回退，理由第 6 条把这一取舍表述为先冻结逻辑边界、再保留物理部署弹性。

**Application 作为默认独立项目。** 决策第 5 条写明 Application「作为 Web 项目内部目录存在，而不是默认独立项目」；理由第 3 条给出的落选理由是内部目录既能保留命令、查询、事件处理器的职责分区，又能「避免为早期服务引入过多项目数量」。

**Contracts 作为每个服务的默认项目层。** 决策第 6 条否决该默认形态，只在「确有跨进程共享 DTO、协议或 SDK 抽象时才按需拆出」；原文只记录了这条按需条件，未展开更多落选理由。

**Gateway 预设为重型多层项目拆分。** 决策第 7 条选择「默认采用薄 Web 服务，而不是预设为重型多层项目拆分」；原文只记录了该选择，未保留重型拆分一侧的落选理由。

**SharedKernel、Common、Utils 一类无边界聚合共享库。** 决策第 8 条明令禁止创建此类库。落选理由在背景与理由第 5 条均有记录：「过宽的共享库会快速侵蚀服务边界，导致项目重新退化成逻辑耦合的大单体」，因此对 common 实施窄共享约束以「从制度上抑制跨服务耦合蔓延」。

**把 Connector Host 与 Connector 并入 backend 解决方案。** 决策第 3 条要求二者物理上保持在独立的 connector-hosts 根目录中；实施说明第 2 条记录的理由是双方通过 Platform SDK 与版本化公开契约协作，且「主平台与 Connector Host 在同一主版本内不要求同步发布或同步小版本升级」。

**通过共享数据库表实现跨服务协作。** 决策第 9 条禁止该做法，后果第 4 条进一步要求即使开发环境合并部署也「坚持独立 schema、独立契约和独立审计边界」；原文只记录了这条禁止与随之而来的边界要求，未展开更多落选理由。

**平台 HTTP 服务沿用 `.Host` 命名主线。** 后果第 5 条把平台 HTTP 服务的默认命名统一为 `.Web`、`.Domain`、`.Infrastructure`，`.Host` 只保留给 Connector Host 宿主或后台进程类工程；理由第 2 条给出的落选理由是三项目主线「更贴近 netcorepal-cloud-framework 官方项目结构」，能减少框架落地时的额外适配成本。
...
5. 解决方案模板中应明确每个服务的 Web、Domain、Infrastructure 项目命名规则，以及 Contracts 何时作为按需例外出现。
6. README、上下文映射（context map）与仓库布局（repo layout）文档必须与本 ADR 保持一致。
