...
2. 0 到 1 阶段优先使用同一 PostgreSQL 集群下按服务隔离 schema 的方式控制运维成本；当工具链、AppHost 资源（resource）或 `EnsureCreated()` 本地验证要求更强隔离时，可以在同一集群内按服务拆分数据库（database），并继续保留服务自有 schema。
3. 缓存与分布式协作默认采用 Redis。
4. 应用级缓存默认采用 FusionCache，使用进程内 L1 缓存加 Redis L2 缓存；多实例部署需要启用 Redis backplane 以同步本地缓存失效。
5. 异步集成事件与后台异步任务分发通过 `Messaging:Provider` 与持久化 profile（配置方案）解耦；默认 `InMemory` 使用 CAP 进程内传输，`RabbitMQ` 作为多实例、跨进程和生产扩展 profile。
6. 对象文件、备份包、日志归档附件等二进制内容默认采用 MinIO。
7. File Storage 服务拥有文件元数据、访问授权、上传下载会话、Upload Provider 抽象、对象存储键（key）与保留策略；其它服务通过 File Storage API/SDK 引用 `fileId` 或 `FileReference`，不直接暴露对象存储键。
8. 向量索引与相似检索默认采用 Qdrant。
9. 观测基线统一采用 OpenTelemetry；可视化与观测聚合作为部署期可选适配，不默认绑定特定商业平台或收费体系。
10. 日志、审计、业务事务数据必须分开建模，不允许共用同一语义表或同一 retention（保留）策略。
11. 跨服务状态传播优先通过集成事件完成，禁止通过共享数据库表或直接写入他服务 schema 的方式协作。
12. 数据库替换采用显式 database profile（数据库配置方案），而不是让业务代码直接感知具体 provider（提供程序）；默认 profile 为 PostgreSQL。
...
15. 后端应用代码统一依赖 `Microsoft.Extensions.Logging` 抽象；服务宿主默认采用 Serilog 作为结构化日志 provider（提供程序），并通过 Console 和 OpenTelemetry/OTLP 输出日志事件。
...
19. 内置日志持久化 profile 采用滚动 JSONL 热日志、Log Archive Worker、File Storage 压缩 chunk（分块）和独立 `observability` 索引元数据组合；原始日志正文不进入业务 schema，也不复用 Ops `AuditRecord`。
20. `observability` 索引数据库默认使用 PostgreSQL 独立 schema 或独立 database；SQLite 只允许作为单机诊断索引，Elastic/OpenSearch/ClickHouse 等只作为外部增强 adapter（适配器）。

## 理由

1. 这组基础设施兼顾私有化可落地性、社区成熟度与后续演进空间，适合作为通用应用管理平台的默认基线。
2. 在同一 PostgreSQL 集群下按服务隔离 schema 或服务级 database，能在保持服务级数据边界的同时降低早期运维复杂度。
3. 统一消息 provider 配置、缓存、对象存储与观测方案，可以减少平台服务之间的技术碎片化，同时让单机部署不承担不必要的 broker（消息代理）依赖。
...
3. 某些 Microsoft-first（Microsoft 优先）或信创客户环境可能会要求 SQL Server、GaussDB、DMDB 或其它数据库替代，这会形成后续 profile 适配工作。
4. 统一基线有助于减少认知负担，但也意味着少量特殊服务不能随意自行选型。
5. 团队需要显式管理缓存键、TTL、tag（标签）、失效策略和 fail-safe（故障安全）边界，避免缓存把权限、会话、动作状态或审计事实变成隐形真相源。
...
1. infra 层需要提供 PostgreSQL、Redis、MinIO、Qdrant、OpenTelemetry 的本地开发编排；RabbitMQ 保留为 `Messaging:Provider=RabbitMQ` 时启用的可选资源。完整平台拓扑后续通过平台级 Aspire AppHost 承接，Docker Compose 作为生成或兜底目标。
2. backend/services/FileStorage 需要封装文件元数据、上传下载授权、Upload Provider 抽象和 MinIO/object storage（对象存储）适配；业务服务不直接把对象存储 key、tus endpoint 或 S3 multipart 细节当成长期公开契约。
...
5. 跨服务 IntegrationEvent 默认采用 CAP outbox 或等价可靠发布机制，outbox 记录必须与领域事实处于同一事务边界；默认单机 profile 使用 CAP InMemory message queue（消息队列），只承诺进程内分发，不承诺跨进程投递。
...
13. Log Archive Worker 只能处理已关闭或具有稳定 checkpoint（检查点）的滚动日志文件，上传前必须压缩、计算 `sha256` 并记录时间范围、服务、实例、level（级别）、`operationTaskId`、`correlationId`、`traceId` 和过期时间等索引元数据。
...
3. 不在本 ADR 中决定 Microsoft-first（Microsoft 优先）或信创环境下的具体替代产品选型、验收证书和生产迁移脚本。
4. 不在本 ADR 中冻结每个业务查询的缓存键、TTL 和 tag（标签）明细；这些规则由 docs/architecture/caching-baseline.md 和具体服务实现承接。
