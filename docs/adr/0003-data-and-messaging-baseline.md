...
5. 异步集成事件与后台异步任务分发通过 `Messaging:Provider` 与持久化 profile 解耦；默认 `InMemory` 使用 CAP 进程内传输，`RabbitMQ` 作为多实例、跨进程和生产扩展 profile。
...
3. 统一消息 provider 配置、缓存、对象存储与观测方案，可以减少平台服务之间的技术碎片化，同时让单机部署不承担不必要的 broker 依赖。
...
5. 跨服务 IntegrationEvent 默认采用 CAP outbox 或等价可靠发布机制，outbox 记录必须与领域事实处于同一事务边界；默认单机 profile 使用 CAP InMemory message queue，只承诺进程内分发，不承诺跨进程投递。
