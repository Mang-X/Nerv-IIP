...
3. **本 ADR 之后新开的业务字节面不得把 FileStorage 的 download grant id 交给调用方。** grant id 是 FileStorage 全服务共用命名空间，其兑换面不校验用途；一旦交给调用方，任一业务门面的读权限持有者都能兑换其它门面签发的 grant。新开业务面的下载授权必须由网关在服务端签发并立即兑换，对外只暴露以业务标识（如 `fileId`）为入参的单跳字节路由。
   ...
   **既有存量：已登记的缺口，本 ADR 既不豁免也不追认。** 本 ADR 之前已经开出、且把 FileStorage
   download grant id 交给调用方的业务文件面继续存在，本决策不追溯要求其改造。**但本决策不主张它
   安全，也不为它设立任何条件式豁免。** 它带有与本条所禁止者同类的跨口径兑换风险：grant id 出自
   全服务共用命名空间、其兑换面不校验用途，因此「签发该 id 的权限口径」与「能兑换该 id 的权限
   口径」是否一致，不由该存量面自己决定，也不是它能够单独保证的性质。**该风险当前未经论证排除。**

   存量面的收敛路径有两条：改成「授权在服务端签发并立即兑换、不交出 id」，或由 FileStorage 提供
   按 grant id 反查其所属文件的能力，使网关能在兑换时复核用途。两条路的取舍取决于 FileStorage
   是否提供该能力，属实现裁量，不由本 ADR 决定。

   本条对新开业务面的禁令不因这条存量而放宽——它的作用正是不再扩大这个暴露面。
...
7. 决策 3 的既有存量是**已登记、未论证安全的缺口**，不是豁免。本 ADR 不为它设立条件式判据——一条「条件成立就安全」的表述会让后来人以为此处已经论证过，而实际并没有。它的风险状态与收敛责任由承接票承担，不由本 ADR 维护。
