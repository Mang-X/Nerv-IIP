...
3. **本 ADR 之后新开的业务字节面不得把 FileStorage 的 download grant id 交给调用方。** grant id 是 FileStorage 全服务共用命名空间，其兑换面不校验用途；一旦交给调用方，任一业务门面的读权限持有者都能兑换其它门面签发的 grant。新开业务面的下载授权必须由网关在服务端签发并立即兑换，对外只暴露以业务标识（如 `fileId`）为入参的单跳字节路由。
   ...
   **既有存量例外（登记，不追认为合规形状）**：本 ADR 之前已经开出、且把 download grant id 交给调用方的业务文件面继续存在，本决策不追溯要求其改造。该豁免**不是无条件的**。判据不是「某一条路由自己只有一个权限码」——单条路由天然只有一个，那样写永远成立、等于没有条件。真正的判据是**接受调用方 grant id 的网关路由的集合**：**当且仅当该集合里所有路由要求同一个权限码时，交出 grant id 才不构成跨口径兑换**。只要集合中出现第二个权限码，一个口径签发的 grant 就可能被另一个口径的持有者兑换，豁免即告失效。

   **本决策不主张该条件当前成立。** 存量面属登记在案的已知缺口，不是「条件满足所以安全」；决策 3 对新开面的禁令正是为了不再扩大这个集合。集合当前的成员与各自权限码是随实现变化的事实，由权限口径与网关路由本身承担，不在本 ADR 维护。
...
7. 决策 3 的存量例外是**已登记的缺口，不是已论证安全的豁免**。它的判据是「接受调用方 grant id 的网关路由集合是否只有一个权限码」，而不是任何单条路由自己的权限码数量——后者恒为一，用它做判据会得出恒真的假安全结论。该集合的当前成员由权限口径与网关路由承担，不由本 ADR 维护。
