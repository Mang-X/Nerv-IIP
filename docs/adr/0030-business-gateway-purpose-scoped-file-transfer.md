...
- 自我修订（本 ADR 决策 3）：适用范围由「本 ADR 之后新开的**业务**字节面」扩为「**网关暴露的**文件字节面」，并取消既有存量的不追溯登记。扩面把 PlatformGateway 的文件字节面纳入本 ADR 约束，因此本 ADR 的适用对象不再只有 BusinessGateway（标题保留原措辞，不就地改写历史标题）。理由与复评条件写在决策 3 内；替代方案 3 的原文保持不动，其中被推翻的一句另附说明。
...
3. **网关暴露的文件字节面不得把 FileStorage 的 download grant id 交给调用方。** grant id 是 FileStorage 全服务共用命名空间，其兑换面既不校验用途、也不记录签发门面；一旦交给调用方，任一门面的读权限持有者都能兑换其它门面签发的 grant，而 IAM 对 user 主体只按权限码判定、不看资源归属，因此**没有任何一层能在兑换时识别归属**。下载授权必须由网关在服务端签发并立即兑换，对外只暴露以业务标识（如 `fileId`）为入参的单跳字节路由。
   ...
   **本条对所有网关文件字节面成立，不按开出时间区分。** 本条初版只约束「本 ADR 之后新开的业务字节面」，并为既有面登记了一条不追溯的说明；该范围限定已取消——按开出时间划线会让「某条路由开得早」成为继续交出 grant id 的理由，而本条要禁的那个机制与路由何时开出无关。
...
7. 决策 3 使「以 download grant id 为入参的网关路由」成为一个**必须为空的集合**。因此它的承担方不是逐条路由的运行期判断，而是公开契约的形状：网关契约里出现任何以 grant id 为入参、或把 grant id 交给调用方的路由，都是对本条的违反，无论其权限码写成什么。
