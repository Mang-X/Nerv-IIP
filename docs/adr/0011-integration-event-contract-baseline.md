...
1. **把 NuGet 包版本、应用版本或数据库迁移版本作为事件版本。** 理由第 3 条记录「使用正整数事件版本比把 NuGet 包版本、应用版本或数据库迁移版本混入事件契约更直接，也更适合 routing key 绑定与并行迁移」。
2. **以服务内部模型作为公开 schema 的事实来源。** 决策第 12 条记录「服务内部 Domain event、EF entity、Infrastructure model、queue message adapter 和测试 fixture 都不得成为公开 schema 的事实来源」；背景记录「集成事件是公开契约，不是服务内部领域事件的直接序列化结果」，理由第 5 条记录以 `backend/common/Contracts/` 作为事实来源可以「避免服务内部模型泄漏成跨进程契约」。
3. **发布方与消费者各自复制一份事件 DTO。** 决策第 13 条记录双方「通过 Contracts 或 Platform SDK 消费同一契约，不复制 DTO」。原文只记录了该选择，未保留落选理由。
4. **在事务提交前直接把跨服务事件发送到具体 broker。** 决策第 18 条记录「不得在事务提交前直接把跨服务事件发送到具体 broker」；理由第 6 条记录 CAP outbox 或等价机制把发布与领域事实提交放在同一事务边界，「可以避免数据库事实已提交但消息丢失，或消息已发出但事实回滚」。
