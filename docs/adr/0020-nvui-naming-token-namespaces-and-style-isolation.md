...
> 直接以 `Nv*` canonical 名落地（§1.2 **R1** — 全新组件、无存量映射）：
> - `NvMetricRing`（环形构成指标：多色分段环 + 图例联动，表达「部分与整体」）
> - `NvMetricStrip`（横向多指标条：一卡多口径，分隔线代卡缝）
>
> 判定依据（R1→R2）：二者与既有 `NvMetricCard` 是**结构不同的独立件**而非其变体——
> `NvMetricRing` 是横向 gauge + 图例的构成图，`NvMetricStrip` 是横排多 cell 的指标条，
> 均无法并入 `NvMetricCard` 的纵向单值骨架（故不走"一件多模式"）。素名优先 → `Nv` + 语义名
> （`MetricRing`/`MetricStrip`），与大屏层既有 `NvRingGauge`（纯仪表原语、`--nv-scr-*` 深色）
> 分属不同表面、按"跨表面建两件"各自命名，无 clash。文件名即 `NvMetricRing.vue`/
> `NvMetricStrip.vue`，内部无 `*-pro`/`.ds-*`/`.sb-*`。已并入
