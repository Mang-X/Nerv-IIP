...
- 执行批次：本 ADR **不改任何代码**。库侧改名与层落地 = MAN-433；分应用代码转换 =
  MAN-435；守护与收口 = MAN-436。这些批次的执行输入是逐件映射表（原附录 A/B/C），
  已移出本 ADR，住在 [`nvui-naming-map.md`](../../frontend/DESIGN/nvui-naming-map.md)。
...
2. **PC 层取素名**：`pc/`（去 `Pro` 后缀）、`blocks/` 活跃件、`layout/` 全部升级为
...
3. **场景层与 PC 潜在同名者保留场景词根**：`NvScreenButton`、`NvMobileDialog`、
   ...
   判定按 1.2 流程，逐件结果冻结在 [`nvui-naming-map.md`](../../frontend/DESIGN/nvui-naming-map.md)
   附录 A。
...
5. **非组件导出不改名**：composable（`useTheme`、`useScreenData`、`useSidebar`…）、
   ...
   全表见 [`nvui-naming-map.md`](../../frontend/DESIGN/nvui-naming-map.md) 附录 A）。
   `messagePro`/`notificationPro` 两个函数名的 `Pro` 尾巴保留到收口批
...
- **R2** 素名与 shadcn 原版导出名相同，或与 PC 层（pc/blocks/layout 改名后）素名相同
...
`blocks/DataTable`、`blocks/DataTablePagination`（由 `NvDataTable`/`NvPagination`
...
│                       迁移为 --nv-brand、--nv-success、--nv-ease-out-quart…
│                       （全表见 frontend/DESIGN/nvui-naming-map.md 附录 C）
├ 场景命名空间          screen --nv-scr-*（现 --sb-* 全表迁移，见
│                              frontend/DESIGN/nvui-naming-map.md 附录 B）
...
[`nvui-naming-map.md`](../../frontend/DESIGN/nvui-naming-map.md) 附录 C 改名同步）之外，新增：
...
3. **screen 命名空间**：`--nv-scr-*` 全表（[`nvui-naming-map.md`](../../frontend/DESIGN/nvui-naming-map.md)
   附录 B 30 项）存在；别名期断言
...
| 步 | 批次 | 内容 | 验收口径 |
|---|---|---|---|
| S1 别名过渡 | MAN-433 | 库侧：按 [`nvui-naming-map.md`](../../frontend/DESIGN/nvui-naming-map.md) 附录 A 新增 `Nv*` 为规范实现名；旧名以 `export { NvButton as ButtonPro }` 形式保留并标 JSDoc `@deprecated`（IDE 划线）；token 新名落地 + 旧名 var 别名（`--sb-bg: var(--nv-scr-bg)`）；4.1/4.3 层结构落地；4.4 契约守护（别名期形态）上线；design-system 站组件页标题/示例切新名（文档站是唯一教学面，先切断旧名传播） | `pnpm -C frontend typecheck && test && build` 全绿；契约测试别名期断言绿；文档站构建绿 + 实机抽查（亮/暗/大屏）；既有业务代码**零改动**仍编译通过（别名兜底的直接证据） |
| S2 按应用进行代码转换 | MAN-435 | 按应用逐个 PR 替换导入与模板引用：business-console（93 页）→ business-pda（15）→ screen（10）→ console（10）。代码转换以 [`nvui-naming-map.md`](../../frontend/DESIGN/nvui-naming-map.md) 附录 A 为唯一映射输入（脚本 + 人工复核），同 PR 内完成该应用的 `--sb-*`/类名引用替换 | 每个应用 PR：typecheck/test/build 三绿 + 真机逐模块抽查；该应用源码旧名/旧 token **零匹配**（CI grep）；不允许半迁移合入 |
...
2. **全场景强制词根（screen 件一律 NvScreenXxx）**：判定简单，但 `NvScreenOeeHero`、
   ...
   （NvScanBar/NvOeeHero）矛盾。被否，改用 R1–R5 判定流程 + [`nvui-naming-map.md`](../../frontend/DESIGN/nvui-naming-map.md) 附录 A
   冻结逐件结果。
...
必须依赖 [`nvui-naming-map.md`](../../frontend/DESIGN/nvui-naming-map.md) 附录 A
而非人脑记忆；类型名连带改名（`DataTableProColumn` 等）会出现在业务代码签名里，
代码转换须覆盖类型导入。

**后续维护者须知。** [`nvui-naming-map.md`](../../frontend/DESIGN/nvui-naming-map.md)
的三份映射表（原附录 A/B/C）是 MAN-433/435 的执行合同，执行中发现映射遗漏先修订
那份映射表再动代码；新组件命名一律走 1.2 流程，R3b 专名词表扩词须在 PR 中注明；
...
按 `docs/architecture/decision-record-governance.md` 的分层判据，逐件映射表随代码增长
（每新增一个组件就要增补一次），不属于「只被新决策取代」的决策内容，因此三份全表已移出
本 ADR，落在 [`frontend/DESIGN/nvui-naming-map.md`](../../frontend/DESIGN/nvui-naming-map.md)：

| 原小节 | 内容 |
| --- | --- |
| 附录 A | 组件旧名 → 新名完整映射表（含 A1–A7 分层与「不参与改名的导出」） |
| 附录 B | `--sb-*` → `--nv-scr-*` token 全表映射（30 项） |
| 附录 C | PC 自有语义 token nv 化清单 |

本 ADR 保留判定规则本身（1.2 判定流程、1.3 处置、决策 3 的命名空间划分与契约层冻结
名单）。**新增或改名组件时更新那份映射表，不要回头编辑本 ADR**；若需要改动判定规则，
那是决策变更，走新 ADR。
