...
- **旧兼容入口：** `docs/architecture/implementation-readiness.md` 以及 M2 明确登记的迁移兼容页只负责导航，不得重新添加项目状态、报告正文、实现说明、事故过程或裁决。
...
- **当前架构：** 先读 `docs/architecture/README.md`，只描述当前组件、边界、事实所有权、依赖方向和交互，不混入项目进度、历史报告或事故过程。
- **任务进度与证据：** 留在 GitHub/Linear 和对应 PR，不复制为仓库长期总账。

不得为状态页、报告目录或迁移分类新增永久 manifest、自然语言 checker、生成脚本或独立 CI step。
