...
- **Architecture：** 当前组件、边界、事实所有权、依赖方向和交互从 `docs/architecture/README.md` 路由；当前正文进入其主题目录，不混入项目进度、Product/Runbook/Reference/Governance 正文或历史报告。
...
- **Architecture 根目录：** M4 后只保留 `docs/architecture/README.md` 与 Current Architecture 主题目录。旧平铺 `.md` 路径已退役；当前文档、README、Agent 指令和代码旁说明只能链接 canonical owner，不得重新建立永久 shim。
- **冻结历史引用：** ADR、Report、Status archive 与 `docs/superpowers/**` 中的旧 Architecture URL 不为迁移清理批量改写；它们只反映当时上下文。遇到旧路径时通过对应历史提交或 Git 追溯，不因历史坐标不存在而恢复兼容页。
