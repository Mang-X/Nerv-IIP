...
| 设备监控（IoT） | `/equipment/telemetry/connectors` | 已落地（MAN-442/#796，只读状态墙） | 采集健康连接器卡片墙，消费 `listBusinessConsoleTelemetryConnectorCollectionHealth`（AppHub 连接器采集健康列表，读用 `business.iiot.telemetry.read`）；展示连接器名/协议类型（OPC UA·Modbus·MQTT，来自 sourceSystem）/派生状态（在线·断线·采集停滞）/心跳/采样/采样速率（前端按 counter epoch 相邻轮询差值算）/丢样/错误；断线（心跳失联）红色置顶、采集停滞（心跳在线但指标停更）单列区分。10 秒自动轮询。当前只读，无 CRUD（配置面仍在 Connector Host 边界）；点击卡片展开采集明细，逐条 tag 清单需 connector→tag 读面（缺口，见 #947），暂不伪造。不显示 organization/environment/debug/source metadata。 |
