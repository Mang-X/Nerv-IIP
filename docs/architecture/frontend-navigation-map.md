...
| 设备监控（IoT） | `/equipment/telemetry/connectors` | 已落地（MAN-442/#796，只读状态墙） | 采集健康连接器卡片墙，消费 `listBusinessConsoleTelemetryConnectorCollectionHealth`（AppHub 连接器采集健康列表，读用 `business.iiot.telemetry.read`）；展示连接器名/协议类型（OPC UA·Modbus·MQTT，来自 sourceSystem）/派生状态（在线·断线·异常停止，仅由无歧义事实派生）/心跳/采样/采样速率（前端按 counter epoch 相邻轮询差值算）/丢样/错误；断线=心跳超窗停报（真断连，红色置顶，断线时长基于冻结的最后心跳单调增长）、异常停止=连接器自报终态（成因未必是断连，单列区分不显示断线时长）；采集停滞不做派生（LastSampleAtUtc 是源采样活动非采集存活，静默源/时钟偏差不可靠），仅展示原始吞吐事实由操作员判断。10 秒自动轮询。当前只读，无 CRUD（配置面仍在采集侧边界）；点击卡片展开采集明细，逐条 tag 清单需 connector→tag 读面（缺口，见 #947），暂不伪造。不显示 organization/environment/debug/source metadata。 |
