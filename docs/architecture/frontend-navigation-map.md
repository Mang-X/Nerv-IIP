...
| 设备监控（IoT） | `/equipment/telemetry/connectors` | 部分落地（MAN-442/#796，状态墙 tranche；#796 拔线验收未满足→#951、connector→tag→#947 未收口，收口后再标已落地） | 采集健康连接器卡片墙，消费 `listBusinessConsoleTelemetryConnectorCollectionHealth`（AppHub 连接器采集健康列表，读用 `business.iiot.telemetry.read`）；展示连接器名/协议类型（OPC UA·Modbus·MQTT，来自 sourceSystem）/派生状态（在线·断线·异常停止·待采集，仅由无歧义事实派生）/心跳/采样/采样速率（前端按 counter epoch 相邻轮询差值算）/丢样/错误；断线=心跳超窗停报（真断连，红色置顶，断线时长基于冻结的最后心跳单调增长）、异常停止=连接器自报终态（成因未必是断连，单列区分不显示断线时长）、待采集=在心跳且 running 但无采样证据（无 mapping/未采集，unknown，不计入在线）；采集停滞不做派生（LastSampleAtUtc 是源采样活动非采集存活，静默源/时钟偏差不可靠），仅展示原始吞吐事实由操作员判断。**注：#796「拔线后 1 个轮询周期内反映断线」验收无法由现有协议事实满足（Host 掉线受 2 分钟心跳窗口约束，现场链路拔线时 Host 仍心跳只会派生异常停止），需连接存活事实+端到端拔线时序，已拆 follow-up #951。**10 秒自动轮询。当前只读，无 CRUD（配置面仍在采集侧边界）；点击卡片展开采集明细，逐条 tag 清单需 connector→tag 读面（缺口，见 #947），暂不伪造。不显示 organization/environment/debug/source metadata。 |
