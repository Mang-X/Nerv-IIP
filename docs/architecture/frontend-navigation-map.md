...
| 设备监控（IoT） | `/equipment/telemetry/connectors` | 代码已落地（MAN-442/#796，#947/#951；Docker 真实拔线验收 3/3） | 采集健康连接器卡片墙每 10 秒刷新，分别展示 Host 心跳、现场连接 `unknown/alive/lost`、collector health/counters 和 tag sample presence，不从采样静默反推断线。现场连接断开与采集主机离线使用不同原因和时长基准；配置停用、启用失败、已激活但等待首条数据、已采样保持独立文案。点击卡片才按 canonical `collectionConnectorId` 懒加载 current manifest，折叠时不请求；`manifestStatus=unavailable` 表示旧 Host 尚未上报，不等于空配置。coverage 只说明 sample presence，不宣称质量或新鲜度。真实 `pwsh scripts/verify-connector-health-disconnect.ps1 -Runs 3` 当前代码头的最近成功证据为 3/3（端到端 3181/1213/1267 ms，最大 3181 ms），满足固定 10 秒 deadline。当前只读，无配置 CRUD；不显示 organization/environment/debug/hash/revision。 |
