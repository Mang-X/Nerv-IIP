...
| 制造执行 | `/mes/work-orders`、`/mes/work-orders/:workOrderId` | 已落地（FE-8 金标准） | 列表按 FE-4 原型重做（PageHeader + 来源条 + 派工分组卡 + SectionCards + Toolbar[状态/工作中心] + DataTable + 分页）；急单与生产报工改 Dialog（报工对象只读、上下文带入）；详情页改 PageHeader + SectionCards + 工序/用料 DataTable。详情不是常驻菜单项。 |
...
| 制造执行 | `/mes/dispatch` | 已落地（FE-8 金标准） | 派工看板按 FE-4 原型重做（PageHeader + SectionCards + Toolbar + DataTable + 分页 + 阻塞原因可读化）；长期应消费 APS/设备 readiness 结果。 |
...
| 制造执行 | `/mes/receipts` | 已落地（FE-8 金标准） | 完工入库按 FE-4 原型重做（PageHeader + SectionCards + Toolbar + DataTable + 分页）；新增入库改 Dialog，工单/物料只读由工单详情或报工完成带出。 |
