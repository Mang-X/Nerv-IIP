...
3. 后续按 provider 扩展邮件、短信、企业微信、钉钉、Webhook 等外部通道。
...
3. NotificationPreference：用户、角色或组织维度的通知偏好。
4. NotificationSubscription：用户、角色或外部客户端对某类平台事件的订阅关系。
5. DeliveryAttempt：对站内、邮件、企业 IM、Webhook 等通道的一次投递尝试。
6. NotificationReadState：消息已读、未读、归档、忽略等用户状态。
...
3. 邮件、短信、企业微信、钉钉和 Webhook 只作为 provider 扩展点，不进入领域模型作为固定依赖。
...
2. 支持 NotificationIntent、NotificationMessage、NotificationReadState 和基础 DeliveryAttempt。
...
1. 增加邮件、企业 IM 或 Webhook provider 中的一到两个最小实现。
2. 增加 NotificationPreference 和 NotificationSubscription。
