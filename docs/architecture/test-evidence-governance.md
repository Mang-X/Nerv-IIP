...
- **政策闭合与责任执行通道推导。**每个快速分片排除选择器都必须解析到至少一个政策测试身份，其规则必须是带真实依赖 `requiredLane` 的 `environment-gated`。除非本文档的跳过政策已登记某项测试，否则不得将其移出默认门禁。分片声明的 `excludedTestLanes` 随后必须等于这些 `requiredLane` 值通过 `heavyLanes[].policyLane` 映射到的高成本执行通道，因此分片不能把 `redis-cap` 或 `full-chain` 排除归给真实 PostgreSQL 责任脚本。当前全部 66 个选择器都解析为 `postgres`、`redis-cap` 或 `full-chain`；强制机制位于 `verify-backend-test-shards.ps1`。当前 active core manifest 为 15 个成员、119 个冻结身份，其中 Quality manifest 冻结 15 个身份，`test-evidence-policy.json` 另冻结 16 个 Quality 身份（含 hosted lane 未执行的 WorldHistory 规模种子规则）。NERV-688 拆解②与③八批加 #1561、#2231 及后续接入已把 Inventory 的 3 个、MasterData 的 13 个、Scheduling 的 6 个、AppHub 3 个、BarcodeLabel/FileStorage/Maintenance 各 1 个、IndustrialTelemetry 20 个、Quality 15 个、MES 37 个、WMS 9 个、ERP 4 个、DemandPlanning 3 个、跨业务 Acceptance 3 个，共 119 个身份登记进 active `postgres` manifest；其中 hosted job 选择 14 个成员、111 个身份，明确不选择 `masterdata-device-reference-concurrency` 的 8 个身份。拆解④把 DemandPlanning 的 2 个 Redis/CAP 用例接入 hosted `redis-cap` job；其余 PostgreSQL 登记仍由按需 `scripts/verify-backend-real-postgres-tests.ps1` 承接，不能据此宣称全量已运行。
...
每个运行时跳过都必须恰好匹配一条适用于当前上下文的规则。当前清单包含 43 个来源赋值：
...
`postgres` lane 的成员内不变量由 `scripts/tests/postgres-test-lane.Tests.ps1` 结构性强制，不再依赖作者自觉：runner 归属成员的每条冻结身份必须在方法体里调用本服务的 schema 重置、且不得出现任何内层库工厂或建库语句（工厂清单从行为自动穷举，再用调用点反查做第二条独立推导的交叉核对）；每处重置实现必须是 `DROP SCHEMA IF EXISTS <schema> CASCADE`；test-owned 归属成员则必须走受治理的共享 helper。用例侧的库归属断言比对 host、port 与库名三元组，而不只是库名。此外，政策里每条 `requiredLane: postgres` 的身份必须落在 manifest 成员身份（active 或 deferred）或显式的 `scaleSeedExemptions` 里，让"漏网的第三个类"无法无声出现。

采集器是单执行通道采集器：一次调用拥有一个物理 `-Lane`。`-SelectedLanes` 可以指定该物理分片或其逻辑基础执行通道；不得使用同级分片选择器声称该调用分别认证了每个同级分片。zero-execution 仅按逻辑基础执行通道对多个选中同级选择器分组，以避免重复/虚假的同级分片失败；它能识别基础选择器对应的当前分片执行，并且在选中当前分片确实没有 passed/failed 结果时仍然失败。MAN-669 新增了执行通道名称与调用，但没有改变该采集器契约。当前 CI 接入 `backend-shard-1` … `backend-shard-4` 和 `connector-host`（均为 `realDependency: false`），并接入 `postgres`（`realDependency: true`）顺序执行 active manifest 的 hosted 子集：14 个成员、111 个冻结身份。active manifest 本身为 15 个成员、119 个身份；`masterdata-device-reference-concurrency` 虽为 active，仍未由 hosted job 选择，其 8 个身份不能计入 hosted 执行证据。active 表示 runner 可执行且受清单治理，不等于已经获得 hosted 执行证据。其中 Quality manifest 为 15 个身份，policy 规则为 16 个身份。其余 PostgreSQL 成员以及 FullChain、性能和 Connector 真实依赖作业仍属后续工作。契约测试只证明 inventory/workflow 关系与 zero-execution 函数；只有 hosted `postgres` job 的实际运行与证据才证明该真实依赖通道已经执行。

<!-- postgres-lane-inventory active-members=15 active-identities=119 hosted-members=14 hosted-identities=111 active-not-hosted=masterdata-device-reference-concurrency active-not-hosted-identities=8 -->
