...
| `postgres` | `PostgreSQL Provider Tests` | `inventory-postgres-profile`、`masterdata-postgres-profile`、`scheduling-postgres-profile`、`apphub-postgres-profile`（`test-owned`）、`barcodelabel-postgres-profile`、`filestorage-postgres-profile`、`industrialtelemetry-postgres-profile`、`quality-postgres-profile`、`mes-postgres-profile`、`wms-postgres-profile`、`maintenance-device-pause-postgres`（拆解②与③前七批） |
