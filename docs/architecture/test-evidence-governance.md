...
Rule 3 is deliberately scoped. `backend-tests`, `erp-sales-order-demand-acceptance`, and `connector-host-tests` have `if: always()` collection/upload steps and therefore something to lose, so their job budgets are inflated well past any real runtime to clear the step sum. `frontend`, `openapi-client-drift`, and `script-governance` have no step that can run after an earlier one failed: nothing is preserved by outliving their own budget, so inflating them past the step sum would only convert a fast red into a slow red.
