...
- **Policy closure.** Every fast-shard exclusion selector must resolve to at least one policy test identity whose rule is `environment-gated` with a real-dependency `requiredLane`. A test cannot be dropped from the default gate unless this document's skip policy already registers it. All 49 current selectors resolve to `postgres` or `full-chain` rules; enforcement lives in `verify-backend-test-shards.ps1`. Passed-execution proof for those selectors stays with the opt-in `scripts/verify-backend-real-postgres-tests.ps1`.
