...
- **政策闭合与责任执行通道推导。**每个快速分片排除选择器都必须解析到至少一个政策测试身份，其规则必须是带真实依赖 `requiredLane` 的 `environment-gated`。除非本文档的跳过政策已登记某项测试，否则不得将其移出默认门禁。分片声明的 `excludedTestLanes` 随后必须等于这些 `requiredLane` 值通过 `heavyLanes[].policyLane` 映射到的高成本执行通道，因此分片不能把 `redis-cap` 或 `full-chain` 排除归给真实 PostgreSQL 责任脚本。当前全部 69 个选择器都解析为 `postgres`、`redis-cap` 或 `full-chain`；强制机制位于 `verify-backend-test-shards.ps1`。当前 active core manifest 为 15 个成员、110 个冻结身份，其中 Quality hosted manifest 冻结 15 个身份，`test-evidence-policy.json` 另冻结 16 个 Quality 身份（含 hosted lane 未执行的 WorldHistory 规模种子规则）。NERV-688 拆解②与③八批加后续接入已把 Inventory 的 3 个、MasterData 的 13 个、Scheduling 的 6 个、AppHub 3 个、BarcodeLabel/FileStorage/Maintenance 各 1 个、IndustrialTelemetry 7 个、Quality 15 个、MES 39 个、WMS 9 个、ERP 6 个、DemandPlanning 3 个、跨业务 Acceptance 3 个，共 110 个身份登记进 hosted `postgres` manifest；拆解④把 DemandPlanning 的 2 个 Redis/CAP 用例接入 hosted `redis-cap` job；其余 PostgreSQL 登记仍由按需 `scripts/verify-backend-real-postgres-tests.ps1` 承接，不能据此宣称全量已运行。
