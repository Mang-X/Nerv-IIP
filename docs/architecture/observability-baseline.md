...
本文档定义 Nerv-IIP 的 Observability（可观测性）事实归属、索引边界和后续建表前置条件。它承接 `docs/architecture/deployment-baseline.md` 中的日志持久化方案，并按 ADR 0016 将 VictoriaLogs 作为当前内置仅日志集中存储与检索后端。

## 定位

Observability（可观测性）负责让平台运行状态可诊断、可关联、可导出。它不拥有业务事实，也不替代 Ops 审计或 Notification 通知。

## 事实归属

Observability（可观测性）拥有：
...
5. 观测数据的 retention、清理记录和归档结果。

Observability（可观测性）不拥有：
...
2. OpenTelemetry/OTLP 日志写入 VictoriaLogs，用作当前默认的集中日志存储和 LogsQL 检索后端。
3. OpenTelemetry Collector 接收日志、trace 和 metric，可启用本地 `file_storage` 作为发送队列；Compose/PoC/生产路径可由 Collector 将日志转发到 VictoriaLogs。
4. Log Archive Worker 将关闭后的 JSONL 文件压缩为 `.jsonl.gz` chunk，上传到 File Storage，作为归档和诊断包路径。
5. `observability` schema 只保存 chunk、可选 entry 索引和元数据，不保存完整原始日志正文；#304 不创建该 schema 或 migration。
6. PlatformGateway 查询日志时调用 VictoriaLogs adapter 或后续索引/chunk adapter，并对返回结果执行鉴权、分页、脱敏和限流。
...
| 表 | 现在建表？ | 用途 | 说明 |
| --- | --- | --- | --- |
| `log_chunks` | 否 | 一个压缩日志块的索引，记录 fileId、服务、环境、实例、时间范围、行数、大小、hash、level 摘要和保留到期时间。 | 首批必须表。 |
| `log_entry_indexes` | 否 | 可选细粒度索引，记录 chunk 内特定日志行的 timestamp、level、operationTaskId、correlationId、traceId、lineNumber 或 byteOffset。 | 只有查询性能需要时启用。 |
| `diagnostic_packages` | 否 | 一次诊断导出包的元数据，引用 File Storage 中的归档文件。 | 用于现场排障和交付支持。 |
| `archive_jobs` | 否 | 日志归档任务执行记录。 | 记录成功、失败、重试和耗时。 |
| `retention_runs` | 否 | retention 清理执行记录。 | 证明索引和 File Storage chunk 同步清理。 |
...
1. 内置仅日志后端为 VictoriaLogs `v1.50.0`，容器镜像固定为 `victoriametrics/victoria-logs:v1.50.0`，许可证为 Apache License 2.0。
