...
2. 防止平台服务、前端应用、Agent、基础设施与文档混杂在同一层级。
...
- Contracts 只在确有跨进程共享契约需求时才按需拆出。
- backend/tests 只放跨服务测试宿主与集成测试；服务自身测试优先放在各自目录下的 tests。
- 示例：backend/services/AppHub/src/Nerv.IIP.AppHub.Web、backend/services/AppHub/src/Nerv.IIP.AppHub.Domain、backend/services/AppHub/src/Nerv.IIP.AppHub.Infrastructure。

### agents

- 单独承载 Agent Host 与各类 Connector。
- 与 backend 分开维护 solution，但遵守同一套构建与版本治理约定。
...
1. 真实运行时代码必须放在 frontend、backend、agents 之一，禁止放在根目录。
...
5. Agent Host 与 Connector 永远不并入 backend solution，防止平台核心服务与目标环境适配逻辑耦合。
6. 平台 HTTP 服务入口项目统一使用 .Web 命名；仅 Agent 宿主或后台进程类项目保留 .Host 命名。
