...
| `lib/FullStackSessionRuntime.ps1` | `library` | 已受治理/精确 session cleanup | 临时 AppHost 把精确 `NERV_IIP_SESSION_ID` 显式注入全部项目资源；FullStack managed run 同时从 session 创建、场景、`aspire logs` 诊断到 `aspire stop` 完成都保持该子进程作用域，使业务服务与每次 CLI 调用启动的 `aspire-managed` helper 都继承会话归属。stop 先按 manifest 的 PID + 启动时间停止 coordinator/CLI/guardian；Linux 再通过 `/proc` 读取该标识并在停止后重新枚举，任何残留业务进程或 helper 都使 cleanup 失败关闭。Windows/Linux 同时保留允许进程名加精确 worktree 命令行的兼容路径；macOS 保留 manifest PID/进程树口径，不声明命令行或环境扫描能力。两个 FullChain FullStack 场景还会在测试前通过 owned PostgreSQL container 有界验证冻结 relation 集合，消除 resource healthy 早于 migration 完成的竞态。不得退化为按通用进程名或仓库外命令行清理。 |
