...
| `scripts/lib/CiRequiredSummary.ps1` | `check` library | 已受治理 | NERV-668 required-summary 结构模型；以 Ruby YAML 结构化读取 `.github/workflows/ci.yml`，要求稳定 `ci-summary` / `CI Summary` job 精确依赖影响计划、当前五个 required 候选 job、ERP Acceptance、OpenAPI Drift 和 PostgreSQL Provider Tests，并使用 `if: always()`。唯一 step 无条件执行、固定 `bash --noprofile --norc -euo pipefail {0}`：稳定 Backend/frontend 聚合必须成功；ERP Acceptance、Connector Host Tests、Script Governance、OpenAPI Drift 与 PostgreSQL Provider Tests 在被选择时必须成功，只有计划成功且明确未选中时才接受 skipped，并把 Backend 与前四个直接 lane 的 `skipped by design`、PostgreSQL lane 的 `skipped by policy` 及实际结果写入 Actions Summary。影响计划、选中 lane 的失败/取消/错误跳过、未选中 lane 的意外结果、身份缺席、step 条件、非 fail-fast shell、吞退出码或 `continue-on-error` 均返回 finding。这些 job 只是 Summary 内的条件依赖，不新增 branch-protection context。 |
