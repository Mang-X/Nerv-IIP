...
| `scripts/lib/CiImpactPlan.ps1` | `check` library | 已受治理 | NERV-668 拆解②的纯路径影响模型；规范化并 ordinal 去重排序仓库相对路径，输出 schema v1 粗域、共享层、Gateway/OpenAPI、前端分层、真实依赖、业务服务集合与逐信号理由。workflow/规则自改、未知业务服务和未分类路径均保守选择全部影响；不执行 Git 或网络调用。 |
