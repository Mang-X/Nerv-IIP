...
| `tests/ci-impact-plan.Tests.ps1` | `check` | 已受治理 | 覆盖纯文档、脚本治理登记文档、单业务服务、Contracts/Testing/Persistence/Messaging 扩散、全部当前 `backend/common/**` 共享目录、Backend fast lane 控制输入、ERP acceptance 输入闭包与 Connector/OpenAPI 后端中央构建输入、BusinessGateway/OpenAPI、PostgreSQL lane 三个自有路径、前端、connector-hosts、infra、FullChain、规则自改、非法路径及入口 JSON/output/summary 序列化；另以结构化 workflow 合同钉住 Backend 五个执行 job、ERP Sales Order Demand Acceptance、Connector Host Tests、Script Governance、OpenAPI Drift 与 PostgreSQL Provider Tests 的精确 fail-open 且 cancellation-aware 的 PR 路由，并禁止其余 job 偷接影响计划。删除控制输入路由、错误 signal、删除计划失败分支、漏掉 ERP 闭包输入、缺 dependency 或额外接入 job 的变异均失败；临时输出在 `finally` 清理，并由 Script Governance 独立 step 执行。 |
