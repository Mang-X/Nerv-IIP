...
| `scripts/lib/ScriptVariableBinding.ps1` | `check` library | 已受治理 | #1509 的共享静态变量绑定核；统一 `Set-Variable`/`New-Variable` 的 canonical、别名、模块限定名、大小写、参数元数据配对、字面量集合展开与作用域前缀归一化，并提供显式 `Set-Item variable:` provider path 识别。治理 checker 仍独占 assignment Left-shape 与作用域结构裁决，WMS verifier 合同只复用命令绑定核；计算名称、splat、`PSVariable.Set`、动态 provider path 与 ref 间接改写继续作为已登记残余，不被这个 helper 冒充已证明。只读取 PowerShell alias/cmdlet metadata，不执行被分析脚本。 |
