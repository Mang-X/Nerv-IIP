...
| `tests/ci-required-summary.Tests.ps1` | `check` | 已受治理 | 对真实 workflow 只启动一次生产 verifier，并以同一 workflow 上 dot-source 的生产 finding 函数建立等价 baseline；随后每个 OS 临时 YAML mutation 直接调用该生产函数，避免为每个变体启动新 PowerShell。覆盖 stable aggregate dependency/result、裸值/单双引号 collector lane、upload-artifact v4/v5、artifact name 包含匹配、非 v1 owner 冒用及 v1 collector/artifact 缺失；每个 mutation 都先证明文本实际改变，再要求命中精确 diagnostic。原有合同继续覆盖旧 ERP、稳定聚合消费边界、`always()`、fail-fast、退出码与 CRLF。夹具在 `finally` 清理，CI 在 Script Governance job 中执行。 |
| `scripts/lib/CiImpactPlan.ps1` | `check` library | 已受治理 | NERV-668 拆解②的纯路径影响模型；规范化并 ordinal 去重排序仓库相对路径，输出 schema v1 粗域、共享层、Gateway/OpenAPI、前端分层、真实依赖、ERP acceptance、业务服务集合与逐信号理由。普通 Markdown 归 docs，`frontend/**` Markdown 还保留所属前端层；全部当前 `backend/common/**` 共享代码目录保守扩散到所有业务服务并选择 Connector 与 ERP lane，两个后端中央构建文件也同时选择 OpenAPI、Connector 与 ERP，`IntegrationEventHandlers` 归 Redis/CAP 消息链；Backend fast lane 的 manifest、evidence policy、runner、verifier、`OrdinalString.ps1`、selectors、diagnostics、合同与 manifest 引用的 heavy-lane owner scripts 精确选择 `scripts+backend`，PostgreSQL lane 的入口、库和 manifest 精确选择 `postgresql`。验收矩阵既有 planning owning paths 选择 `scripts+backend+full_chain`，三个 shadow runtime/equivalence owning paths 选择 `scripts+backend+full_chain+erp_sales_order_demand` 且业务服务集合为空。ERP 专属闭包另覆盖 ERP、DemandPlanning、MasterData、FullChain 探针、acceptance 入口及本地 compose 拓扑；`NuGet.config` 与共享运行库 `scripts/lib/ScriptAutomation.ps1` 因被多个既有 lane 消费而保留全信号 fail-open。workflow/规则自改、未知业务服务和未分类路径均保守选择全部影响；不执行 Git 或网络调用。 |
| `get-ci-impact-plan.ps1` | `check` | 已受治理 | 接受显式 changed paths，或通过受治理 Git helper 读取 PR merge-base / push range diff；写 `artifacts/ci-impact-plan/impact-plan.json`，并可追加 GitHub outputs 与 Actions Summary。本入口只报告选择与逐信号理由；workflow 当前消费 `scripts`、`backend`、`openapi_codegen`、`connector_hosts`、`postgresql`、`redis_cap`、`full_chain`、`erp_sales_order_demand` 八个标量 output 及 frontend workspace 选择，本脚本自身不启动测试、不修改 branch protection。 |
| `tests/ci-impact-plan.Tests.ps1` | `check` | 已受治理 | 覆盖纯文档、脚本治理登记文档、单业务服务、Contracts/Testing/Persistence/Messaging 扩散、全部当前 `backend/common/**` 共享目录、Backend fast lane 控制输入（含 selectors 直接依赖的 `OrdinalString.ps1`）、ERP acceptance 输入闭包与 Connector/OpenAPI 后端中央构建输入、BusinessGateway/OpenAPI、PostgreSQL lane 三个自有路径、验收矩阵 planning 与 shadow runtime owning paths、前端、connector-hosts、infra、FullChain、规则自改、非法路径及入口 JSON/output/summary 序列化；runtime owning path 对删除、错误大小写、alias 与路径漂移逐项运行生产库变异，且每个规范路径都必须精确选择四个 shadow 信号并保持业务服务集合为空。另以结构化 workflow 合同钉住 Backend 五个执行 job、ERP Sales Order Demand Acceptance、Connector Host Tests、Script Governance、OpenAPI Drift 与 PostgreSQL Provider Tests 的精确 fail-open 且 cancellation-aware 的 PR 路由，并禁止其余 job 偷接影响计划；验收矩阵允许现行 hosted `acceptance-scenario-matrix-runtime` job，并在 Script Governance 内保留三个独立纯 fixture 合同 step（planning、runtime、equivalence），不允许把生产入口接入 fixture step 或新增 job。删除 `OrdinalString.ps1` 或 PostgreSQL 控制输入路由、错误 signal、删除计划失败分支、漏掉 ERP 闭包输入、缺 dependency 或额外接入 job 的变异均失败；临时输出在 `finally` 清理，并由 Script Governance 独立 step 执行。 |
| `scripts/acceptance-scenario-matrix.json` | `manifest`（机器输入） | 已受治理 | NERV-673②-1 schema v2 六场景清单；运行时不写文件、无 cleanup、无独立环境要求，由 `AcceptanceScenarioMatrix.ps1` 消费并与 v1 五场景双向闭合。CI 以 planning job 生成受 provenance 约束的选择，并在 `sales-order-demand` 选中时驱动 hosted shadow runtime 与三轨等价；Script Governance 和 `compat-fast` 仍只执行纯 fixture 合同。 |
...
| `scripts/lib/AcceptanceScenarioMatrixRuntime.ps1` | `library` + `check` | 已受治理/shadow 合同 | 读取并校验 planning artifact、v2/v1 权威 manifest、workflow 预算与唯一 `sales-order-demand` 选择，冻结进程内 action 合同、稳定等价向量、失败分类、cleanup readback 和原子 final summary。planning source attempt 与 runtime 自身 attempt 分开验证：前者只绑定 planning artifact，后者继续进入 shadow result/summary provenance。库本身只调用注入的进程内 action；真实依赖由生产入口装配，不改变 v1 evidence owner。 |
| `run-acceptance-scenario-matrix.ps1` | `verify` | 已受治理/hosted shadow 接线 | shadow controller 的生产入口；装配 `verify-erp-sales-order-demand-planning.ps1` 作为真实业务 action，并由 `acceptance-scenario-matrix-runtime` job 在 planning 精确选中时调用。入口分别接受 `PlanningRunAttempt` 与当前 `RunAttempt`，普通首轮/全量重跑仍可同值，selective rerun 则保留真实 producer attempt。Writes：runtime summary、shadow canonical result 与受治理业务/cleanup evidence；cleanup：验证精确 owned-resource cleanup 证据；requires：PowerShell 7、.NET SDK 10、Docker PostgreSQL/Redis。代码接线不构成 hosted 成功证据。 |
| `tests/acceptance-scenario-matrix-runtime.Tests.ps1` | `check` | 已受治理/fixture CI 接线 | 用 OS 临时 workflow/artifact/summary 和进程内 fixture action 验证 runtime preflight、物理权威路径、planning/runtime attempt 分离、唯一场景、结果 schema、稳定等价字段、失败分类、cleanup 与原子 summary；另直接执行生产 runtime library，覆盖 push、schedule、workflow_dispatch all-active 与 conservative PR 的 active/core 全集：顺序变化通过，真子集、额外成员、重复和非法成员失败。它不连接 PostgreSQL、Redis、Docker、Aspire 或业务服务；由 Script Governance 独立 5 分钟 step 和 `compat-fast` 执行，不能作为 hosted runtime 或 shadow 等价证据。 |
| `scripts/lib/AcceptanceScenarioMatrixEquivalence.ps1` | `library` + `check` | 已受治理/纯比较 | 严格校验 planning provenance 与 v1、shadow、legacy ERP 三份 canonical result 的 schema、固定 track/path/source-attempt 绑定、稳定事实和 cleanup 结果；共同 runId、SHA、manifest/scenario 必须一致，物理 source attempt 不进入稳定业务向量 digest。report 只记录 planning/各 track 的 source attempt 和脱敏 digest，不泄漏 volatile 对象。只进行 ordinal 比较，不连接真实依赖。v1 仍是唯一 `full-chain` evidence owner。 |
| `verify-acceptance-scenario-matrix-equivalence.ps1` | `check` + `generate` | 已受治理/hosted equivalence 接线 | 在三轨前置 job 成功后按各 producer 的 `needs` artifact identity 读取 planning 与 canonical artifacts，并把各自物理 source attempt 传给纯 comparator，原子写入 equivalence report；空值、非规范 attempt、artifact name 与 runId/attempt 不一致均在下载前失败关闭，失败时不保留成功形态的 partial report。真实 workflow 仅在 `sales-order-demand-selected=true` 时运行。artifact name 含 producer attempt 且不启用 overwrite，保留 selective rerun 前后两轮证据。 |
| `tests/acceptance-scenario-matrix-equivalence.Tests.ps1` | `check` | 已受治理/fixture CI 与 compat-fast 接线 | 用 OS 临时 planning/canonical fixtures 验证同 run 中 planning=1、v1=2、shadow=1、legacy ERP=1 的 mixed selective-rerun 正例，以及三轨固定 path/track/source-attempt 绑定、schema/provenance/cleanup 失败关闭、稳定向量不受物理 attempt 影响、脱敏 source-attempt report 与原子写入；不连接真实依赖。由 Script Governance 独立 5 分钟 step 与 `compat-fast` 执行；纯合同通过不构成 hosted 等价证据。 |
...
| `install/migrate-platform-databases.ps1` | `release-install` | 已受治理 | 从 platform/business allowlist manifest 选择目标；只对选中服务验证连接变量、expected database、project 顶层 DbContext 源码与 startup project 顶层同 namespace design-time factory 配对。apply 前通过 `psql` 只读连接逐库确认目标已存在，全部通过后才串行执行 EF migrations；不负责备份、seed、建库、删库或回滚。 |
| `install/migrate-business-databases.ps1` | `release-install` | 已受治理 | 业务 13 库的操作员入口；固定选择 business manifest，正常、非零与超时路径均保留独立子进程 stdout/stderr 日志，安全回放脱敏的逐服务诊断与失败原因，并透传受控服务子集、release/correlation identity 和 ValidateOnly；不复制连接串到命令行或日志。 |
