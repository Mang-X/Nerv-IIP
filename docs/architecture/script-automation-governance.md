...
| `scripts/lib/CiRequiredSummary.ps1` | `check` library | 已受治理 | NERV-668 拆解①的 required-summary 结构模型；以 Ruby YAML 结构化读取 `.github/workflows/ci.yml`，要求稳定 `ci-summary` / `CI Summary` job 精确依赖当前四个 required job、使用 `if: always()`，且唯一 step 只含四条独立 `result = success` 断言。依赖身份缺席、额外依赖、非 success 白名单、no-op、吞退出码或 `continue-on-error` 均返回 finding。当前非 required 的 ERP/OpenAPI 不在本层被升级。 |
...
| `tests/ci-required-summary.Tests.ps1` | `check` | 已受治理 | 对真实 workflow 先跑生产 verifier，再逐个写 OS 临时 YAML 变体；覆盖缺少 dependency、删除 `always()`、no-op、`\|\| true`、job/step `continue-on-error` 以及把严格成功判断改为 `failure`/`cancelled`/`skipped`。每个变体都要求生产 verifier 非零且给出对应诊断，夹具在 `finally` 清理。CI 在 Script Governance job 中执行。 |
