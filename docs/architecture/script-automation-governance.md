...
| `scripts/lib/AcceptanceScenarioMatrixRuntime.ps1` | `library` + `check` | 已受治理/shadow 合同 | 读取并校验 planning artifact、v2/v1 权威 manifest、workflow 预算与唯一 `sales-order-demand` 选择，冻结进程内 action 合同、稳定等价向量、失败分类、cleanup readback 和原子 final summary。库本身只调用注入的进程内 action；真实依赖由生产入口装配，不改变 v1 evidence owner。 |
| `run-acceptance-scenario-matrix.ps1` | `verify` | 已受治理/hosted shadow 接线 | shadow controller 的生产入口；装配 `verify-erp-sales-order-demand-planning.ps1` 作为真实业务 action，并由 `acceptance-scenario-matrix-runtime` job 在 planning 精确选中时调用。Writes：runtime summary、shadow canonical result 与受治理业务/cleanup evidence；cleanup：验证精确 owned-resource cleanup 证据；requires：PowerShell 7、.NET SDK 10、Docker PostgreSQL/Redis。代码接线不构成 hosted 成功证据。 |
| `tests/acceptance-scenario-matrix-runtime.Tests.ps1` | `check` | 已受治理/fixture CI 接线 | 用 OS 临时 workflow/artifact/summary 和进程内 fixture action 验证 runtime preflight、物理权威路径、唯一场景、结果 schema、稳定等价字段、失败分类、cleanup 与原子 summary；不连接 PostgreSQL、Redis、Docker、Aspire 或业务服务。由 Script Governance 独立 5 分钟 step 和 `compat-fast` 执行，不能作为 hosted runtime 或 shadow 等价证据。 |
| `scripts/lib/AcceptanceScenarioMatrixEquivalence.ps1` | `library` + `check` | 已受治理/纯比较 | 严格校验 planning provenance 与 v1、shadow、legacy ERP 三份 canonical result 的 schema、身份、稳定事实和 cleanup 结果；只进行 ordinal 比较，不连接真实依赖、不写文件。v1 仍是唯一 `full-chain` evidence owner。 |
| `verify-acceptance-scenario-matrix-equivalence.ps1` | `check` + `generate` | 已受治理/hosted equivalence 接线 | 在三轨前置 job 成功后读取 planning 与 canonical artifacts，调用纯 comparator 并原子写入 equivalence report；失败时不保留成功形态的 partial report。真实 workflow 仅在 `sales-order-demand-selected=true` 时运行。 |
| `tests/acceptance-scenario-matrix-equivalence.Tests.ps1` | `check` | 已受治理/fixture CI 与 compat-fast 接线 | 用 OS 临时 planning/canonical fixtures 验证三轨身份与稳定事实等价、schema/provenance/cleanup 失败关闭及原子报告，不连接真实依赖。由 Script Governance 独立 5 分钟 step 与 `compat-fast` 执行；纯合同通过不构成 hosted 等价证据。 |
