...
| `tests/ci-required-summary.Tests.ps1` | `check` | 已受治理 | 对真实 workflow 先跑生产 verifier，再逐个写 OS 临时 YAML 变体；覆盖缺少 dependency、删除 summary 的 `always()`、断言 step 加条件、改用非 fail-fast shell、`\|\| true`、job/step `continue-on-error`、Connector/Script/OpenAPI/PostgreSQL selected lane 错收 skipped、unselected lane 错收 success、忽略影响计划失败、隐藏 `skipped by design` / `skipped by policy` 审计，以及 verifier library 为 CRLF 时的等价判定。每个变体都要求生产 verifier 非零且给出对应诊断，夹具在 `finally` 清理。CI 在 Script Governance job 中执行。 |
| `scripts/lib/CiImpactPlan.ps1` | `check` library | 已受治理 | NERV-668 拆解②的纯路径影响模型；规范化并 ordinal 去重排序仓库相对路径，输出 schema v1 粗域、共享层、Gateway/OpenAPI、前端分层、真实依赖、业务服务集合与逐信号理由。普通 Markdown 归 docs，`frontend/**` Markdown 还保留所属前端层；全部当前 `backend/common/**` 共享代码目录保守扩散到所有业务服务并选择 Connector lane，两个后端中央构建文件也同时选择 OpenAPI 与 Connector，`IntegrationEventHandlers` 归 Redis/CAP 消息链；PostgreSQL lane 的入口、库和 manifest 三个自有路径精确选择 `postgresql`。workflow/规则自改、未知业务服务和未分类路径均保守选择全部影响；不执行 Git 或网络调用。 |
| `get-ci-impact-plan.ps1` | `check` | 已受治理 | 接受显式 changed paths，或通过受治理 Git helper 读取 PR merge-base / push range diff；写 `artifacts/ci-impact-plan/impact-plan.json`，并可追加 GitHub outputs 与 Actions Summary。本入口只报告选择与逐信号理由；workflow 当前消费 `scripts`、`backend`、`openapi_codegen`、`connector_hosts`、`postgresql` 五个既有 output，本脚本自身不启动测试、不修改 branch protection。 |
| `tests/ci-impact-plan.Tests.ps1` | `check` | 已受治理 | 覆盖纯文档、脚本治理登记文档、单业务服务、Contracts/Testing/Persistence/Messaging 扩散、全部当前 `backend/common/**` 共享目录与后端中央构建文件的 Connector 编译输入闭包、BusinessGateway/OpenAPI、OpenAPI job 的后端根级与前端 workspace 输入、PostgreSQL lane 三个自有路径、前端、connector-hosts、infra、FullChain、规则自改、非法路径及入口 JSON/output/summary 序列化；另以结构化 workflow 合同钉住 Connector Host Tests、Script Governance、OpenAPI Drift 与 PostgreSQL Provider Tests 的精确 fail-open 且 cancellation-aware 的 PR 路由，并禁止其余 job 提前消费影响计划。删除 PostgreSQL 自有路径路由、使用错误 signal、删除计划失败分支、漏掉 backend 治理面、缺 dependency 或额外接入 job 的变异均失败；临时输出在 `finally` 清理，并由 Script Governance 独立 step 执行。 |
