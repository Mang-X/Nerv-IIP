...
1. `compat-fast`：在 macOS 或 Linux 环境运行 `pwsh scripts/check-script-governance.ps1`、`pwsh scripts/tests/ordinal-string.Tests.ps1`、`pwsh scripts/tests/check-script-governance.Tests.ps1`、`pwsh scripts/tests/script-governance-scan-boundary.Tests.ps1`、`pwsh scripts/tests/ordinal-comparison-layers.Tests.ps1`、`pwsh scripts/tests/test-evidence.Tests.ps1`、`pwsh scripts/tests/acceptance-scenario-matrix.Tests.ps1`、`pwsh scripts/tests/acceptance-scenario-matrix-runtime.Tests.ps1` 和 `git diff --check`。其中 PowerShell 脚本统一由独立 `pwsh -Command` 进程内的调用运算符执行，覆盖 GitHub Actions 包装脚本产生的 script scope 语义，不以 `pwsh -File` 的全局作用域结果替代；脚本路径、具名参数字典与位置参数数组通过 CLIXML + Base64 只作为数据传输，在子进程内分别用 hashtable splat 与 array splat 绑定，不把参数值拼进可执行文本。`scripts/tests/script-compatibility-arguments.Tests.ps1` 通过现有 script-governance fixture 合同永久覆盖 switch、具名参数、普通值、空字符串、空格、单引号、前导连字符、注入形载荷、非零退出与超时。
...
| `tests/ci-required-summary.Tests.ps1` | `check` | 已受治理 | 对真实 workflow 先跑生产 verifier，再逐个写 OS 临时 YAML 变体；覆盖缺少 dependency、删除 summary 的 `always()`、断言 step 加条件、改用非 fail-fast shell、`\|\| true`、job/step `continue-on-error`、ERP/Connector/Script/OpenAPI/PostgreSQL selected lane 错收 skipped、unselected lane 错收 success、忽略影响计划失败、隐藏 `skipped by design` / `skipped by policy` 审计，以及 verifier library 为 CRLF 时的等价判定。每个变体都要求生产 verifier 非零且给出对应诊断，夹具在 `finally` 清理。CI 在 Script Governance job 中执行。 |
...
| `scripts/acceptance-scenario-matrix.json` | `manifest`（机器输入） | 已受治理/纯规划 | NERV-673②-1 schema v2 六场景清单；运行时不写文件、无 cleanup、无独立环境要求，由 `AcceptanceScenarioMatrix.ps1` 消费并与 v1 五场景双向闭合。CI 只运行 Script Governance 的 planning/runtime 纯 fixture 合同；当前没有 planning/matrix hosted runtime。 |
...
| `plan-acceptance-scenario-matrix.ps1` | `check` + `generate` | 已受治理/尚未接 runtime | 读取 manifest 与 workflow，对每个选中项目执行一次 `dotnet restore` 和一次 Release `--no-restore --list-tests`。Writes：NuGet cache、project restore intermediates、`artifacts/acceptance-scenario-matrix/planning.json` 与 `artifacts/script-logs/**`；cleanup：入口先删 stale success artifact，失败不留 partial artifact，并恢复 MSBuild node-reuse/dotnet build-server 环境变量；requires：PowerShell 7、.NET SDK 10。当前真实 `ci.yml` 不含具名 planning job/step，入口会在零外部命令下失败关闭；CI 不直接调用本入口。 |
...
| `scripts/lib/AcceptanceScenarioMatrixRuntime.ps1` | `library` + `check` | 已受治理/shadow 纯合同 | 读取并校验 planning artifact、v2/v1 权威 manifest、workflow 预算与唯一 `sales-order-demand` 选择，冻结进程内 action 合同、稳定等价向量、失败分类、cleanup readback 和原子 final summary。只调用注入的进程内 action；本文件不连接真实依赖，也不是 hosted runtime 入口。 |
| `run-acceptance-scenario-matrix.ps1` | `verify` | 已受治理/尚未接真实 runtime | shadow controller 的生产入口；Writes：调用方声明的 runtime summary；cleanup：验证注入 action 返回的精确 owned-resource cleanup 证据；requires：PowerShell 7。当前真实 workflow 不含 runtime job/入口，生产入口会先在 preflight 失败关闭且零外部副作用；只有 preflight 通过但未注入 action 时，默认 action 才以 `not wired` 失败。v1 FullChain runner 与旧 ERP job 仍是实际运行权威，真实 shadow 接线属于后续 PR。 |
| `tests/acceptance-scenario-matrix-runtime.Tests.ps1` | `check` | 已受治理/fixture CI 接线 | 用 OS 临时 workflow/artifact/summary 和进程内 fixture action 验证 runtime preflight、物理权威路径、唯一场景、结果 schema、稳定等价字段、失败分类、cleanup 与原子 summary；不调用生产 action，不连接 PostgreSQL、Redis、Docker、Aspire 或业务服务。由 Script Governance 唯一独立 5 分钟 step 和 `compat-fast` 执行，不能作为 hosted runtime 或 shadow 等价证据。 |
