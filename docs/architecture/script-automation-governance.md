...
| `scripts/lib/CiRequiredSummary.ps1` | `check` library | 已受治理 | NERV-668 required-summary 结构模型；以 Ruby YAML 结构化读取 `.github/workflows/ci.yml`，要求稳定 `ci-summary` / `CI Summary` job 精确依赖影响计划、当前五个 required 候选 job、ERP Acceptance、OpenAPI Drift、PostgreSQL Provider Tests、Redis/CAP Transport Tests 与 Business FullChain Acceptance，并使用 `if: always()`。唯一 step 无条件执行、固定 `bash --noprofile --norc -euo pipefail {0}`：稳定 Backend/frontend 聚合必须成功；七个直接条件 lane 在被选择时必须成功，只有计划成功且明确未选中时才接受 skipped；旧 ERP 的选择同时镜像自身 ERP impact 与 FullChain 等价路由。Summary 只消费稳定 FullChain 聚合，不直接依赖 v1、shadow 或 equivalence job；各条件 lane 的政策与实际结果写入 Actions Summary。影响计划、选中 lane 的失败/取消/错误跳过、未选中 lane 的意外结果、身份缺席、step 条件、非 fail-fast shell、吞退出码或 `continue-on-error` 均返回 finding。这些 job 只是 Summary 内的条件依赖，不新增 branch-protection context。 |
...
| `tests/ci-required-summary.Tests.ps1` | `check` | 已受治理 | 对真实 workflow 先跑生产 verifier，再逐个写 OS 临时 YAML 变体；覆盖稳定 FullChain 名称/依赖、planning 与 v1 必须成功、`sales-order-demand-selected` 只接受精确小写 `true`/`false`、selected/unselected shadow/equivalence 结果矩阵、selected legacy ERP 成功、旧 ERP 入口/路由与 error/14-day artifact，以及 `CI Summary` 只消费稳定聚合并保留旧 ERP 直接依赖。既有 mutation 继续覆盖缺少 dependency、删除 summary 的 `always()`、非 fail-fast shell、`\|\| true`、`continue-on-error`、条件 lane 错误结果与审计文本，以及 verifier library 为 CRLF 时的等价判定。每个变体都要求生产 verifier 或聚合合同非零，夹具在 `finally` 清理。CI 在 Script Governance job 中执行。 |
