...
| `install/migrate-platform-databases.ps1` | `release-install` | 已受治理 | 从 platform/business allowlist manifest 选择目标；在数据库动作前验证 service、连接变量、expected database、project、startup project 与 DbContext 配对，输出脱敏发布诊断并串行执行 EF migrations；不负责备份、seed、删库或回滚。 |
| `install/migrate-business-databases.ps1` | `release-install` | 已受治理 | 业务 13 库的操作员入口；固定选择 business manifest，安全回放共享执行器的逐服务诊断与失败原因，并透传受控服务子集、release/correlation identity 和 ValidateOnly；不复制连接串到命令行或日志。 |
