...
| `tests/ci-impact-plan.Tests.ps1` | `check` | 已受治理 | 覆盖纯文档、脚本治理登记文档、单业务服务、Contracts/Testing/Persistence/Messaging 扩散、全部当前 `backend/common/**` 共享目录与后端中央构建文件的 Connector 编译输入闭包、BusinessGateway/OpenAPI、OpenAPI job 的后端根级与前端 workspace 输入、前端、connector-hosts、infra、FullChain、规则自改、非法路径及入口 JSON/output/summary 序列化；另以结构化 workflow 合同钉住 Connector Host Tests、Script Governance、OpenAPI Drift 与 PostgreSQL Provider Tests 的精确 fail-open 且 cancellation-aware 的 PR 路由，并禁止其余 job 提前消费影响计划。错误 signal、删除计划失败分支、漏掉 backend 治理面、缺 dependency 或额外接入 job 的变异均失败；临时输出在 `finally` 清理，并由 Script Governance 独立 step 执行。 |
