...
1. `compat-fast`：在 macOS 或 Linux 环境运行 `pwsh scripts/check-script-governance.ps1`、`pwsh scripts/tests/ordinal-string.Tests.ps1`、`pwsh scripts/tests/check-script-governance.Tests.ps1`、`pwsh scripts/tests/script-governance-scan-boundary.Tests.ps1`、`pwsh scripts/tests/ordinal-comparison-layers.Tests.ps1`、`pwsh scripts/tests/test-evidence.Tests.ps1`、`pwsh scripts/tests/acceptance-scenario-matrix.Tests.ps1` 和 `git diff --check`。
...
| `tests/ci-impact-plan.Tests.ps1` | `check` | 已受治理 | 覆盖纯文档、脚本治理登记文档、单业务服务、Contracts/Testing/Persistence/Messaging 扩散、全部当前 `backend/common/**` 共享目录、Backend fast lane 控制输入（含 selectors 直接依赖的 `OrdinalString.ps1`）、ERP acceptance 输入闭包与 Connector/OpenAPI 后端中央构建输入、BusinessGateway/OpenAPI、PostgreSQL lane 三个自有路径、前端、connector-hosts、infra、FullChain、规则自改、非法路径及入口 JSON/output/summary 序列化；另以结构化 workflow 合同钉住 Backend 五个执行 job、ERP Sales Order Demand Acceptance、Connector Host Tests、Script Governance、OpenAPI Drift 与 PostgreSQL Provider Tests 的精确 fail-open 且 cancellation-aware 的 PR 路由，并禁止其余 job 偷接影响计划。删除 `OrdinalString.ps1` 或 PostgreSQL 控制输入路由、错误 signal、删除计划失败分支、漏掉 ERP 闭包输入、缺 dependency 或额外接入 job 的变异均失败；临时输出在 `finally` 清理，并由 Script Governance 独立 step 执行。 |
| `scripts/acceptance-scenario-matrix.json` | `manifest`（机器输入） | 已受治理/纯规划 | NERV-673②-1 schema v2 六场景清单；运行时不写文件、无 cleanup、无独立环境要求，由 `AcceptanceScenarioMatrix.ps1` 消费并与 v1 五场景双向闭合。CI 入口仅为 Script Governance 的 `Test acceptance scenario matrix contract` fixture step；当前没有 planning/matrix runtime。 |
