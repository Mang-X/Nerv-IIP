...
| `run-postgres-test-lane.ps1` | `verify` | 已受治理/真实 PostgreSQL | NERV-688 拆解②模板与③逐服务入口；读取 `postgres-test-lane.json` 的显式 active 成员批次，以无凭据命令行参数的 `psql` 协议探针为每个成员创建 run/attempt 专属数据库，精确 discovery 后顺序运行筛选测试，要求各成员冻结身份全部 Passed 且零 skip；失败时按成员声明的受限 schema 保留脱敏诊断，再在 `finally` 删除对应数据库，写含聚合值与逐成员事实的 schema v2 dependency summary。当前 active manifest 为 15 个成员、120 个冻结身份；hosted job 仅选择其中 14 个成员、112 个身份。唯一未由 hosted 选择的 active 成员 `masterdata-device-reference-concurrency` 有 8 个身份，原因是共享 workflow 接入仍待独立 Scope-Gate；它目前只由按需真实 PostgreSQL lane 承接，active 不能替代 hosted 执行证据。两处自管生命周期特例按裁决登记为 `status: deferred`，不由本 runner 执行。 |
