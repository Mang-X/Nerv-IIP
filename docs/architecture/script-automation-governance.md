...
1. `compat-fast`：在 macOS 或 Linux 环境运行 `pwsh scripts/check-script-governance.ps1`、`pwsh scripts/tests/ordinal-string.Tests.ps1`、`pwsh scripts/tests/check-script-governance.Tests.ps1`、`pwsh scripts/tests/script-governance-scan-boundary.Tests.ps1`、`pwsh scripts/tests/ordinal-comparison-layers.Tests.ps1`、`pwsh scripts/tests/test-evidence.Tests.ps1`、`pwsh scripts/tests/acceptance-scenario-matrix.Tests.ps1`、`pwsh scripts/tests/acceptance-scenario-matrix-runtime.Tests.ps1` 和 `git diff --check`。
...
| `run-acceptance-scenario-matrix.ps1` | `verify` | 已受治理/尚未接真实 runtime | shadow controller 的生产入口；Writes：调用方声明的 runtime summary；cleanup：验证注入 action 返回的精确 owned-resource cleanup 证据；requires：PowerShell 7。未注入 action 时固定以 `not wired` 失败，当前 workflow 不调用本入口；v1 FullChain runner 与旧 ERP job 仍是实际运行权威，真实 shadow 接线属于后续 PR。 |
