...
**五份声明的强度上界怎么读**（#1509 六轮更正）。此前这里写的是「盲区之外的构造，声明成立」，那句话把「盲区表」当成了扫描面的补集——实测不成立：`-lt`/`-le`/`-gt`/`-ge`、`[string]::Compare`/`.CompareTo`、`[StringComparer]::InvariantCulture`、`.Sort()` 无参四类都是 culture-aware，既不在覆盖轴也不在盲区表。四类现已全部补进覆盖轴。准确的上界是分两半的：
