...
| `lib/FullStackSessionRuntime.ps1` | `library` | 已受治理/精确 session cleanup | 临时 AppHost 把精确 `NERV_IIP_SESSION_ID` 显式注入全部项目资源；FullStack stop 先按 manifest 的 PID + 启动时间停止 coordinator/CLI/guardian，Linux 再通过 `/proc` 读取子进程继承的该标识，收口命令行不携带 worktree 的 `aspire-managed` 与业务服务进程，Windows/Linux 同时保留允许进程名加精确 worktree 命令行的兼容路径。停止后 PID 读回不为空则 cleanup 失败。macOS 保留 manifest PID/进程树口径，不声明命令行或环境扫描能力。两个 FullChain FullStack 场景还会在测试前通过 owned PostgreSQL container 有界验证冻结 relation 集合，消除 resource healthy 早于 migration 完成的竞态。不得退化为按通用进程名或仓库外命令行清理。 |
