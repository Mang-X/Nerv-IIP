...
| `run-postgres-test-lane.ps1` | `verify` | 已受治理/真实 PostgreSQL | NERV-688 拆解② hosted PostgreSQL 模板；读取 `postgres-test-lane.json` 的 active 成员，以无凭据命令行参数的 `psql` 协议探针创建 run/attempt 专属数据库，精确 discovery 后运行筛选测试，要求冻结身份全部 Passed 且零 skip；失败时按成员声明的受限 schema 保留脱敏诊断，再在 `finally` 删除数据库、写机器可读 dependency summary。当前只接 Inventory 单服务 core 试点；逐服务铺开属于拆解③。 |
