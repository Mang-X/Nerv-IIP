...
| `scripts/lib/CiRequiredSummary.ps1` | `check` library | 已受治理 | NERV-668 required-summary 结构模型；以 Ruby YAML 结构化读取 `.github/workflows/ci.yml`，要求稳定 `ci-summary` / `CI Summary` job 精确依赖影响计划、当前五个 required 候选 job 和首批纳管的 OpenAPI Drift，并使用 `if: always()`。唯一 step 无条件执行、固定 `bash --noprofile --norc -euo pipefail {0}`：未条件化 lane 必须成功；Script Governance/OpenAPI Drift 在被选择时必须成功，只有计划成功且明确未选中时才接受 skipped，并把 `skipped by design` 与实际结果写入 Actions Summary。影响计划、选中 lane 的失败/取消/错误跳过、未选中 lane 的意外结果、身份缺席、step 条件、非 fail-fast shell、吞退出码或 `continue-on-error` 均返回 finding。OpenAPI 只是 summary 内的条件依赖，不新增 branch-protection context。 |
...
| `get-ci-impact-plan.ps1` | `check` | 已受治理 | 接受显式 changed paths，或通过受治理 Git helper 读取 PR merge-base / push range diff；写 `artifacts/ci-impact-plan/impact-plan.json`，并可追加 GitHub outputs 与 Actions Summary。本入口只报告选择与逐信号理由；拆解③首批由 workflow 消费 `scripts`、`backend`、`openapi_codegen` 三个既有 output，本脚本自身不启动测试、不修改 branch protection。 |
| `tests/ci-impact-plan.Tests.ps1` | `check` | 已受治理 | 覆盖纯文档、脚本治理登记文档、单业务服务、Contracts/Testing/Persistence/Messaging 扩散、BusinessGateway/OpenAPI、OpenAPI job 的后端根级与前端 workspace 输入、前端、connector-hosts、infra、FullChain、规则自改、非法路径及入口 JSON/output/summary 序列化；另以结构化 workflow 合同钉住 Script Governance/OpenAPI Drift 的精确 fail-open 且 cancellation-aware 的 PR 路由，并禁止其余 job 提前消费影响计划。错误 signal、删除计划失败分支、漏掉 backend 治理面、缺 dependency 或额外接入 job 的变异均失败；临时输出在 `finally` 清理，并由 Script Governance 独立 step 执行。 |
