...
| `tests/acceptance-scenario-matrix-runtime.Tests.ps1` | `check` | 已受治理/fixture CI 接线 | 用 OS 临时 workflow/artifact/summary 和进程内 fixture action 验证 runtime preflight、物理权威路径、planning/runtime attempt 分离、显式场景 adapter、结果 schema、稳定等价字段、失败分类、cleanup 与原子 summary；WMS 纯合同另以单条件反例覆盖业务 checkpoint 未收敛、非布尔 checkpoint、零执行、managed process/database/provider 残留、cleanup error、诊断采集/脱敏声明及敏感值替换，并以真实临时 artifact 执行共享写入与后置扫描。生产 verifier 的 AST 合同只证明 catch 中唯一 failure exporter 调用排在 `finally` cleanup 之前，不代表失败分支已实际执行。另直接执行生产 runtime library，覆盖 push、schedule、workflow_dispatch all-active 与 conservative PR 的 active/core 全集：顺序变化通过，真子集、额外成员、重复和非法成员失败。它不连接 PostgreSQL、Redis、Docker、Aspire 或业务服务；由 Script Governance 独立 5 分钟 step 和 `compat-fast` 执行，不能作为 hosted runtime 或 shadow 等价证据。 |
