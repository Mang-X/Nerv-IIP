...
| `verify-solution-configuration-membership.ps1` | `check` | 已受治理 | MAN-669 PR-C 解决方案配置映射门禁；对**每个**解决方案（默认 `backend/Nerv.IIP.sln` 与 `connector-hosts/Nerv.IIP.ConnectorHost.sln`）计算成员项目的 `ProjectReference` 传递闭包，要求闭包内不得出现非成员。MSBuild 通过解决方案的 configuration map 解析项目配置，只能传递可达的项目没有 map 条目、回落到自身默认配置，于是 `--configuration Release` 的构建把它产出到 `bin/Debug`，而构建输出里没有任何东西会失败。真实踩过两次：`Nerv.IIP.Contracts.Mes`（backend 解决方案，PR-B）与 `Sdk.Ops`/`Contracts.Ops`/`Contracts.IntegrationEvents`（ConnectorHost 解决方案，PR-C；run `31143773140`、`31138913408` 的 Connector Host job 日志各三行 `bin/Debug`）。与上面 `verify-backend-test-shards.ps1` 那条**目录规则**互补而非替代——目录规则按构造只认识一个解决方案，看不见本次泄漏（那三个项目是 backend 解决方案的合法成员）；反过来目录规则能抓到「没人引用的孤儿项目」，闭包规则抓不到。展开 MSBuild glob（`**` 跨目录，`*`/`?` 不跨），因为 `Nerv.IIP.MigrationGovernance.Tests` 用了 `..\services\**\*.Infrastructure.csproj`——按字面路径处理会报一个不存在的项目并把真实发现全埋在后面。**无 allowlist、无 owner-issue 豁免通道**，理由同上一条：一条登记的例外等于「明知配置错误仍然构建」。只读；接入 required check `Script Governance`。 |
| `tests/solution-configuration-membership.Tests.ps1` | `check` | 已受治理 | 上一条的契约测试：真仓库必须通过；**两个解决方案必须都在默认检查集合内**（防止门禁被悄悄收窄回 backend-only —— 那正是 PR-B 的目录规则漏掉 ConnectorHost 泄漏的方式）；用临时夹具解决方案证明「传递非成员必红，且失败信息同时点名非成员与把它拉进来的成员」「登记后必绿」「glob 必须展开、`**` 跨目录、叶子模式不误伤同目录的其它项目、字面 glob 文本绝不能被当作项目路径上报」。已做变异验证：把 glob 展开削弱成返回空集，glob 用例立刻红。夹具全部建在 OS 临时目录并在 `finally` 清理，不改动仓库。CI 在 Script Governance job 中以真实退出码执行。 |
