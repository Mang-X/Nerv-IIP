...
| `run-redis-cap-test-lane.ps1` | `verify` | 已受治理/真实 PostgreSQL + Redis | NERV-688 拆解④的 Redis Streams transport 入口；读取 `redis-cap-test-lane.json` 的显式 active 成员，分别执行 `psql` 与 `redis-cli` 协议探针，为成员创建 run/attempt 专属数据库和 CAP version，精确 discovery 后要求冻结身份全部 Passed 且零 skip。失败时只读取受限 PostgreSQL schema 与 Redis stream/group/pending 元数据，不读取消息正文；`finally` 只删除本成员相对运行前新增的 Redis keys 并强制删除动态数据库，禁止 `FLUSHALL`，最终写 schema v2 dependency summary。 |
