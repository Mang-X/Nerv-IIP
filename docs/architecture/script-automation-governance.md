...
| `run-postgres-test-lane.ps1` | `verify` | 已受治理/真实 PostgreSQL | NERV-688 拆解②模板与③逐服务入口；读取 `postgres-test-lane.json` 的显式 active 成员批次，以无凭据命令行参数的 `psql` 协议探针为每个成员创建 run/attempt 专属数据库，精确 discovery 后顺序运行筛选测试，要求各成员冻结身份全部 Passed 且零 skip；失败时按成员声明的受限 schema 保留脱敏诊断，再在 `finally` 删除对应数据库，写含聚合值与逐成员事实的 schema v2 dependency summary。当前 hosted job 接入 Inventory、MasterData、Scheduling、AppHub、BarcodeLabel、FileStorage、IndustrialTelemetry、Quality、MES、WMS 与 Maintenance 设备停机验收；其余服务仍由拆解③后续批次承接。 |
