...
| `lib/FullStackSessionRuntime.ps1` | `library` | 已受治理/精确 session cleanup | FullStack stop 先按 manifest 的 PID + 启动时间停止 coordinator/CLI/guardian，再按允许的进程名与精确 worktree 命令行收口脱离原进程树的服务进程；Windows 使用 CIM、Linux 使用 `/proc`，停止后 PID 读回不为空则 cleanup 失败。macOS 保留 manifest PID/进程树口径，不声明命令行扫描能力。两个 FullChain FullStack 场景还会在测试前通过 owned PostgreSQL container 有界验证冻结 relation 集合，消除 resource healthy 早于 migration 完成的竞态。不得退化为按通用进程名或仓库外命令行清理。 |
