...
| `scripts/lib/CiRequiredSummary.ps1` | `check` library | 已受治理 | NERV-668 拆解①的 required-summary 结构模型；以 Ruby YAML 结构化读取 `.github/workflows/ci.yml`，要求稳定 `ci-summary` / `CI Summary` job 精确依赖当前五个 required 候选 job（含 NERV-685 PR①新增的 `Frontend Unit Tests`）、使用 `if: always()`，且唯一 step 无条件执行、固定 `bash --noprofile --norc -euo pipefail {0}` 并只含五条独立 `result = success` 断言。依赖身份缺席、额外依赖、非 success 白名单、step 条件、非 fail-fast shell、no-op、吞退出码或 `continue-on-error` 均返回 finding。当前非 required 的 ERP/OpenAPI 不在本层被升级。 |
...
| `tests/ci-required-summary.Tests.ps1` | `check` | 已受治理 | 对真实 workflow 先跑生产 verifier，再逐个写 OS 临时 YAML 变体；覆盖缺少 dependency、删除 `always()`、断言 step 加条件、改用非 fail-fast shell、no-op、`\|\| true`、job/step `continue-on-error` 以及把严格成功判断改为 `failure`/`cancelled`/`skipped`。每个变体都要求生产 verifier 非零且给出对应诊断，夹具在 `finally` 清理。CI 在 Script Governance job 中执行。 |
...
| `get-ci-impact-plan.ps1` | `check` | 已受治理 | 接受显式 changed paths，或通过受治理 Git helper 读取 PR merge-base / push range diff；写 `artifacts/ci-impact-plan/impact-plan.json`，并可追加 GitHub outputs 与 Actions Summary。本入口只报告选择，不启动测试、不修改 workflow/branch protection。 |
| `tests/ci-impact-plan.Tests.ps1` | `check` | 已受治理 | 覆盖纯文档、单业务服务、Contracts/Testing/Persistence/Messaging 扩散、BusinessGateway/OpenAPI、前端、connector-hosts、infra、FullChain、规则自改、非法路径、入口 JSON/output/summary 序列化及 workflow 只观测反向合同；临时输出在 `finally` 清理，并由 Script Governance 独立 step 执行。 |
