...
| `scripts/lib/CiRequiredSummary.ps1` | `check` library | 已受治理 | NERV-668 拆解①的 required-summary 结构模型；以 Ruby YAML 结构化读取 `.github/workflows/ci.yml`，要求稳定 `ci-summary` / `CI Summary` job 精确依赖当前四个 required job、使用 `if: always()`，且唯一 step 无条件执行、固定 `bash --noprofile --norc -euo pipefail {0}` 并只含四条独立 `result = success` 断言。依赖身份缺席、额外依赖、非 success 白名单、step 条件、非 fail-fast shell、no-op、吞退出码或 `continue-on-error` 均返回 finding。当前非 required 的 ERP/OpenAPI 不在本层被升级。 |
