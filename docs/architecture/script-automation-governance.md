...
| `tests/ci-required-summary.Tests.ps1` | `check` | 已受治理 | 对真实 workflow 先跑生产 verifier，再逐个写 OS 临时 YAML 变体；覆盖缺少 dependency、删除 `always()`、断言 step 加条件、改用非 fail-fast shell、`\|\| true`、job/step `continue-on-error`、selected lane 错收 skipped、unselected lane 错收 success、忽略影响计划失败和隐藏 `skipped by design` 审计。每个变体都要求生产 verifier 非零且给出对应诊断，夹具在 `finally` 清理。CI 在 Script Governance job 中执行。 |
...
| `tests/ci-impact-plan.Tests.ps1` | `check` | 已受治理 | 覆盖纯文档、单业务服务、Contracts/Testing/Persistence/Messaging 扩散、BusinessGateway/OpenAPI、前端、connector-hosts、infra、FullChain、规则自改、非法路径及入口 JSON/output/summary 序列化；另以结构化 workflow 合同钉住 Script Governance/OpenAPI Drift 的精确 fail-open PR 路由，并禁止其余 job 提前消费影响计划。错误 signal、删除计划失败分支、漏掉 backend 治理面、缺 dependency 或额外接入 job 的变异均失败；临时输出在 `finally` 清理，并由 Script Governance 独立 step 执行。 |
