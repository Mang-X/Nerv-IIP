...
| `scripts/lib/ScriptVariableBinding.ps1` | `check` library | 已受治理 | #1509 的共享静态变量绑定核；统一 `Set-Variable`/`New-Variable` 的 canonical、别名、模块限定名、大小写、参数元数据配对、字面量集合展开与作用域前缀归一化，并按 `Set-Item` cmdlet metadata 消费包括公共参数在内的全部参数值，再识别显式 `variable:` provider path。治理 checker 仍独占 assignment Left-shape 与作用域结构裁决，WMS verifier 合同只复用命令绑定核；计算名称、splat、`PSVariable.Set`、动态 provider path 与 ref 间接改写继续作为已登记残余，不被这个 helper 冒充已证明。只读取 PowerShell alias/cmdlet metadata，不执行被分析脚本。 |
| `scripts/lib/OrdinalComparisonContract.ps1` | `check` library | 已受治理 | #1509 的序数比较扫描面：`Get-NervOrdinalComparisonFindings` 解析一个 PowerShell 文件并报出 culture-aware 的标识符比较；`Get-NervOrdinalContractCoveredAxes` / `Get-NervOrdinalContractBlindSpots` 把「能扫到什么」和「扫不到什么」都做成可枚举的数据，由调用方用合成源码逐条正反对照。显式 `Set-Item variable:` 写入复用 `ScriptVariableBinding.ps1` 的 provider-path 核，其余 item mutation 轴保持本库权威。具名豁免按「函数名 + 表达式原文精确相等」匹配（不是子串——子串豁免能被放宽成裸 cmdlet 名并顺带吞掉未来的调用点，#1509 三轮实证；“精确相等”由 `[StringComparer]::Ordinal` 构造的查表兑现，四轮更正了裸 `@{}` 实为 OrdinalIgnoreCase 这个口径差），且必须恰好命中一处，死豁免与过宽豁免都会红。它存在的理由是 `test-evidence.Tests.ps1` 与 `backend-test-shards.Tests.ps1` 对两个不同的库做同一句声明——两份手写扫描器等于两句不同的声明。纯函数，只解析不执行。 |
