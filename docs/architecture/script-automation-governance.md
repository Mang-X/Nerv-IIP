...
| `run-full-chain-test-lane.ps1` | `verify` | 已受治理/真实 PostgreSQL + Redis + 多服务 | NERV-767 的五场景顺序执行入口；读取 `full-chain-test-lane.json`，先精确 discovery 再复用受治理 fullstack/acceptance 入口或直接运行冻结测试，统一收集成员 TRX。只管理本次拥有的 Compose PostgreSQL/Redis service；expected/discovered/passed、零 skip、依赖 readiness、逐成员结果和 cleanup 任一不完整均失败关闭，并写 schema v2 dependency summary。 |
...
| `tests/full-chain-test-lane.Tests.ps1` | `check` | 已受治理 | 用 OS 临时 manifest/TRX fixture 锁定五场景清单、唯一身份、入口与诊断登记，以及零发现、身份漂移、skip、失败和 cleanup 失败关闭；另钉住 runner 的环境注入与 summary 证据契约，不连接真实依赖。 |
