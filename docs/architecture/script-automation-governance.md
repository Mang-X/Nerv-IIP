...
| `scripts/lib/CiRequiredSummary.ps1` | `check` library | 已受治理 | NERV-668 required-summary 结构模型；以 Ruby YAML 结构化读取 `.github/workflows/ci.yml`，要求稳定 `ci-summary` / `CI Summary` job 精确依赖影响计划、当前五个 required 候选 job、ERP Acceptance、OpenAPI Drift 和 PostgreSQL Provider Tests，并使用 `if: always()`。唯一 step 无条件执行、固定 `bash --noprofile --norc -euo pipefail {0}`：未条件化 lane 必须成功；ERP Acceptance、Connector Host Tests、Script Governance、OpenAPI Drift 与 PostgreSQL Provider Tests 在被选择时必须成功，只有计划成功且明确未选中时才接受 skipped，并把前四者的 `skipped by design`、PostgreSQL lane 的 `skipped by policy` 与实际结果写入 Actions Summary。影响计划、选中 lane 的失败/取消/错误跳过、未选中 lane 的意外结果、身份缺席、step 条件、非 fail-fast shell、吞退出码或 `continue-on-error` 均返回 finding。这些 job 只是 Summary 内的条件依赖，不新增 branch-protection context。 |
...
| `tests/ci-impact-plan.Tests.ps1` | `check` | 已受治理 | 覆盖纯文档、脚本治理登记文档、单业务服务、Contracts/Testing/Persistence/Messaging 扩散、全部当前 `backend/common/**` 共享目录、ERP acceptance 输入闭包与 Connector/OpenAPI 后端中央构建输入、BusinessGateway/OpenAPI、PostgreSQL lane 三个自有路径、前端、connector-hosts、infra、FullChain、规则自改、非法路径及入口 JSON/output/summary 序列化；另以结构化 workflow 合同钉住 ERP Sales Order Demand Acceptance、Connector Host Tests、Script Governance、OpenAPI Drift 与 PostgreSQL Provider Tests 的精确 fail-open 且 cancellation-aware 的 PR 路由，并禁止其余 job 提前消费影响计划。删除 PostgreSQL 自有路径路由、错误 signal、删除计划失败分支、漏掉 ERP 闭包输入、缺 dependency 或额外接入 job 的变异均失败；临时输出在 `finally` 清理，并由 Script Governance 独立 step 执行。 |
