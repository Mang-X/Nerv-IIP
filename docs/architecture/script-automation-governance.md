...
4. `Stop-ProcessTree`：基于 root PID 清理自有进程树；Windows 读取 CIM 父子关系，Linux 从 `/proc/<pid>/stat` 读取精确 PPID 并递归收集后代，避免 `dotnet run` 父进程停止后业务可执行文件残留；停止后逐个读回冻结的全部 PID，任一仍存在即失败关闭。
...
7. `Get-ScriptAutomationSignalExit`、`Format-ScriptAutomationSignalExit`、`Get-ScriptAutomationInheritedSignalExit`：把子进程退出码分类为信号终止，渲染可被上一层识别的 `NERV-SIGNAL-EXIT` 单行标记，并从子进程捕获输出里回收更深一层的信号结论。分类只在非 Windows 平台成立（Windows 退出码是任意 32 位值，`128 + signal` 约定不存在），只覆盖编号在 Linux 与 macOS 之间一致的信号名，其余一律报泛化的 `SIG<n>`。

helper 必须异步或文件重定向 stdout/stderr，避免子进程缓冲区阻塞。超时后必须先尝试温和停止，再强制清理自有进程树，并把 killed PID 写入诊断。

`Invoke-NativeCommandWithTimeout` 与 `Invoke-NativeCommandOutput` 的非零退出路径必须在失败信息里给出信号结论：进程自身被信号杀死时直接命名信号，子进程只是普通非零退出但其捕获输出里带有 `NERV-SIGNAL-EXIT` 标记时继承那条结论。两条路径都必须原样保留 `exited with <code>` 子串——`scripts/verify-backend-test-determinism.ps1` 与两处契约测试用 `exited with (?<exitCode>\d+)` 解析它。
...
2. **`DynamicInvocation` 收窄为「注入式 action seam」。** 库可以 `& $Action`，条件是该变量在**本次调用所在的作用域链上可证明是 script block**：要么是 `[scriptblock]` 类型的参数，要么是被 `{ … }` 字面量（含 `.GetNewClosure()`）赋值的变量。作用域单位是一个 `ScriptBlockAst`（函数体或 `{ … }` 字面量），查找沿包围链向外走——与 PowerShell 自身的作用域一致：外层的 seam 对内层可见，**兄弟函数的同名变量永远不可见**，且**内层一旦自己绑定同名变量就遮蔽外层**（最内层绑定说了算：函数体里写 `$x = 'dotnet'` 就是一个局部，文件级的 `$x = { … }` 替它作不了保）。**「绑定」的准确范围见下表**——它不是「与运行期解析一致」，运行期还有两种残余拼写这条规则看不见，而作用域限定符那一侧又比运行期更严。这一条是刻意的：若按整文件收集，A 函数里的 `$action = { }` 就会替 B 函数里的 `$action = 'dotnet'; & $action` 放行，等于用一个流行参数名重新打开这条规则要堵的洞。这正是本仓库构造可测试库所依赖的注入 seam。`& 'dotnet'`、`& "$exe"`、`& (Get-Command …)`、`& $stringVariable` 一律仍是违规——最后一种是 `ForbiddenCommand` 在结构上看不见的那个洞，所以这条规则必须留在库作用域，而不是整体豁免。
   ...
   **「绑定一个名字」的拼写从 AST 类型穷举，不再逐轮追加**（#1509 四轮）。二轮只修了 `=`；三轮实测另外八种拼写照样 exit 0；四轮又实测出两种（`$a, $b = …` 多重赋值、`[string] $a = …` 类型化赋值）依然 exit 0，**并且被测进程真的执行了外部命令**。每一轮都只补了上一轮被点名的那几种，所以每一轮都落后一轮。现在 `AssignmentStatementAst.Left` 交给 `Get-SeamAssignmentTargets` **按类型层次递归展开**，下表的 Left 形状是把 `Left` 能取的类型写全的结果，而不是被报告过的拼写清单：
   ...
   | `Set-Variable` / `New-Variable`（名字实参**逐个字面量**展开；命名 `-Name`、`-Name:x` 冒号实参或位置参数；别名从 PowerShell 的别名表枚举，当前解出 `set`/`sv`/`nv`；命令名大小写不敏感；参数前缀缩写；模块限定名剥掉限定符后同样命中） | `CommandAst`，命令名按别名表 + 限定符归一化，元素配对按 cmdlet 参数元数据，名字实参按 AST 类型递归展开（`Get-NervScriptVariableBinderLiteralNames`） | ✅ | ❌ | `set-variable-shadows-seam`、`new-variable-shadows-seam`、`set-variable-positional-name-after-{valued,value,switch}-parameter`、`new-variable-positional-name-after-valued-parameter`、`set-variable-alias-positional-name-after-valued-parameter`、`set-variable-abbreviated-{valued,name}-parameter`、`set-variable-shipped-alias-{,uppercase-}shadows-seam`、`{set,new}-variable-module-qualified-shadows-seam`、`set-variable-module-qualified-mixed-case-shadows-seam`、`set-variable-module-qualified-alias-shadows-seam`、`set-variable-colon-argument-{single,multiple}-name{,s}`、`binder-alias-removed-from-session` |
   ...
   **命令名这一半也从元数据枚举**（#1509 七轮）。六轮只把**参数配对**改成读 cmdlet 元数据，**命令名识别**仍是手写的四条清单（`set-variable`/`sv`/`new-variable`/`nv`），于是同一个缺陷类换个拼写照样放行——实测 exit 0 且运行期真的执行了外部命令（以 `/bin/echo` 回显证实）：
   ...
   现在别名来自 `Get-Alias -Definition` 对 `$script:nervScriptVariableBinderCanonicalNames`（只声明 `Set-Variable`/`New-Variable` 两个 cmdlet）的枚举，模块限定名按最后一个 `\` 序数切分后取末段再比对。比较用 `OrdinalIgnoreCase`：**刻意的大小写不敏感**，因为 PowerShell 解析命令名本来就不分大小写（`SET`、`microsoft.powershell.utility\SET-VARIABLE` 实测都真的绑定），而 `OrdinalIgnoreCase` 只折叠大小写、不折叠任何可忽略字符，所以仍在本 PR 的序数口径内。
   ...
   **名字实参的「集合性」这一轴**（#1509 八轮）。六轮把**参数配对**改成读 cmdlet 元数据，七轮把**命令名识别**改成从别名表枚举，两轮都没问过第五个轴：**参数声明类型本身是不是集合**。`Get-Command Set-Variable` 说 `-Name` 是 `[string[]]`——一个实参可以拼出好几个绑定，而实现只认单个 `StringConstantExpressionAst`，于是整条绑定被丢掉。实测 exit 0 且运行期真的执行了外部命令（用 `/bin/echo` 回显证实：seam 应返回 `'seam'`，实际输出为空）：
   ...
   这**不是**残余表里的「名字只在运行期存在」：`action,zz` 是两个静态字面量，而文档那一行按字面读（「**字面量**名字，命名 `-Name` 或位置参数」）恰好覆盖这个拼写——又一次文档强度 > 实现强度。现在名字实参交给 `Get-NervScriptVariableBinderLiteralNames` **按 AST 类型递归展开**，与 `Get-SeamAssignmentTargets` 对 `$a, $b = …` 的处理同构，两处用的是同一条判据而不是两套强度。
   ...
   **这一轴被守成契约，不只是被修好**：`binder parameter collection types` 从 `Get-Command` 枚举两个 binder cmdlet 的**全部**参数，断言集合类型的恰好是 `Set-Variable` 的 `Name`/`Include`/`Exclude` 三个（单向：PowerShell 新增一个集合类型参数就红，逼人裁决；少一个不红），并分别断言 `Set-Variable -Name` 是 `[string[]]`、`New-Variable -Name` 是 `[string]`——上面两条裁决各自站在哪个测量结果上，是断言而不是记忆。`Get-NervScriptVariableBinderLiteralNames dispatch set` 则按 `Left type set` 的同一形式断言源码里对 `$Argument` 的 `-is` 分派集合恰好是那五种类型，并用一份语料实测这五种拼写真的解析成那五种 AST——删掉任一分支即便行为用例没覆盖到也会红。
...
| `scripts/lib/TestEvidenceArtifacts.ps1` | 全文件按上述扫描面**零发现**，具名豁免 **1 条**：`New-NervTestEvidenceSummary` 里 `Group-Object { Get-NervRetainedSkipReason $_ }`（按人读文案**分组**；同一行的**排序**不在豁免内）。豁免按「函数名 + 表达式原文精确相等」匹配，不是子串，且必须恰好命中一处；「精确相等」是真序数（查表用 `[StringComparer]::Ordinal` 构造，裸 `@{}` 的默认比较器是 OrdinalIgnoreCase，#1509 四轮更正）。 | `scripts/tests/test-evidence.Tests.ps1` |
...
**八份声明的强度上界怎么读**（#1509 六轮更正）。此前这里写的是「盲区之外的构造，声明成立」，那句话把「盲区表」当成了扫描面的补集——实测不成立：`-lt`/`-le`/`-gt`/`-ge`、`[string]::Compare`/`.CompareTo`、`[StringComparer]::InvariantCulture`、`.Sort()` 无参四类都是 culture-aware，既不在覆盖轴也不在盲区表。四类现已全部补进覆盖轴。准确的上界是分两半的：
...
1. `compat-fast`：在 macOS 或 Linux 环境运行 `pwsh scripts/check-script-governance.ps1`、`pwsh scripts/tests/ordinal-string.Tests.ps1`、`pwsh scripts/tests/check-script-governance.Tests.ps1`、`pwsh scripts/tests/script-governance-scan-boundary.Tests.ps1`、`pwsh scripts/tests/ordinal-comparison-layers.Tests.ps1`、`pwsh scripts/tests/test-evidence.Tests.ps1`、`pwsh scripts/tests/acceptance-scenario-matrix.Tests.ps1`、`pwsh scripts/tests/acceptance-scenario-matrix-runtime.Tests.ps1`、`pwsh scripts/tests/acceptance-scenario-matrix-equivalence.Tests.ps1` 和 `git diff --check`。其中 PowerShell 脚本统一由独立 `pwsh -Command` 进程内的调用运算符执行，覆盖 GitHub Actions 包装脚本产生的 script scope 语义，不以 `pwsh -File` 的全局作用域结果替代；脚本路径、具名参数字典与位置参数数组通过 CLIXML + Base64 只作为数据传输，在子进程内分别用 hashtable splat 与 array splat 绑定，不把参数值拼进可执行文本。`scripts/tests/script-compatibility-arguments.Tests.ps1` 通过现有 script-governance fixture 合同永久覆盖 switch、具名参数、普通值、空字符串、空格、单引号、前导连字符、注入形载荷、非零退出与超时。
...
| `scripts/lib/TestEvidencePolicy.ps1` | `check` library | 已受治理 | façade-owned 的 evidence policy 导入、静态校验、source Skip 扫描与 runtime 违规判定职责，精确定义 `New-NervTestEvidenceViolation`、`Import-NervTestEvidencePolicy`、`Test-NervQuarantineRuleMetadata`、`Get-NervSourceSkipAssignments`、`Test-NervTestEvidencePolicy`、`Test-NervRuleApplies`、`Get-NervTestEvidenceViolations` 7 个函数；依赖 `TestEvidence.ps1` 先行定义的 quoted-text、ordinal、property 与 lane helper，不支持消费者直接加载。quarantine metadata 的 policy/runtime 两调用点复用同一纯校验；保持 source assignment、classification、lane applicability、skip registration 与 real-dependency zero-execution 契约；不自行加载其他库。 |
| `scripts/lib/TestEvidencePrivacy.ps1` | `check` library | 已受治理 | façade-owned 的 retained test-evidence 隐私脱敏职责，精确定义 `ConvertTo-NervRetainedDisplayName`、`ConvertTo-NervRetainedFailureText`、`Get-NervRetainedSkipReason` 3 个函数；依赖 `TestEvidence.ps1` 先行定义的 `Find-NervQuotedTextEnd` 与 `Test-NervHasProperty`，并沿用调用方先加载的 `Protect-ScriptAutomationText`，不支持消费者直接加载。保持 body marker/digest、固定 failure 文案、skip reason policy 文案与 512 字符截断契约；若将来确需证据专属脱敏，应新建带实际行为的函数，不重新引入仅转发共享 redactor 的 Middle Man。纯转换，不读写文件，不自行加载其他库。 |
| `scripts/lib/TestEvidenceParsing.ps1` | `check` library | 已受治理 | façade-owned 的 TRX 与 dotnet console 输入解析职责，精确定义 `Get-NervTrxSkipReason`、`Read-NervTrxResults`、`ConvertFrom-NervDotNetConsoleSummary` 3 个函数；依赖 `TestEvidence.ps1` 先行定义的 lane、metadata 数据契约、outcome、stable-guid 与 ordinal helper，并消费先加载 Privacy 暴露的 retained display/failure helper，不支持消费者直接加载。保持 raw/normalized TRX、counter/provenance/error 顺序、单一 parse-result 返回对象与 console baseline import 契约；纯解析，不自行加载其他库。 |
| `scripts/lib/TestEvidenceArtifacts.ps1` | `check` library | 已受治理 | façade-owned 的 summary 与脱敏 artifact 渲染/写入职责，精确定义 `New-NervTestEvidenceSummary`、`Write-NervUtf8NoBom`、`ConvertTo-NervEvidenceIdentity`、`Write-NervEvidenceOutputPath`、`New-NervNormalizedTrxFileNameSet`、`Add-NervNormalizedTrxFileName`、`Get-NervNormalizedTrxHashedFileName`、`Resolve-NervNormalizedTrxFileNames`、`Write-NervTestEvidenceArtifacts`、`Write-NervTestEvidenceFailureArtifacts` 10 个函数；依赖 `TestEvidence.ps1` 先行定义的 retained schema、ordinal 与 lane helper，并沿用调用方先加载的 `Protect-ScriptAutomationText`，不支持消费者直接加载。collector 经 façade 消费 summary/成功与失败 artifact，baseline generator 也经 façade 直接消费通用输出原语 `Write-NervUtf8NoBom`；该跨职责消费不改变定义归属。唯一 ordinal 豁免是 summary 内按人读文案分组的 skip reason；叶子子库不自行加载其他库。 |
| `scripts/lib/TestEvidenceProvenance.ps1` | `check` library | 已受治理 | façade-owned 的 GitHub Actions provenance、跨 lane 权威性与 prior-attempt 判定职责，精确定义 10 个函数；依赖 `TestEvidence.ps1` 先行定义的 6 个 ordinal/lane helper，并沿用调用方先加载的 `Protect-ScriptAutomationText`，不支持消费者直接加载。run 身份固定 8 字段跨 lane 严格等值，per-job 环境固定 `runnerOs`/`runnerImage`/`dotnetSdk` 3 字段逐条校形并按 lane 对照权威 job log；prior-attempt 只有 run/job/SHA/lane/attempt 与唯一上一轮 failure 全部成立时才返回 verified。纯解析与校验，不读写文件，不自行加载其他库。 |
| `scripts/lib/TestEvidenceBaseline.ps1` | `check` library | 已受治理 | façade-owned 的 Baseline 生成职责，只定义 `New-NervTestEvidenceBaseline`；依赖 `TestEvidence.ps1` 先行定义的七个 ordinal/lane helper，不支持消费者直接加载。保持 schema 2、per-lane provenance 双向覆盖、authoritative job 绑定、复合键分组与 canonical ordinal 排序；不读写文件，不自行加载 `ScriptAutomation.ps1` 或 façade。composite-key 隔离变异 harness 必须把本文件与 façade closure 一并复制，并由结构断言防止后续子库拆分漏复制。 |
| `scripts/lib/ScriptVariableBinding.ps1` | `check` library | 已受治理 | #1509 的共享静态变量绑定核；统一 `Set-Variable`/`New-Variable` 与 `Set-Item`/`New-Item` family 的 canonical、`Get-Alias` 枚举、内建 alias 下界、模块限定名、大小写和 cmdlet metadata 参数配对。item family 识别 `-Path`/`-LiteralPath variable:name`，以及 `New-Item -Path variable: -Name name`；显式 `variable:` 根配动态 `-Name` 按 may-write 处理。`-Force`/`-PassThru` 不隐藏写入；`-WhatIf` 在静态分析中仍保守按 may-write。治理 checker 仍独占 assignment Left-shape 与作用域结构裁决，WMS verifier 合同复用同一命令绑定核；计算 binder 名称、splat、动态 provider path 与 ref 间接改写继续作为治理 checker 的已登记 fail-open residual，不被这个 helper 冒充已证明。只读取 PowerShell alias/cmdlet metadata，不执行被分析脚本。 |
| `scripts/lib/OrdinalComparisonContract.ps1` | `check` library | 已受治理 | #1509 的序数比较扫描面：`Get-NervOrdinalComparisonFindings` 解析一个 PowerShell 文件并报出 culture-aware 的标识符比较；`Get-NervOrdinalContractCoveredAxes` / `Get-NervOrdinalContractBlindSpots` 把「能扫到什么」和「扫不到什么」都做成可枚举的数据，由调用方用合成源码逐条正反对照。显式 variable provider 的 `Set-Item`/`New-Item` 写入及其 alias/参数绑定统一复用 `ScriptVariableBinding.ps1`，本库不再保留私有 New-Item 模型；其余 item mutation 轴保持本库权威。具名豁免按「函数名 + 表达式原文精确相等」匹配（不是子串——子串豁免能被放宽成裸 cmdlet 名并顺带吞掉未来的调用点，#1509 三轮实证；“精确相等”由 `[StringComparer]::Ordinal` 构造的查表兑现，四轮更正了裸 `@{}` 实为 OrdinalIgnoreCase 这个口径差），且必须恰好命中一处，死豁免与过宽豁免都会红。它存在的理由是 `test-evidence.Tests.ps1` 与 `backend-test-shards.Tests.ps1` 对两个不同的库做同一句声明——两份手写扫描器等于两句不同的声明。纯函数，只解析不执行。 |
...
| `scripts/lib/CiRequiredSummary.ps1` | `check` library | 已受治理 | NERV-668 required-summary 结构模型；以 Ruby YAML 结构化读取 `.github/workflows/ci.yml`。除稳定 `ci-summary` / `CI Summary` 的精确依赖与 selected/skipped 政策外，还要求 FullChain stable aggregate 精确闭合 planning、v1、shadow 与 equivalence 的 dependencies/result 矩阵。formal owner 静态检查识别 workflow step 中直接调用 `./scripts/collect-test-evidence.ps1` 且 `-Lane` 为裸值、单引号或双引号 `full-chain`，并识别 `actions/upload-artifact@v4`/`@v5` 中 artifact name 字面包含 `test-evidence-full-chain-`；wrapper 与动态构造不在证明范围。Summary 只消费稳定 FullChain aggregate、不直接依赖 v1/shadow/equivalence；各条件 lane 的政策与实际结果写入 Actions Summary。依赖缺失、选中轨道错误接受 skipped、直接证据 owner 冒用、身份缺席、step 条件、非 fail-fast shell、吞退出码或 `continue-on-error` 均返回 finding。这些 job 只是 Summary 内的条件依赖，不新增 branch-protection context。 |
| `verify-ci-required-summary.ps1` | `check` | 已受治理 | CI Required Summary 生产门禁入口；输出全部结构 finding 后非零退出，已在 Script Governance 中以独立 step 执行。它验证 workflow 契约，不冒充 GitHub hosted job 已运行，也不修改 branch protection。 |
| `verify-ci-dotnet-sdk-authority.ps1` | `check` | 已受治理 | #2407 PR B 的 CI/restore SDK authority 门禁。manifest 根 `project` 必须唯一、为字符串且精确指向 BusinessGateway Web 根项目；SDK 期望只从同一 manifest 根 `toolchain.sdk` 读取。门禁在 YAML mapping key 被普通 parser 折叠前以 Psych AST 要求每个受管 `actions/setup-dotnet` step 恰有一个 `dotnet-version` key，再结构化核对 12 个受管 job，并用真实 `dotnet --version` 核对当前进程；不读取 workflow 注释、伪造日志或调用方传入的期望版本。由 `tests/ci-impact-plan.Tests.ps1` 在现有 Script Governance step 中执行，覆盖同步 manifest/workflow SDK 升级的独立旧期望控制、同 step 重复 key，以及 manifest owner/root 缺失、错误、重复和类型错误；缺失、重复、浮动、任一侧漂移或真实进程不一致均非零退出。不修改 workflow、manifest、lock 或业务代码。 |
| `tests/ci-required-summary.Tests.ps1` | `check` | 已受治理 | 对真实 workflow 只启动一次生产 verifier，并以同一 workflow 上 dot-source 的生产 finding 函数建立等价 baseline；随后每个 OS 临时 YAML mutation 直接调用该生产函数，避免为每个变体启动新 PowerShell。覆盖 stable aggregate dependency/result、裸值/单双引号 collector lane、upload-artifact v4/v5、artifact name 包含匹配、非 v1 owner 冒用及 v1 collector/artifact 缺失；每个 mutation 都先证明文本实际改变，再要求命中精确 diagnostic。合同继续覆盖旧 ERP surface 不得回流、稳定聚合消费边界、`always()`、fail-fast、退出码与 CRLF。夹具在 `finally` 清理，CI 在 Script Governance job 中执行。 |
| `scripts/lib/CiImpactPlan.ps1` | `check` library | 已受治理 | NERV-668 拆解②的纯路径影响模型；规范化并 ordinal 去重排序仓库相对路径，输出 schema v1 粗域、共享层、Gateway/OpenAPI、前端分层、真实依赖、业务服务集合与逐信号理由。普通 Markdown 归 docs，`frontend/**` Markdown 还保留所属前端层；全部当前 `backend/common/**` 共享代码目录保守扩散到所有业务服务并选择 Connector 与 FullChain，两个后端中央构建文件也同时选择 OpenAPI、Connector 与 FullChain，`IntegrationEventHandlers` 归 Redis/CAP 消息链；Backend fast lane 的 manifest、evidence policy、runner、verifier、`OrdinalString.ps1`、selectors、diagnostics、合同与 manifest 引用的 heavy-lane owner scripts 精确选择 `scripts+backend`，PostgreSQL lane 的入口、库和 manifest 精确选择 `postgresql`。验收矩阵 planning 与 shadow runtime/equivalence owning paths 均选择 `scripts+backend+full_chain` 且业务服务集合为空。旧 ERP 专属闭包覆盖的 ERP、DemandPlanning、MasterData、FullChain 探针、acceptance verifier、中央构建输入及本地 compose 拓扑均保守选择 `full_chain`；`NuGet.config` 与共享运行库 `scripts/lib/ScriptAutomation.ps1` 因被多个既有 lane 消费而保留全信号 fail-open。workflow/规则自改、未知业务服务和未分类路径均保守选择全部影响；不执行 Git 或网络调用。 |
| `get-ci-impact-plan.ps1` | `check` | 已受治理 | 接受显式 changed paths，或通过受治理 Git helper 读取 PR merge-base / push range diff；写 `artifacts/ci-impact-plan/impact-plan.json`，并可追加 GitHub outputs 与 Actions Summary。本入口只报告选择与逐信号理由；workflow 当前消费 `scripts`、`backend`、`openapi_codegen`、`connector_hosts`、`postgresql`、`redis_cap`、`full_chain` 七个标量 output 及 frontend workspace 选择，本脚本自身不启动测试、不修改 branch protection。 |
| `tests/ci-impact-plan.Tests.ps1` | `check` | 已受治理 | 覆盖纯文档、脚本治理登记文档、单业务服务、Contracts/Testing/Persistence/Messaging 扩散、全部当前 `backend/common/**` 共享目录、Backend fast lane 控制输入（含 selectors 直接依赖的 `OrdinalString.ps1`）、旧 ERP 输入闭包向 FullChain 的保守折叠与 Connector/OpenAPI 后端中央构建输入、BusinessGateway/OpenAPI、PostgreSQL lane 三个自有路径、验收矩阵 planning 与 shadow runtime owning paths、前端、connector-hosts、infra、FullChain、规则自改、非法路径及入口 JSON/output/summary 序列化；runtime owning path 对删除、错误大小写、alias 与路径漂移逐项运行生产库变异，且每个规范路径都必须精确选择 `scripts+backend+full_chain` 并保持业务服务集合为空。另以结构化 workflow 合同钉住旧 ERP job/output/signal 不得回流，以及 Backend 五个执行 job、Connector Host Tests、Script Governance、OpenAPI Drift 与 PostgreSQL Provider Tests 的精确 fail-open 且 cancellation-aware PR 路由；验收矩阵允许现行 hosted `acceptance-scenario-matrix-runtime` job，并在 Script Governance 内保留三个独立纯 fixture 合同 step（planning、runtime、equivalence），不允许把生产入口接入 fixture step 或新增 job。删除 `OrdinalString.ps1` 或 PostgreSQL 控制输入路由、错误 signal、删除计划失败分支、漏掉 FullChain 输入闭包、缺 dependency 或额外接入 job 的变异均失败；临时输出在 `finally` 清理，并由 Script Governance 独立 step 执行。 |
| `scripts/acceptance-scenario-matrix.json` | `manifest`（机器输入） | 已受治理 | NERV-673②-1 schema v2 六场景清单；运行时不写文件、无 cleanup、无独立环境要求，由 `AcceptanceScenarioMatrix.ps1` 消费并与 v1 五场景双向闭合。CI 以 planning job 生成受 provenance 约束的选择，并在 `sales-order-demand` 选中时驱动 hosted shadow runtime 与 v1/shadow 两轨等价；Script Governance 和 `compat-fast` 仍只执行纯 fixture 合同。 |
| `scripts/lib/AcceptanceScenarioMatrix.ps1` | `library` + `check` + `generate` | 已受治理/纯规划 | 读取 v2/v1 manifest、仓库 impact 静态根与调用方 workflow，提供选择、预算、discovery 和 artifact 严格合同，并可经 `ScriptAutomation` 对选中项目执行 restore/discovery。Writes：调用方声明的 planning artifact；cleanup：失败时删除 stale/partial artifact；requires：PowerShell 7。不是独立入口；CI 由 Script Governance 合同 step 与 `compat-fast` 间接执行。 |
| `plan-acceptance-scenario-matrix.ps1` | `check` + `generate` | 已受治理/hosted planning 接线 | 读取 manifest 与 workflow，对每个选中项目执行一次 `dotnet restore` 和一次 Release `--no-restore --list-tests`。Writes：NuGet cache、project restore intermediates、`artifacts/acceptance-scenario-matrix/planning.json` 与 `artifacts/script-logs/**`；cleanup：入口先删 stale success artifact，失败不留 partial artifact，并恢复 MSBuild node-reuse/dotnet build-server 环境变量；requires：PowerShell 7、.NET SDK 10。真实 `ci.yml` 的 `acceptance-scenario-matrix-planning` 调用本入口并输出 `sales-order-demand-selected`、tested SHA 与 digest；该代码接线不替代 hosted 运行证据。 |
| `tests/acceptance-scenario-matrix.Tests.ps1` | `check` | 已受治理/fixture CI 接线 | 用 OS 临时 manifest/workflow/artifact 与 fake `dotnet` 验证六场景 schema、v1 闭合、选择、预算、discovery、provenance、stale artifact 和真实 workflow 前置失败合同。Writes：OS 临时目录内的 JSON/YAML、fake executable、planning artifact，以及 fake-dotnet command/environment logs；cleanup：最外层 `finally` 精确递归删除本次唯一 `$fixtureRoot` 及其中全部自有 fixture/log；requires：PowerShell 7。由 Script Governance 独立 5 分钟 step 和 `compat-fast` 执行，不连接 PostgreSQL、Redis、Docker、Aspire 或业务服务。 |
| `scripts/lib/ScriptCompatibility.ps1` | `check` library | 已受治理 | 构造 compat-fast 的 CI 等价 `pwsh -Command` 参数；脚本路径、具名参数字典与位置参数数组经 CLIXML + Base64 传输，子进程内分别以 hashtable splat 与 array splat 调用目标脚本。命令文本只嵌入 Base64，不拼接路径或参数值；不执行进程。 |
| `tests/script-compatibility-arguments.Tests.ps1` | `check` | 已受治理/fixture CI 接线 | 用 OS 临时参数脚本和真实 `pwsh -Command` 子进程验证 switch/具名/位置参数、空值、空格、单引号、前导连字符、注入形载荷、非零退出与超时；由现有 `check-script-governance.Tests.ps1` 调用，因此随 Script Governance 与 `compat-fast` 执行，不新增 workflow step。fixture 与日志只写本次 OS 临时目录并在 `finally` 精确清理。 |
| `scripts/lib/AcceptanceScenarioMatrixRuntime.ps1` | `library` + `check` | 已受治理/shadow 合同 | 读取并校验 planning artifact、v2/v1 权威 manifest、workflow 预算与 caller 明确选择的受支持场景，冻结按场景分派的进程内 action、稳定等价向量、失败分类、cleanup readback 和原子 final summary。显式 adapter 当前支持 `sales-order-demand` 与 `wms-delivery-erp`，未知场景失败关闭；WMS canonical builder 消费 verifier 记录的业务布尔、诊断 evidence、精确 TRX 计数与 cleanup readback，并把公开 outbound 的 Completed + 非空 completedAtUtc 终态作为独立 `outboundCompleted` 稳定事实。共享 WMS 诊断写入器先扫描同目录临时文件，再原子发布并对 final 文件读回扫描；独立 secret pattern 对 Bearer、password/pwd 与非空 URI userinfo 的拦截是可证伪轴，caller 声明值扫描与 final reread 是防御纵深和发布完整性检查。发布前扫描失败会删除未发布临时文件且不覆盖既有 final；扫描 actual final 命中时删除该精确发布路径并失败关闭。失败采集能力由同一顶层 try 的 catch 中唯一、无条件且不在函数体内的 exporter 与 `finally` 存在性结构合同证明。picking、completion、completed outbound 与 `$businessEvidence` 必须各由唯一顶层 `New-Variable -Option Constant` 首次绑定；合同精确验证变量名、Constant 选项和 RHS/命令来源，completed outbound 还必须来自唯一 `Wait-WmsOutboundOrder -RequireCompleted`。该函数的同一 return-if condition 必须把 `rows.Count == 1` 与 `!RequireCompleted || Test-NervAcceptanceWmsCompletedOutboundReadback` 正确 AND 组合并返回公开 readback；同一 `$completedOutboundOrder` 的纯成员读再沿 `$businessEvidence.wmsOutboundOrder.completionReadback` 直接键路径接入 `status`/`completedAtUtc`。未调用函数与未调用 scriptblock/closure 均不属于顶层生产接线。静态写入识别复用 `ScriptVariableBinding.ps1` 的 #1509 Set/New-Variable 与 Set/New-Item 核；顶层 `PSVariable.Set` 也使合同失败，Constant 运行期守卫进一步拒绝字面量或计算名称的重绑定。计算 binder 名称、splat、动态 provider path 与 ref 间接改写仍是静态 residual，但不再能在 verifier 运行期改写这四个权威事实。focused test 以 `if ($false)`、未调用函数、追加覆盖、去除/修改 Constant、双谓词单绑定、断开 readback 与重复 exporter 变异验证结构合同，但不把 structural-only 证明冒充失败 capture、真实 provider 或 hosted 运行覆盖；真实失败执行仍须由故障注入证据证明。planning source attempt 与 runtime 自身 attempt 分开验证：前者只绑定 planning artifact，后者继续进入 shadow result/summary provenance。库本身只调用注入的进程内 action；真实依赖由生产入口装配，不改变 v1 evidence owner。 |
| `run-acceptance-scenario-matrix.ps1` | `verify` | 已受治理/hosted shadow 接线 | shadow controller 的生产入口；按 `-ScenarioId` 显式分派 `verify-erp-sales-order-demand-planning.ps1` 或 `verify-erp-wms-delivery-completion.ps1` 作为真实业务 action，并由 `acceptance-scenario-matrix-runtime` job 在 planning 精确选中时调用。入口分别接受 `PlanningRunAttempt` 与当前 `RunAttempt`，普通首轮/全量重跑仍可同值，selective rerun 则保留真实 producer attempt。Writes：runtime summary、shadow canonical result 与受治理业务/诊断/cleanup evidence；cleanup：验证精确 owned-resource cleanup 证据；requires：PowerShell 7、.NET SDK 10、Docker PostgreSQL/Redis。当前 workflow 仍只接线 sales shadow；WMS adapter 的代码接线不构成 hosted WMS shadow 成功证据。 |
| `tests/acceptance-scenario-matrix-runtime.Tests.ps1` | `check` | 已受治理/fixture CI 接线 | 用 OS 临时 workflow/artifact/summary 和进程内 fixture action 验证 runtime preflight、物理权威路径、planning/runtime attempt 分离、显式场景 adapter、结果 schema、稳定等价字段、失败分类、cleanup 与原子 summary；WMS 纯合同另以单条件反例覆盖 picking 数量/状态/时间、completion requestId、公开 outbound Completed + completedAtUtc、业务 checkpoint 未收敛、非布尔 checkpoint、零执行、cleanup、诊断声明及 Bearer/password/URI userinfo 异源扫描。生产 verifier 的 AST 合同以 `if ($false)`、未调用函数、追加覆盖、去除/修改 Constant、pipeline mapper、双谓词单绑定、重复 exporter、Set/New-Variable 与 Set/New-Item 的真实绑定拼写、`-Force`/`-PassThru`、顶层 literal/computed `PSVariable.Set`、完成 helper 绕过、未调用 scriptblock、`businessEvidence` 覆盖、死 dummy readback、常量加死成员读、null readback 与缺失 completionReadback 变异验证顶层唯一接线和 completed outbound readback；另以运行期 fixture 证明 Constant 变量绑定拒绝 literal/computed `PSVariable.Set` 重绑定，但不宣称其所引用对象深不可变。计算 binder 名称、splat、动态 provider path 与 ref 间接改写仍是明示静态负控制；`-WhatIf` 的保守 may-write 鉴别只属于 ordinal layers suite。它不代表失败分支、真实 provider 或 hosted shadow/equivalence 已实际执行；equivalence fixture 只回归新增稳定字段的 schema/比较行为，不是本轮库变异的 RED 证据。另直接执行生产 runtime library，覆盖 push、schedule、workflow_dispatch all-active 与 conservative PR 的 active/core 全集：顺序变化通过，真子集、额外成员、重复和非法成员失败。它不连接 PostgreSQL、Redis、Docker、Aspire 或业务服务；由 Script Governance 独立 5 分钟 step 和 `compat-fast` 执行，不能作为 hosted runtime 或 shadow 等价证据。 |
| `scripts/lib/AcceptanceScenarioMatrixEquivalence.ps1` | `library` + `check` | 已受治理/纯比较 | 严格校验 planning provenance 与 v1、shadow 两份 canonical result 的 schema、固定 track/path/source-attempt 绑定、稳定事实和 cleanup 结果；共同 runId、SHA、manifest/scenario 必须一致，物理 source attempt 不进入稳定业务向量 digest。report 只记录 planning/各 track 的 source attempt 和脱敏 digest，不泄漏 volatile 对象。只进行 ordinal 比较，不连接真实依赖。v1 仍是唯一 `full-chain` evidence owner。 |
| `verify-acceptance-scenario-matrix-equivalence.ps1` | `check` + `generate` | 已受治理/hosted equivalence 接线 | 接受显式 `-ScenarioId`，在 v1/shadow 两轨前置 job 成功后按各 producer 的 `needs` artifact identity 读取 planning 与 canonical artifacts，并把各自物理 source attempt 传给纯 comparator，原子写入 scenario 一致的 equivalence report；空值、非规范 attempt、artifact name 与 runId/attempt 不一致均在下载前失败关闭，失败时不保留成功形态的 partial report。真实 workflow 目前仅在 `sales-order-demand-selected=true` 时运行。artifact name 含 producer attempt 且不启用 overwrite，保留 selective rerun 前后两轮证据。 |
...
| `run-postgres-test-lane.ps1` | `verify` | 已受治理/真实 PostgreSQL | NERV-688 拆解②模板与③逐服务入口；读取 `postgres-test-lane.json` 的显式 active 成员批次，以无凭据命令行参数的 `psql` 协议探针为每个成员创建 run/attempt 专属数据库，精确 discovery 后顺序运行筛选测试，要求各成员冻结身份全部 Passed 且零 skip；失败时按成员声明的受限 schema 保留脱敏诊断，再在 `finally` 删除对应数据库，写含聚合值与逐成员事实的 schema v2 dependency summary。当前 hosted job 接入 Inventory、MasterData、Scheduling、AppHub、BarcodeLabel、FileStorage、IndustrialTelemetry、Quality、MES、WMS、ERP、DemandPlanning、跨业务 Acceptance 与 Maintenance 设备停机验收，即拆解③登记的全部非规模种子成员；两处自管生命周期特例按裁决登记为 `status: deferred`，不由本 runner 执行。 |
| `scripts/lib/PostgresTestLane.ps1` | `verify` library | 已受治理 | PostgreSQL lane manifest 与 TRX 的纯验证逻辑；成员必须显式记录 `core/extended`、`active/deferred/blocked`、安全数据库前缀与非空唯一测试身份。 |
| `run-redis-cap-test-lane.ps1` | `verify` | 已受治理/真实 PostgreSQL + Redis | NERV-688 拆解④的 Redis Streams transport 入口；读取 `redis-cap-test-lane.json` 的显式 active 成员，分别执行 `psql` 与 `redis-cli` 协议探针，为成员创建 run/attempt 专属数据库，并从完整 member/run/attempt 输入派生保留 attempt 的 CAP version 与精确 Redis 根命名空间。执行前只扫描并认领该根命名空间，非空或扫描失败立即失败且不删除既有键；精确 discovery 后要求冻结身份全部 Passed 且零 skip。失败时只读取受限 PostgreSQL schema 与该命名空间内的 stream/group/pending 元数据，不读取消息正文；`finally` 只扫描、删除并复核该命名空间，任一枚举、删除或复核失败都把 cleanup 标为失败，同时独立尝试强制删除动态数据库。禁止 `FLUSHALL`，最终写 schema v2 dependency summary。 |
| `scripts/lib/RedisCapTestLane.ps1` | `verify` library | 已受治理 | Redis/CAP lane manifest、TRX 与聚合 summary 的纯验证逻辑；成员必须登记 `core/extended`、`active/deferred/blocked`、安全数据库/CAP version 前缀、受限诊断 schema 与非空唯一测试身份，TRX identity 以 ordinal 精确集合相等判定。 |
| `tests/redis-cap-test-lane.Tests.ps1` | `check` | 已受治理 | 用 OS 临时 TRX/manifest fixture 验证 Redis/CAP member、冻结 identity、零 skip、完整发现和 cleanup 失败关闭；fixture 在 `finally` 清理，不连接真实依赖。 |
| `run-full-chain-test-lane.ps1` | `verify` | 已受治理/真实 PostgreSQL + Redis + 多服务 | NERV-767 的五场景顺序执行入口；读取 `full-chain-test-lane.json`，在所有有副作用入口前对共享项目只做一次 restore 与一次 `--no-restore` Release discovery，先闭合全部冻结身份，再复用受治理 fullstack/acceptance 入口或直接运行冻结测试并统一收集成员 TRX。内部最坏超时严格内嵌于 120 分钟 workflow step，schema v2 dependency summary 在启动、发现、逐成员与清理边界增量落盘。只管理本次拥有的 Compose PostgreSQL/Redis service；expected/discovered/passed、零 skip、依赖 readiness、逐成员结果和 cleanup 任一不完整均失败关闭。 |
| `lib/FullStackSessionRuntime.ps1` | `library` | 已受治理/精确 session cleanup | FullStack managed run 从 session 创建、场景、`aspire logs` 诊断到 `aspire stop` 完成都在精确 `NERV_IIP_SESSION_ID` 子进程作用域内，使每次 CLI 调用启动的 `aspire-managed` helper 继承会话归属。stop 先按 manifest 的 PID + 启动时间停止 coordinator/CLI/guardian；Linux 再通过 `/proc` 读取该标识，收口命令行不携带 worktree 的 helper 与业务服务进程，Windows/Linux 同时保留允许进程名加精确 worktree 命令行的兼容路径。停止后 PID 读回不为空则 cleanup 失败。macOS 保留 manifest PID/进程树口径，不声明命令行或环境扫描能力。两个 FullChain FullStack 场景还会在测试前通过 owned PostgreSQL container 有界验证冻结 relation 集合，消除 resource healthy 早于 migration 完成的竞态。不得退化为按通用进程名或仓库外命令行清理。 |
| `scripts/lib/FullChainTestLane.ps1` | `verify` library | 已受治理 | FullChain manifest、TRX 与聚合 summary 的纯验证逻辑；清单必须保持五个 active/core 场景和唯一冻结身份，TRX identity 使用 ordinal 精确集合相等。 |
| `tests/full-chain-test-lane.Tests.ps1` | `check` | 已受治理 | 用 OS 临时 manifest/TRX fixture 锁定五场景清单、唯一身份、入口与诊断登记，以及零发现、身份漂移、skip、失败和 cleanup 失败关闭；另钉住单次前置 discovery、禁用持久 build server、内部超时预算、增量 summary、fullstack 独立配方兼容性、runner 环境注入与证据契约，不连接真实依赖。 |
