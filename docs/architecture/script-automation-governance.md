...
| `tests/ci-required-summary.Tests.ps1` | `check` | 已受治理 | 对真实 workflow 先跑生产 verifier，再逐个写 OS 临时 YAML 变体；覆盖 stable aggregate 的 planning/v1/shadow/legacy ERP/equivalence dependency 删除、选中轨道错误接受 skipped，以及 shadow 冒充 formal collector/artifact owner、v1 丢失 collector/artifact。原有合同继续覆盖精确 `sales-order-demand-selected`、旧 ERP 入口/路由、`CI Summary` 只消费稳定聚合并保留旧 ERP 直接依赖，以及 `always()`、fail-fast、退出码与 CRLF 等变异。每个变体都要求生产 verifier 或聚合合同非零，夹具在 `finally` 清理。CI 在 Script Governance job 中执行。 |
