...
| `verify-ci-dotnet-sdk-authority.ps1` | `check` | 已受治理 | #2407 PR B 的 CI/restore SDK authority 门禁。只从 `docs/architecture/business-gateway-api-surface-restore.manifest.json` 根 `toolchain.sdk` 读取精确版本，结构化核对 12 个受管 `actions/setup-dotnet` job，并用真实 `dotnet --version` 核对当前进程；不读取 workflow 注释、伪造日志或调用方传入的期望版本。由 `tests/ci-impact-plan.Tests.ps1` 在现有 Script Governance step 中执行，缺失、重复、浮动、任一侧漂移或真实进程不一致均非零退出；不修改 workflow、manifest、lock 或业务代码。 |
