...
MAN-397 将退货单据定义为补偿性事实。已记录的采购收货、已过账凭证、已完成的 WMS 移动、已匹配的供应商发票和已开具的 AR 均保持不可变。因此，退货会创建独立编号的退货/借项/贷项单据及平衡凭证；绝不删除或修改原始事实。
...
所有跨服务事实均使用公开的版本化集成事件、消费者本地 inbox 记录、稳定的业务幂等键，以及不可恢复分歧时的 DLQ。ERP 绝不读取 WMS、Quality、Inventory 或 Finance 的 schema。
...
- BusinessERP 服务在 `erp` schema 中拥有采购退货、借项通知单、RMA、贷项通知单、AP/AR 核销和会计凭证。
