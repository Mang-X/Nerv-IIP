...
2026-09-03 起，AppHost 与 `infra/compose/nerv-iip.platform.yml` 显式注入 `FileStorage__UploadProvider=tus`，compose 另以持久卷承载 `FileStorage__Tus__RootPath`；代码内的默认值仍是 `server-proxy`，改变的是这两处部署配置。同日 `IUploadCommitStorage` 的生产实现落地为 `LocalTusUploadCommitStorage`：complete 的提交证据由它从本地 tus 字节读回实际 size 与 canonical SHA-256，证据与冻结意图的一致性判定仍在 `PostgreSqlFileStorageService`。在此之前生产容器只注册恒不提交的实现，complete 在任何部署下都返回 503。配置为 `server-proxy` 的运行实例没有本地字节面，complete 仍返回 503 并把上传会话重新打开。ADR 0023/0024 要求的按 `ObjectKey` 定位 final、staging/final 分区与 atomic promote 仍未实现，由 #994 与 #1012 承接。
