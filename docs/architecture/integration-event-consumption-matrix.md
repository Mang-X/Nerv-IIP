...
| `Nerv.IIP.Contracts.Wms` | `wms.WcsTaskCancelled` / `WmsIntegrationEvent` | WMS WCS cancellation converter. | WMS consumes the cancellation fact through the WCS adapter boundary and sends an idempotent adapter cancellation command using `AdapterType` and external task id. | `consumed-internally` | Device cancellation now has an in-platform adapter handoff; concrete WCS providers can replace the logging adapter behind the stable boundary. |
