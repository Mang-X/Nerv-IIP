...
| `Nerv.IIP.Contracts.Scheduling` | `scheduling.SchedulePlanInvalidated` / `SchedulePlanInvalidatedIntegrationEvent` | Scheduling input-change invalidation handlers publish the event after recording `SchedulePlanInvalidation` for affected generated plans, including reason, affected resources and affected operations. | MES consumes `SchedulePlanInvalidatedIntegrationEvent` to mark affected operation tasks as schedule-invalidated. Notification consumes it to create a planner reschedule task for configured `Scheduling:InvalidationNotification:RecipientRefs` or the default `role:production-planner`. | `consumed-internally` | No dangling action for invalidation propagation. Automatic re-scheduling remains outside #699. |
