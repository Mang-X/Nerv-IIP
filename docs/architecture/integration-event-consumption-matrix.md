...
| `Nerv.IIP.Contracts.Wms` | `wms.InboundOrderCompleted` / `WmsIntegrationEvent` | WMS inbound completion converter. | No active handler found. | `producer-only-until-feature` | Keep as warehouse execution fact; AP automation should use ERP purchase-receipt facts under `#509` unless a later issue requires WMS receipt projection. |
...
| `Nerv.IIP.Contracts.Wms` | `wms.OutboundOrderCancelled` / `WmsIntegrationEvent` | WMS outbound cancellation converter. | No active ERP handler found. | `needs-business-consumer` | ERP delivery cancellation projection remains a follow-up to the sales delivery flow; cancellation reason is carried in diagnostic message. |
...
| `Nerv.IIP.Contracts.Wms` | `wms.WcsTaskCancelled` / `WmsIntegrationEvent` | WMS WCS cancellation converter. | No active handler found. | `audit-or-external-only` | Adapter/device cancellation is an external WCS integration concern; WMS persists cancellation state and emits the fact. |
...
| ERP | `erp.PurchaseReceiptRecorded` / `ErpIntegrationEvent<TPayload>` | ERP receipt converter. | No finance event consumer path found. | `needs-business-consumer` | AP candidate/payable automation is owned by `#509`; first apply ERP reliability baseline `#507`. |
