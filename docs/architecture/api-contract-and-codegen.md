...
| `executeBusinessConsoleErpPaymentExecution` | `POST /api/business-console/v1/erp/finance/payment-executions` | 执行 AP 付款并按分配行部分或全额核销应付，透传 ERP 付款执行单审计链。 |
| `registerBusinessConsoleErpCashReceipt` | `POST /api/business-console/v1/erp/finance/cash-receipts` | 登记现金收款并匹配 AR，可部分核销应收。 |
