...
22. BusinessGateway WMS facade 已补齐 PDA 一线作业读写面：收货、上架、拣货、复核发货、盘点五类列表只接受 WMS 目录返回的 `self/work-pool/site` 工作范围，并支持状态、关键字、库位和批次过滤。前端只发送 `scopeKind/scopeId`；Gateway 从当前 bearer token 与 IAM 实时结果取得 principal、组织环境和 exact-site grants，按端点注入固定 WMS permission，再由 WMS 作业池资格 fail closed 解析范围，禁止客户端注入 actor、站点或内部派工过滤。收货、上架、拣货、复核和盘点五类聚合均提供受控 assignment facade；上架与拣货任务另提供 start/progress/exception/complete 四段动作，使用任务 `version` 做并发控制、持久幂等回执做同键重放，并只返回服务端计算的 `allowedActions/blockReasons`。对应 3 个范围目录、5 个派工端点和 8 个人工动作在 `facade-coverage-matrix.json` 均声明为 `exposed`；旧 WCS adapter 使用的通用 warehouse-task progress/complete 仍为 `internal`。
