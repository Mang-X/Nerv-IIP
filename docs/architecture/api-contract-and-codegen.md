...
| `getBusinessConsoleMesFoundationReadiness` | `GET /api/business-console/v1/mes/foundation-readiness` | MES 基础准备聚合检查。 |
...
| `listBusinessConsoleMesProductionPlans` | `GET /api/business-console/v1/mes/production-plans` | 生产计划列表。 |
...
| `runBusinessConsoleMesSchedule` | `POST /api/business-console/v1/mes/schedules/run` | 运行 MES 规则排程；不包含甘特，也不承担 APS 算法。 |
