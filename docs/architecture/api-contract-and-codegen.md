...
22. BusinessGateway WMS facade 已补齐 PDA 一线作业读写面：收货、上架、拣货、复核发货、盘点五类列表只接受 WMS 目录返回的 `self/work-pool/site` 工作范围。收货、上架、拣货和复核发货列表支持服务端状态、关键字、库位和批次过滤；盘点列表支持状态、关键字和库位过滤，当前聚合没有 lot 语义，因此不得向 OpenAPI 或前端伪造盘点批次参数。前端只发送 `scopeKind/scopeId`；Gateway 从当前 bearer token 取得 principal，读取 MasterData 当前组织/环境的启用 site 目录并与 IAM 对当前端点权限返回的 grants 求交，再按端点注入固定 WMS permission。Organization grant 只展开为该目录内的启用 site，Site grant 只命中同编码 site；客户端不能注入 actor、授权站点或内部派工过滤。
    ...
    WMS 服务端 `listWmsOperationalCandidates` 通过一个受 internal service authorization 保护的服务路由读取近期持久作业事实；BusinessGateway 将它拆成 `listBusinessConsoleWmsReceiptOperationalCandidates`、`listBusinessConsoleWmsShipmentOperationalCandidates`、`listBusinessConsoleWmsCountOperationalCandidates` 三个公开 operation，并分别固定 `receipts/shipments/counts` 候选域及 `business.wms.receipts.read`、`business.wms.shipments.read`、`business.wms.counts.read`。收货候选只来自入库单/上架任务，发货候选只来自出库单/拣货任务，盘点候选只来自盘点执行。响应必须保留 `sourceKind=wms-operational-facts`、`scopeKind/scopeId`、`asOfUtc`、可空 `freshnessUtc` 和 `truncated`，不得把该有界作业事实窗口解释为 MasterData 库位目录、Inventory `StockLocation` 权威目录或全量批次。该服务 operation 到三个 Gateway operation 的一对三 `exposed` 声明已登记在 `facade-coverage-matrix.json`，BusinessGateway OpenAPI 快照和 generated api-client 均包含三个公开 operation；公开盘点范围目录、候选和列表统一使用 `business.wms.counts.read`，写操作权限不随之扩大。

    收货、上架、拣货、复核和盘点五类聚合均提供受控 assignment facade；上架与拣货任务另提供 start/progress/exception/complete 四段动作，使用任务 `version` 做并发控制、持久幂等回执做同键重放，并只返回服务端计算的 `allowedActions/blockReasons`。对应 3 个范围目录、3 个作业候选、5 个派工端点和 8 个人工动作都通过 BusinessGateway OpenAPI/codegen 两跳公开；旧 WCS adapter 使用的通用 warehouse-task progress/complete 仍为 `internal`。
23. BusinessGateway IndustrialTelemetry/Equipment facade 的 tags、alarm-rules、alarms、equipment alarms 列表均使用服务端分页响应，并支持设备、状态等下游已暴露过滤；equipment alarm facade 默认使用 IndustrialTelemetry `active` lifecycle 过滤，返回 `raised`、`acknowledged` 和 `shelved` 等未 cleared 报警。equipment alarm facade 还暴露 acknowledge、shelve 和 unshelve 写入口，先由 BusinessGateway 执行 `business.iiot.alarms.write` 权限检查，再透传到 IndustrialTelemetry 生命周期命令；BusinessGateway 不在当前页结果上二次模拟全量过滤，也不拥有报警状态机。
...
29. #716 DemandPlanning forecast 与 MRP exception facade 已补齐：BusinessGateway 暴露 `listBusinessConsolePlanningForecasts` / `createOrUpdateBusinessConsolePlanningForecast`，下游对应 DemandPlanning `/api/business/v1/planning/forecasts`。MRP 输入准备会先用同 SKU/site 的 sales-order 与 released MPS 在 forecast period 前后 consumption window 内按规划 UOM 归一消耗 forecast，剩余 forecast 才进入 MRP；若 MasterData planning parameter / UOM conversion 上下文不可用，不跨单位冲减 forecast，只保留同 UOM 的基本消费口径，并通过 MRP 输入源 metadata 记录 MasterData snapshot 的 `none` / `error` 状态。`planning/suggestions` 读面可返回 `reschedule-in`、`reschedule-out` 和 `cancel` 例外消息，用于提示计划收货提前、延后或取消，不代表 ERP/MES 下游建单事件。
30. `getBusinessConsoleMesFinishedGoodsReceiptInventoryLink` 是 MAN-528/#972 的 MES 页面级公开关联：Gateway 必须先同时校验 `business.mes.receipts.read` 与 `business.inventory.ledger.read`，再以 `business-mes + RequestNo + WorkOrderId` 精确读取 Inventory movement/lot/current balance；无精确命中时显式返回未建立或 404，不得以 SKU、批次号或相似单号猜测关联。`linkStatus` 的折叠优先级固定为：库存过账失败（已有精确 movement 时为 `partiallyPosted`，否则为 `postingFailed`）→ 无精确库存事实 `notPosted` → 当前正余额受质量限制 `qualityRestricted` → MES 尚有剩余待过账数量 `partiallyPosted` → `posted`。因此 `qualityRestricted` 可以与 `remainingQuantity > 0` 同时成立；MAN-518 页面必须组合 `linkStatus`、`receiptStatus` 与 `remainingQuantity` 展示，不得把单一状态解释为完整过账进度。该聚合只整理 MES/Inventory 事实，不复制库存余额，也不提前实现 MAN-518 页面。
...
| `matchBusinessConsoleErpCashReceipt` | `POST /api/business-console/v1/erp/finance/cash-receipts/{cashReceiptNo}/match` | 匹配已登记收款并核销 AR，生成收款凭证并同步账龄读面。 |
| `openBusinessConsoleErpAccountingPeriod` | `POST /api/business-console/v1/erp/finance/accounting-periods` | 开启会计期间，建立期间 posting date 守卫范围。 |
| `closeBusinessConsoleErpAccountingPeriod` | `POST /api/business-console/v1/erp/finance/accounting-periods/close` | 关闭会计期间，关闭后拒绝新凭证。 |
| `reopenBusinessConsoleErpAccountingPeriod` | `POST /api/business-console/v1/erp/finance/accounting-periods/reopen` | 以审计原因重开期间，用于迟到集成事件或人工例外补录后再过账。 |
| `getBusinessConsoleErpTrialBalance` | `GET /api/business-console/v1/erp/finance/trial-balance` | 查询指定期间试算平衡。 |
| `getBusinessConsoleErpMonthEndChecklist` | `GET /api/business-console/v1/erp/finance/month-end-checklist` | 查询月结最小检查清单：未过账单据、未匹配发票、GR/IR 余额和凭证数。 |
...
| `queryBusinessConsoleQualitySpcControlChart` | `GET /api/business-console/v1/quality/spc/control-chart` | 查询按 SKU、特性和工作中心投影的 Xbar-R 控制图、控制限和判异结果。 |
| `queryBusinessConsoleQualityProcessCapability` | `GET /api/business-console/v1/quality/spc/process-capability` | 查询同一 SPC 范围内计量实测值的 Cp/Cpk 过程能力。 |
...
| `shelveBusinessConsoleEquipmentAlarm` | `POST /api/business-console/v1/equipment/alarms/{alarmEventId}/shelve` | 限时搁置报警，要求 `business.iiot.alarms.write`。 |
| `unshelveBusinessConsoleEquipmentAlarm` | `POST /api/business-console/v1/equipment/alarms/{alarmEventId}/unshelve` | 手工解除报警搁置，要求 `business.iiot.alarms.write`。 |
...
| `queryBusinessConsoleTelemetryOee` | `GET /api/business-console/v1/telemetry/oee` | 基于设备状态与 MES 报工投影查询窗口 OEE；`availabilityRate` 保持兼容，口径为 shared EquipmentRuntime 的 productive runtime / loading runtime。性能率为 MES 总产出 ÷（productive runtime × 工序计划数量/标准工时快照的理论速率）；质量率为良品 ÷（良品 + 报废 + 返工）；OEE 仅在三个因子都可计算时返回乘积。响应同时返回报工数量、单位、理论速率、理论产出、`isDegraded` 和 `degradedReasons`，缺少状态、报工、统一单位、理论速率或 productive runtime 时相关因子/OEE 返回 `null`，绝不以 `1` 补位。 |
...
| `listBusinessConsoleWmsPutawayTasks` | `GET /api/business-console/v1/wms/putaway-tasks` | 按授权工作范围查询 WMS 上架任务，返回任务版本、派工事实、批次/序列号、允许动作与阻塞原因。 |
| `startBusinessConsoleWmsPutawayTask` / `recordBusinessConsoleWmsPutawayTaskProgress` / `reportBusinessConsoleWmsPutawayTaskException` / `completeBusinessConsoleWmsPutawayTask` | `POST /api/business-console/v1/wms/putaway-tasks/{warehouseTaskId}/{action}` | 在当前授权范围内开始、累计报进度、上报异常或完成上架任务；使用持久幂等键与任务版本并发门禁。 |
| `listBusinessConsoleWmsOutboundOrders` | `GET /api/business-console/v1/wms/outbound-orders` | 查询 WMS 出库/拣货/复核/发货列表。 |
| `listBusinessConsoleWmsPickingTasks` | `GET /api/business-console/v1/wms/picking-tasks` | 按授权工作范围查询 WMS 拣货任务，返回任务版本、派工事实、批次/序列号、允许动作与阻塞原因。 |
| `startBusinessConsoleWmsPickingTask` / `recordBusinessConsoleWmsPickingTaskProgress` / `reportBusinessConsoleWmsPickingTaskException` / `completeBusinessConsoleWmsPickingTask` | `POST /api/business-console/v1/wms/picking-tasks/{warehouseTaskId}/{action}` | 在当前授权范围内开始、累计报进度、上报异常或完成拣货任务；短拣必须提交差异原因，终态不可再次操作。 |
...
| `resumeBusinessConsoleMesOperationTask` | `POST /api/business-console/v1/mes/operation-tasks/{operationTaskId}/resume` | 工序恢复；沿用同一显式范围与幂等契约。 |
| `completeBusinessConsoleMesOperationTask` | `POST /api/business-console/v1/mes/operation-tasks/{operationTaskId}/complete` | 工序完工；沿用同一显式范围与幂等契约。 |
...
| `getBusinessConsoleMesFinishedGoodsReceiptInventoryLink` | `GET /api/business-console/v1/mes/finished-goods-receipt-requests/{requestNo}/inventory-link` | 以 MES 请求号与工单号精确关联 Inventory movement、产出批次和当前质量状态余额；要求 MES 入库读与库存台账读双权限，无匹配不猜测。 |
