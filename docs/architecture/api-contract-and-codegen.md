...
15. `getBusinessConsoleMesFinishedGoodsReceiptInventoryLink` 是 MAN-528/#972 的 MES 页面级公开关联：Gateway 必须先同时校验 `business.mes.receipts.read` 与 `business.inventory.ledger.read`，再以 `business-mes + RequestNo + WorkOrderId` 精确读取 Inventory movement/lot/current balance；无精确命中时显式返回未建立或 404，不得以 SKU、批次号或相似单号猜测关联。该聚合只整理 MES/Inventory 事实，不复制库存余额，也不提前实现 MAN-518 页面。
