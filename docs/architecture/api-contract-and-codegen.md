...
23. BusinessGateway MasterData facade 已补齐通用资源 detail/update/disable/enable、typed list 字段、reference-data `codeSet` 过滤、workshop/team-member 接口，以及 BusinessPartner 多角色和 `taxId` 字段。BusinessGateway 只透传 MasterData lifecycle 请求和 IAM 权限检查，不在 facade 层重写主数据生命周期。
