...
MAN-422 扩展的是上述既有 Scheduling plan response schema，不新增 route：`SchedulePlanContract.provenance` 为 nullable additive object，包含 `engineId`、`ruleProviderId`、`ruleProfileId`、`ruleProfileVersion`、nullable `engineInputFingerprint`、`traceSchemaVersion`、`replayStatus` 和 `constraintSources[]`。每个 constraint source 只公开 `sourceId`、`sourceVersion`、`outcome`、`factCount`、`factsFingerprint` 与 `reasonCodes[]`；`algorithmVersion` 继续是唯一 engine version 字段，不并行新增 `engineVersion`。新方案返回 `replayStatus=available`；无法恢复 exact effective input 的历史方案返回 `legacy-unavailable` 且 fingerprint 可空。既有 preview/create/workbench/revision/detail exposed facade 原样透传该 schema；route、permission、operationId 和 facade declaration 不变，因此 `facade-coverage-matrix.json` 不增加或修改行。OpenAPI snapshot 与 generated client 必须由 Task 6 从 Gateway 机械生成，不在文档任务中手改。

MES `runBusinessConsoleMesSchedule` 及其 `/mes/schedules` UI 仍是 deprecated legacy exception，不是 BusinessScheduling 的第二个 engine。它的既有 exposed facade 继续存在，迁移/删除另立 follow-up；新功能不得依赖该 route。MES 与 BusinessScheduling 的 canonical API/assembly boundary 是 `Nerv.IIP.Contracts.Scheduling` 的 released/revoked events，MES 不消费 provider/engine implementation。
