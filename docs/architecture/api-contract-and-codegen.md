...
22. BusinessGateway WMS facade 已补齐 PDA 一线作业读写面：收货、上架、拣货、复核发货、盘点五类列表接收服务端授权的 `self/team/site/organization` 工作范围及状态、关键字、库位/批次过滤；前端只发送 `scopeKind/scopeId`，Gateway 从当前主体解析真实人员、班组、站点候选并 fail closed，禁止客户端注入内部 actor 或派工过滤。上架与拣货任务另提供 start/progress/exception/complete 四段动作，使用任务 `version` 做并发控制、持久幂等回执做同键重放，并只返回服务端计算的 `allowedActions/blockReasons`。
