...
| O3   | 公开列表返回一条 `assignedUserId=user-emp-012` 的 Queued 后序任务，至少一个更小 `operationSequence` 未 Completed | 通过公开 Gateway 尝试开始；PDA 375×812 路径消费同一门禁事实 | 服务端返回“前序工序尚未完成（工序 N）”，不泄露 blocking/current operation task raw ID；PDA 展示同一可读原因且不渲染开始按钮，目标任务状态不变。                                                                 |
