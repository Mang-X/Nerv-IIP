...
上述缺口已整批发 consolidated issue **#374**（拣货/上架/盘点 list、个人任务过滤、扫码 resolve）给后端，落地后再起计划。处置口径：缺口落地前，对应 PDA 作业页保持 disabled / feature-flag hidden 或以父单进入，不做半截入口或空跳转；「我的任务」v1 先按工作中心/状态在客户端聚合现有 list，缺口落地后切换为后端个人过滤。

分期落地（WMS 一线闭环）：**收货入库 `/wms/inbound` 与复核发货 `/wms/review` 已建（Plan 2，facade 为 inbound/outbound 的 list + complete）**，应用墙对应入口已点亮；拣货 `wms.pick`、上架 `wms.putaway`、盘点 `wms.count` 与扫码 resolve 仍待 #374，保持 `routeReady:false` / disabled。
