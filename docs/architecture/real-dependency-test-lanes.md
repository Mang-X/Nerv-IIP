...
| --- | --- | --- |
| `postgres` | `PostgreSQL Provider Tests` | 单服务或单测试宿主，证明 PostgreSQL 查询翻译、migration、约束、事务、并发及持久 inbox/outbox 等生产数据库语义。只有当测试不声称验证真实消息 transport 时，才可使用 CAP InMemory。当前 `backend-test-shards.json` 的 `real-postgres` 是过渡执行别名，第二层接线时必须收敛到既有证据 ID `postgres`，不得形成第四条 lane。 |
| `redis-cap` | `Redis/CAP Transport Tests` | 使用真实 Redis Streams，证明发布、消费、重复、乱序、重试、幂等及消费恢复；允许 PostgreSQL 作为持久化依赖，但断言终点是 transport 或单个消费者的持久结果，不冒充跨服务业务闭环。 |
...
| 事件 | 运行策略 | 门禁关系 |
| --- | --- | --- |
| pull request | NERV-668 按变更影响选择精确 lane、政策规则或 FullChain scenario；在该路由落地前，PR 上的真实依赖只允许 `workflow_dispatch` 预检，不建立一套临时 PR 过滤；main/nightly 自动运行不受此限制。 | lane job 不直接设为 required；被选择结果必须汇入唯一稳定的 `CI Required Summary`，失败、取消、缺席或零执行均阻止合并。 |
...
失败时先保留脱敏的 PostgreSQL、Redis/CAP、inbox/outbox/DLQ 与业务状态诊断，再由 `always()` 路径清理动态数据库、Redis 命名空间和子进程，并把清理结果写入证据；不得保存连接串、凭据、header 或业务正文。
...
Nightly 失败必须自动创建或更新按 lane 去重的 GitHub 故障 Issue，记录 lane、失败类别、tested SHA、run/attempt、证据链接和最后一次清理结果，并指向 **Nerv-IIP 平台 CI/测试治理**责任方；不得直接创建 Linear Issue，由现有 GitHub → Linear 集成同步。

第一次失败即把该 lane 标为“可信度降级”并阻止 release；连续两次失败或超过一个工作日未恢复时升级为 P1。后续成功运行须在同一 Issue 附恢复证据后关闭，不能只依赖 GitHub Actions 的个人通知或人工记忆。

## 非目标与后续边界

本页不写 workflow YAML、不修改测试、脚本、`ci.yml`、证据 policy 或 manifest，也不设计各业务 scenario 的步骤。第二层负责 PostgreSQL job 模板和单服务试点，第三层逐服务接入并收编 NERV-423，第四层接入 Redis/CAP，第五层由 NERV-767 接入 FullChain；NERV-673、NERV-677 消费本页裁决但保留各自业务矩阵和种子分层职责。
