...
Console or PlatformGateway
  ...
  -> Ops operation task
  -> Connector Host claims operation task lease
...
1. `Nerv.IIP.Contracts.Ops` 固化 Ops 公开 DTO，包括创建任务、claim lease task、任务详情、heartbeat、abandon 和执行结果。
2. `Nerv.IIP.Sdk.Ops` 提供 Connector Host 使用的 claim/heartbeat/abandon 与 result 回传客户端。
...
4. 高风险动作审批端点不信任请求 body 中的 actor；审批 actor 从服务端认证主体或受信内部转发头 `X-Authenticated-Actor` 派生，缺少可信 actor 时拒绝审批，请求人不能审批或拒绝自己发起的任务。
...
2. 不提供生产级分布式调度、持久化 outbox 或复杂重试退避策略。
