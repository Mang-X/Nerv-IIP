...
| Aspire AppHost | Required deployment/development orchestration baseline; platform-level AppHost exists at `infra/aspire/Nerv.IIP.AppHost` and currently covers PlatformGateway, AppHub, IAM, Ops, FileStorage, Connector Host, Console, PostgreSQL, Redis, optional RabbitMQ, MinIO and OpenTelemetry Collector. RabbitMQ is created only for the `Messaging:Provider=RabbitMQ` profile. | [.NET Aspire docs](https://learn.microsoft.com/dotnet/aspire/) | [dotnet/aspire](https://github.com/dotnet/aspire) |
...
| RabbitMQ | Optional messaging provider for multi-instance or cross-process event delivery. Default single-node PostgreSQL deployments use `Messaging:Provider=InMemory` with CAP InMemory message queue; RabbitMQ remains the broker profile for production expansion. | [RabbitMQ docs](https://www.rabbitmq.com/docs) | [rabbitmq/rabbitmq-server](https://github.com/rabbitmq/rabbitmq-server) |
| Savorboard.CAP.InMemoryMessageQueue | Required default CAP transport for single-node deployments where `Messaging:Provider` is omitted or set to `InMemory`; CAP outbox storage still uses the service database profile. | [CAP In-Memory Queue docs](https://cap.dotnetcore.xyz/user-guide/en/transport/in-memory-queue/) | [dotnetcore/CAP](https://github.com/dotnetcore/CAP) |
