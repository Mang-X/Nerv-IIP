...
本页仅保留 M2 旧路径兼容导航，不再承载 Product 正文。

当前 Product：[`../product/master-data/design.md`](../product/master-data/design.md)

<<<<<<< HEAD
迁移由 M2-C / #2395 完成；兼容页不得追加产品正文，删除条件由 M2-M/M4 统一处理。
=======
## 0. 为什么重做（现状问题，已核代码事实）
...
├── 技能目录      /master-data/skill-catalog  [SkillCatalog 受控技能定义]
└── 人员技能      /master-data/skills         [PersonnelSkill 矩阵(工人×技能)]
【受控数据】
├── 产品分类      /master-data/product-categories [ProductCategory 层级目录]
...
└── 编码规则      /master-data/code-rules     [编码规则与版本维护]
```
...
| **工装与模具** | **平表 + 注册 Sheet + 状态/使用动作** | **新页** | 关键字与状态走服务端筛选；登记真实工作中心 × SKU 适用组合；展示累计使用/寿命预警与排程资格；可用→保养、完成保养→可用、退役终态，状态原因必填；完成保养时，只有累计使用已达到保养寿命才会清零计数，页面须在确认前披露；`resources.read` 可查看，`resources.manage` 才可写 |
...
### 6.1 通用页面骨架（`@nerv-iip/ui` 区块组件，即「FE-2 区块」）
...
- **班组成员关系变更原因必填**（#1655）：从班组移除成员时，确认框必须收集 1–500 字的真实业务原因，显示字符计数并拒绝空、纯空白或超长输入；提交前去除首尾空白，失败时保留输入便于重试，成功或取消后清空。Gateway 与 MasterData handler 双层按同一口径校验，审计只写入该真实原因，不允许客户端或服务端代填占位值。
- **编码规则版本变更原因必填**（#1656）：新建编码规则版本时必须填写 1–500 字的真实业务原因，页面显示字符计数，并在提交后对空、Unicode 空白或超长输入给出内联提示；非法输入不得发出请求。提交前去除首尾空白，服务失败保留输入便于原地重试，成功或取消后清空；Gateway、MasterData handler 与领域模型同口径拒绝空原因，版本历史只记录用户提交的真实原因，不允许前端或服务端代填。
...
>>>>>>> ea2a55773 (fix(console): 披露工装保养清零条件)
