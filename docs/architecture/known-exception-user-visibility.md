...
> 优先保留受治理的语义字符串；只有既有 `success=false` envelope 缺少 `code` 或 `code`
> 等于 HTTP 状态时，才允许把安全的稳定 `message` 作为语义码。成功、400 和本地错误路径的
> 数值型 HTTP `code` 不变，HTTP 状态始终原样 fail-closed。
...
| BusinessGateway 通用 client，下游非 2xx 且非 400 | `preserve`（有条件） | 只处理 `success=false` 对象 envelope；优先保留最长 128 字符、只含 ASCII 字母/数字/`-_.` 且非纯数字的 `code`。既有无 `code` envelope 或 `code` 等于 HTTP 状态时，仅同规则的稳定 `message` 可作为语义码；畸形/不安全 envelope 固定回退 `downstream-request-failed`。HTTP 状态原样保留，`errorData`/`data` 仅输出过滤后的非敏感语义字段 | [`BusinessServiceHttpClient.cs`](../../backend/gateway/BusinessGateway/src/Nerv.IIP.BusinessGateway.Web/Application/BusinessServices/Shared/BusinessServiceHttpClient.cs)；[`BusinessServiceProxyException.cs`](../../backend/gateway/BusinessGateway/src/Nerv.IIP.BusinessGateway.Web/Application/BusinessServices/Shared/BusinessServiceProxyException.cs) |
