...
> 2026-08-26 NERV-1371 补充：BusinessGateway 代理下游非 400 失败时，公开 envelope 的 `code`
> 优先保留受治理的语义字符串；既有 `success=false` envelope 缺少 `code` 或 `code`
> 等于 HTTP 状态时，Shared client 默认不把 `message` 提升为语义码，只有 capability 明确登记的
> 现有稳定码才可兼容提升。成功、400 和本地错误路径的数值型 HTTP `code` 不变，HTTP 状态始终
> 原样 fail-closed。非 400 下游失败只有在 `message` 与最终校验通过的语义 `code` 完全一致时才
> 公开该 `message`；任何不同的自由文本均固定回退为 `downstream-request-failed`。
...
| 边界 | 政策 | 当前行为 | 源码证据 |
| --- | --- | --- | --- |
| BusinessGateway 通用 `BusinessServiceHttpClient`，下游 HTTP 400 | `preserve`（有条件） | 从失败信封读取 `message`，只对 400 使用 business-message 路径；消息须非空、最长 500 字符、首字符非空白，且不含控制字符或 `<>{}/\\`，否则改为 `downstream-request-failed`；公开 `code` 仍为数值 400 | [`BusinessServiceHttpClient.cs`](../../backend/gateway/BusinessGateway/src/Nerv.IIP.BusinessGateway.Web/Application/BusinessServices/Shared/BusinessServiceHttpClient.cs)；[`BusinessServiceProxyException.cs`](../../backend/gateway/BusinessGateway/src/Nerv.IIP.BusinessGateway.Web/Application/BusinessServices/Shared/BusinessServiceProxyException.cs) |
| BusinessGateway 通用 `BusinessServiceHttpClient`，下游 HTTP 2xx 且 `success=false` | `preserve`（有条件） | 默认从成功状态的失败信封读取 `message`，再走与 400 相同的 business-message 安全过滤；调用方设置 `failClosedOnFailureEnvelope=true` 时内部先抛 `InvalidOperationException`，随后在请求包装层转为 `BusinessServiceProxyException("downstream-invalid-response")`，公开响应不保留原消息 | [`BusinessServiceHttpClient.cs`](../../backend/gateway/BusinessGateway/src/Nerv.IIP.BusinessGateway.Web/Application/BusinessServices/Shared/BusinessServiceHttpClient.cs) 第 76-96、101-140 行；[`BusinessServiceProxyException.cs`](../../backend/gateway/BusinessGateway/src/Nerv.IIP.BusinessGateway.Web/Application/BusinessServices/Shared/BusinessServiceProxyException.cs) 第 43-52、73-88 行 |
| BusinessGateway 通用 client，下游非 2xx 且非 400 | `preserve`（有条件） | 只处理 `success=false` 对象 envelope；直接 `code` 必须是最长 128 字符、至少两个小写 kebab 段且不含敏感段的稳定语义码。既有无 `code` envelope 或 `code` 等于 HTTP 状态时，Shared 默认拒绝从 `message` 猜测 code，只兼容仓库现有跨域稳定协议 `idempotency-conflict`、`lifecycle-conflict`；capability 还可精确登记自己的现有公开协议（当前为 WMS 已登记 reason 集及 MES 的 `ROUTING_SNAPSHOT_MISSING`）。已登记 uppercase-snake 值仍须通过稳定码形态校验，未登记值不会被提升。公开 `message` 只有在与最终校验通过的语义 `code` 完全一致时才保留，任何不同的下游自由文本固定回退 `downstream-request-failed`。畸形、未登记或不安全 envelope 同样固定回退，HTTP 状态原样保留。`errorData`/`data` 只投影对象中的 `field`、`reason`：`field` 必须是非敏感 ASCII 标识符，`reason` 必须满足同一稳定语义码规则；未知字段、不安全值及投影后空对象一律删除 | [`BusinessServiceHttpClient.cs`](../../backend/gateway/BusinessGateway/src/Nerv.IIP.BusinessGateway.Web/Application/BusinessServices/Shared/BusinessServiceHttpClient.cs)；[`BusinessServiceProxyException.cs`](../../backend/gateway/BusinessGateway/src/Nerv.IIP.BusinessGateway.Web/Application/BusinessServices/Shared/BusinessServiceProxyException.cs)；[`BusinessConsoleWmsClient.cs`](../../backend/gateway/BusinessGateway/src/Nerv.IIP.BusinessGateway.Web/Application/BusinessServices/BusinessConsoleWmsClient.cs)；[`BusinessServiceClients.cs`](../../backend/gateway/BusinessGateway/src/Nerv.IIP.BusinessGateway.Web/Application/BusinessServices/BusinessServiceClients.cs) |
| BusinessGateway `AuthorizedBusinessProxyEndpoint` | `preserve` | 捕获带 `SemanticCode` 的下游 envelope 异常时，把语义码、安全 `Message` 和受控 `ErrorData` 写入公开失败 envelope；没有语义码的既有 400、本地错误及成功响应仍使用数值型 HTTP `code` | [`AuthorizedBusinessProxyEndpoint.cs`](../../backend/gateway/BusinessGateway/src/Nerv.IIP.BusinessGateway.Web/Application/Auth/AuthorizedBusinessProxyEndpoint.cs)；[`ResponseDataEndpointResults.cs`](../../backend/gateway/BusinessGateway/src/Nerv.IIP.BusinessGateway.Web/ResponseDataEndpointResults.cs) |
| BusinessGateway 直接调用公开构造器 `new BusinessServiceProxyException(status, message)` | `redact` | 构造器刻意丢弃 `message`，异常文本固定为 `downstream-request-failed` | [`BusinessServiceProxyException.cs`](../../backend/gateway/BusinessGateway/src/Nerv.IIP.BusinessGateway.Web/Application/BusinessServices/Shared/BusinessServiceProxyException.cs) 第 9-17 行 |
...
| BusinessGateway → AppHub | `redact` | client 虽读取下游信封，但失败时调用会丢弃 `message` 的公开 `BusinessServiceProxyException` 构造器 | [`BusinessAppHubClient.cs`](../../backend/gateway/BusinessGateway/src/Nerv.IIP.BusinessGateway.Web/Application/BusinessServices/BusinessAppHubClient.cs) 第 28-38 行；[`BusinessServiceProxyException.cs`](../../backend/gateway/BusinessGateway/src/Nerv.IIP.BusinessGateway.Web/Application/BusinessServices/Shared/BusinessServiceProxyException.cs) 第 9-17 行 |
