...
| `delivery_orders` / `delivery_order_lines` | business | ERP 发货请求与 WMS 实际出库完成投影。 | `delivery_order_no` 是业务编号；头记录 `released` / `partially-shipped` / `completed` / `cancelled`、首次 `shipped_at_utc`、整单 `completed_at_utc` 以及取消审计；行引用 SO line no，复制 SKU、UOM、location、可选 lot，并累计 `shipped_quantity`。 | delivery order no 唯一索引用于 WMS/AR 下游引用；`consumer_name + idempotency_key` inbox 保证同一 WMS 完成事件不重复累计，迁移前记录的 shipped quantity 默认为 0。 | release 后通过公开 WMS outbound-requested 事件请求出库；WMS 完成事件按 `organization_id + environment_id + DeliveryOrderNo` 和全部行实际数量增量投影，任一行未足量时保持 `partially-shipped`，全部行足量后才进入 `completed`、记录完成时间并幂等创建 AR；WMS cancellation 只允许在尚未出库时回写 `cancelled`。 |
