...
15. `backend/services/Business/MasterData/src/Nerv.IIP.Business.MasterData.Infrastructure/Migrations/20260706065509_DeepenDeviceAssetLedger.cs`
16. `backend/services/Business/MasterData/src/Nerv.IIP.Business.MasterData.Infrastructure/Migrations/20260713020248_AddMasterDataLifecycleAudit.cs`
17. `backend/services/Business/MasterData/src/Nerv.IIP.Business.MasterData.Infrastructure/Migrations/20260728232043_AddPrincipalScopeContextAudit.cs`
...
| `master_data_lifecycle_audit` | audit | MasterData 资源 enable/disable 的耐久操作事实，不以 domain event 或消息总线作为唯一审计存储。 | `id` 为 Guid v7；记录 `organization_id`、`environment_id`、`resource_type`、`resource_id`、`resource_code`、`resource_identity`、`target_enabled`、`actor_id`、`reason`、`operation_id` 和 `occurred_at_utc`；actor 来自可信认证上下文。 | `organization_id + environment_id + operation_id` 唯一，保护操作重放；`organization_id + environment_id + resource_type + resource_code + occurred_at_utc` 支持资源审计追溯。 | 仅在资源真实状态变更时与聚合同一 UnitOfWork 追加；同 operation 同 payload 重放幂等，同 key 冲突 payload 拒绝；历史不覆盖。 |
| `master_data_scope_context_audit` | audit | 记录会改变当前主体候选作业范围的 MasterData 关系变更，不把调岗、成员移除或层级迁移伪装成 lifecycle enable/disable。 | `id` 为 Guid v7；记录稳定 `operation_kind`、资源类型/ID/code/identity、可信 `actor_id`、`correlation_id`、`causation_id`、`operation_id`、授权相关字段的 `before_json/after_json`、reason 与 UTC 时间；JSON 不写姓名、电话等无关 PII。 | `organization_id + environment_id + operation_id` 支持按请求链追溯；`organization_id + environment_id + resource_type + resource_code + occurred_at_utc` 支持按资源回放。 | Worker/Site/Workshop/Team/ProductionLine/WorkCenter 创建、Worker 在岗状态变更、TeamMember 增删及范围层级调整与业务修改同一 UnitOfWork 追加；历史只增不改，资源启停仍由 lifecycle audit 表表达。 |
...
| `workers` | business | 工厂员工主数据，**「人」的业务权威源**（2026-07-26 裁决；IAM 只管登录身份，`iam.users` 的显示名/工号/部门为账号展示冗余）。承载工号、姓名、部门、岗位、在岗状态与联系电话；`user_id` 是班组成员、人员技能与 MES 派工共同引用的人员标识。 | `id` 为 Guid v7 强类型 ID；`organization_id + environment_id + code` 与 `organization_id + environment_id + user_id` 分别是工号与人员标识的业务唯一键；`employment_status` 取 `active`/`on-leave`/`resigned`；`disabled` 为归档软停用标记。 | 工号与人员标识各有唯一索引；`organization_id + environment_id + department_code + disabled` 支持按部门列员工。 | 员工离职用 `employment_status = resigned`，归档用 `disabled`，均不物理删除，保证历史派工记录可追溯。 |
...
| `device_assets` | business | 设备资产主数据，记录设备型号、厂区/车间/产线/工位层级、工作中心归属、采购/成本/保修/供应商、资产类别、静态容量、关键等级、父设备关系和可维护/可遥测标记。 | `id` 为 Guid v7 强类型 ID；`organization_id + environment_id + code` 是业务唯一键；`site_code`、`workshop_code`、`line_code`、`station_code` 与 `work_center_code` 为跨聚合业务引用；`purchase_date`、`purchase_cost`、`purchase_currency_code`、`warranty_expires_on` 和 `supplier_partner_code` 记录台账生命周期；`parent_device_id` 记录父设备公共 ID；`retired_on` 表达退役日期；`minimum_capacity`、`maximum_capacity` 与 `capacity_uom_code` 记录流程设备静态能力。 | 唯一索引保护设备 code；`work_center_code + disabled` 支持按工作中心列活跃设备；`site_code + workshop_code + line_code + disabled` 支持按资源层级列设备。 | 聚合创建后保留；退役使用 `retired_on` 表示生命周期结束，停用仍通过 `disabled` 退出新业务使用；Maintenance 和 Business Console 可只读消费保修/供应商信息，但主数据规则仍归 MasterData；PLC/DCS/SCADA 密钥、tag、报警和状态快照不进入本表。 |
| `device_asset_components` | business | 设备资产拥有的关键部件清单，用于最小父子设备/部件结构展示和维修上下文。 | `id` 为 Guid v7 强类型 ID；`device_asset_id` 指向所属设备聚合；`component_code`、`component_name`、`quantity` 和 `critical` 描述部件行。 | `device_asset_id + component_code` 唯一，防止同一设备内重复部件；`device_asset_id` 支持按设备加载部件结构。 | 子实体生命周期跟随 `device_assets` 聚合；不表达库存备件余额、维修领料或设备运行状态，这些分别属于 Inventory、Maintenance 和 IndustrialTelemetry。 |
| `tooling_assets` / `tooling_applicability` | business | MasterData 权威工装/模具主数据及工作中心、SKU 适用范围。 | 工装 code 由 Coding engine 的 `tooling-asset` 规则分配；状态为 Available/Maintenance/Retired；`maintenance_life_count` 与 `usage_count` 表达保养寿命门禁。 | `organization_id + environment_id + code` 唯一；适用行按 `tooling_asset_id + work_center_code + sku_code` 唯一。 | Retired 为终态；达到寿命自动转 Maintenance，只有 Available 且未到寿命的适用工装可进入 Scheduling。 |
| `changeover_matrix_entries` / `changeover_required_tooling` | business | 按工作中心、来源 SKU 或现有 ProductCategory、目标 SKU 维护权威换型分钟与所需工装。 | `source_type + source_code` 以非空规范键表达 exact SKU 或产品分类，避免 nullable 唯一键漏洞；exact SKU 匹配优先；setup 必须为正且至少关联一套工装。 | `organization_id + environment_id + work_center_code + source_type + source_code + to_sku_code` 唯一；所需工装按矩阵行和 tooling code 唯一，子行 FK 非空并级联。 | MasterData 拥有配置生命周期；批量入口按规范键 upsert 并可停用；Scheduling 只通过内部 facts API 读取，矩阵命中时覆盖 routing 默认 setup，未命中时保留 ProductEngineering 发布快照中的基础 setup。 |
...
20. `backend/services/Business/Quality/src/Nerv.IIP.Business.Quality.Infrastructure/Migrations/20260724073658_AddQualityReinspectionHistory.cs`
21. `backend/services/Business/Quality/src/Nerv.IIP.Business.Quality.Infrastructure/Migrations/20260730150913_AddInspectionTaskAssignmentScope.cs`
...
| `nonconformance_reports.close_reason / closed_by_actor` | audit columns | NCR 关闭耐久审计事实。 | `close_reason` 最长 500；`closed_by_actor` 最长 200，由 Quality 从可信认证/内部上下文解析，客户端不能提交 actor。 | 随 NCR 聚合在同一 UnitOfWork 写入。 | 仅关闭时写入；自动关闭调用方必须提供明确 reason 和 system actor。 |
...
| `code_idempotency_keys` | business | Quality 创建与检验任务提交的幂等回执，把客户端 key 绑定到 payload fingerprint 和首次业务结果。 | `organization_id + environment_id + rule_key + idempotency_key` 是唯一键；`code` 保存首次分配的业务 code 或权威检验记录 ID。 | 唯一索引阻止同一 key 在同一规则内重复创建不同资源或以不同载荷重放。 | 随创建/提交请求写入，保留用于超时重试去重、权威结果回放和冲突诊断。 |
...
| `inspection_tasks` | business | Quality 待检任务事实，由 ERP/WMS 收货、MES 工序完成和完工入库请求事件按激活的检验计划自动生成。 | `id` 为 Guid v7 强类型 ID；`inspection_plan_id` 记录命中的计划；`inspection_record_id` 在任务完成后引用生成的检验记录；`source_type + source_service + source_document_id + source_document_line_id + sku_code` 保留来源单据/行/工序上下文；`quantity`、`uom_code`、`batch_no`、`serial_no` 保留待检实物上下文；`assigned_user_id` / `assigned_team_id` 保存当前个人或班组归属；`version` 是派工、领取、完成共享的乐观并发版本，初始及历史升级回填值均为 `1`；`status` 为 `pending`、`in-progress` 或 `completed`；`due_at_utc` 和 `overdue_reminder_sent_at_utc` 支撑超期提醒。 | `organization_id + environment_id + status + due_at_utc` 支持组织待检和超期扫描；`organization_id + environment_id + assigned_user_id + status + due_at_utc`、对应 team 索引支撑 Self/Team 队列；来源业务唯一约束防止同一来源重复生成任务；`organization_id + environment_id + trigger_idempotency_key` 唯一约束支撑事件重投幂等。 | 事件生成后为 `pending` 并按来源事实派给检验员或班组；个人任务或授权班组任务经原子领取后进入 `in-progress`，仅当前检验员可提交并变为 `completed`；完成态只读；超期提醒只标记首次发送时间，不关闭任务。 |
| `inspection_task_assignment_receipts` | audit | 检验任务派工、转派和领取的耐久幂等与审计回执。 | 保存组织/环境、任务强 ID、`action`、稳定 `idempotency_key`、载荷指纹、可信 `actor_principal_id`、变更前后 inspector/team、转派原因、结果版本和 UTC 时间。 | `organization_id + environment_id + inspection_task_id + action + idempotency_key` 唯一，阻止同一意图重复产生不同结果；任务/时间索引支撑审计回读。 | 与任务状态变化在同一 UnitOfWork 写入；同键同载荷返回原结果，同键异载荷返回 409；转派必须保留原因。 |
| `inspection_records` | business | 检验执行记录和最终结果事实，保留来源单据、被检 SKU、数量、批次/序列号、库存放行维度、逐次复检谱系和处置引用。 | `id` 为 Guid v7 强类型 ID；`inspection_plan_id` 可选指向执行的计划版本；`source_type + source_service + source_document_id` 保留来源业务单据；`attempt_number` 是同来源/SKU 的一基尝试序号；`reinspection_of_inspection_record_id` 指向直接前序记录，初检为空；`uom_code`、`site_code`、`location_code`、`source_quality_status`、`owner_type`、`owner_id` 是 Inventory 事件消费所需的可选库存状态转换维度；`result` 为 `passed`、`rejected` 或 `conditional-release`。 | `organization_id + environment_id + source_service + source_document_id` 支持按来源追踪；`organization_id + environment_id + source_type + source_service + source_document_id + sku_code + attempt_number` 唯一约束既保留初检幂等又允许显式复检；`reinspection_of_inspection_record_id` 非空唯一约束保证一个前序只有一个直接后继，避免把请求重放误建为新业务尝试；`organization_id + environment_id + source_type + result` 支持场景+结果查询；`organization_id + environment_id + result` 支持仅按结果过滤。 | 初检为 attempt 1；仅 rejected/conditional-release 可对最新记录发起复检并追加 attempt，passed 为终态；每次记录创建都形成独立结果事实并发出 record-scoped 幂等的 passed/conditional-release/rejected 集成事件；计划复检沿用初检的精确计划版本，逐次结果/NCR 不覆盖；通过 `OpenNcrFromInspection` 每次记录最多关联一个 NCR。 |
...
| `stock_ledgers` | business | Inventory 拥有的当前库存余额事实，按 SKU、UOM、站点、地点、批次、序列号、生产日期、效期、受控库存状态和 owner 维度聚合。 | `id` 为 Guid v7 强类型 ID；`production_date` 和 `expiry_date` 记录批次生产/效期；`shelf_life_days` 与 `expiry_date_source`（`direct`、`derived`、`mixed` 或 null）记录可证明的效期来源，历史或混合来源不猜测保质期；`on_hand_quantity`、`reserved_quantity`、`moving_average_unit_cost`、`inventory_value`、`is_frozen_for_count`、`ledger_version` 和 `row_version` 维护余额、计价、盘点冻结和并发控制；`quality_status` 只允许 `unrestricted`、`quality`、`restricted`、`blocked`。 | 唯一索引保护同一库存维度只有一条余额并纳入生产日期/效期；SKU/site/expiry 索引用于 FEFO 与近效期查询；状态 check constraint 防自由文本库存状态。 | 由库存移动、预留/释放、状态转移和盘点调整驱动更新；FEFO 预留按 `expiry_date` 优先，过期批次默认禁止常规预留/出库；不由 WMS、MES 或 ERP 平行维护余额。 |
| `stock_movements` | business | 追加式库存移动事实，记录来源服务、来源单据、幂等键、有符号数量、批次效期和移动金额。 | `id` 为 Guid v7 强类型 ID；`source_service + source_document_id + idempotency_key` 保护外部调用幂等；`source_document_line_id` 保留工单/来源行公开关联；`production_date`、`expiry_date` 保留移动入账时的批次效期；`quantity` 为正负移动量；`unit_cost` 和 `movement_amount` 记录移动平均计价输入/结果。 | 幂等唯一索引防重复入账；`organization_id + environment_id + source_service + source_document_id + source_document_line_id + posted_at_utc` 支撑按来源单据/工单稳定查询 movement 与成品批次；SKU/site/location 索引用于追踪库存历史；SKU/site/expiry 索引用于效期追溯；状态 check constraint 防自由文本库存状态。 | 创建后不可变；余额、预留、计价变化通过 ledger 更新体现；MAN-528 的公开来源查询只以持久 source keys 命中，不按 SKU 或相似 lot 猜测。 |
...
21. `backend/services/Business/Mes/src/Nerv.IIP.Business.Mes.Infrastructure/Migrations/20260718021716_AddMesSchedulePlanProvenance.cs`
22. `backend/services/Business/Mes/src/Nerv.IIP.Business.Mes.Infrastructure/Migrations/20260718051406_AddMesScheduleReleaseWatermark.cs`
23. `backend/services/Business/Mes/src/Nerv.IIP.Business.Mes.Infrastructure/Migrations/20260718091703_AddMesSkuAvailabilityProjection.cs`
24. `backend/services/Business/Mes/src/Nerv.IIP.Business.Mes.Infrastructure/Migrations/20260724073623_AddMesWorkOrderCapitalizedUnitCost.cs`
25. `backend/services/Business/Mes/src/Nerv.IIP.Business.Mes.Infrastructure/Migrations/20260726054012_AddMesAssignedUserName.cs`
26. `backend/services/Business/Mes/src/Nerv.IIP.Business.Mes.Infrastructure/Migrations/20260728095215_AddOperationTaskTeamAndShiftSemantics.cs`
27. `backend/services/Business/Mes/src/Nerv.IIP.Business.Mes.Infrastructure/Migrations/20260729024918_AddMesMaterialRequirementSnapshotOutcome.cs`

| Table | Kind | Purpose | Key columns | Index intent | Lifecycle |
| --- | --- | --- | --- | --- | --- |
| `work_orders` | business | MES 持久化工单事实，记录 SKU、生产版本引用、计划数量、单位、优先级、交期、执行状态、累计良品/报废数量、ERP 权威资本化单位成本、完工关闭时间，以及从 DemandPlanning 或等价来源计划转单时复制的 durable source plan reference。 | `id` 为 Guid v7 强类型 ID；`organization_id + environment_id + work_order_id` 是业务唯一键；`production_version_id` 是 ProductEngineering 业务引用；`uom_code` 为来源计划单位快照；`completed_quantity`、`scrap_quantity` 和 `over_receipt_tolerance_percent` 支撑执行进度；nullable `capitalized_unit_cost` 是 ERP 资本化事件的耐久投影，用于桥接完工入库申请与成本事件乱序；`material_requirement_snapshot_status` 仅允许 `captured`/`no-requirements`，与 `material_requirement_snapshot_evaluated_at_utc`、`material_requirement_snapshot_production_version_id` 共同证明列表和开工命令消费的最新齐套快照结果，证明版本必须等于当前 `production_version_id`，空值或版本漂移按未证明失败关闭；`source_system + source_document_type + source_document_id + source_demand_reference` 是跨服务来源计划/需求回溯引用，不建立跨 schema FK；`source_demand_references`（text[]）持久化来源建议 peg 到的全部需求源引用（MRP 合批建议一张工单对应多张需求源单据，含主引用、按序去重），升级前历史行为 NULL，追溯读面回退单值 `source_demand_reference`。 | 唯一约束保护同一组织/环境内工单号；SKU/交期索引用于排产扫描；`source_system + source_document_id` 索引用于从来源计划回查已转 MES 工单。 | 工单创建后可 release/start/hold/complete/close/cancel/scrap；complete 表示计划数量已由良品或报废执行覆盖，成品入库仍由 `output_lot_genealogies` 的产出批次强引用单独门控；齐套快照在工单发布时记录结果，`no-requirements` 不伪造物料行，开工命令不在可回滚动作事务内补抓；升级前的既有需求行无法可靠归属当前生产版本，迁移不回填证明列并保持失败关闭，须由 governed snapshot 重新确认；工程版本自动重绑会清空证明并删除旧需求行，缺少当前版本证明时禁止把 `start` 暴露为可执行动作；资本化消费者与完工入库创建命令按 organization/environment/work-order scope 取得 PostgreSQL transaction-scoped advisory lock，资本化事件先到时投影与 consumer inbox 同事务提交，后续入库申请直接采用该成本，入库申请先到时由消费者回填；相同成本重放幂等，不同成本改写 fail closed；历史保留用于报工、入库请求、成本追踪和需求/MRP/MES 计划来源追溯。 |
| `mes_sku_availabilities` | business projection | MES 消费 MasterData `SkuDisabledIntegrationEvent` 后形成的 SKU 禁用投影，只用于阻止新的计划转工单和急单创建，不改写已有工单。 | `id` 为 Guid v7 强类型 ID；`organization_id + environment_id + sku_code` 唯一；`status`、`changed_at_utc`、`disabled_reason` 和 `source_event_id` 保存最新禁用事实。 | scope + SKU 唯一索引支撑创建门禁和并发收敛；`changed_at_utc` 是乐观并发 token。 | 消费者 inbox 先按 consumer/idempotency key 去重；PostgreSQL 按 scope/SKU 事务 advisory lock 串行禁用投影与新工单创建，较旧事件成功 no-op，非法来源或 payload 进入 DLQ。当前只消费禁用事件，SKU 重新启用仍由独立 `SkuChangedIntegrationEvent` 消费切片治理。 |
| `operation_tasks` | business | MES 工序任务事实，保存工序顺序、工作中心、可选工作中心、持续时间、执行窗口、暂停累计、实际人工/机器工时、派工班次/人员/设备事实、ProductEngineering 标准工序编码以及 Quality 报检触发快照。 | `id` 为 Guid v7 强类型 ID；`organization_id + environment_id + operation_task_id` 是业务唯一键；`operation_code` 是可选 StandardOperation 编码，用于 MES/PDA 查询当前生效 SOP/电子作业指导书；`sku_code`、`uom_code`、`planned_quantity` 和 `requires_quality_inspection` 复制工单/Routing 报检上下文；`assigned_user_id`、`device_asset_id`、`shift_id` 和 `assigned_at_utc` 记录当前派工事实；`manual_dispatch_revision` 默认 0，是随真实设备派工及其清除单调递增的生命周期版本，`has_active_manual_dispatch` 默认 false，标记当前是否仍拥有 MES 手工设备派工锁；`schedule_plan_id` 与 `schedule_release_revision` 记录当前 APS 派工的真实来源版本，旧撤销只能清除完全匹配的来源；`scheduled_at_utc` 仅由已下达 APS 排程分配写入、人工派工不写；`schedule_invalidation_reason_code` 记录排程失效原因码；`paused_at_utc`、`paused_duration_ticks`、`labor_time_ticks`、`machine_time_ticks` 记录暂停扣时后的执行工时事实。 | 工单/工序顺序索引用于按工单加载工序并执行前序门禁；`organization_id + environment_id + schedule_plan_id` 索引用于按排程版本撤销派工；不建立跨 schema FK。 | 随工单创建或执行调整保留；新版 release 更新来源水位，匹配 revoke 清除 APS 来源并标记失效，但保留 active manual dispatch 与执行中/终态的设备/指派事实；升级后的首个 governed release 会清理未被新版覆盖且 `scheduled_at_utc` 可证明为旧 APS 派工的任务；事件重放和旧撤销晚到均幂等。 |
| `schedule_release_watermarks` | business | MES 按组织/环境保存已消费的最高排程撤销版本，阻止跨 topic 乱序时先到的 revoke 被后到旧 release 反向复活。 | `organization_id + environment_id` 为复合主键；`revoked_plan_id`、`revoked_release_revision`、`revoked_at_utc` 保存最高撤销事实。 | 复合主键保证每个业务 scope 只有一个单调撤销水位。 | revoke 消费时只向更高 revision 推进；release revision 不高于水位时记录 dead letter 并无业务副作用；与事件 inbox 一起保证重放幂等。 |
| `production_reports` | business | MES 报工事实，记录工单/工序的良品数、报废数、返工数、完工标记、报工时间、可选报废原因/不良记录号、产出批次/序列号谱系，以及冲销单到原报工的审计链。 | `id` 为 Guid v7 强类型 ID；`organization_id + environment_id + report_no` 是报工业务号唯一键；`work_order_id` 与 `operation_task_id` 为 MES 业务引用；`material_movement_count` 固化本报工发出的生产耗料 Inventory movement 数；`source` 标识 `manual` 或 `telemetry`；冲销负数行以可空 `reversed_by varchar(100)` 固化经认证的操作者引用，历史与普通报工保持 `NULL`；OEE 字段是创建时固化的投影快照，冲销复用原报工快照，不建立跨服务 FK。 | 报工号唯一索引用于重试和追踪；工单/工序/时间索引用于执行时间线查询；`organization_id + environment_id + reversed_report_no` 唯一索引用于防重复冲销。 | 报工创建后作为执行历史保留；物料消耗通过公共 Inventory movement request 表达，工单累计 report/material count 随 `WorkOrderCompleted` 提供 ERP 成本闭包证明。 |
| `telemetry_production_report_candidates` | business | MES 接收 IndustrialTelemetry 生产计数 delta 后的人工处理队列；它保存配置为 draft 的自动报工，或在活动报警、未映射工作中心、无唯一在制工单时暂停的计数。 | `id` 为 Guid v7 强类型 ID；`organization_id + environment_id + source_idempotency_key` 是跨服务计数事实唯一边界；设备、tag、bucket、good quantity 和可选工作中心/工单/工序记录可审计的解析上下文；状态由 `draft`/`pending-confirmation` 转为 `confirmed`/`dismissed`，终态同时记录操作者、时间、处置原因或正式报工 ID。 | 唯一索引阻止同一 IIoT event idempotency key 重复生成候选；范围、状态和创建时间索引支撑过滤分页。 | #871 提供受治理读面和人工确认/忽略闭环；确认复用正式报工命令和稳定 `telemetry:{source_idempotency_key}`，不会产生重复计数。 |
| `telemetry_production_report_candidate_transitions` | business | 遥测报工候选状态变更的不可变审计历史。 | 记录 candidate、前后状态、认证 actor、可选处置原因和 UTC 时间。 | `candidate_id + occurred_at_utc` 索引支持详情时间线读取，候选删除时级联清理。 | 只由候选聚合领域方法追加，不允许 handler 裸改状态。 |
| `production_report_material_consumptions` | business | MES 报工引用的实际物料批次消耗事实，用于工单、报工、物料批次追溯和 Inventory 过账失败诊断；冲销报工写入负数量表示线边库存回补。 | `id` 为 Guid v7 强类型 ID；`organization_id + environment_id + report_no` 指向报工号；记录 `material_id`、`material_lot_id`、`uom_code`、消耗数量、线边领料申请号、耗料所在 `site_code`/`location_code`（取自供料领料单的线边目标库位）、`inventory_posting_failure_code`、`inventory_posting_failure_message` 和 `inventory_posting_failed_at_utc`。 | 物料批次索引用于按批次追溯；工单索引用于工单谱系；报工/物料/批次唯一索引用于报工明细加载和重复写入兜底。 | 随报工写入，作为执行追溯历史保留；不拥有 Inventory 批次余额；Inventory 拒绝生产消耗过账时由 MES 消费失败事件并回写到对应消耗事实，报工读面暴露最近失败诊断；历史缺失 UOM 的记录以 `UNSPECIFIED` 显式标记，不能用于发送库存事件。 |
...
| `quality_hold_transitions` | audit | MES quality hold 的 append-only lifecycle 事实，用于独立还原多周期 applied/automatic release/manual release 时间线。 | `id` 为 Guid v7；记录 org/env、source service/document、hold cycle、correlation、event kind、actor、time、reason、inspection record/document、origin 和可选 `idempotency_key`。 | 完整 scope/source/cycle/correlation/event kind 唯一索引保护自动事件重放；带 `idempotency_key` 的完整 scope 唯一索引保护人工操作；timeline 索引按 org/env/source/time/id 稳定读取。 | 与 `quality_hold_contexts` 当前投影在同一 UnitOfWork 追加，不 update/覆盖；旧投影不回填无法可信推导的 actor/reason/time。 |
...
| `material_issue_requests` | business | MES 领料/备料申请与线边接收事实，跟踪 requested/received 数量、单位、实际接收批次和 Inventory 过账失败诊断。 | `id` 为 Guid v7 强类型 ID；`organization_id + environment_id + request_no` 是领料申请业务号唯一键；记录工单、工序、物料、`uom_code`、批次、请求/接收数量、状态、`inventory_posting_failure_code`、`inventory_posting_failure_message`、`inventory_posting_failed_at_utc` 和 `inventory_posting_rollback_key`；线边调拨位置与在途过账状态由 `source_site_code`/`source_location_code`（库存实际持仓来源）、`target_site_code`/`target_location_code`（工位线边）、`pending_receipt_quantity`、`receipt_attempt`、`pending_posting_token`、`pending_issue_leg_posted`、`pending_receipt_leg_posted` 承载；WMS 回执三列 `wms_request_id`（WMS 为本申请建立的出库单号）、`wms_picking_task_no`（首个拣货任务号）、`wms_prepared_at_utc`（仓库接单时间）记录仓库接单结果。 | 请求号唯一索引用于重试和下游引用；工单/物料索引用于齐套汇总；工序索引用于工序开工检查。 | 创建后作为 MES 执行事实保留，并通过公共 Inventory movement request 事件表达线边领料出库意图；收料确认只登记在途数量，`received_quantity` 与状态必须等仓库出库、线边入库两条腿都回执成功才推进（齐套因此不会先于库存实扣转绿）；Inventory 过账拒绝只丢弃尚未落账的在途数量并保留失败诊断，尝试序号 `receipt_attempt` 使重试产生全新幂等键、不会被库存去重吞掉；一腿成功一腿失败时 `pending_issue_leg_posted`/`pending_receipt_leg_posted` 跨尝试保留，重试只重发未过账的腿，已实扣的腿不会被重复提交；历史缺失 UOM 的记录以 `UNSPECIFIED` 显式标记，写入侧在受理时即以业务错误回绝（不再等到事件转换处抛异常）；创建时发 `mes.MaterialIssueRequested`，WMS 据此建出库单与拣货任务，再以 `wms.MaterialIssueOutboundPrepared` 回写上述三列——单号与任务号始终由 WMS 拥有，MES 只记录，未回写即表示仓库尚未接单；WMS 拣货执行仍由 WMS 边界拥有。 |
| `finished_goods_receipt_requests` | business | MES 完工入库请求事实，向 Inventory 边界表达成品收货意图，并保留可选产出批次/序列号、生产日期、效期、入库单位成本、Inventory 累计过账数量、过账引用和失败诊断。 | `id` 为 Guid v7 强类型 ID；`organization_id + environment_id + request_no` 是入库请求业务号唯一键；记录 `work_order_id`、`sku_id`、`quantity`、`uom_code`、`unit_cost`、`status`、请求时间、`produced_lot_no`、`serial_no`、`production_date`、`expiry_date`、`posted_quantity`、`posted_inventory_movement_id`、`posted_at_utc`、`inventory_posting_failure_code`、`inventory_posting_failure_message` 和 `inventory_posting_failed_at_utc`；`posted_quantity` 支撑 `PartiallyPosted` 与剩余数量重试；`unit_cost` 对 legacy 与等待 ERP 资本化的记录可为空，获得正数成本后才向 Inventory 传递移动平均计价请求。 | 请求号唯一索引用于下游引用；工单/SKU/时间索引用于入库请求追踪；创建请求的幂等 fingerprint 包含请求 payload 的 `unit_cost`、`production_date` 和 `expiry_date`，同一业务重试 key 携带不同请求成本或效期时视为 payload 冲突。 | 创建时若请求未携带成本但工单已有 `capitalized_unit_cost`，立即采用权威投影并发出公共 Inventory movement request；否则保持无成本等待，资本化事件到达后回填并发出请求。Inventory 拥有正式库存移动和计价结果，MES 保存请求成本、累计已过账数量、已知过账引用和 Inventory 拒绝诊断；失败或部分过账请求通过同一请求号重试剩余数量，不创建新的入库申请。 |
...
| `code_idempotency_keys` | business | MES 创建请求与工序生命周期命令的幂等回执，把客户端 key 绑定到 payload fingerprint 和首次业务结果。 | `organization_id + environment_id + rule_key + idempotency_key` 是唯一键；`code` 保存首次分配 code，或工序命令的权威 status/changed-at 结果。 | 唯一索引阻止同一 key 在同一规则内重复执行不同资源、动作或载荷。 | 随创建/生命周期请求写入，保留用于超时重试去重、权威结果回放和冲突诊断。 |
| `cap_published_messages` | system | CAP published message outbox，由 netcorepal/CAP 基础设施维护。 | 主键由 CAP 类型定义。 | CAP 内部索引用于投递扫描和过期清理。 | 系统表随服务数据库迁移创建；业务代码不直接读写。 |
| `cap_received_messages` | system | CAP received message inbox，由 netcorepal/CAP 基础设施维护。 | 主键由 CAP 类型定义。 | CAP 内部索引用于消费幂等、分组扫描和过期清理。 | 系统表随服务数据库迁移创建；业务代码不直接读写。 |
| `cap_locks` | system | CAP distributed lock table，由 netcorepal/CAP 基础设施维护。 | 主键 `Key`。 | 主键用于 CAP 内部协调。 | 系统表随服务数据库迁移创建；业务代码不直接读写。 |
...
9. `backend/services/Business/DemandPlanning/src/Nerv.IIP.Business.DemandPlanning.Infrastructure/Migrations/20260719045306_AddSalesOrderDemandProjection.cs`
   ...
   - reclassifies legacy manual rows with `demand_type='sales-order'` and no upstream document id to `manual` before integrated order facts arrive; if a true `manual` row already owns the same reference, both are retained and the former sales-order reference receives the first unoccupied deterministic `legacy-so` suffix derived from its stable row id.
10. `backend/services/Business/DemandPlanning/src/Nerv.IIP.Business.DemandPlanning.Infrastructure/Migrations/20260731005011_AddMrpRunFailureReason.cs`

| Table | Kind | Purpose | Key columns | Index intent | Lifecycle |
| --- | --- | --- | --- | --- | --- |
| `demand_sources` | business | DemandPlanning 拥有的销售订单、手工需求和安全库存等需求来源事实；预测输入已拆到 `forecast_inputs` 并在进入 MRP 前做订单消耗。 | `id` 为 Guid v7 强类型 ID；`organization_id + environment_id + demand_type + source_reference + source_line_reference` 是业务唯一键；销售订单需求还保留 stable document id、customer、source version/status、SKU、UOM、site、数量和要求交期快照。 | 业务唯一索引允许同一销售订单的多行幂等投影；SKU/日期/类型索引用于 MPS/MRP 输入扫描。 | 手工需求创建或调整后保留；ERP sales-order full snapshot 按业务版本更新，缺失/取消行归零并标记 cancelled；只有 active 且数量大于零的需求进入 MRP。 |
| `sales_order_demand_projections` | business projection | ERP 销售订单到 DemandPlanning 的订单级消费水位与墓碑，不替代 ERP 订单事实。 | `organization_id + environment_id + sales_order_id` 为复合主键；记录 sales order no、customer、site、latest order version/status、last event id 和 occurred time。 | scope + sales order no 唯一索引支持业务编号追溯；主键保证同一 ERP stable id 只有一个水位。 | release/change/cancel 只接受严格更高业务版本；取消水位阻止迟到 release/change 复活需求。 |
| `processed_integration_events` | system | DemandPlanning 服务本地消费者 inbox，按公共事件幂等键阻止重复投影。 | Guid v7 主键；consumer name、event id/type/version、source service、idempotency key、processed time。 | `consumer_name + idempotency_key` 唯一；source/type/time 索引用于审计。 | 与需求投影在同一次显式 SaveChanges 中提交；并发唯一冲突按已处理收敛。 |
| `integration_event_dead_letters` | system | 保存 envelope 或业务载荷校验失败的 ERP 销售订单事件，避免形成永久 poison message。 | 通用持久 DLQ envelope、failure code/message、status 和 replay timestamps。 | consumer/status/time 与 consumer/event id 索引用于运维扫描。 | 非瞬态业务拒绝进入 Pending；基础设施瞬态失败仍交由 CAP retry。 |
...
| `mrp_runs` | business | MRP 计算运行头和输入快照元数据。 | `id` 为 Guid v7 强类型 ID；`run_id` 为外部可见运行编号；保存计划窗口、状态（Created=排队/Running/Completed/Failed）、`failure_reason`（异步计算失败原因）、输入快照摘要、输入来源汇总和输入覆盖周期。 | run id 唯一索引支持幂等读取；状态/创建时间索引用于运行列表。 | 每次 MRP 运行生成独立事实；不直接创建 ERP/MES 正式单据，运行结果可展示 MPS、销售需求、预测、安全库存等输入来源。 |
...
8. `backend/services/Business/Approval/src/Nerv.IIP.Business.Approval.Infrastructure/Migrations/20260711114639_AddApprovalRoutingAuditAndPendingIdentity.cs`
...
| `approval_template_steps` | business | 模板中的有序审批步骤定义，支持同 step no 的会签/或签和结构化条件路由。 | `id` 为 Guid v7 强类型 ID；`approval_template_id` 归属模板；`step_no` 为模板内顺序；`completion_policy` 为 `all`/`any`；条件字段覆盖金额上下限、单据类型、来源服务、组织和部门，旧 `condition_expression` 继续兼容。 | template + step no + approver 唯一，防止同一模板步骤重复配置。 | 随模板维护；运行链启动后会复制为 runtime steps。 |
| `approval_chains` | business | 运行中的业务审批链实例，绑定来源服务和来源单据并保留路由审计上下文。 | `id` 为 Guid v7 强类型 ID；记录 chain id、source service、source document id、status、`round_no`、`row_version`，以及 `routing_amount`、`routing_organization_id`、`routing_department_id`；`pending_identity_key` 是待审批期间的 SHA-256 业务身份键。 | 来源单据索引用于业务反查；状态索引用于待审批列表；`row_version` 作为 EF concurrency token；nullable unique `pending_identity_key` 在数据库层阻止同一来源单据并发创建两个 pending 链。 | 从 pending 进入 approved/rejected/returned/withdrawn 等状态时清空 pending identity；returned/withdrawn 重提时恢复 identity、推进 `round_no`；路由上下文持久保留用于审计，不替代 Ops 运维审批。 |
...
| Table | Kind | Purpose | Key columns | Index intent | Lifecycle |
| --- | --- | --- | --- | --- | --- |
| `inbound_orders` | business | WMS 入库执行单头，记录来源单据、SiteCode 仓库范围、状态、创建时作业人/工作池快照、收货完成与取消审计事实。 | `id` 为 Guid v7 强类型 ID；`organization_id + environment_id + inbound_order_no` 是业务唯一键；`assigned_operator_user_id`/`assigned_pool_code` 是可空创建快照；`cancellation_reason` 和 `cancelled_at_utc` 记录取消原因与 UTC 时间。 | 入库单号唯一索引防重复；`source_document_type + source_document_id + status` 索引支持 ERP 按来源关闭开放入库预期；组织、环境、状态、SiteCode 与作业人/工作池索引支持现场任务查询。 | 入库单创建后推进收货、上架、完成或取消；V1 以 `site_code` 表达仓库范围，不伪造 Warehouse 主数据；库存移动由 Inventory 边界承接。 |
...
| `outbound_orders` | business | WMS 出库执行单头、创建时作业人/工作池快照、复核包装事实和取消审计。 | `id` 为 Guid v7 强类型 ID；`organization_id + environment_id + outbound_order_no` 是业务唯一键；`assigned_operator_user_id`/`assigned_pool_code` 是可空创建快照；`version` 是每次聚合变更推进的乐观并发令牌；`cancellation_reason` 和 `cancelled_at_utc` 记录取消原因与取消时间。 | 出库单号唯一索引防重复；组织、环境、状态、SiteCode 与作业人/工作池索引支持现场任务查询；并发 posting 回调通过 `version` 冲突重试后重新判断全行状态。 | 出库单创建后推进拣货、复核、包装、完成或取消；V1 以 `site_code` 表达仓库范围；取消会释放 Inventory reservation 并保留审计事实。 |
...
| `warehouse_tasks` | business | 上架、拣货和补货建议任务事实，记录来源批次/序列号、创建时作业人/工作池快照、库位、执行量和人工执行审计。 | `id` 为 Guid v7 强类型 ID；`version` 从 1 开始并在每次成功变更后推进，是 EF concurrency token；异常与完成分别保留 code/reason/time/actor；补货建议以 BackorderOrder 为来源单据。 | 组织、环境、任务号唯一索引防重复；组织、环境、类型、状态、SiteCode 与作业人/工作池索引支持现场任务队列。 | `Open → InProgress → Completed`；拣货显式短拣可带原因进入 `CompletedWithDifference`；`Exception`、`Cancelled` 与完成态均为阻断后续执行的终态；V1 以 `site_code` 表达仓库范围。 |
| `warehouse_task_action_receipts` | business | 人工仓储任务动作的耐久幂等回执，保存首个成功响应以支持安全重放。 | `id` 为 Guid v7 强类型 ID；记录 task/action/idempotency key、payload fingerprint，以及结果状态、版本、执行量与差异量。 | `organization_id + environment_id + warehouse_task_id + action + idempotency_key` 唯一索引阻止重复动作；task + created time 索引支持回执诊断。 | 首次成功动作与 WarehouseTask 变更同事务写入；同 payload 重放原结果，不同 payload 复用 key 时拒绝，历史回执不删除。 |
| `count_executions` | business | WMS 盘点执行、创建时作业人/工作池快照和差异输出事实。 | `id` 为 Guid v7 强类型 ID；记录 count execution id、SiteCode/库位/SKU/差异数量；`assigned_operator_user_id`/`assigned_pool_code` 是可空创建快照；`inventory_count_task_id` 保存 Inventory count task public id，用于冻结台账和确认调整。 | execution id 唯一索引防重复；组织、环境、状态、SiteCode 与作业人/工作池索引支持盘点列表。 | 创建时通过 Inventory count-task 边界冻结对应台账，完成时通过 Inventory adjustment 边界确认差异；V1 以 `site_code` 表达仓库范围；外部 `count-adjustment` 事件不作为 WMS 盘点闭环。 |
...
| `inventory_movement_requests` | business | WMS 向 Inventory 请求库存移动的本地 pending/posted/failed 元数据，也是盘点 Inventory RPC 路径的持久完成回执。 | `id` 为 Guid v7 强类型 ID；记录业务来源、幂等键、movement type、posting 状态、Inventory movement id、可选 `inventory_reservation_id`、`production_date`、`expiry_date` 和失败代码/消息。 | 幂等键索引用于防重复 request；状态索引用于补偿扫描；posted/failed 事件按组织、环境、movement type、来源单据、来源行和幂等键匹配本地 request。 | 由 WMS completion 在本地事务内创建 pending request，并通过公共 `Nerv.IIP.Contracts.Inventory` movement-requested / stock-movement-posted / stock-movement-posting-failed 集成事件异步闭环；盘点 RPC 成功后直接持久化 posted receipt，超时重放只回放同一结果、不再次调用 Inventory；入库 request 携带收货效期，出库 request 可携带 Inventory reservation id 供 Inventory 过账时分配预留；Inventory 业务拒绝会把 request 标记为 Failed，释放对应预留并冻结对应入库/出库单，retry 只重提 failed 行。 |
...
14. `backend/services/Business/Erp/src/Nerv.IIP.Business.Erp.Infrastructure/Migrations/20260723164736_AddDeliveryOrderSiteCode.cs`
...
| `work_center_cost_rates` | business | ERP 拥有的工作中心实际人工小时费率追加式修订历史。 | 每行以 Guid v7 `id` 标识；`organization_id + environment_id + work_center_id` 是硬 scope，`hourly_rate numeric(18,6)` 必须为正数，`currency_code char(3)` 是规范化大写币种；`effective_from_utc` 包含、可空 `effective_to_utc` 排除，`revision` 在 scope 内单调递增；`changed_by` 保存规范化认证 actor，`reason` 与 `changed_at_utc` 保存不可变审计事实。历史单行数据迁移为 revision 1、CNY、Unix epoch 起效，并标记 `system:migration` / `legacy cost-rate migration`。 | `ux_work_center_cost_rates_scope_revision` 唯一约束 `organization_id + environment_id + work_center_id + revision`；`ix_work_center_cost_rates_effective_lookup` 覆盖 scope + 有效窗 + revision。PostgreSQL 写入事务按 scope 取得 transaction-scoped advisory lock 后分配下一 revision，唯一索引仍作为最终完整性门禁。 | MES 报工事件按真实数量/理论节拍折算小时，ERP 按报工发生时刻选择有效的最高 revision；没有有效费率时在 consumer inbox 之前进入持久 dead letter，禁止默认费率或跨组织/环境/工位回退，配置后可重放同一事件。 |
...
| `quotations` / `quotation_lines` | business | 销售报价和报价行。 | `quotation_no` 是业务编号；行记录 SKU、数量、单价和交期；`converted_sales_order_no` / `converted_at_utc` 记录报价转出的销售订单引用（null 表示未转出）。 | quotation no 唯一索引用于销售订单创建。 | 报价需显式 approve 才能创建销售订单；已转出报价重复转订单幂等返回既有订单号，不再新建。 |
...
| `delivery_orders` / `delivery_order_lines` | business | ERP 发货请求与 WMS 实际出库完成投影。 | `delivery_order_no` 是业务编号；头复制销售订单的真实 `site_code`，并记录 `released` / `partially-shipped` / `completed` / `cancelled`、首次 `shipped_at_utc`、整单 `completed_at_utc`、取消审计和单调 `version` 并发令牌；行引用 SO line no，复制 SKU、UOM、location、可选 lot，并累计 `shipped_quantity`。 | delivery order no 唯一索引用于 WMS/AR 下游引用；`consumer_name + idempotency_key` inbox 保证同一 WMS 完成事件不重复累计；`version` 乐观并发令牌拒绝不同幂等键在多副本消费时的陈旧累计写。新增 `site_code` 通过同租户、同环境、同销售订单号从 `sales_orders` 前滚回填后才收紧为非空，拒绝伪造默认库区。 | release 后按头 `site_code` 与行 `location_code` / `lot_no` 通过公开 WMS outbound-requested 事件请求真实库存分账出库；WMS 只有在 Inventory 对全部出库行返回 posted 后才发布完成事件，ERP 随后按 `organization_id + environment_id + DeliveryOrderNo` 和全部行实际数量增量投影，并幂等创建 AR 与凭证。库存过账失败保持 ERP 发货单未完成，WMS 公开失败码、失败原因与精确分账键并允许受控重试；并发消费最多重试三次，晚于 `cancelled` / `completed` 的完成事实和晚于首次出库的取消事实进入 `stale-delivery-state` dead-letter。 |
...
| `business_partner_availabilities` | business projection | ERP 消费 MasterData 业务伙伴变更后形成的最新可用性投影，用于采购订单供应商与销售订单客户创建/提交门禁。 | `id` 为 Guid v7 强类型 ID；`organization_id + environment_id + partner_code` 唯一；`status`、`changed_at_utc` 和 `source_event_id` 保留最新来源事实。 | 组织、环境、伙伴代码唯一索引保证每个 ERP 伙伴只有一条当前投影。 | `BusinessPartnerChangedIntegrationEvent` 按 inbox 幂等消费；较旧的 `changed_at_utc` 不覆盖新状态，active/disabled 事件更新同一行，不跨 schema 查询 MasterData。 |
...
16. `backend/services/Business/IndustrialTelemetry/src/Nerv.IIP.Business.IndustrialTelemetry.Infrastructure/Migrations/20260717100647_AddConnectorTagManifestCoverage.cs`
17. `backend/services/Business/IndustrialTelemetry/src/Nerv.IIP.Business.IndustrialTelemetry.Infrastructure/Migrations/20260717113612_AddConnectorManifestExactOrdering.cs`
18. `backend/services/Business/IndustrialTelemetry/src/Nerv.IIP.Business.IndustrialTelemetry.Infrastructure/Migrations/20260718040416_HardenConnectorTagBindingConstraints.cs`
...
| `alarm_shelve_idempotency` | business | 报警搁置操作的持久幂等台账，按 alarm + 幂等键唯一，存 payload fingerprint。 | `id` 为 Guid v7 强类型 ID；`organization_id + environment_id + alarm_event_id + idempotency_key` 是唯一幂等键；`payload_fingerprint` 是 shelve payload（shelvedAtUtc/durationMinutes/shelvedBy/reason）的 SHA-256 hex：同键同 fingerprint 视为重放（不重复应用），同键异 fingerprint 视为冲突拒绝；`created_at_utc` 记录首次落账时间。 | alarm+幂等键唯一索引使搁置操作跨窗口/跨独立操作（A→B→延迟A）durably 幂等，并把并发重复收敛为单次应用。 | 只增不改；作为搁置命令的判重台账，不参与报警业务读面。 |
| `connector_tag_manifests` | business | IndustrialTelemetry 接受的采集连接器 tag manifest 头，保存每个 canonical connector 当前获准 revision 和来源观测时间。 | `id` 为 Guid v7 强类型 ID；`organization_id + environment_id + collection_connector_id` 是业务唯一键；`manifest_revision` 是 lowercase SHA-256；`manifest_observed_at_utc` 用于 UTC 展示，`manifest_observed_at_utc_ticks` 保存不受 PostgreSQL 微秒精度折叠影响的 .NET UTC ticks 业务全序；`concurrency_version` 是应用维护的 bigint 乐观锁版本。 | org/env/canonical connector 唯一索引保证每个 collection connector 只有一个当前 manifest root；`concurrency_version` 防止多实例 stale write 覆盖。 | 同 exact-ticks observation 同 revision 幂等、异 revision 冲突；旧 observation 拒绝；新 observation 可接受回滚 revision；任何接受的 root mutation 递增 version。 |
| `connector_tag_bindings` | business | Connector manifest 的可复用 device/tag 绑定投影，同时保留 current、retired 和 activation 诊断状态。 | `id` 为 Guid v7 强类型 ID；`organization_id + environment_id + collection_connector_id + device_asset_id + tag_key` 是业务唯一键；`is_current`/`retired_at_utc` 表示 manifest 成员生命周期；`activation_observed_at_utc` 用于 UTC 展示，`activation_observed_at_utc_ticks` 保存 exact-ticks activation 全序；`concurrency_version` 是应用维护的 bigint 乐观锁版本；错误字段有界清洗。 | 完整业务键唯一索引防重复投影；manifest+current 索引支撑按 root 加载当前绑定；`concurrency_version` 防止并发 definition/retire/reactivate/activation 覆盖；数据库 CHECK 强制 activation status 枚举和 `is_current`/`retired_at_utc` 成对语义。 | replace omission 软退役；再次出现时复用原投影 ID；activation 仅允许 `pending`/`active`/`error`/`disabled`，旧 exact-ticks 观测不覆盖新状态；任何实际 binding mutation 递增 version。 |
| `telemetry_summaries` | business | 粗粒度采集汇总 bucket，保存 tag 数值摘要、可选来源元数据和 nullable canonical collection connector provenance，兼容报警规则评估和旧数据面。 | `id` 为 Guid v7 强类型 ID；`organization_id + environment_id + source_system + source_connector + device_asset_id + tag_key + source_sequence` 是原有幂等唯一键，且 source 元数据为空时按 not-distinct 参与唯一性；`collection_connector_id` 不改变历史幂等身份但参与 payload compatibility；`sample_count`、`min_value`、`max_value`、`average_value` 保存摘要；`bucket_end_unix_time_milliseconds` 是由 `bucket_end_utc` 派生的内部排序/迟到桶判断键。 | 来源 scope+序列唯一索引防重复；设备+tag+bucket 起点和 bucket end Unix ms 索引保留；org/env+canonical connector+device+tag+bucket end 索引仅在 summary 上支撑 manifest coverage join。 | 可由 raw historian 重算；旧行 canonical connector 保持 null，不从 `source_connector` 回填。 |
| `telemetry_raw_samples` | business | historian raw 层采样明细 bucket，保存采集端最细粒度写入的 min/max/avg/first/last、来源元数据和 nullable canonical collection connector provenance。 | `id` 为 Guid v7 强类型 ID；`organization_id + environment_id + source_system + source_connector + device_asset_id + tag_key + source_sequence` 是原有幂等唯一键，且 source 元数据为空时按 not-distinct 参与唯一性；`collection_connector_id` 不改变历史幂等身份但参与 payload compatibility；`bucket_start_utc`、`bucket_end_utc`、`hourly_window_start_utc` 和 `bucket_end_unix_time_milliseconds` 定位采样窗口与 raw→hourly 降采样窗口；其余字段保存 raw bucket 明细。 | 原有来源 scope+序列唯一索引和 historian 查询/retention 索引保持不变；raw 不增加 connector coverage join 索引。 | raw 层按保留策略清理；旧行 canonical connector 保持 null，不从 `source_connector` 回填。 |
...
13. `backend/services/Business/Maintenance/src/Nerv.IIP.Business.Maintenance.Infrastructure/Migrations/20260712095604_AddMaintenanceActualTechnician.cs`
14. `backend/services/Business/Maintenance/src/Nerv.IIP.Business.Maintenance.Infrastructure/EntityConfigurations/MaintenanceDeviceStateEntityTypeConfiguration.cs`
15. `backend/services/Business/Maintenance/src/Nerv.IIP.Business.Maintenance.Infrastructure/Migrations/20260713085152_PauseMaintenancePlansForDisabledDevices.cs`
16. `backend/services/Business/Maintenance/src/Nerv.IIP.Business.Maintenance.Infrastructure/Migrations/20260716050408_MakeMaintenancePlanTriggersOptional.cs`

| Table | Kind | Purpose | Key columns | Index intent | Lifecycle |
| --- | --- | --- | --- | --- | --- |
| `maintenance_work_orders` | business | 维修工单、报警引用、PM 来源、点检失败来源、诊断描述、结构化 failure mode/cause、计划指派技师、实际主执行技师、工时、费用、有效修复开始时间、报警恢复标记、设备不可用和完工事实。 | `id` 为 Guid v7 强类型 ID；`organization_id + environment_id + source_alarm_id` 防止同一报警重复开单；`source_plan_code` 记录 PM 自动开单来源；`source_type + source_reference_id` 记录 alarm、plan、inspection 等来源事实引用；`diagnostic_description` 保存自动开单诊断文本；`failure_mode_code`、`failure_cause_code` 支撑 ISO 14224 风格故障分类；`assigned_technician_user_id` 记录计划指派的外部用户引用，`actual_technician_user_id` 记录完工时的实际主执行技师并在未填写时回退计划指派；`estimated_labor_minutes`、`actual_labor_minutes`、`spare_part_cost_amount`、`external_service_cost_amount` 和 `cost_currency_code` 支撑维修人力与费用统计；`repair_started_at_utc` 支撑有效修复段 MTTR；`alarm_cleared` / `alarm_cleared_at_utc` 记录报警已解除但待维修确认状态；`device_asset_id` 引用 MasterData 设备。 | source alarm 唯一索引用于报警幂等；`ux_maintenance_work_orders_source_reference` 防止同一来源事实重复开单；状态/设备字段支撑维修看板查询；设备、实际技师、时间窗用于可靠性和工时费用统计。 | 手工、报警、PM 到期、用量到期或点检失败生成；完成后保留计划指派与实际执行差异、停机、修复段、实际工时、费用汇总和备件引用事实。 |
...
| `maintenance_plans` | business | 预防性维护计划、可选保养周期、可选下一次到期日、可选运行小时触发阈值和可选 runtime availability 维护窗口事实。 | `id` 为 Guid v7 强类型 ID；`organization_id + environment_id + plan_code` 是业务唯一键；`interval`（可空）、`starts_on`、`last_generated_on`、`next_due_on`（可空）、`runtime_hour_interval`、`last_generated_runtime_hours`、`next_due_runtime_hours`、`paused`、`owner`、`window_start_utc` 和 `window_end_utc` 描述计划、PM 生成状态、用量触发状态与维护窗口。 | 计划编码唯一索引用于防重复；设备与窗口字段支撑 availability 查询；`next_due_on` 支撑 PM 到期扫描；运行小时字段支撑用量触发 PM 追赶。 | 创建后作为计划事实保留；**不变式：每个计划至少有一个触发——日历 `interval`（ISO day interval，如 `P7D`）和/或 `runtime_hour_interval`**。纯运行小时计划 `interval`/`next_due_on` 为 `NULL`、无日历到期，只在累计运行小时越过阈值时开单；日历计划按原 `next_due_on` 追赶多周期；窗口边界必须成对提供并按 UTC 保存；MasterData 设备停用事件将同组织/环境/设备下的计划设置为 `paused=true`，暂停期间不生成工单且不推进到期游标。 |
| `maintenance_device_states` | system | Maintenance 对 MasterData 设备 active/disabled 状态的本地最新事实投影，用于计划创建与 PM 生成门禁。 | `organization_id + environment_id + device_asset_id` 为复合主键；`disabled` 是最新状态；`changed_at_utc` 和 `source_event_id` 保留事件顺序与追溯信息。 | `organization_id + environment_id + disabled` 支撑 PM 扫描中的停用设备过滤；主键保证每台设备仅有一条当前投影。 | 由 `DeviceAssetChangedIntegrationEvent` 幂等更新；旧事件不得覆盖较新状态。停用先于计划到达时，后续计划创建仍会暂停；active 更新投影但不会自动恢复已暂停计划。 |
...
| `code_idempotency_keys` | business | Maintenance 创建与完工命令的幂等回执，把客户端 key 绑定到 payload fingerprint 和首次业务结果。 | `organization_id + environment_id + rule_key + idempotency_key` 是唯一键；`code` 保存首次创建的工单 ID，或完工命令的权威 status/changed-at 结果。 | 唯一索引阻止同一 key 在同一规则内重复创建工单或以不同完工载荷重放。 | 随创建/完工请求写入，保留用于超时重试去重、权威结果回放和冲突诊断。 |
...
`maintenance_plans.window_start_utc` 与 `maintenance_plans.window_end_utc` 是 #207 设备 runtime availability 的可选维护窗口列；窗口边界必须成对提供，且按 UTC 存储。`maintenance_plans.next_due_on` 与 `last_generated_on` 支撑 #416 PM 到期自动开单；迁移会把已有计划的 `next_due_on` 回填为 `starts_on`。#554 起，PM 生成按原 `next_due_on` 多周期追赶，`runtime_hour_interval`、`last_generated_runtime_hours` 和 `next_due_runtime_hours` 支撑运行小时用量触发。MAN-440/#794 迁移 `20260716050408_MakeMaintenancePlanTriggersOptional` 把 `maintenance_plans.interval` 与 `next_due_on` 从 NOT NULL 改为可空，落地不变式「每个计划至少有一个触发（日历 `interval` 和/或 `runtime_hour_interval`）」：纯运行小时计划 `interval`/`next_due_on` 为 `NULL`，`generate-due` 的日历分支对 `next_due_on IS NULL` 天然跳过，只走运行小时阈值分支。同迁移新增三个 CHECK 约束作数据库层加固（领域层已强制，二者互为兜底）：`ck_maintenance_plans_has_trigger`（`interval IS NOT NULL OR runtime_hour_interval IS NOT NULL`）、`ck_maintenance_plans_calendar_trigger_paired`（`(interval IS NULL) = (next_due_on IS NULL)`）与 `ck_maintenance_plans_runtime_trigger_paired`（`(runtime_hour_interval IS NULL) = (next_due_runtime_hours IS NULL)`），禁止写入无触发计划、日历列不成对或运行小时阈值与游标不成对的永不到期/不完整状态。MAN-401/#717 增加 `paused` 与 `maintenance_device_states` 双重门禁：MasterData 设备停用消费会耐久暂停现有计划并阻止后续计划继续调度；事件重放由 Maintenance consumer inbox 幂等去重，设备状态更新、计划创建和 PM 生成由同一组织/环境分布式锁串行化。
...
15. `backend/services/Business/Scheduling/src/Nerv.IIP.Business.Scheduling.Infrastructure/Migrations/20260730035140_AddOrderUrgencyBucketLookupIndex.cs`

| Table | Kind | Purpose | Key columns | Index intent | Lifecycle |
| --- | --- | --- | --- | --- | --- |
| `schedule_problems` | business | APS lite 排程问题快照，保留输入 problem id、组织/环境、计划窗口、deterministic fingerprint 和 normalized problem JSON。 | `id` 为 Guid v7 强类型 ID；`organization_id + environment_id + problem_id` 是排程问题幂等范围；`problem_fingerprint` 记录输入快照指纹；`problem_json` 用于后续锁定/override 的真实工序和资源校验；`horizon_start_utc` / `horizon_end_utc` 记录排程窗口。 | `organization_id + environment_id + problem_id` 唯一索引用于同一业务上下文内重放、幂等诊断和从 plan 追溯输入。 | 生成排程方案时捕获并保留；不由下游服务直接修改。 |
| `schedule_plans` | business | APS lite 排程方案头，记录生成/发布/取代/撤销状态、算法版本、合同版本、问题指纹和方案 KPI。 | `id` 为 Guid v7 强类型 ID；`plan_id` 是公开方案 ID；`release_revision` 是组织/环境 scope 内单调发布版本；`revoked_at_utc`、`revocation_reason` 与 `superseded_by_plan_id` 保存撤销事实；`locked_operation_count` / `optimizable_operation_count` 明确 KPI 优化空间。 | `plan_id` 唯一；`organization_id + environment_id` 上 `status = 'Released'` 的 PostgreSQL partial unique index 保证 scope 仅一个 active release；scope + release revision 唯一索引保护版本水位。 | 新版发布在同一事务/PG advisory lock 下先把旧版转为 superseded，再发布新版；显式撤销转为 revoked；两种撤销均保留历史与 revision 并发布明确事件。 |
...
| `order_urgency_snapshots` | business | `order-urgency-v1` 不可变计算结果，保留三类贡献项、原因码、来源时间、计算时间、版本和输入指纹。 | scope、order/business reference、level、model version、input fingerprint、priority revision、15 分钟 calculation bucket、result JSON。 | scope + order + model + fingerprint + priority revision + bucket 唯一保证重算幂等；`organization_id + environment_id + order_id + calculation_bucket_utc` 覆盖周期刷新已计算 bucket 的持久化跳过判断；scope + order + calculated time 支撑 latest-per-order；business reference + calculated time 支撑跨页一致读取；scope + calculated time 支撑 retention scan。新增索引 `ix_order_urgency_snapshot_scope_order_bucket` 的数据库注释说明其用于 bucket 查找和重复刷新跳过。 | 方案生成、优先级变化或后台周期追加；GET 不写入。按 organization/environment 默认在线保留 180 天、总保留 1,095 天；同 order 的 latest 永远在线。只有 exact-version 归档证据完整且存在显式限时授权时才删除旧源行。 |
...
9. `backend/services/AppHub/src/Nerv.IIP.AppHub.Infrastructure/Migrations/20260713020020_AddConnectorCollectionHealthProjection.cs`
10. `backend/services/AppHub/src/Nerv.IIP.AppHub.Infrastructure/Migrations/20260717075316_AddConnectorConnectionState.cs`
11. `backend/services/AppHub/src/Nerv.IIP.AppHub.Infrastructure/Migrations/20260718040045_HardenConnectorConnectionProjection.cs`
...
| `application_instances` | business | 应用实例聚合根，记录实例键、版本、节点、状态、健康、Connector Host 归属和 connector 上报扩展信息。 | `OrganizationId + EnvironmentId + InstanceKey` 唯一；`OrganizationId + EnvironmentId + ApplicationKey` 支持按应用查实例；`OrganizationId + EnvironmentId + ConnectorHostId` 支持按 Connector Host 扫描心跳生命周期；拥有 heartbeat、状态历史和状态变化。 |
...
| `registration_idempotency` | business | 注册请求幂等记录，避免 connector 重试导致重复实例注册。 | `OrganizationId + EnvironmentId + IdempotencyKey` 唯一；记录 `RegistrationId` 和 `InstanceKey`；迁移从旧的全局唯一 InstanceKey 可靠回填 scope，无法推导时 fail-fast。 |
| `connector_collection_health` | business projection | Connector Host 真实采集健康的最新耐久投影，与进程心跳分离。 | 记录 org/env/connector/source system、counter epoch、metrics report time、last sample、nullable received/dropped/error counters；连接事实包含 nullable `ConnectionStatus`、显示时间 `ConnectionObservedAtUtc`、exact-ticks 全序 `ConnectionObservedAtUtcTicks`、连接/断开起点及有界诊断；`ConcurrencyVersion` 是应用维护的乐观锁版本。nullable 表示 legacy 或尚无事实，不伪造 0、不历史回填连接状态。 | `ApplicationInstanceId` 一对一；`OrganizationId + EnvironmentId + ConnectorId` 唯一；同 epoch 保持 counters 单调，new epoch 表达 process reset，bounded retired epoch lineage 防止旧进程迟到回写；counter ordering 与 connection ordering 相互独立；数据库 CHECK 强制 connection status 枚举及 `alive`/`lost`/`unknown` 字段形状完整，仓储以有限重试合并并发投影。 |
...
4. `ConnectionStatus` 只接受 Connector Protocol 的 `unknown`、`alive`、`lost`；legacy rows 保持 null，不从 heartbeat、collector status 或 sample silence 回填。
...
5. `backend/services/Iam/src/Nerv.IIP.Iam.Infrastructure/Migrations/20260519032000_AddUserCaseInsensitiveUniqueIndexes.cs`
6. `backend/services/Iam/src/Nerv.IIP.Iam.Infrastructure/Migrations/20260628081213_AddIamSecurityAuditRecords.cs`
7. `backend/services/Iam/src/Nerv.IIP.Iam.Infrastructure/Migrations/20260705132145_AddIamUserLifecyclePasswordPolicy.cs`
8. `backend/services/Iam/src/Nerv.IIP.Iam.Infrastructure/Migrations/20260726055246_AddUserWorkerProfile.cs`
...
| `users` | business | 后台用户认证事实，记录 login name、email、password hash、启用状态、security stamp、permission version、账号有效期、密码变更/过期/强制改密状态、登录时间、失败计数、最近失败时间、锁定截止时间，以及工人档案（显示名/工号/部门显示名，三者均可空，只用于工人目录展示，不参与授权）。 | `IX_users_LoginName_Lower` 与 `IX_users_Email_Lower` 分别通过 `lower(LoginName)`、`lower(Email)` 表达式唯一索引保护大小写不敏感唯一性；`AccountExpiresAtUtc` 支持扫描即将过期或已过期账号；`Id` 为调用方提供的有界 string 强类型 ID。 |
