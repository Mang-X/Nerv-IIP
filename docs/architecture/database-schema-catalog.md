...
`maintenance_plans.window_start_utc` 与 `maintenance_plans.window_end_utc` 是 #207 设备 runtime availability 的可选维护窗口列；窗口边界必须成对提供，且按 UTC 存储。`maintenance_plans.next_due_on` 与 `last_generated_on` 支撑 #416 PM 到期自动开单；迁移会把已有计划的 `next_due_on` 回填为 `starts_on`。#554 起，PM 生成按原 `next_due_on` 多周期追赶，`runtime_hour_interval`、`last_generated_runtime_hours` 和 `next_due_runtime_hours` 支撑运行小时用量触发。MAN-401/#717 增加 `paused` 门禁：MasterData 设备停用消费会耐久暂停匹配计划，事件重放由 Maintenance consumer inbox 幂等去重。
