...
| `calibration_records` | business | 已接受的量具校准审计记录。 | `measuring_device_id` 为聚合内外键；`calibration_no` 为校准凭据编号；`calibrated_at_utc`、`calibrated_by` 和可选 `certificate_file_id` 保留结果与证书引用。 | `measuring_device_id + calibration_no` 唯一，防止重复导入同一校准记录。 | 新记录将量具恢复为 in-use 并以配置周期重算下次到期日；随量具删除。 |
...
| `production_reports` | business | MES 报工事实，记录工单/工序的良品数、报废数、返工数、完工标记、报工时间、可选报废原因/不良记录号、产出批次/序列号谱系，以及冲销单到原报工的审计链。 | `id` 为 Guid v7 强类型 ID；`organization_id + environment_id + report_no` 是报工业务号唯一键；`work_order_id` 与 `operation_task_id` 为 MES 业务引用；`produced_lot_no` 和 `serial_no` 是追溯公开标识；`reversed_report_no` 指向被冲销的同服务报工业务号，`reversal_reason` 记录冲销原因；`oee_work_center_id`、`oee_device_asset_id`、`oee_uom_code` 和 `oee_theoretical_rate_per_hour` 是创建时固化的 OEE 投影快照，冲销复用原报工快照而不从当前工序重算，不建立跨服务 FK。 | 报工号唯一索引用于重试和追踪；工单/工序/时间索引用于执行时间线查询；`organization_id + environment_id + reversed_report_no` 唯一索引用于防重复冲销和审计链查询，PostgreSQL 允许普通报工的 NULL 值重复。 | 报工创建后作为执行历史保留；冲销不会修改原报工，而是追加负向报工并引用原报工及其 OEE 快照；物料消耗通过公共 Inventory movement request 集成事件表达库存扣减或冲销回补意图，Quality/ERP 仍拥有各自正式事实。 |
