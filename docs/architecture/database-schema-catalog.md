...
| `work_calendar_working_times` | business | 工作日历拥有的周期性工作日标记。 | `id` 为 owned row Guid；`work_calendar_id` 指向 `work_calendars`；`day_of_week` 表示本地周期性工作日，具体工作时间由 Shift 表达。 | `work_calendar_id` 支持按日历加载所有工作日；随聚合级联维护。 | owned collection，生命周期完全跟随 `work_calendars` 聚合。 |
