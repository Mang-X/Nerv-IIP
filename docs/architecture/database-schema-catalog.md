...
| `changeover_matrix_entries` / `changeover_required_tooling` | business | 按工作中心、来源 SKU 或产品族、目标 SKU 维护权威换型分钟与所需工装。 | `source_type + source_code` 以非空规范键表达 exact SKU 或产品族，避免 nullable 唯一键漏洞；exact SKU 匹配优先；setup 必须为正且至少关联一套工装。 | `organization_id + environment_id + work_center_code + source_type + source_code + to_sku_code` 唯一；所需工装按矩阵行和 tooling code 唯一，子行 FK 非空并级联。 | MasterData 拥有配置生命周期；批量入口按规范键 upsert 并可停用；Scheduling 只通过内部 facts API 读取，不复制或接受调用方内联 setup/tooling 作为权威事实。 |
