...
| Table | Kind | Purpose | Key columns | Index intent | Lifecycle |
| --- | --- | --- | --- | --- | --- |
| `inbound_orders` | business | WMS 入库执行单头，记录来源单据、SiteCode 仓库范围、状态、创建时作业人/班组快照、收货完成与取消审计事实。 | `id` 为 Guid v7 强类型 ID；`organization_id + environment_id + inbound_order_no` 是业务唯一键；`assigned_operator_user_id`/`assigned_team_id` 是可空创建快照；`cancellation_reason` 和 `cancelled_at_utc` 记录取消原因与 UTC 时间。 | 入库单号唯一索引防重复；`source_document_type + source_document_id + status` 索引支持 ERP 按来源关闭开放入库预期；组织、环境、状态、SiteCode 与作业人/班组索引支持现场任务查询。 | 入库单创建后推进收货、上架、完成或取消；V1 以 `site_code` 表达仓库范围，不伪造 Warehouse 主数据；库存移动由 Inventory 边界承接。 |
...
| `outbound_orders` | business | WMS 出库执行单头、创建时作业人/班组快照、复核包装事实和取消审计。 | `id` 为 Guid v7 强类型 ID；`organization_id + environment_id + outbound_order_no` 是业务唯一键；`assigned_operator_user_id`/`assigned_team_id` 是可空创建快照；`version` 是每次聚合变更推进的乐观并发令牌；`cancellation_reason` 和 `cancelled_at_utc` 记录取消原因与取消时间。 | 出库单号唯一索引防重复；组织、环境、状态、SiteCode 与作业人/班组索引支持现场任务查询；并发 posting 回调通过 `version` 冲突重试后重新判断全行状态。 | 出库单创建后推进拣货、复核、包装、完成或取消；V1 以 `site_code` 表达仓库范围；取消会释放 Inventory reservation 并保留审计事实。 |
...
| `warehouse_tasks` | business | 上架、拣货和补货建议任务事实，记录来源批次/序列号、创建时作业人/班组快照、库位、执行量和人工执行审计。 | `id` 为 Guid v7 强类型 ID；`version` 从 1 开始并在每次成功变更后推进，是 EF concurrency token；异常与完成分别保留 code/reason/time/actor；补货建议以 BackorderOrder 为来源单据。 | 组织、环境、任务号唯一索引防重复；组织、环境、类型、状态、SiteCode 与作业人/班组索引支持现场任务队列。 | `Open → InProgress → Completed`；拣货显式短拣可带原因进入 `CompletedWithDifference`；`Exception`、`Cancelled` 与完成态均为阻断后续执行的终态；V1 以 `site_code` 表达仓库范围。 |
...
| `count_executions` | business | WMS 盘点执行、创建时作业人/班组快照和差异输出事实。 | `id` 为 Guid v7 强类型 ID；记录 count execution id、SiteCode/库位/SKU/差异数量；`assigned_operator_user_id`/`assigned_team_id` 是可空创建快照；`inventory_count_task_id` 保存 Inventory count task public id，用于冻结台账和确认调整。 | execution id 唯一索引防重复；组织、环境、状态、SiteCode 与作业人/班组索引支持盘点列表。 | 创建时通过 Inventory count-task 边界冻结对应台账，完成时通过 Inventory adjustment 边界确认差异；V1 以 `site_code` 表达仓库范围；外部 `count-adjustment` 事件不作为 WMS 盘点闭环。 |
