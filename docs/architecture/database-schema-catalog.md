...
| `maintenance_plans` | business | 预防性维护计划、保养周期、下一次到期日、可选运行小时触发阈值和可选 runtime availability 维护窗口事实。 | `id` 为 Guid v7 强类型 ID；`organization_id + environment_id + plan_code` 是业务唯一键；`interval`、`starts_on`、`last_generated_on`、`next_due_on`、`runtime_hour_interval`、`last_generated_runtime_hours`、`next_due_runtime_hours`、`paused`、`owner`、`window_start_utc` 和 `window_end_utc` 描述计划、PM 生成状态、用量触发状态与维护窗口。 | 计划编码唯一索引用于防重复；设备与窗口字段支撑 availability 查询；`next_due_on` 支撑 PM 到期扫描；运行小时字段支撑用量触发 PM 追赶。 | 创建后作为计划事实保留；PM 调度支持 ISO day interval（如 `P7D`）按原 `next_due_on` 追赶多周期，也支持运行小时阈值触发；窗口边界必须成对提供并按 UTC 保存；MasterData 设备停用事件将同组织/环境/设备下的计划设置为 `paused=true`，暂停期间不生成工单且不推进到期游标。 |
...
`maintenance_plans.window_start_utc` 与 `maintenance_plans.window_end_utc` 是 #207 设备 runtime availability 的可选维护窗口列；窗口边界必须成对提供，且按 UTC 存储。`maintenance_plans.next_due_on` 与 `last_generated_on` 支撑 #416 PM 到期自动开单；迁移会把已有计划的 `next_due_on` 回填为 `starts_on`。#554 起，PM 生成按原 `next_due_on` 多周期追赶，`runtime_hour_interval`、`last_generated_runtime_hours` 和 `next_due_runtime_hours` 支撑运行小时用量触发。MAN-401/#717 增加 `paused` 与 `maintenance_device_states` 双重门禁：MasterData 设备停用消费会耐久暂停现有计划并阻止后续计划继续调度；事件重放由 Maintenance consumer inbox 幂等去重，设备状态更新、计划创建和 PM 生成由同一组织/环境分布式锁串行化。
