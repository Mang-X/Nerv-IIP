...
| `stock_reservations` | business | Inventory 拥有的库存预留/分配事实，按来源服务、来源单据和库存维度保留预留数量。 | `id` 为 Guid v7 强类型 ID；`source_service + source_document_id + idempotency_key` 保护预留幂等；`production_date`、`expiry_date` 保存被预留批次效期；`reserved_quantity`、`released_quantity`、`allocated_quantity`、`open_quantity` 和 `status` 记录生命周期；`quality_status` 只允许受控库存状态。 | 来源单据唯一索引防重复预留；SKU/site/location/status 索引用于 ATP 和补偿查询；SKU/site/expiry/status 索引用于 FEFO 和临期预留追踪；状态 check constraint 防自由文本库存状态。 | 创建后可部分释放或分配给出库；FEFO 预留可产生多个 reservation allocation，但 WMS 当前仅消费单 allocation 拣货；不删除历史预留事实。 |
...
| `operation_tasks` | business | MES 工序任务事实，保存工序顺序、工作中心、可选工作中心、持续时间、执行窗口、暂停累计、实际人工/机器工时、派工班次/人员/设备事实，以及 Quality 报检触发快照。 | `id` 为 Guid v7 强类型 ID；`organization_id + environment_id + operation_task_id` 是业务唯一键；`sku_code`、`uom_code`、`planned_quantity` 和 `requires_quality_inspection` 复制工单/Routing 报检上下文；`assigned_user_id`、`device_asset_id`、`shift_id` 和 `assigned_at_utc` 记录当前派工事实；`paused_at_utc`、`paused_duration_ticks`、`labor_time_ticks`、`machine_time_ticks` 记录暂停扣时后的执行工时事实。 | 工单/工序顺序索引用于按工单加载工序并执行前序门禁；外键索引用于报工与工单追踪；完工事件把 `requires_quality_inspection` 和 SKU 快照传递给 Quality，用于生成过程检任务。 | 随工单创建或执行调整保留；状态表达排产、派工和执行进度；完工后工时字段作为 OEE/绩效核算输入，报检字段作为事件驱动待检任务生成依据。 |
| `production_reports` | business | MES 报工事实，记录工单/工序的良品数、报废数、返工数、完工标记、报工时间、可选报废原因/不良记录号、产出批次/序列号谱系，以及冲销单到原报工的审计链。 | `id` 为 Guid v7 强类型 ID；`organization_id + environment_id + report_no` 是报工业务号唯一键；`work_order_id` 与 `operation_task_id` 为 MES 业务引用；`produced_lot_no` 和 `serial_no` 是追溯公开标识；`reversed_report_no` 指向被冲销的同服务报工业务号，`reversal_reason` 记录冲销原因，不建立跨服务 FK。 | 报工号唯一索引用于重试和追踪；工单/工序/时间索引用于执行时间线查询；`organization_id + environment_id + reversed_report_no` 唯一索引用于防重复冲销和审计链查询，PostgreSQL 允许普通报工的 NULL 值重复。 | 报工创建后作为执行历史保留；冲销不会修改原报工，而是追加负向报工并引用原报工；物料消耗通过公共 Inventory movement request 集成事件表达库存扣减或冲销回补意图，Quality/ERP 仍拥有各自正式事实。 |
...
| `scan_records` | business | 扫码记录事实，记录扫码对象、结果、设备/人员、幂等键、GS1 解析字段和下游业务动作选择。 | `id` 为 Guid v7 强类型 ID；记录 scanned value、source workflow、source document、result、idempotency key、GTIN、lot、serial、SSCC、quantity、SKU/UOM/site/location 和 downstream event id。 | 幂等键索引用于 PDA/Connector 重试去重；accepted scanned value、EPC URI、lot-present GTIN/lot/serial 和 no-lot GTIN/serial 唯一索引用于防止同一序列化条码跨幂等键重复扫码；rejected scanned value 可重复记录；SSCC 索引用于物流单元追溯；source workflow/document 索引用于业务追溯。 | append-only 扫码事实；`inventory.receipt`、`inventory.issue`、`inventory.adjustment` 会发布 Inventory movement-requested 事件；`wms.receiving`、`production.report`、`quality.inspection` 和 `inventory.count` 会记录显式 downstream scan action 并发布通用 BarcodeScanAccepted 事实，具体业务状态仍归各下游服务。 |
...
| `inbound_order_lines` | business | 入库行，记录 SKU、数量、批次/序列号、生产日期、效期和收货结果。 | `id` 为 Guid v7 强类型 ID；`inbound_order_id` 归属入库单；`production_date`、`expiry_date` 是 WMS 收货采集并传递给 Inventory 的批次效期。 | inbound order 索引用于加载明细。 | 生命周期随入库单推进并保留历史；完成入库时生成携带批次效期的 Inventory movement request。 |
...
| `purchase_orders` / `purchase_order_lines` | business | 采购订单头和行，记录订单币种、金额、收货容差与已收数量。 | `purchase_order_no` 是业务编号；`currency_code` 是订单币种；`approval_chain_id` 记录 BusinessApproval 审批链；行记录 ordered/received quantity、over/under receipt tolerance percent 和 final delivery 标志。 | PO no 唯一索引用于收货反查。 | 创建后先进入 `PendingApproval`；BusinessApproval 完成事件批准后 release 才可被收货推进；收货允许在 over tolerance 内超过订单数量，final delivery 可关闭剩余 open quantity；拒绝/退回后取消；不写 Inventory 余额。 |
...
| `sales_orders` / `sales_order_lines` | business | 销售订单和行，记录已释放发货数量。 | `sales_order_no` 是业务编号；行记录 ordered/delivered quantity。 | SO no 唯一索引用于发货请求反查。 | ERP 只拥有订单事实；客户信用额度、已开放应收和 active SO exposure 超限时订单进入 `credit-held` 而非丢弃，信用放行后才可发货；WMS 拥有出库执行。 |
...
| `account_payables` | business | 应付候选事实。 | `payable_no` 是业务编号；记录来源单据、供应商、币种、金额、已付金额、本位币金额、汇率、发票日、到期日和付款条件。 | AP no 唯一索引用于幂等生成。 | 支付推进不允许超过 open amount；付款命令支持一次核销多张 AP、on-account 预付和付款汇率差异，生成本位币平衡凭证及已实现汇兑损益行；列表可按 as-of date 输出账龄 bucket；供应商发票匹配创建的 AP 通过 GR/IR clearing 入账，直接 AP 创建走费用/应付凭证，不触碰库存或 GR/IR；完整总账月结后置。 |
| `account_receivables` | business | 应收候选事实。 | `receivable_no` 是业务编号；记录来源单据、客户、币种、金额、已收金额、本位币金额、汇率、发票日、到期日和付款条件。 | AR no 唯一索引用于幂等生成。 | 收款推进不允许超过 open amount；列表和汇总按币种输出本位币 open amount；创建和收款核销会生成平衡子分类账凭证，完整收款核销后置。 |
...
| `journal_vouchers` / `journal_voucher_lines` | business | 平衡凭证事实和借贷行。 | `voucher_no` 是业务编号；行记录 account code、document currency debit/credit、exchange rate 和 local currency debit/credit。 | voucher no 唯一索引用于凭证审计。 | posted 后不可变，凭证按本位币借贷平衡；AP/AR/Cost 创建和 AP/AR 清算命令会自动写入最小子分类账凭证。 |
...
| BusinessQuality | `quality` | Implemented | Yes | Yes | No | 已有 NCR、InspectionPlan、InspectionRecord、InspectionTask、persistent DLQ schema、MES defect source 唯一约束、migration、schema convention tests 和 verify script；客户 release bundle 仍待后续。 |
