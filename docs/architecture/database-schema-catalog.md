...
| `schedule_plans` | business | 排程方案头，记录生成/发布/取代/撤销状态、唯一 engine version、engine/rule/constraint provenance、回放状态、合同版本、问题指纹和方案 KPI。 | `id` 为 Guid v7 强类型 ID；`plan_id` 是公开方案 ID；`algorithm_version` 保持唯一 engine version；`engine_id`、`rule_provider_id`、`rule_profile_id`、`rule_profile_version` 保存稳定执行身份；`constraint_sources_json` 是确定性、版本化约束来源摘要；`trace_schema_version` 与 `replay_status` 记录 trace 兼容和回放可用性；`release_revision` 是 scope 内单调发布版本。 | `plan_id` 唯一；`organization_id + environment_id` 上 `status = 'Released'` 的 PostgreSQL partial unique index 保证 scope 仅一个 active release；scope + release revision 唯一索引保护版本水位。 | 新方案 provenance 必填，generated plan 替换在同一 aggregate/UoW 中同时替换输出和 trace；历史已知 engine/rule 身份回填为默认实现，constraint trace 与 replay 明确为 `legacy-unavailable`。新版发布/撤销治理语义不变。 |
