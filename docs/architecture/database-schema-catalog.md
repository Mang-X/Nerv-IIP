...
| Table | Kind | Purpose | Key columns | Index intent | Lifecycle |
| --- | --- | --- | --- | --- | --- |
| `work_orders` | business | MES 持久化工单事实，记录 SKU、生产版本引用、计划数量、单位、优先级、交期、执行状态、累计良品/报废数量、ERP 权威资本化单位成本、完工关闭时间，以及从 DemandPlanning 或等价来源计划转单时复制的 durable source plan reference。 | `id` 为 Guid v7 强类型 ID；`organization_id + environment_id + work_order_id` 是业务唯一键；`production_version_id` 是 ProductEngineering 业务引用；`uom_code` 为来源计划单位快照；`completed_quantity`、`scrap_quantity` 和 `over_receipt_tolerance_percent` 支撑执行进度；nullable `capitalized_unit_cost` 是 ERP 资本化事件的耐久投影，用于桥接完工入库申请与成本事件乱序；`source_system + source_document_type + source_document_id + source_demand_reference` 是跨服务来源计划/需求回溯引用，不建立跨 schema FK。 | 唯一约束保护同一组织/环境内工单号；SKU/交期索引用于排产扫描；`source_system + source_document_id` 索引用于从来源计划回查已转 MES 工单。 | 工单创建后可 release/start/hold/complete/close/cancel/scrap；complete 表示计划数量已由良品或报废执行覆盖，成品入库仍由 `output_lot_genealogies` 的产出批次强引用单独门控；资本化消费者与完工入库创建命令按 organization/environment/work-order scope 取得 PostgreSQL transaction-scoped advisory lock，资本化事件先到时投影与 consumer inbox 同事务提交，后续入库申请直接采用该成本，入库申请先到时由消费者回填；相同成本重放幂等，不同成本改写 fail closed；历史保留用于报工、入库请求、成本追踪和需求/MRP/MES 计划来源追溯。 |
