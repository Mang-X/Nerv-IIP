...
| `payment_executions` / `payment_execution_allocations` | business | AP 付款执行单及核销分配，记录 approved 到 executed 的最小付款执行审计链。 | `payment_execution_no` 是业务编号；头记录 supplier、amount、currency、payment date、cash account、status、approver/executor；分配行记录 payable no 和 amount。 | payment execution no 唯一索引用于付款幂等和凭证追溯；分配行归属索引用于加载核销明细。 | 付款命令创建 approved 单据并执行，核销 AP open amount，同步账龄读面，并生成付款凭证。 |
...
| `cash_receipts` / `cash_receipt_allocations` | business | 现金收款登记单及 AR 匹配分配，记录收款到部分核销的审计链。 | `cash_receipt_no` 是业务编号；头记录 customer、amount、currency、receipt date、cash account、status；分配行记录 receivable no 和 amount。 | cash receipt no 唯一索引用于收款幂等和凭证追溯；分配行归属索引用于加载核销明细。 | 收款登记后匹配 AR，可部分核销 open amount，同步账龄读面，并生成收款凭证。 |
