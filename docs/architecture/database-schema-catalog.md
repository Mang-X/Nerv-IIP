...
| `account_payables` | business | 应付候选事实。 | `payable_no` 是业务编号；记录来源单据、供应商、币种、金额、已付金额、本位币金额、汇率、发票日、到期日和付款条件。 | AP no 唯一索引用于幂等生成。 | 支付推进不允许超过 open amount；付款命令支持一次核销多张 AP、on-account 预付和付款汇率差异，生成本位币平衡凭证及已实现汇兑损益行，并持久化 `payment_executions` 审计单据；列表可按 as-of date 输出账龄 bucket；供应商发票匹配创建的 AP 通过 GR/IR clearing 入账，直接 AP 创建走费用/应付凭证，不触碰库存或 GR/IR；完整总账月结后置。 |
...
| `account_receivables` | business | 应收候选事实。 | `receivable_no` 是业务编号；记录来源单据、客户、币种、金额、已收金额、本位币金额、汇率、发票日、到期日和付款条件。 | AR no 唯一索引用于幂等生成。 | 收款推进不允许超过 open amount；列表和汇总按币种输出本位币 open amount；创建和收款核销会生成平衡子分类账凭证，并持久化 `cash_receipts` 审计单据；完整收款核销后置。 |
...
| `telemetry_raw_samples` | business | historian raw 层采样明细 bucket，保存采集端最细粒度写入的 min/max/avg/first/last 和来源元数据。 | `id` 为 Guid v7 强类型 ID；`organization_id + environment_id + source_system + source_connector + device_asset_id + tag_key + source_sequence` 是幂等唯一键，且 source 元数据为空时按 not-distinct 参与唯一性；`bucket_start_utc`、`bucket_end_utc` 和 `bucket_end_unix_time_milliseconds` 定位采样窗口；`sample_count`、`min_value`、`max_value`、`average_value`、`first_value`、`last_value` 保存 raw bucket 明细。 | 来源 scope+序列唯一索引防重复写入；设备+tag+bucket 起点索引用于历史明细查询；设备+tag+bucket end Unix ms 索引用于 retention、迟到桶和范围扫描。 | raw 层按保留策略清理；hourly/daily rollup 生成后可缩短 raw 保留。 |
| `telemetry_rollups` | business | historian hourly/daily 降采样结果，保存 min/max/加权 avg/first/last。 | `id` 为 Guid v7 强类型 ID；`organization_id + environment_id + device_asset_id + tag_key + grain + window_start_utc` 是 rollup 唯一键；`grain` 为 `Hourly` 或 `Daily`；`window_start_utc`、`window_end_utc` 和 `window_end_unix_time_milliseconds` 定位汇总窗口；`source_sequence` 是 deterministic historian 序列。 | 设备+tag+grain+window 起点唯一索引防重复 downsampling；设备+tag+grain+window end Unix ms 索引用于历史查询和 retention 清理。 | hourly 和 daily 层分别按保留策略清理；不跨 schema 引用设备主数据。 |
| `CAPLock` | system | CAP distributed lock table，由 netcorepal/CAP 基础设施维护。 | 主键由 CAP 类型定义。 | CAP 内部协调。 | 系统表随服务数据库迁移创建；业务代码不直接读写。 |
| `CAPPublishedMessage` | system | CAP published message outbox，由 netcorepal/CAP 基础设施维护。 | 主键由 CAP 类型定义。 | CAP 内部投递扫描。 | 系统表随服务数据库迁移创建；业务代码不直接读写。 |
| `CAPReceivedMessage` | system | CAP received message inbox，由 netcorepal/CAP 基础设施维护。 | 主键由 CAP 类型定义。 | CAP 内部消费幂等。 | 系统表随服务数据库迁移创建；业务代码不直接读写。 |
