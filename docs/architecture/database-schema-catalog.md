...
| `schedule_operation_overrides` | business | Scheduling API 手动调整或 MES 真实手动派工形成的当前固定工序分配。 | 保存真实工单/工序、资源/工作中心、UTC 起止窗口、锁定原因、来源事件/操作者和来源时间；较旧事件不覆盖较新事实。 | `organization_id + environment_id + operation_id` 唯一索引保证每个业务上下文中的工序只有一个当前 override。 | 后续排程装配、预览和生成自动转换为 locked assignment；不承载 #701 的 release/revoke 版本治理。 |
