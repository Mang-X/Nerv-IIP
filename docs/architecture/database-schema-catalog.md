...
| --- | --- | --- | --- | --- | --- |
| `skus` | business | 物料和产品 SKU 主数据，用于计划、库存和执行识别。 | `id` 为 Guid v7 强类型 ID；`organization_id + environment_id + code` 是业务唯一键；`unit` 记录默认计量单位。 | 唯一索引防止同一组织/环境内 SKU code 重复；`category + disabled` 支持按品类过滤可用 SKU 列表。 | 聚合创建后保留审计时间；停用通过 `disabled` 软关闭，不物理删除。 |
...
| `work_centers` | business | 工作中心主数据，用于产能计划和执行路由。 | `id` 为 Guid v7 强类型 ID；`organization_id + environment_id + code` 是业务唯一键；`capacity_minutes_per_day` 记录每日标称产能分钟数。 | 唯一索引保护工作中心 code；`disabled` 支持快速扫描活跃工作中心。 | 聚合创建后保留；停用表示不再接收计划或执行任务。 |
...
| `device_assets` | business | 设备资产主数据，记录设备型号、产线和工作中心归属。 | `id` 为 Guid v7 强类型 ID；`organization_id + environment_id + code` 是业务唯一键；`line_code` 与 `work_center_code` 为跨聚合业务引用。 | 唯一索引保护设备 code；`work_center_code + disabled` 支持按工作中心列活跃设备。 | 聚合创建后保留；退役或不可用使用 `disabled` 停用。 |
