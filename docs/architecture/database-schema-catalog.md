...
| `outbound_orders` | business | WMS 出库执行单头、复核包装事实和取消审计。 | `id` 为 Guid v7 强类型 ID；`organization_id + environment_id + outbound_order_no` 是业务唯一键；`version` 是每次聚合变更推进的乐观并发令牌；`cancellation_reason` 和 `cancelled_at_utc` 记录取消原因与取消时间。 | 出库单号唯一索引防重复；并发 posting 回调通过 `version` 冲突重试后重新判断全行状态。 | 出库单创建后推进拣货、复核、包装、完成或取消；取消会释放 Inventory reservation 并保留审计事实。 |
