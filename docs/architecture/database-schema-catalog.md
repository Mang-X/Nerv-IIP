...
| `connector_collection_health` | business projection | Connector Host 真实采集健康的最新耐久投影，与进程心跳分离。 | 记录 org/env/connector/source system、counter epoch、metrics report time、last sample、nullable received/dropped/error counters，以及独立按 connection observed time 排序的 `unknown/alive/lost` 现场连接事实和 transition timestamps/reason/diagnostic；nullable 表示 legacy 或尚无事实，不伪造 0、不历史回填连接状态。 | `ApplicationInstanceId` 一对一；`OrganizationId + EnvironmentId + ConnectorId` 唯一；同 epoch 保持 counters 单调，new epoch 表达 process reset，bounded retired epoch lineage 防止旧进程迟到回写；counter ordering 与 connection ordering 相互独立。 |
