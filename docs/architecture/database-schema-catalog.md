...
| `connector_collection_health` | business projection | Connector Host 真实采集健康的最新耐久投影，与进程心跳分离。 | 记录 org/env/connector/source system、counter epoch、metrics report time、last sample 以及 nullable received/dropped/error counters；nullable 表示尚无事实，不伪造 0。 | `ApplicationInstanceId` 一对一；`OrganizationId + EnvironmentId + ConnectorId` 唯一；同 epoch 保持 counters 单调，new epoch 表达 process reset，bounded retired epoch lineage 防止旧进程迟到回写。 |
