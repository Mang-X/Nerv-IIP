...
| `operation_tasks` | business | MES 工序任务事实，保存工序顺序、工作中心、可选工作中心、持续时间、执行窗口、暂停累计、实际人工/机器工时、派工班次/人员/设备事实、ProductEngineering 标准工序编码以及 Quality 报检触发快照。 | `id` 为 Guid v7 强类型 ID；`organization_id + environment_id + operation_task_id` 是业务唯一键；`operation_code` 是可选 StandardOperation 编码，用于 MES/PDA 查询当前生效 SOP/电子作业指导书；`sku_code`、`uom_code`、`planned_quantity` 和 `requires_quality_inspection` 复制工单/Routing 报检上下文；`assigned_user_id`、`device_asset_id`、`shift_id` 和 `assigned_at_utc` 记录当前派工事实；`manual_dispatch_revision` 默认 0，是随真实设备派工及其清除单调递增的生命周期版本，`has_active_manual_dispatch` 默认 false，标记当前是否仍拥有 MES 手工设备派工锁；升级时不能可靠推断历史设备来源，因此 revision 0 + inactive + 已有 device 表示 legacy-unknown，不会被长期标为 active，但设备清空/工序取消会保守发布 revision 1 tombstone，随后新手工派工使用 revision 2；`scheduled_at_utc` 仅由已下达 APS 排程分配（`ApplyScheduleAssignment`）写入、人工派工不写，据以区分「已排程/未排程」；`schedule_invalidation_reason_code` 记录排程失效原因码（`status = ScheduleInvalidated` 时有值，被已下达排程重排回队列时清空）；`paused_at_utc`、`paused_duration_ticks`、`labor_time_ticks`、`machine_time_ticks` 记录暂停扣时后的执行工时事实。 | 工单/工序顺序索引用于按工单加载工序并执行前序门禁；外键索引用于报工与工单追踪；`operation_code` 不建立跨 schema FK，仅作为 ProductEngineering SOP 查询条件；完工事件把 `requires_quality_inspection` 和 SKU 快照传递给 Quality，用于生成过程检任务。 | 随工单创建或执行调整保留；真实设备手工派工激活锁并推进版本，设备清除或工序取消会再次推进版本并发布清除事实；状态表达排产、派工和执行进度；完工后工时字段作为 OEE/绩效核算输入，SOP 由 ProductEngineering 当前版本查询决定，报检字段作为事件驱动待检任务生成依据。 |
...
| `schedule_plans` | business | APS lite 排程方案头，记录生成/发布状态、算法版本、合同版本、问题指纹和方案 KPI。 | `id` 为 Guid v7 强类型 ID；`plan_id` 是公开方案 ID；`problem_id` 和 `problem_fingerprint` 指向输入快照；`status` 表示 generated/released 等生命周期；`locked_operation_count` / `optimizable_operation_count` 明确 KPI 优化空间，延期/准时指标只统计非锁定工序；其余资源指标保持全方案口径。 | `plan_id` 唯一索引用于 detail、Gantt 和 release 查询。 | 生成后可发布；发布后作为 MES、Gantt 和 APS KPI 看板消费的稳定事实保留。 |
