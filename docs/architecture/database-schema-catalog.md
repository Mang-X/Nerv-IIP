...
| `workers` | business | 工厂员工主数据，承载工号、姓名、部门、岗位、在岗状态与联系电话；`user_id` 是班组成员、人员技能与 MES 派工共同引用的人员标识。 | `id` 为 Guid v7 强类型 ID；`organization_id + environment_id + code` 与 `organization_id + environment_id + user_id` 分别是工号与人员标识的业务唯一键；`employment_status` 取 `active`/`on-leave`/`resigned`；`disabled` 为归档软停用标记。 | 工号与人员标识各有唯一索引；`organization_id + environment_id + department_code + disabled` 支持按部门列员工。 | 员工离职用 `employment_status = resigned`，归档用 `disabled`，均不物理删除，保证历史派工记录可追溯。 |
