...
`maintenance_plans.window_start_utc` 与 `maintenance_plans.window_end_utc` 是 #207 设备 runtime availability 的可选维护窗口列；窗口边界必须成对提供，且按 UTC 存储。`maintenance_plans.next_due_on` 与 `last_generated_on` 支撑 #416 PM 到期自动开单；迁移会把已有计划的 `next_due_on` 回填为 `starts_on`。#554 起，PM 生成按原 `next_due_on` 多周期追赶，`runtime_hour_interval`、`last_generated_runtime_hours` 和 `next_due_runtime_hours` 支撑运行小时用量触发。MAN-440/#794 迁移 `20260716050408_MakeMaintenancePlanTriggersOptional` 把 `maintenance_plans.interval` 与 `next_due_on` 从 NOT NULL 改为可空，落地不变式「每个计划至少有一个触发（日历 `interval` 和/或 `runtime_hour_interval`）」：纯运行小时计划 `interval`/`next_due_on` 为 `NULL`，`generate-due` 的日历分支对 `next_due_on IS NULL` 天然跳过，只走运行小时阈值分支。同迁移新增两个 CHECK 约束作数据库层加固（领域层已强制，二者互为兜底）：`ck_maintenance_plans_has_trigger`（`interval IS NOT NULL OR runtime_hour_interval IS NOT NULL`）与 `ck_maintenance_plans_calendar_trigger_paired`（`(interval IS NULL) = (next_due_on IS NULL)`），禁止写入无触发计划或日历列不成对的永不到期状态。MAN-401/#717 增加 `paused` 与 `maintenance_device_states` 双重门禁：MasterData 设备停用消费会耐久暂停现有计划并阻止后续计划继续调度；事件重放由 Maintenance consumer inbox 幂等去重，设备状态更新、计划创建和 PM 生成由同一组织/环境分布式锁串行化。
