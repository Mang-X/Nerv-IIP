...
| `finished_goods_receipt_requests` | business | MES 完工入库请求事实，向 Inventory 边界表达成品收货意图，并保留可选产出批次/序列号、入库单位成本、Inventory 累计过账数量、过账引用和失败诊断。 | `id` 为 Guid v7 强类型 ID；`organization_id + environment_id + request_no` 是入库请求业务号唯一键；记录 `work_order_id`、`sku_id`、`quantity`、`uom_code`、`unit_cost`、`status`、请求时间、`produced_lot_no`、`serial_no`、`posted_quantity`、`posted_inventory_movement_id`、`posted_at_utc`、`inventory_posting_failure_code`、`inventory_posting_failure_message` 和 `inventory_posting_failed_at_utc`；`posted_quantity` 支撑 `PartiallyPosted` 与剩余数量重试；`unit_cost` 对 legacy 记录可为空，新建请求需提供正数以传递给 Inventory 移动平均计价。 | 请求号唯一索引用于下游引用；工单/SKU/时间索引用于入库请求追踪；创建请求的幂等 fingerprint 包含 `unit_cost`，同一业务重试 key 携带不同成本时视为 payload 冲突。 | 创建后通过公共 Inventory movement request 事件表达入库意图；Inventory 拥有正式库存移动和计价结果，MES 保存请求成本输入、累计已过账数量、已知过账引用和 Inventory 拒绝诊断；失败或部分过账请求通过同一请求号重试剩余数量，不创建新的入库申请。 |
