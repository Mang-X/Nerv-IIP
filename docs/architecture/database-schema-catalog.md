...
9. `backend/services/Business/Erp/src/Nerv.IIP.Business.Erp.Infrastructure/Migrations/20260629035019_AddErpLongTailCurrencyTolerance.cs`
...
| `purchase_receipts` / `purchase_receipt_lines` | business | ERP 采购收货事实，记录质量状态摘要和库存过账维度快照。 | `purchase_receipt_no` 是业务编号；行引用 PO line no，并复制 SKU、UOM、location 和可选 lot。 | receipt no 唯一索引用于质量/WMS/GR-IR/Inventory 下游引用。 | 记录后不可变；收货通过公开 Inventory movement request 事件请求库存入账，并由 ERP Finance 生成借库存、贷 GR/IR 的暂估凭证；质量检验和 AP 仍由公开事件/API 接线，收货本身不创建 AP。 |
| `supplier_invoices` / `supplier_invoice_lines` | business | 供应商发票三单匹配事实，匹配 PO、采购收货和发票行。 | `invoice_no` 是业务编号；头记录 PO/receipt/supplier/date/due date/currency/match status，行记录 PO line、receipt line、SKU、UOM、数量和单价。 | invoice no 唯一索引用于防重复；行归属索引用于加载匹配明细。 | 匹配通过后冲 GR/IR 并创建 AP，生成借 GR/IR、贷 AP 的应付子分类账凭证；发票币种必须与 PO 币种一致；超出数量、金额或价格百分比容差、累计已开票数量超过收货数量时标记 `PaymentHeld`，不创建 AP/凭证；人工释放 held 发票后执行同样的 GR/IR clearing + AP 生成，作废 held 发票后状态为 `Voided` 且数量不再占用后续累计开票匹配。 |
