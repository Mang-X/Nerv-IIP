...
23. `backend/services/Business/Mes/src/Nerv.IIP.Business.Mes.Infrastructure/Migrations/20260718081034_AddMesSkuAvailabilityProjection.cs`
...
| `mes_sku_availabilities` | business projection | MES 消费 MasterData `SkuDisabledIntegrationEvent` 后形成的 SKU 禁用投影，只用于阻止新的计划转工单和急单创建，不改写已有工单。 | `id` 为 Guid v7 强类型 ID；`organization_id + environment_id + sku_code` 唯一；`status`、`changed_at_utc`、`disabled_reason` 和 `source_event_id` 保存最新禁用事实。 | scope + SKU 唯一索引支撑创建门禁和并发收敛；`changed_at_utc` 是乐观并发 token。 | 消费者 inbox 先按 consumer/idempotency key 去重；PostgreSQL 按 scope/SKU 事务 advisory lock 串行投影，较旧事件成功 no-op，非法来源或 payload 进入 DLQ。当前只消费禁用事件，SKU 重新启用仍由独立 `SkuChangedIntegrationEvent` 消费切片治理。 |
