...
| `production_reports` | business | MES 报工事实，记录工单/工序的良品数、报废数、返工数、完工标记、报工时间、可选报废原因/不良记录号、产出批次/序列号谱系，以及冲销单到原报工的审计链。 | `id` 为 Guid v7 强类型 ID；`organization_id + environment_id + report_no` 是报工业务号唯一键；`work_order_id` 与 `operation_task_id` 为 MES 业务引用；`source` 标识 `manual` 操作员录入或 `telemetry` 计数自动报工；`produced_lot_no` 和 `serial_no` 是追溯公开标识；`reversed_report_no` 指向被冲销的同服务报工业务号，`reversal_reason` 记录冲销原因；`oee_work_center_id`、`oee_device_asset_id`、`oee_uom_code` 和 `oee_theoretical_rate_per_hour` 是创建时固化的 OEE 投影快照，冲销复用原报工快照而不从当前工序重算，不建立跨服务 FK。 | 报工号唯一索引用于重试和追踪；工单/工序/时间索引用于执行时间线查询；`organization_id + environment_id + reversed_report_no` 唯一索引用于防重复冲销和审计链查询，PostgreSQL 允许普通报工的 NULL 值重复。 | 报工创建后作为执行历史保留；冲销不会修改原报工，而是追加负向报工并引用原报工及其 OEE 快照；物料消耗通过公共 Inventory movement request 集成事件表达库存扣减或冲销回补意图，Quality/ERP 仍拥有各自正式事实。 |
| `telemetry_production_report_candidates` | business | MES 接收 IndustrialTelemetry 生产计数 delta 后的人工处理队列；它保存配置为 draft 的自动报工，或在活动报警、未映射工作中心、无唯一在制工单时暂停的计数。 | `id` 为 Guid v7 强类型 ID；`organization_id + environment_id + source_idempotency_key` 是跨服务计数事实唯一边界；设备、tag、bucket、good quantity 和可选工作中心/工单/工序记录可审计的解析上下文；`status` 仅为 `draft` 或 `pending-confirmation`，`suspension_reason` 解释自动过账被抑制的原因。 | 唯一索引阻止同一 IIoT event idempotency key 重复生成候选；范围、状态和创建时间索引支撑待人工确认队列扫描。 | 候选保留为自动报工审计和人工确认输入；它不修改工单进度，只有 `posted` 模式且设备—工作中心—在制工序匹配、无活动报警的计数才会生成正式 `production_reports`。 |
