...
| `order_urgency_archive_batches` | audit | 紧急度快照归档、源删除与 archive version 删除的耐久批次证据。 | stable batch id、organization/environment、source snapshot ids、状态、水位、object key、exact version id、SHA-256、大小、删除授权和错误。 | scope + batch 唯一保证重试幂等；scope + status + max calculated time 支撑恢复删除和总保留扫描。 | pending/failed 不删除源；exact-version 回读完成后转 archived；授权源删除后转 source-deleted；独立授权且超过总保留后转 archive-deleted。审计行保留。 |
