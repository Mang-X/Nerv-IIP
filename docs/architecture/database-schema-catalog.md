...
6. `backend/services/Business/BarcodeLabel/src/Nerv.IIP.Business.BarcodeLabel.Infrastructure/Migrations/20260629034438_Issue556Gs1EpcisAggregationAndSerialUniqueness.cs`
...
| `scan_records` | business | 扫码记录事实，记录扫码对象、结果、设备/人员、幂等键、GS1 解析字段和下游业务动作选择。 | `id` 为 Guid v7 强类型 ID；记录 scanned value、source workflow、source document、result、idempotency key、GTIN、lot、serial、SSCC、quantity、SKU/UOM/site/location 和 downstream event id。 | 幂等键索引用于 PDA/Connector 重试去重；scanned value、EPC URI 和 GTIN/lot/serial 唯一索引用于防止同一序列化条码跨幂等键重复扫码；SSCC 索引用于物流单元追溯；source workflow/document 索引用于业务追溯。 | append-only 扫码事实；`inventory.receipt`、`inventory.issue`、`inventory.adjustment` 会发布 Inventory movement-requested 事件；`wms.receiving`、`production.report`、`quality.inspection` 和 `inventory.count` 会记录显式 downstream scan action 并发布通用 BarcodeScanAccepted 事实，具体业务状态仍归各下游服务。 |
| `epcis_events` | business | BarcodeLabel 生成的 EPCIS 追溯事件，覆盖 serialized label commissioning、accepted scan object event，以及 SSCC parent 与 SGTIN child 的 aggregation/disaggregation 语义。 | `id` 为 Guid v7 强类型 ID；记录 event type、action、CBV business step URI、disposition、label value、GTIN、lot、serial、EPC URI、parent SSCC、可选 parent EPC URI、source workflow/document、可选 print item 或 scan record id。 | event type + EPC URI 和 event type + GTIN/lot/serial 唯一索引用于防止同一序列化事件重复写入；parent SSCC 索引用于按物流单元回放聚合关系；source workflow/document 索引用于按业务单据回放；print item/scan record 索引用于从标签或扫码事实跳转。 | append-only 追溯事实；不建立跨 schema FK，不替代 MES/WMS/Inventory 自有业务状态。 |
