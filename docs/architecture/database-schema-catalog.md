...
| `connector_tag_manifests` | business | IndustrialTelemetry 接受的采集连接器 tag manifest 头，保存每个 canonical connector 当前获准 revision 和来源观测时间。 | `id` 为 Guid v7 强类型 ID；`organization_id + environment_id + collection_connector_id` 是业务唯一键；`manifest_revision` 是 lowercase SHA-256；`manifest_observed_at_utc` 独立排序 replace-style manifest。 | org/env/canonical connector 唯一索引保证每个 collection connector 只有一个当前 manifest root。 | 同 observation 同 revision 幂等、异 revision 冲突；旧 observation 拒绝；新 observation 可接受回滚 revision。 |
| `connector_tag_bindings` | business | Connector manifest 的可复用 device/tag 绑定投影，同时保留 current、retired 和 activation 诊断状态。 | `id` 为 Guid v7 强类型 ID；`organization_id + environment_id + collection_connector_id + device_asset_id + tag_key` 是业务唯一键；`is_current`/`retired_at_utc` 表示 manifest 成员生命周期；activation 状态与 observation 独立排序并保存有界清洗后的错误。 | 完整业务键唯一索引防重复投影；manifest+current 索引支撑按 root 加载当前绑定。 | replace omission 软退役；再次出现时复用原投影 ID；activation 仅允许 `pending`/`active`/`error`/`disabled`，旧观测不覆盖新状态。 |
