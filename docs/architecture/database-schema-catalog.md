...
| `device_control_commands` | business | 设备控制命令台账，投影 Ops operation task 的下发事实，记录设备、命令类型、tag/值或参数集、请求人、原因、幂等/关联键、Ops operation task id 和下发时状态/审批快照，支撑命令 result/history 读面（#842/#838）。 | `id` 为 Guid v7 强类型 ID；`operation_task_id` 是 Ops 命令的唯一自然键，也是读面按 commandId 定位的外部键；`command_type` 为 `write-tag`/`start-stop`/`parameter-set`；`tag_key`/`value` 用于单 tag 命令，`parameters_json` 保存参数集命令输入；`status`/`approval_status` 是下发时 Ops 快照，单命令读面会实时从 Ops 刷新状态；`requested_at_unix_time_milliseconds` 是由 `requested_at_utc` 派生的 provider-neutral 历史范围过滤/排序键。 | `operation_task_id` 唯一索引防重复台账并支撑幂等下发；org/env+时间索引与 org/env+设备+时间索引支撑历史按设备/时间过滤分页。 | 下发时追加台账事实；PLC/DCS/SCADA 凭据或连接 payload 不进入本 schema；命令执行进度以 Ops operation task 为权威，历史行状态实时进展作为后续（Ops OperationTaskCompleted/Failed 消费者）深化。 |
| `CAPLock` | system | CAP distributed lock table，由 netcorepal/CAP 基础设施维护。 | 主键由 CAP 类型定义。 | CAP 内部协调。 | 系统表随服务数据库迁移创建；业务代码不直接读写。 |
| `CAPPublishedMessage` | system | CAP published message outbox，由 netcorepal/CAP 基础设施维护。 | 主键由 CAP 类型定义。 | CAP 内部投递扫描。 | 系统表随服务数据库迁移创建；业务代码不直接读写。 |
| `CAPReceivedMessage` | system | CAP received message inbox，由 netcorepal/CAP 基础设施维护。 | 主键由 CAP 类型定义。 | CAP 内部消费幂等。 | 系统表随服务数据库迁移创建；业务代码不直接读写。 |
