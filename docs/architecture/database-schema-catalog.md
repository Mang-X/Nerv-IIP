...
| `users` | business | 后台用户认证事实，记录 login name、email、password hash、启用状态、security stamp、permission version、账号有效期、密码变更/过期/强制改密状态、登录时间、失败计数、最近失败时间和锁定截止时间。 | `IX_users_LoginName_Lower` 与 `IX_users_Email_Lower` 分别通过 `lower(LoginName)`、`lower(Email)` 表达式唯一索引保护大小写不敏感唯一性；`AccountExpiresAtUtc` 支持扫描即将过期或已过期账号；`Id` 为调用方提供的有界 string 强类型 ID。 |
