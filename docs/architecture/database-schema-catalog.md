...
| `purchase_orders` / `purchase_order_lines` / `purchase_order_line_sources` | business | 采购订单头、行和来源申请行追溯，记录订单币种、金额、收货容差与已收数量。 | `purchase_order_no` 是业务编号；`currency_code` 是订单币种；`approval_chain_id` 记录 BusinessApproval 审批链；行记录 ordered/received quantity、over/under receipt tolerance percent 和 final delivery 标志；source 行记录来源 requisition no、申请行号和 pegged quantity。 | PO no 唯一索引用于收货反查；source 行在同一 PO line 下按 requisition no + line no 唯一。 | 创建后先进入 `PendingApproval`；BusinessApproval 完成事件批准后 release 才可被收货推进；收货允许在 over tolerance 内超过订单数量，final delivery 可关闭剩余 open quantity；拒绝/退回后取消；不写 Inventory 余额；多申请合并转单通过 source 行保留 MRP/申请来源追溯。 |
