...
13. `backend/services/Business/Quality/src/Nerv.IIP.Business.Quality.Infrastructure/Migrations/20260626170759_AddQualityNcrInventoryDispositionRouting.cs`
14. `backend/services/Business/Quality/src/Nerv.IIP.Business.Quality.Infrastructure/Migrations/20260629074947_AddQualityLongtailReviewFixes.cs`
...
| `operation_tasks` | business | MES 工序任务事实，保存工序顺序、工作中心、可选工作中心、持续时间、执行窗口、暂停累计、实际人工/机器工时和派工班次/人员/设备事实。 | `id` 为 Guid v7 强类型 ID；`organization_id + environment_id + operation_task_id` 是业务唯一键；`assigned_user_id`、`device_asset_id`、`shift_id` 和 `assigned_at_utc` 记录当前派工事实；`paused_at_utc`、`paused_duration_ticks`、`labor_time_ticks`、`machine_time_ticks` 记录暂停扣时后的执行工时事实。 | 工单/工序顺序索引用于按工单加载工序并执行前序门禁；外键索引用于报工与工单追踪。 | 随工单创建或执行调整保留；状态表达排产、派工和执行进度；完工后工时字段作为 OEE/绩效核算输入。 |
...
| `production_report_material_consumptions` | business | MES 报工引用的实际物料批次消耗事实，用于工单、报工、物料批次追溯和 Inventory 过账失败诊断。 | `id` 为 Guid v7 强类型 ID；`organization_id + environment_id + report_no` 指向报工号；记录 `material_id`、`material_lot_id`、`uom_code`、消耗数量、线边领料申请号、`inventory_posting_failure_code`、`inventory_posting_failure_message` 和 `inventory_posting_failed_at_utc`。 | 物料批次索引用于按批次追溯；工单索引用于工单谱系；报工/物料/批次唯一索引用于报工明细加载和重复写入兜底。 | 随报工写入，作为执行追溯历史保留；不拥有 Inventory 批次余额；Inventory 拒绝生产消耗过账时由 MES 消费失败事件并回写到对应消耗事实，报工读面暴露最近失败诊断；历史缺失 UOM 的记录以 `UNSPECIFIED` 显式标记，不能用于发送库存事件。 |
...
| `finished_goods_receipt_requests` | business | MES 完工入库请求事实，向 Inventory 边界表达成品收货意图，并保留可选产出批次/序列号、入库单位成本、Inventory 过账引用和失败诊断。 | `id` 为 Guid v7 强类型 ID；`organization_id + environment_id + request_no` 是入库请求业务号唯一键；记录 `work_order_id`、`sku_id`、`quantity`、`uom_code`、`unit_cost`、`status`、请求时间、`produced_lot_no`、`serial_no`、`posted_inventory_movement_id`、`posted_at_utc`、`inventory_posting_failure_code`、`inventory_posting_failure_message` 和 `inventory_posting_failed_at_utc`；`unit_cost` 对 legacy 记录可为空，新建请求需提供正数以传递给 Inventory 移动平均计价。 | 请求号唯一索引用于下游引用；工单/SKU/时间索引用于入库请求追踪；创建请求的幂等 fingerprint 包含 `unit_cost`，同一业务重试 key 携带不同成本时视为 payload 冲突。 | 创建后通过公共 Inventory movement request 事件表达入库意图；Inventory 拥有正式库存移动和计价结果，MES 只保存请求成本输入、已知过账引用和 Inventory 拒绝诊断。 |
...
| `alarm_events` | business | 工业报警生命周期事实，记录 raise/clear、严重级别、独立优先级、触发 tag、观测值、阈值、单位和外部报警 ID。 | `id` 为 Guid v7 强类型 ID；`organization_id + environment_id + device_asset_id + alarm_code + external_alarm_id` 是幂等唯一键；`status` 表示 raised/cleared；`tag_key`、`observed_value`、`threshold_value`、`unit_code` 为 RCA 上下文字段。 | 外部报警唯一索引防重复；设备+时间索引用于报警时间线和 RCA 查询。 | 报警创建后可清除；清除只补充 cleared facts，不删除历史。 |
...
| Table | Kind | Purpose | Key columns | Index intent | Lifecycle |
| --- | --- | --- | --- | --- | --- |
| `maintenance_work_orders` | business | 维修工单、报警引用、PM 来源、点检失败来源、诊断描述、结构化 failure mode/cause、有效修复开始时间、报警恢复标记、设备不可用和完工事实。 | `id` 为 Guid v7 强类型 ID；`organization_id + environment_id + source_alarm_id` 防止同一报警重复开单；`source_plan_code` 记录 PM 自动开单来源；`source_type + source_reference_id` 记录 alarm、plan、inspection 等来源事实引用；`diagnostic_description` 保存自动开单诊断文本；`failure_mode_code`、`failure_cause_code` 支撑 ISO 14224 风格故障分类；`repair_started_at_utc` 支撑有效修复段 MTTR；`alarm_cleared` / `alarm_cleared_at_utc` 记录报警已解除但待维修确认状态；`device_asset_id` 引用 MasterData 设备。 | source alarm 唯一索引用于报警幂等；`ux_maintenance_work_orders_source_reference` 防止同一来源事实重复开单；状态/设备字段支撑维修看板查询。 | 手工、报警、PM 到期、用量到期或点检失败生成；完成后保留停机、修复段和备件引用事实。 |
...
| BusinessDemandPlanning | `demand_planning` | Implemented | Yes | Yes | No | 已有需求来源、带评审/发布生命周期的 MPS、记录输入来源/覆盖周期的 MRP run、pegging、带 required/release date 的计划建议和 numbering counter/idempotency tables schema、migration、schema convention tests 和 verify script；客户 release bundle 仍待后续。 |
