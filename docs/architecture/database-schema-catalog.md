...
| `operation_tasks` | business | MES 工序任务事实，保存工序顺序、工作中心、可选工作中心、持续时间、执行窗口、暂停累计、实际人工/机器工时、派工班次/人员/设备事实、ProductEngineering 标准工序编码以及 Quality 报检触发快照。 | `id` 为 Guid v7 强类型 ID；`organization_id + environment_id + operation_task_id` 是业务唯一键；`operation_code` 是可选 StandardOperation 编码，用于 MES/PDA 查询当前生效 SOP/电子作业指导书；`sku_code`、`uom_code`、`planned_quantity` 和 `requires_quality_inspection` 复制工单/Routing 报检上下文；`assigned_user_id`、`device_asset_id`、`shift_id` 和 `assigned_at_utc` 记录当前派工事实；`manual_dispatch_revision` 默认 0，是随真实设备派工及其清除单调递增的生命周期版本，`has_active_manual_dispatch` 默认 false，标记当前是否仍拥有 MES 手工设备派工锁；`schedule_plan_id` 与 `schedule_release_revision` 记录当前 APS 派工的真实来源版本，旧撤销只能清除完全匹配的来源；升级时不能可靠推断历史设备来源，因此 revision 0 + inactive + 已有 device 表示 legacy-unknown；`scheduled_at_utc` 仅由已下达 APS 排程分配写入、人工派工不写；`schedule_invalidation_reason_code` 记录排程失效原因码；`paused_at_utc`、`paused_duration_ticks`、`labor_time_ticks`、`machine_time_ticks` 记录暂停扣时后的执行工时事实。 | 工单/工序顺序索引用于按工单加载工序并执行前序门禁；外键索引用于报工与工单追踪；排程来源列用于消费者内精确版本匹配，不建立跨 schema FK。 | 随工单创建或执行调整保留；新版 release 更新来源水位，匹配 revoke 清除 APS 来源/资源并标记失效，但保留 active manual dispatch 与执行中/终态；事件重放和旧撤销晚到均幂等。 |
