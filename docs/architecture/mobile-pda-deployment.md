...
```powershell
# 本仓不设全局 Android env——须显式传给脚本进程（路径按本机实际调整）：
$env:ANDROID_HOME = 'C:\Users\hp\android-sdk'
$env:JAVA_HOME = 'C:\Users\hp\.jdks\jdk-21.0.11+10'   # 须 JDK 21+，见第 2 节前置工具链
...
步骤：env 前置检查（ANDROID_HOME/JAVA_HOME/node_modules，缺失即失败并提示约定路径）→
