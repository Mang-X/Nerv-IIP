...
2. 当前 legacy Compose 文件已拆为依赖/观测层与平台服务层：`infra/compose/nerv-iip.dependencies.yml` 覆盖 PostgreSQL、Redis、MinIO、OTel Collector、RabbitMQ profile 和 Aspire Dashboard profile；Redis 默认启用持久卷、AOF 和 60 秒/1 key change RDB snapshot，以支持 `Messaging:Provider=Redis` 单机 profile。`infra/compose/nerv-iip.platform.yml` 覆盖平台核心服务、部分业务服务、Connector Host、Console 和 Business Console 的镜像构建与运行环境，但它没有覆盖所有后续 AppHost 资源和全部网关下游 `BaseUrl` 注入，不能再被描述为完整平台服务图。完整平台服务拓扑以 AppHost 和 `.\nerv.ps1 publish-compose` 的 Aspire 生成产物为准，legacy overlay 只作为迁移期验证资产。
