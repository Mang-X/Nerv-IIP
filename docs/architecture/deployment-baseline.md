...
当前 `infra/docker-compose.dev.yml` 是本地依赖服务兜底入口。`infra/compose/nerv-iip.dependencies.yml` 提供 PoC/安装前置依赖与观测 profile 的受控 Compose 入口；`infra/compose/nerv-iip.platform.yml` 提供完整平台服务镜像拓扑 overlay，使用 `infra/docker/dotnet-service.Dockerfile` 和 `infra/docker/vite-spa.Dockerfile` 构建主平台 HTTP 服务、Connector Host 与前端 SPA。平台级 AppHost 仍是服务拓扑真相源；Compose overlay 是受控交付产物，不新增第二套业务编排模型。

## Aspire AppHost 范围

当前平台级 AppHost 已覆盖 PlatformGateway、AppHub、IAM、Ops、FileStorage、Notification、BusinessMasterData、BusinessProductEngineering、BusinessInventory、BusinessQuality、BusinessMES、BusinessDemandPlanning、BarcodeLabel、BusinessApproval、WMS、BusinessIndustrialTelemetry、BusinessMaintenance、Connector Host、Console、PostgreSQL、Redis、MinIO 和 OpenTelemetry Collector；RabbitMQ 作为 `Messaging:Provider=RabbitMQ` 时启用的可选资源。后续 AppHost 应继续覆盖：
...
7. 当前完整平台 Compose 使用 `infra/compose/nerv-iip.dependencies.yml` + `infra/compose/nerv-iip.platform.yml` 组合验证；生产配置从 `infra/compose/nerv-iip.production.env.example` 复制后填入现场密钥，不在仓库保存真实值。
...
2. 当前 Compose 文件已拆为依赖/观测层与平台服务层：`infra/compose/nerv-iip.dependencies.yml` 覆盖 PostgreSQL、Redis、MinIO、OTel Collector、RabbitMQ profile 和 Aspire Dashboard profile；`infra/compose/nerv-iip.platform.yml` 覆盖 PlatformGateway、BusinessGateway、AppHub、IAM、Ops、FileStorage、Notification、全部业务服务、Connector Host、Console 和 Business Console 的镜像构建与运行环境。完整平台服务拓扑仍以 AppHost 为准。
3. 平台级 AppHost 位于 `infra/aspire/Nerv.IIP.AppHost`，当前用于表达 PlatformGateway、AppHub、IAM、Ops、FileStorage、Notification、BusinessMasterData、BusinessProductEngineering、BusinessInventory、BusinessQuality、BusinessMES、BusinessDemandPlanning、BarcodeLabel、BusinessApproval、WMS、BusinessIndustrialTelemetry、BusinessMaintenance、Connector Host、Console、PostgreSQL、Redis、MinIO 和 OpenTelemetry Collector 的本地开发拓扑；RabbitMQ 只在 `Messaging:Provider=RabbitMQ` profile 下加入拓扑。AppHost 为 AppHub/IAM/Ops/Notification 与 Business Wave 1/Wave 2/Equipment Reliability 服务使用独立 PostgreSQL database resource；业务服务启动期迁移必须受 `Persistence:AutoMigrate=true` 显式控制，且只允许 Development 环境自动执行。
4. `infra/docker-compose.dev.yml` 继续作为验证脚本拉起本地依赖的稳定入口；PostgreSQL 默认映射到本机 `15432`。PoC/容器化私有化可使用 `docker compose -f infra/compose/nerv-iip.dependencies.yml -f infra/compose/nerv-iip.platform.yml --env-file infra/compose/nerv-iip.production.env up -d`，其中 RabbitMQ 与 Aspire Dashboard 通过 Compose profiles 显式启用。`scripts/verify-production-deployment-artifacts.ps1` 验证 Compose config 与部署文件存在性。
