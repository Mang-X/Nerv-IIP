...
6. migration-based local verification、Compose config 验证、生产环境文件模板、AppHost 启动脚本、AppHub/IAM/Ops/Notification 受治理平台 migration 入口、FileStorage 独立 migration 入口、zip package 入口、disposable Compose 发布演练和性能阈值化 gate 已经存在；FileStorage 在所有环境只允许 PostgreSQL metadata，Development 由 AppHost 注入 `FileStorageDb` 和 Development-only AutoMigrate；缺 provider、选择 InMemory、缺连接或非 Development 启用 Web-host AutoMigrate 都会在启动时 fail fast。客户 PoC/私有化发布前仍需补齐业务数据库显式迁移，并按 docs/architecture/database-release-runbook.md 完成备份恢复演练、seed 清单、Windows Service/systemd 注册和现场诊断输出。
