...
当前平台级 AppHost 已覆盖 PlatformGateway、AppHub、IAM、Ops、FileStorage、Notification、BusinessMasterData、BusinessProductEngineering、BusinessInventory、BusinessQuality、BusinessMES、Connector Host、Console、PostgreSQL、Redis、MinIO 和 OpenTelemetry Collector；RabbitMQ 作为 `Messaging:Provider=RabbitMQ` 时启用的可选资源。后续 AppHost 应继续覆盖：
...
3. 平台级 AppHost 位于 `infra/aspire/Nerv.IIP.AppHost`，当前用于表达 PlatformGateway、AppHub、IAM、Ops、FileStorage、Notification、BusinessMasterData、BusinessProductEngineering、BusinessInventory、BusinessQuality、BusinessMES、Connector Host、Console、PostgreSQL、Redis、MinIO 和 OpenTelemetry Collector 的本地开发拓扑；RabbitMQ 只在 `Messaging:Provider=RabbitMQ` profile 下加入拓扑。AppHost 为 AppHub/IAM/Ops/Notification 与 Business Wave 1 服务使用独立 PostgreSQL database resource；Wave 1 启动期迁移必须受 `Persistence:AutoMigrate=true` 显式控制，且只允许 Development 环境自动执行。
