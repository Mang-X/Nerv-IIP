...
当前平台级 AppHost 已覆盖 PlatformGateway、AppHub、IAM、Ops、FileStorage、Connector Host、Console、PostgreSQL、Redis、MinIO 和 OpenTelemetry Collector；RabbitMQ 作为 `Messaging:Provider=RabbitMQ` 时启用的可选资源。首批完整 AppHost 后续应继续覆盖：
...
3. 平台级 AppHost 位于 `infra/aspire/Nerv.IIP.AppHost`，当前用于表达 PlatformGateway、AppHub、IAM、Ops、FileStorage、Connector Host、Console、PostgreSQL、Redis、MinIO 和 OpenTelemetry Collector 的本地开发拓扑；RabbitMQ 只在 `Messaging:Provider=RabbitMQ` profile 下加入拓扑。AppHub/IAM/Ops 在 AppHost 中使用独立 PostgreSQL database resource，避免共享 database 下 `EnsureCreated()` 对既有表误判。
