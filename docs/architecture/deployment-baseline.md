...
11. 非 Development 部署必须显式选择 `Messaging:Provider=Redis` 或 `RabbitMQ`，不得回落到 InMemory。Redis profile 必须提供 `Parameters:redis-password`（release-install 参数 `-RedisPassword`）；legacy Compose 对应 `NERV_IIP_REDIS_PASSWORD`，Redis 服务、健康检查和应用连接串共同消费该值，模板只保留 `change-me` 占位符。
12. 非 Development IAM 必须覆盖静态开发 MFA code：AppHost secret 参数为 `Parameters:iam-enterprise-identity-mfa-code`，release-install 参数为 `-IamEnterpriseIdentityMfaCode`，legacy Compose 环境变量为 `NERV_IIP_IAM_ENTERPRISE_IDENTITY_MFA_CODE`。缺失或仍为 `000000` 时必须 fail closed；该输入不得写入日志。IAM seed 在非 Development 由 AppHost 固定关闭，因此 release-install 不要求 seed admin password。
...
2. Aspire 生成的完整平台 Compose 产物、安装包和整合安装脚本必须消费同一组 `Xxx:BaseUrl` 配置键；Compose service DNS、客户域名、反向代理地址或脚本生成的配置文件都只是这些键的不同来源。legacy Compose overlay 只作为迁移期验证资产，不作为第二套完整服务图。
...
5. 支持完整 configured-tag coverage 的 Host 必须显式配置稳定 `collectionConnectorId` 并上报 connector tag manifest 与 connection object；旧 Host 仍兼容，但 Business Console 将显示 connection unknown / manifest unavailable。升级不从 `sourceConnector` 或旧样本回填 canonical identity。
6. 发布验收可运行 `pwsh scripts/verify-connector-health-disconnect.ps1 -Runs 3`，证据写入 `artifacts/script-logs/connector-health-disconnect/<timestamp>/evidence.json`。该命令依赖健康的 Docker/PostgreSQL runtime；当前代码头的最近成功证据 `20260718T062424954Z/evidence.json` 为 3/3，端到端耗时 3181/1213/1267 ms（现场检测 401/82/767 ms，检测后 Gateway 可见 2783/1132/501 ms），最大 3181 ms，满足固定 10 秒 deadline。AppHost/DCP 未启动成功的尝试属于环境启动失败，不计入拔线轮次，也不得覆盖最近成功证据。

## 当前阶段

1. 当前仓库已具备平台控制面、业务 waves、真实基础设施、migration-based persistence、生产部署 artifacts 和性能 smoke 的验证入口，但尚不是面向真实用户的最终安装产品。
2. 当前 legacy Compose 文件已拆为依赖/观测层与平台服务层：`infra/compose/nerv-iip.dependencies.yml` 覆盖 PostgreSQL、Redis、MinIO、OTel Collector、RabbitMQ profile 和 Aspire Dashboard profile；Redis 默认要求 `NERV_IIP_REDIS_PASSWORD`，并启用持久卷、AOF 和 60 秒/1 key change RDB snapshot，以支持 `Messaging:Provider=Redis` 单机 profile。`infra/compose/nerv-iip.platform.yml` 覆盖平台核心服务、部分业务服务、Connector Host、Console 和 Business Console 的镜像构建与运行环境，但它没有覆盖所有后续 AppHost 资源和全部网关下游 `BaseUrl` 注入，不能再被描述为完整平台服务图。完整平台服务拓扑以 AppHost 和 `.\nerv.ps1 publish-compose` 的 Aspire 生成产物为准，legacy overlay 只作为迁移期验证资产。
3. 平台级 AppHost 位于 `infra/aspire/Nerv.IIP.AppHost`，当前用于表达 PlatformGateway、BusinessGateway、AppHub、IAM、Ops、FileStorage、Notification、BusinessMasterData、BusinessProductEngineering、BusinessInventory、BusinessQuality、BusinessMES、BusinessDemandPlanning、BusinessBarcodeLabel、BusinessApproval、BusinessWMS、BusinessIndustrialTelemetry、BusinessMaintenance、BusinessERP、BusinessScheduling、Connector Host、Console、BusinessConsole、PostgreSQL、Redis、MinIO、可选 OpenTelemetry Collector、Aspire Dashboard 和 Aspire Docker Compose deployment target 的本地开发与部署拓扑；RabbitMQ 只在 `Messaging:Provider=RabbitMQ` profile 下加入拓扑，Redis profile 不启动 RabbitMQ。AppHost 为 AppHub/IAM/Ops/FileStorage/Notification 与业务服务使用独立 PostgreSQL database resource；FileStorage 固定使用 `file-storage-db` / `nerv_iip_filestorage`，以 `FileStorageDb` reference 注入连接并在数据库就绪后启动；BusinessQuality 和 BusinessDemandPlanning 通过 AppHost 注入 `Erp:BaseUrl`、resource reference 和 wait dependency 消费 BusinessERP HTTP 边界；BusinessScheduling 通过独立 `business-scheduling-db` 和 5120 本地端口接入 APS lite；业务服务启动期迁移必须受 `Persistence:AutoMigrate=true` 显式控制，且只允许 Development 环境自动执行。
...
6. migration-based local verification、Compose config 验证、生产环境文件模板、AppHost 启动脚本、AppHub/IAM/Ops/Notification 平台 migration 入口、13 个业务数据库 manifest migration 入口、FileStorage 独立 migration 入口、zip package 入口、disposable Compose 发布演练和性能阈值化 gate 已经存在；18 个 AppHost PostgreSQL database resource 均有受治理显式迁移路径。FileStorage 在所有环境只允许 PostgreSQL metadata，Development 由 AppHost 注入 `FileStorageDb` 和 Development-only AutoMigrate；缺 provider、选择 InMemory、缺连接或非 Development 启用 Web-host AutoMigrate 都会在启动时 fail fast。客户 PoC/私有化发布前仍需按 `docs/runbooks/database-release.md` 完成备份恢复演练、seed 清单、Windows Service/systemd 注册和现场诊断输出。
7. 第四阶段历史本地验收入口 `scripts/verify-fourth-slice-real-infra.ps1` 已删除，不再作为兼容路径或部署入口；当前真实基础设施验证使用 AppHost/fullstack 与专用 PostgreSQL/Redis/provider lane，发布演练使用 `scripts/verify-production-release-rehearsal.ps1`。若镜像拉取受限，可以先按现场网络要求配置 Docker Desktop proxy。
8. 脚本自动化治理已冻结为 ADR 0010；现有验证脚本按 `docs/governance/script-automation.md` 迁移到共享 helper 和静态门禁，迁移完成前不得把 legacy verify 脚本解释为 release-install 入口。`scripts/install/start-nerv-iip-apphost.ps1` 是受治理的 release-install 启动入口，负责设置受控环境变量、Aspire 参数、生产 CORS origins、内部 token 和密钥后启动平台 AppHost；它不注册 Windows Service/systemd，也不绕过 AppHost 拓扑。`scripts/install/migrate-platform-databases.ps1` 以显式 allowlist manifest 执行平台数据库迁移，并在任何 EF 动作前通过 `psql` 只读连接确认目标库已存在；`scripts/install/migrate-business-databases.ps1` 是业务 13 库的受治理包装入口，保留独立子进程日志并把脱敏的逐服务诊断回放给操作员。`scripts/install/migrate-file-storage.ps1` 是 FileStorage 的显式 release migrator：连接串只从当前进程的 `NERV_IIP_FILE_STORAGE_DB` 读取，输出脱敏 release/service/database/migration/correlation/log 状态，不负责备份、删库或 seed；PoC/production 必须先完成 runbook 的备份 preflight 再执行。`scripts/package/create-nerv-iip-package.ps1` 是受治理的 zip 交付物生成入口。
...
12. 仓储站点/库位配置（MES `Inventory__SiteCode` / `Inventory__SourceLocationCodes__N` / `Inventory__LineSideLocationCode` / `Inventory__FinishedGoodsLocationCode`，WMS `MaterialIssue__SourceLocationCode` / `MaterialIssue__LineSideLocationCode`）受 AppHost 环境门控：只有 AppHost 自身 `EnvironmentName=Development` 时才回落到与 MasterData `inventory-location` 字典对齐的主线产品位置词汇（`SITE-001` + `loc-raw-01` / `loc-semi-01` / `loc-fg-01` / `loc-line-01`）；这四项是跨域部署配置约定，不是本票创建的 Inventory `StockLocation` 实体或在手量 seed，实际库存库位仍由 Inventory 独立创建与拥有。其他环境必须由部署方用同名配置键显式给出真实站点/库位，`scripts/install/start-nerv-iip-apphost.ps1` 的 `-InventorySiteCode`、`-InventorySourceLocationCodes`、`-InventoryLineSideLocationCode`、`-InventoryFinishedGoodsLocationCode`、`-MaterialIssueSourceLocationCode`、`-MaterialIssueLineSideLocationCode` 是受治理的传入入口。未显式配置时 AppHost 一个键都不下发，由服务侧 fail-closed 显式暴露：WMS 领料消息进死信 `unresolved-location` 并伴随 `LogWarning`，MES 抛 `MATERIAL_SUPPLY_LOCATION_UNCONFIGURED` / `FINISHED_GOODS_LOCATION_UNCONFIGURED`；产品级回落值不会把世界观库位随权威发布路径带入私有化部署。`Inventory__BaseUrl` 是服务端点而非仓储位置，不受该门控影响。
13. legacy overlay `infra/compose/nerv-iip.platform.yml` 明确**不支持 MES→WMS 领料链路**与 MES 线边收料/完工入库过账：它没有下发上面第 12 条的仓储站点/库位键，该路径下领料消息会全量进死信。这是 legacy overlay 定位的直接结论（见本节第 2 条），不再补一份会与 AppHost 漂移的手工配置；需要完整平台拓扑与领料链路时走 `.\nerv.ps1 publish-compose` 生成的 Aspire 产物，再用 `prepare-compose` / `deploy-compose` 部署。
14. AppHost 的 21 个 .NET 项目资源统一从 `builder.Environment.EnvironmentName` 下发 `ASPNETCORE_ENVIRONMENT` 与 `DOTNET_ENVIRONMENT`，不再把子服务固定为 Development。18 个 PostgreSQL 项目资源的 `Persistence__AutoMigrate` 以及 walkthrough、ERP demo、Leader Demo / World History、IAM seed 等开发或演示注入使用同一 Development 判据；非 Development 产物明确关闭这些注入，使各服务的生产守卫真正生效。该合同已用 Development / Production 两份 Aspire Compose 发布产物逐资源核对，但只证明生成产物的环境与开发注入正确，不代表 Production 全栈已经完成显式迁移、TLS/代理配置或真实起栈；后续输入闭合与运行验收分别由 #2032、#2030 跟踪。
15. Production AppHost 与 release-install 的必填输入已闭合 Redis password、IAM pepper、IAM MFA override、Gateway CORS 和 Connector Host scope：AppHost 对非 Development 的 InMemory messaging、缺失 CORS、缺失 Connector Host ID/organization/environment 均直接失败；release-install 对 Redis 缺 password、IAM MFA 缺失或仍为 `000000` 直接失败，并把两项作为 Aspire secret parameter 下发。legacy Compose 与生产 env 样例使用同一命名口径，Redis 启用认证，IAM 显式接收 pepper/MFA。此处只证明输入合同与发布产物可生成，不等同于 Production 全栈已完成迁移或真实起栈；运行验收仍由 #2030 跟踪。
16. BusinessBarcodeLabel 的打印传输配置按部署环境失败关闭：Development AppHost 显式注入受控 `LabelPrinter__Mode=simulated`；非 Development AppHost 与 `scripts/install/start-nerv-iip-apphost.ps1` 固定选择 `zpl-tcp`，并要求部署方传入逻辑 printer id、host、port、连接/写入超时、DPI 和支持码制。legacy Compose 通过 `NERV_IIP_BARCODE_LABEL_PRINTER_*` 接收同一组输入，`infra/compose/nerv-iip.production.env.example` 只保留空白 id/host 与非现场默认值，不提交真实地址。服务启动时拒绝缺失/未知/禁用 mode、重复 id、非法路由或非开发模拟；这些门禁与 loopback 测试只证明配置和 TCP transport 合同，不等同于真实设备可达、物理出纸或现场验收。
