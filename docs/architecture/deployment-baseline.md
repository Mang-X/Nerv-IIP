...
3. 平台级 AppHost 位于 `infra/aspire/Nerv.IIP.AppHost`，当前用于表达 PlatformGateway、BusinessGateway、AppHub、IAM、Ops、FileStorage、Notification、BusinessMasterData、BusinessProductEngineering、BusinessInventory、BusinessQuality、BusinessMES、BusinessDemandPlanning、BusinessBarcodeLabel、BusinessApproval、BusinessWMS、BusinessIndustrialTelemetry、BusinessMaintenance、BusinessERP、Connector Host、Console、BusinessConsole、PostgreSQL、Redis、MinIO 和 OpenTelemetry Collector 的本地开发拓扑；RabbitMQ 只在 `Messaging:Provider=RabbitMQ` profile 下加入拓扑。AppHost 为 AppHub/IAM/Ops/Notification 与业务服务使用独立 PostgreSQL database resource；业务服务启动期迁移必须受 `Persistence:AutoMigrate=true` 显式控制，且只允许 Development 环境自动执行。
4. `infra/docker-compose.dev.yml` 继续作为验证脚本拉起本地依赖的稳定入口；PostgreSQL 默认映射到本机 `15432`。PoC/容器化私有化可使用 `docker compose -f infra/compose/nerv-iip.dependencies.yml -f infra/compose/nerv-iip.platform.yml --env-file infra/compose/nerv-iip.production.env up -d`，其中 RabbitMQ 与 Aspire Dashboard 通过 Compose profiles 显式启用。`scripts/verify-production-deployment-artifacts.ps1` 验证 Compose config 与部署文件存在性；`scripts/verify-production-release-rehearsal.ps1 -Profile dependencies|platform-smoke` 使用独立 Compose project 和高位端口执行 opt-in 发布演练。
5. migration-based local verification、Compose config 验证、生产环境文件模板、AppHost 启动脚本、zip package 入口、disposable Compose 发布演练和性能阈值化 gate 已经存在；客户 PoC/私有化发布前仍需按 docs/architecture/database-release-runbook.md 补齐备份恢复演练、seed 清单、Windows Service/systemd 注册和现场诊断输出。
