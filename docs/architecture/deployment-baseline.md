...
6. 发布验收可运行 `pwsh scripts/verify-connector-health-disconnect.ps1 -Runs 3`，证据写入 `artifacts/script-logs/connector-health-disconnect/<timestamp>/evidence.json`。该命令依赖健康的 Docker/PostgreSQL runtime；最近成功证据 `20260718T025049736Z/evidence.json` 为 3/3，耗时 9816/2749/1935 ms，最大 9816 ms，满足固定 10 秒 deadline。AppHost/DCP 未启动成功的后续尝试属于环境启动失败，不计入拔线轮次，也不得覆盖最近成功证据。
