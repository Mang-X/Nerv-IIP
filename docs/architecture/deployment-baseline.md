...
6. migration-based local verification、Compose config 验证、生产环境文件模板、AppHost 启动脚本、FileStorage 受治理 migration 入口、zip package 入口、disposable Compose 发布演练和性能阈值化 gate 已经存在；FileStorage 的 Development profile 只有显式 `Persistence:Provider=InMemory` 才允许进程内 metadata，AppHost 默认改用 PostgreSQL；PoC/production 缺 provider、选择 InMemory、缺 `FileStorageDb` 连接或启用 Web-host AutoMigrate 都会在启动时 fail fast。客户 PoC/私有化发布前仍需按 docs/architecture/database-release-runbook.md 完成备份恢复演练、seed 清单、Windows Service/systemd 注册和现场诊断输出。
