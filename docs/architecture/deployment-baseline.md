...
6. 发布验收可运行 `pwsh scripts/verify-connector-health-disconnect.ps1 -Runs 3`，证据写入 `artifacts/script-logs/connector-health-disconnect/<timestamp>/evidence.json`。该命令依赖健康的 Docker/PostgreSQL runtime；当前环境执行为 0/3，不能记录成通过，须在 runtime 恢复后复验三轮并保留最大耗时证据。
