...
7. `scripts/verify-fourth-slice-real-infra.ps1` 是第四阶段历史本地验收入口，已按 #2157 退役并保留为明确失败的兼容墓碑；当前真实基础设施验证使用 AppHost/fullstack 与专用 provider lane。若镜像拉取受限，可以先按现场网络要求配置 Docker Desktop proxy。
