...
39. PlatformGateway 当前提供实例列表、实例详情、实例 restart、operation task detail、Console Notification、Console IAM Admin 和 FileStorage 文件列表/上传/下载授权 facade；这些 Console API 需要 bearer token，并由 Gateway 转发到 IAM 做权限校验。BusinessGateway 当前提供 Business Console MasterData、ProductEngineering、Inventory、Quality、DemandPlanning、Scheduling、Equipment runtime facts、WMS 收货/出库/WCS 读面、BarcodeLabel 分页读面、IndustrialTelemetry tags/alarm-rules/alarms/history/OEE/runtime-availability、Maintenance 工单/计划/点检/备件/availability-windows、BusinessApproval 审批中心和 MES facade，使用用户 bearer token 到 IAM 做权限校验，并使用 internal service token 调用业务服务。
