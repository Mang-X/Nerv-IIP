...
| IndustrialTelemetry | 已有 Domain/Infrastructure/Web、PostgreSQL migration 和测试；已覆盖 TelemetryTag、DeviceStateSnapshot、AlarmEvent 和 TelemetrySummary，并接入 solution/AppHost/IAM seed/schema catalog，提供 `scripts/verify-business-industrial-telemetry-mvp.ps1`；#207 已补 source_system/source_connector 幂等 scope、current-state/runtime availability 查询和 BusinessGateway equipment facade 消费边界。保持 PLC/DCS/SCADA 控制命令和凭据在外部边界。 | #129、#207 |
