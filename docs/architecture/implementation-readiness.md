...
56. #78 甘特/排产前端已落地为独立组件包 `frontend/packages/scheduling`（`@nerv-iip/scheduling`）：交付两个可外部消费的复合组件 `GanttChart`（工单甘特）与 `ResourceSchedulerBoard`（资源排产板），以及组合它们的 `SchedulingWorkbench`。「引擎无关」两层架构:Vue 组件层 → `SchedulingEngine` 适配器（本包只保留 `DhtmlxEngine`，封装 DHTMLX Gantt 9.x 专业版；正式自研引擎在后续 PR 落地）→ `ScheduleModel` + `aps-mapper`（消费 #206 Scheduling facade 的 `SchedulePlanContract`，只映射真实字段、不伪造）。DHTMLX 评估许可禁止分发,库文件不入 git（`.gitignore vendor/`，私有源 `@dhx/trial-gantt`）:开发用试用版、生产部署给客户手动分发正式版;未装 vendor 时经条件 vite alias 指向 stub,`vite build` 不失败。编辑为「锁定—重预览」闭环(贴合后端确定性重排,撤销/重做为前端快照栈;未注入 `preview` 时隐藏重排动作,不做假反馈)。设计系统文档站(`apps/design-system`)提供组件示例与接口说明;DESIGN 契约见 `frontend/DESIGN/components/gantt-chart.md`、`resource-scheduler-board.md`、`patterns/blocks/scheduling-workbench.md`。当前后端缺口:从计划+锁定分配重预览、工序依赖编辑端点、资源产能日历。
