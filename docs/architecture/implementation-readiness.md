...
- 已交付事实仍以代码为准：IndustrialTelemetry 当前提供 HTTP bucket 采样入口、TelemetryTag、TelemetrySummary、DeviceStateSnapshot、AlarmRaised/AlarmCleared、OEE/runtime-availability 读面和 BusinessGateway/Business Console 设备运行页面；Maintenance 当前消费报警开单/清除并发布 AssetUnavailable/AssetRestored，且生产分支已通过 `HttpIndustrialTelemetryAssetRuntimeHoursProvider` 将 `IAssetRuntimeHoursProvider` 接到 IndustrialTelemetry OEE 读面并在 OEE 无样本或异常时 fallback 到 Maintenance availability windows；MES 和 Scheduling 当前消费维护可用性事件或标准化 availability/readiness 结果。
- #683 OPC UA 采集连接器已在 Connector Host 落地首条通道：OPC UA client adapter 连接/浏览/订阅 tag，按 bucket 聚合后调用 IndustrialTelemetry `POST /api/business/v1/iiot/samples`，携带 `source_system=opcua`、`source_connector` 和稳定 `source_sequence`；采集健康通过现有 heartbeat/state-snapshot 链路观测，凭据只记录引用不入库不入仓库，首批支持 `env:<PREFIX>` 用户名密码引用。尚未交付但已纳入本批执行入口：#684 Modbus TCP/MQTT 采集连接器、#685 AlarmRaised/AlarmCleared 接入 Notification、#686 报警 ack/shelve/escalation、#687 设备控制命令下发、#688 IndustrialTelemetry 侧运行小时聚合与持久累计、#689 historian raw/hourly/daily 分层存储、#690 DeviceStateChanged 下游消费。
