...
41. #170 事件可靠性基线已落地契约与消费侧守门：`backend/tests/Nerv.IIP.Contracts.IntegrationEvents.Tests` 通过反射验证 Ops、Maintenance、WMS 和 IndustrialTelemetry 公共集成事件暴露 ADR 0011 envelope 字段与结构化 payload；WMS 和 IndustrialTelemetry 事件契约已补齐 correlation、causation、actor、version、source、idempotency 和 payload 形态。`Nerv.IIP.Messaging.CAP` 已提供 `IntegrationEventEnvelopeValidator`、`IntegrationEventConsumerGuard` 和 in-memory DLQ/replay store 抽象；当前已有真实消费者 BusinessMaintenance `AlarmRaised`、Notification `OperationTaskFailed`、BusinessMES `AssetUnavailable`/`AssetRestored`、AppHub `OperationTaskCompleted`/`OperationTaskFailed` 接入版本校验，unsupported version 等消费前拒绝会进入 DLQ，避免执行业务副作用。Notification、AppHub、BusinessMaintenance 和 BusinessMES 的真实消费者均保留 NetCorePal `IntegrationEventConsumer` 标记，并补充 CAP 原生 `ICapSubscribe` 接法复用同一个 handler/guard；BusinessMaintenance 进一步落地了 `integration_event_dead_letters` PostgreSQL 表和 `MaintenanceIntegrationEventDeadLetterStore`，其 DLQ 为持久化；Notification、BusinessMES 和 AppHub 当前使用 in-memory DLQ 基线。持久化业务 inbox 与 replay 管理入口仍可按后续小 issue 深化。
