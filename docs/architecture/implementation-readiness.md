...
- 已交付事实仍以代码为准：IndustrialTelemetry 当前提供 HTTP bucket 采样入口、TelemetryTag、TelemetrySummary、DeviceStateSnapshot、AlarmRaised/AlarmCleared、OEE/runtime-availability 读面和 BusinessGateway/Business Console 设备运行页面；Maintenance 当前消费报警开单/清除并发布 AssetUnavailable/AssetRestored；MES 和 Scheduling 当前消费维护可用性事件或标准化 availability/readiness 结果。
- 尚未交付但已纳入本批执行入口：#683 OPC UA 采集连接器、#684 Modbus TCP/MQTT 采集连接器、#685 AlarmRaised/AlarmCleared 接入 Notification、#686 报警 ack/shelve/escalation、#687 设备控制命令下发、#688 运行小时聚合器与 Maintenance runtime-hours provider、#689 historian raw/hourly/daily 分层存储、#690 DeviceStateChanged 下游消费。
