...
5. Notification 的模板映射、通知合并/静默窗口以及 DLQ 告警与统一看板；边界口径应遵守 docs/architecture/notification-baseline.md。
