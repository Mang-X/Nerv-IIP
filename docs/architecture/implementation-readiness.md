...
45. #173/#528 生产安全硬化已形成可验证闭环：AppHub connector registration 不再固定接受仓库内 `local-connector-secret`；配置 `ConnectorHostCredential:Secret` 后只接受配置值，非 Development 缺失配置会拒绝请求。#528 已把 AppHub heartbeat/state ingestion 从共享 secret 改为注册返回的 per-instance `X-Connector-Ingestion-Token`，token 绑定 registrationId、organizationId、environmentId、connectorHostId 和 instanceKey，body 与 token 不一致的上报会被拒绝；非 Development 还必须显式配置至少 32 bytes 的 `ConnectorIngestionToken:SigningKey`。平台级 AppHost 已把 connector secret 和 `InternalService__BearerToken` 传给子服务，Development fallback 仅保留用于本地无配置测试。IAM 生产环境不论 persistence profile 都要求显式 `Iam:Jwt:SigningKey`，且签名密钥至少 32 bytes、`Iam:Jwt:AccessTokenMinutes` 必须在 1-60 分钟内；PlatformGateway 与 BusinessGateway 在非 Development 下要求 `Security:Cors:AllowedOrigins`、启用 HSTS/HTTPS redirect、CORS 白名单和全局 rate limit；业务 JWT metadata 在非 Development 不再关闭 HTTPS metadata；Ops `audit_records` 新增 `IntegrityHash`，新审计事实带 tamper-evident SHA-256 摘要。
