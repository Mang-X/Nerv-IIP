...
- #683 OPC UA 采集连接器已在 Connector Host 落地首条通道：OPC UA client adapter 连接/浏览/订阅 tag，按 bucket 聚合后调用 IndustrialTelemetry `POST /api/business/v1/iiot/samples`，携带 `source_system=opcua`、`source_connector` 和稳定 `source_sequence`；采集健康通过现有 heartbeat/state-snapshot 链路观测，凭据只记录引用不入库不入仓库，首批支持 `env:<PREFIX>` 用户名密码引用。#685 已补 AlarmRaised/AlarmCleared 接入 Notification；#688 已补 IndustrialTelemetry 侧运行小时查询与 Maintenance RuntimeHourInterval PM 闭环；#690 已补 DeviceStateChanged 下游消费到 IndustrialTelemetry runtime availability 查询投影与 Scheduling generated-plan invalidation；尚未交付但已纳入本批执行入口：#684 Modbus TCP/MQTT 采集连接器、#686 报警 ack/shelve/escalation、#687 设备控制命令下发、#689 historian raw/hourly/daily 分层存储。
