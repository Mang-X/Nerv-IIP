...
5. Notification 的模板映射、通知合并/静默窗口和 DLQ replay 管理入口；边界口径应遵守 docs/architecture/notification-baseline.md。
