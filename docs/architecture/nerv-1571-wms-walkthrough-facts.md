...
合同与 WMS 专属 proof 必须拒绝：组织或环境不匹配、`scopeKind`/`scopeId` 不匹配、`skip`/`take` 不匹配、列表路径、方法、状态或网络错误、入库缺少或错误 `siteCode`、出库带有 `siteCode`，以及没有选择或同时提供两个选择事实。点击后目录尚未完成加载时不得把暂时的零选项数当成缺失；应等待可见目录稳定后再判定唯一性。
