...
1. Capacitor 是面向 Web App 的跨平台原生运行时（native runtime），可创建 Android、iOS 和 PWA 应用，并允许通过插件访问原生能力。
2. Capacitor v8 官方支持 Android、iOS、Web 三类目标；核心要求 Node.js 22+，Android 开发要求 Android Studio 2025.2.1+。
3. Capacitor Android 运行时（runtime）允许 JavaScript 与 Java/Kotlin 通信，Android 支持 API 24+，并依赖 Chrome 60+ 的 Android WebView。
4. Capacitor v8 为当前活跃（active）版本，最低 Android 版本为 Android 7/API 24；v7 已进入维护（maintenance）阶段。
...
6. Capacitor 原生插件可以用 Java/Kotlin 编写，支持权限、Android intent、插件事件（plugin event）和 WebView 导航覆盖（navigation override）。
7. 官方存储指南（storage guide）明确 LocalStorage 和 IndexedDB 存在被系统回收的风险；高性能或大规模本地数据应使用 SQLite 类方案。
8. 官方安全指南（security guide）建议不在客户端嵌入密钥（secret），敏感值使用 Keychain/Keystore，OAuth 使用 PKCE，网络走 HTTPS，WebView 设置 CSP。
9. Zebra DataWedge 支持将硬件扫描头、相机、RFID、MSR、串口等输入通过 Android intent 发送给前台应用，扫描内容通过 `com.symbol.datawedge.data_string` 等额外字段（extra）提供。
10. Honeywell Mobility SDK for Android 提供 Honeywell 移动电脑、打印机和条码阅读器接口。
11. Android Enterprise 支持通过 EMM 分发私有应用（private app）；托管配置（managed configuration）支持 IT 管理员远程下发应用配置。
12. Android 官方离线优先指南（offline-first guidance）强调本地数据源、同步队列、网络监控和 WorkManager。

参考链接：

1. [Capacitor 文档](https://capacitorjs.com/docs)
2. [Capacitor 环境配置](https://capacitorjs.com/docs/getting-started/environment-setup)
3. [Capacitor Android 文档](https://capacitorjs.com/docs/android)
4. [Capacitor 支持策略](https://capacitorjs.com/docs/main/reference/support-policy)
5. [Capacitor 插件概览](https://capacitorjs.com/docs/plugins)
6. [Capacitor 官方插件](https://capacitorjs.com/docs/apis)
7. [Capacitor Android 插件指南](https://capacitorjs.com/docs/plugins/android)
...
12. [Capacitor 存储指南](https://capacitorjs.com/docs/guides/storage)
13. [Capacitor 安全指南](https://capacitorjs.com/docs/guides/security)
14. [Zebra DataWedge Intent Output（意图输出）](https://techdocs.zebra.com/datawedge/latest/guide/output/intent/)
15. [Zebra DataWedge Barcode Input（条码输入）](https://techdocs.zebra.com/datawedge/latest/guide/input/barcode/)
...
17. [Android Enterprise 私有应用分发](https://support.google.com/work/android/answer/9495634?hl=en)
18. [Android 托管配置](https://developer.android.com/work/managed-configurations?hl=en)
19. [Android 离线优先架构](https://developer.android.com/topic/architecture/data-layer/offline-first?hl=en)
...
2. PDA 只访问 BusinessGateway 移动门面（mobile facade），不直连业务服务、数据库、对象存储或消息队列。
3. 扫码、设备、打印、RFID、后台同步等原生能力统一封装在设备桥接层（device bridge）后面，页面不感知厂商 SDK。
4. 离线写入先落本地事实源和发件箱（outbox），再同步到服务端；服务端领域模型决定最终业务事实。
...
1. 使用 Capacitor v8。
2. Android 为首批唯一必须达到的原生目标（native target）。
3. iOS 目标（target）不在 PDA MVP 内创建；如后续需要手机审批类轻应用，可单独评估。
4. Web/PWA 目标（target）只用于开发调试、桌面演示和摄像头扫码兜底（fallback），不作为工业 PDA 正式交付形态。
...
1. Android Enterprise 托管配置（managed configuration）。
2. 管理员扫码配置。
3. 预置测试环境配置。
...
### 扫码适配器（Scanner adapters）

| 适配器（Adapter） | 用途 | 实现方式 | 优先级 |
| --- | --- | --- | --- |
| DataWedge 意图适配器（Intent Adapter） | Zebra/兼容 DataWedge 设备 | Android BroadcastReceiver 或 Activity intent，转为 Capacitor 插件事件（plugin event） | P0 |
| 键盘模拟适配器（Keyboard Wedge Adapter） | 多数 PDA 默认键盘模拟模式 | 捕获 Web input/keydown，保留焦点策略 | P0 兜底（fallback） |
| 摄像头扫码适配器（Camera Scanner Adapter） | 手机、平板、无硬件扫描头设备 | `@capacitor/barcode-scanner` | P1 |
| Honeywell 适配器（Adapter） | Honeywell PDA 深度适配 | Honeywell Mobility SDK 或 intent API | P1/按设备 |
| Urovo/Newland/Seuic 适配器（Adapter） | 国产 PDA 深度适配 | 厂商 SDK/intent | P2/按客户 |
| RFID 适配器（Adapter） | UHF/RFID 场景 | 厂商 SDK 或 DataWedge RFID source | P2 |

### DataWedge 意图（intent）处理
...
### 键盘模拟兜底（Keyboard wedge fallback）

键盘模拟（Keyboard wedge）模式作为兼容兜底，但不能作为唯一推荐模式：

1. 页面必须有隐藏或受控输入接收扫描。
2. 按结束符、扫描间隔和字符速度识别扫描输入。
3. 输入法、手工键入、焦点丢失和软键盘弹出要单独处理。
4. 无法稳定提供码制和原始字节（raw bytes），因此高要求条码解析仍应使用 intent/SDK。

### 反馈（Feedback）

`FeedbackPort` 统一封装：

1. 成功/错误/警告（success/error/warning）三类蜂鸣。
2. 短震动和长震动。
3. 页面提示条（toast）或状态条。
...
2. 页面读取本地投影（projection），写入本地操作（operation），再由发件箱（outbox）同步。
3. 服务端返回的已接受/已拒绝/冲突/重复（accepted/rejected/conflict/duplicate）结果会回写本地操作。
4. 同步策略必须可中断、可恢复、可重试。
5. LocalStorage 只保存非敏感轻量偏好，不能保存任务事实、令牌（token）、发件箱（outbox）或业务草稿。
...
1. 在线提交：页面写入本地操作（operation）后立即触发同步。
2. 网络恢复：Network 插件通知后触发同步。
3. App 启动：启动后触发引导同步（bootstrap sync）。
4. 用户手动：异常队列提供重试按钮。
5. 后台同步：MVP 以 App 前台同步为主；如客户要求后台可靠同步，再增加 Android WorkManager Capacitor 插件。
6. 批量同步：按操作类型（operationType）、创建时间和依赖关系排序，默认批次 50-200。
7. 幂等：每个操作（operation）生成稳定幂等键（idempotencyKey），重放不产生重复业务事实。
8. 冲突：服务端返回机器可读错误码（errorCode）和用户可读消息（message）。

### 冲突处理

| 冲突 | 处理 |
| --- | --- |
| 任务已关闭 | 标记为冲突（conflict），提示刷新任务。 |
| 数量超出 | 标记为已拒绝（rejected），允许修改数量后重试。 |
| 物料/库位不匹配 | 标记为已拒绝（rejected），保留扫描证据。 |
| 权限变化 | 标记为已拒绝（rejected），要求重新登录或联系主管。 |
| 重复提交 | 标记为重复（duplicate），展示已有服务端结果。 |
| 服务端版本过期 | 标记为冲突（conflict），拉取最新任务后人工确认。 |
...
1. `PlatformGateway` 继续服务主平台 Console、AppHub、Ops、IAM 管理端（Admin）、观测等平台控制面接口。
...
4. 当前 `business-pda` 已通过 BusinessGateway 的 business-console 门面（facade）完成 WMS、MES 和设备轻量作业页面；后续移动门面（mobile facade）落地时不得把既有业务控制台契约机械复制一份。

PDA 当前只访问 BusinessGateway/PlatformGateway 的受控门面（facade），不直连业务服务 URL：
...
1. Bearer 令牌校验和权限执行（enforcement）。
2. 组织、环境、设备上下文解析。
3. 移动聚合 DTO。
4. OpenAPI 输出和 api-client 生成。
5. 错误码归一化。
6. 请求限流、幂等头透传、关联 ID（correlationId）注入。
...
当前代码还没有移动 OpenAPI 生成链路；`frontend/packages/api-client/openapi/` 只包含 PlatformGateway 与 BusinessGateway business-console 快照，`src/index.ts` 只重新导出 console、business-console、iam 和 auth 稳定入口。移动 OpenAPI 使用独立 BusinessGateway 契约时，仍应进入现有 `@nerv-iip/api-client` 包，避免 PDA 与 Console 各自维护传输层（transport）、认证请求头（auth header）、错误模型和 envelope 类型。
...
3. `frontend/packages/api-client/openapi-ts.config.ts` 增加移动输入（mobile input），生成到独立目录 `frontend/packages/api-client/src/generated/mobile/`，避免与现有 `src/generated/` PlatformGateway 代码混杂。
4. 手写稳定入口为 `frontend/packages/api-client/src/mobile.ts`，并从 `src/index.ts` 重新导出必要类型、SDK 和 Pinia Colada 在线查询选项（online query options）。
5. PDA 应用只从 `@nerv-iip/api-client` 稳定入口消费，不深层导入（deep import）`src/generated/mobile/*`。
6. BusinessGateway 与 PlatformGateway 可以在部署层共用公网域名和反向代理，也可以分开域名；前端传输层（transport）必须允许 PDA 的 `gatewayBaseUrl` 指向 BusinessGateway，不假设与 Console `baseURL` 相同。

Mobile operationId 固定使用 lower camelCase 并带 `Mobile` 语义前缀，完整表格见 [api-contract-and-codegen.md](api-contract-and-codegen.md)。

所有写 API 必须支持：

1. `Idempotency-Key` 请求头（header）。
2. `X-Correlation-Id` 请求头（header）。
3. 请求体（request body）内的 `clientOperationId`、`deviceId`、`occurredAt`。
...
| 字段 | 来源 | 说明 |
| --- | --- | --- |
| `principalType=user` | 访问令牌（access token） | 员工身份。 |
| `organizationId` | 令牌/上下文选择（token/context selection） | 组织边界。 |
| `environmentId` | 令牌/上下文选择（token/context selection） | 环境边界。 |
| `deviceId` | 设备配置（device profile） | 已登记设备。 |
| `warehouseId/workCenterId` | 移动上下文（mobile context） | 仓库或工位边界。 |
| `permissionCode` | endpoint 声明（declaration） | 业务动作权限。 |

首批复用 [authorization-matrix.md](authorization-matrix.md) 中的业务权限码。若后续引入平台级设备台账，再新增移动设备权限：

| 权限码 | 建议范围（scope） | 说明 |
| --- | --- | --- |
| `mobile.devices.read` | 组织/环境（organization/environment）+ 资源（resource） | 查看 PDA 设备、版本、最近同步状态。 |
| `mobile.devices.manage` | 组织/环境（organization/environment）+ 资源（resource） | 登记、停用、解绑 PDA 设备。 |
| `mobile.diagnostics.write` | 环境（environment）+ 资源（resource） | PDA 上传诊断摘要。 |
...
1. 访问令牌（access token）仅在内存和必要的安全存储中使用。
2. 刷新令牌（refresh token）使用 Android Keystore 或成熟安全存储插件保存。
3. 本地 SQLite 如保存业务敏感数据，应启用加密 SQLite 或文件级加密策略。
4. 设备丢失时，管理员可撤销会话、解绑设备或吊销刷新令牌（refresh token）。

### 网络

1. 生产环境只允许 HTTPS。
2. App 配置 Gateway 允许列表（allowlist），禁止 WebView 任意跳转。
3. CSP 限制资源加载域。
...
1. 首批可复用现有 IAM 登录门面（facade）。
2. 后续如引入 OIDC/SSO，移动端必须使用 Authorization Code + PKCE。
3. 深链回调只携带 code/state，不携带令牌（token）。
4. 离线授权必须有到期时间和可撤销策略。

### WebView 安全

1. 禁止加载非受信任远程页面作为主应用。
2. 禁止把密钥（secret）注入 Vite 环境变量（env）并打包到前端 bundle。
3. 禁止把原生桥接（native bridge）暴露成任意命令执行通道。
...
| 方式 | 适用 | 说明 |
| --- | --- | --- |
| Managed Google Play 私有应用（private app） | 标准 Android Enterprise | 推荐生产路径，可通过 EMM 安装和测试轨灰度。 |
| MDM 直接 APK/AAB 分发 | 私有化/无 Play 环境 | 需要客户 MDM 支持签名包分发。 |
| USB/ADB 安装 | 开发和试点 | 不作为规模生产交付方式。 |

### 托管配置（Managed configuration）

PDA 应用（app）应支持 Android 托管配置（managed configuration），用于远程下发：
...
| 层 | 技术 | 说明 |
| --- | --- | --- |
| UI 状态（UI state） | Vue 组合式函数（composables） | 当前步骤、输入框、局部加载（loading）。 |
| 客户端状态（Client state） | Pinia | 登录摘要、上下文、设备信息、布局偏好。 |
| 在线服务端状态（Server state online） | Pinia Colada | 在线查询、失效、重试、缓存。 |
| 离线状态（Offline state） | SQLite + 本地查询层（local query layer） | 任务快照、草稿、发件箱（outbox）、同步结果。 |
| API 客户端（API client） | `@nerv-iip/api-client` | BusinessGateway 移动 OpenAPI 生成。 |

### Pinia Colada 与 SQLite 边界

PDA 数据流明确拆成在线查询和离线事实源两条路径，组件通过业务 composable 消费统一的 view model，但底层职责不混用：

1. Pinia Colada 只用于在线读取查询（read query），例如引导数据（bootstrap）、任务列表、任务增量拉取、在线条码解释和设备策略刷新。
2. 离线期间组件直接消费 SQLite 本地查询层（local query layer）的投影（projection），不让 Colada 查询兜底（query fallback）读取 SQLite。
3. 发件箱（Outbox）写入完全绕过 Colada 变更（mutate），固定走 `LocalCommand -> SQLite -> Outbox`。
4. 同步运行器（Sync Runner）是唯一把发件箱操作（outbox operation）提交到 BusinessGateway 的组件；同步成功后回写 SQLite，并按需触发 Colada 缓存失效（invalidation）或在线增量刷新。
5. Colada 缓存（cache）可以被清空、失效或重拉，不作为离线事实来源；SQLite schema 和迁移才是 PDA 本地事实源。
6. 页面级组合式函数（composable）负责把在线查询（online query）、本地投影（local projection）、同步状态（sync status）合成 UI 状态，避免页面组件直接同时操作两套数据流。
...
2. 发件箱操作（outbox operation）保存 fileId 或上传会话引用。
...
1. 每个移动 API 请求带 `X-Correlation-Id`。
...
| 环境 | 包类型 | 说明 |
| --- | --- | --- |
| 开发（Dev） | 调试 APK（debug APK） | 开发真机调试。 |
| 测试（Test） | 已签名 APK/AAB（signed APK/AAB） | 测试环境，允许日志增强。 |
| 预生产（Preprod） | 已签名 AAB/APK（signed AAB/APK） | 预生产，接近生产配置。 |
| 生产（Prod） | 已签名 AAB/APK（signed AAB/APK） | 生产分发，日志脱敏，禁用调试。 |

### 版本

1. App 语义版本（semantic version）与构建号（build number）分离。
2. 原生桥接 API 需要独立版本标识。
...
| 层 | 工具 | 目标 |
| --- | --- | --- |
| TypeScript 单元测试（unit） | Vitest | 条码解析、发件箱（outbox）状态机、同步策略。 |
| Vue 组件测试（component） | Vitest + Vue Test Utils | 页面步骤、错误状态、离线提示。 |
| API 契约测试（contract） | OpenAPI 生成测试（generated tests） | DTO 与 BusinessGateway 移动门面（mobile facade）对齐。 |
| Android 单元/仪器测试（unit/instrumented） | Android Studio/Gradle | 原生桥接（native bridge）、intent 解析（parsing）、权限。 |
| E2E 冒烟测试（smoke） | Playwright Web 目标（target） | 非硬件路径和 UI 主流程。 |
| 硬件验收（Hardware acceptance） | 真机脚本和人工用例 | 扫描头、物理按键、弱网、休眠恢复。 |
...
| 普通 Android 手机 | Android 12+ | 摄像头扫码（Camera scanner） | 兜底（fallback）。 |
...
6. 令牌（token）过期、权限撤销、设备解绑。
7. WebView 版本过低告警。
8. MDM 下发配置变更。

## 实施切片

### 切片：架构冻结（Slice 0. Architecture freeze）

1. 冻结 PRD 和架构文档。
2. 确认首批设备型号和扫描模式。
3. 确认 BusinessGateway 移动门面（mobile facade）项目、本地启动入口和反向代理口径。
4. 确认离线动作白名单。

### 切片：技术验证（Slice 1. Technical spike）

1. 扩展 `frontend/apps/business-pda`。
2. 接入 Capacitor Android。
3. 实现 DataWedge intent 或目标设备扫描适配器（adapter）。
4. 实现摄像头兜底（camera fallback）。
5. 实现最小 SQLite/outbox。
6. 打通一次模拟移动操作同步（mock mobile operation）。

### 切片：移动基础（Slice 2. Mobile foundation）

1. 登录、上下文、设备注册、诊断页。
2. 任务中心和离线状态。
3. BusinessGateway 移动 bootstrap/tasks/sync API。
4. api-client 生成和 Pinia/Pinia Colada 接入。
5. 权限执行（enforcement）。

### 切片：WMS MVP（Slice 3. WMS MVP）
...
### 切片：MES/Quality/Maintenance（Slice 4. MES/Quality/Maintenance）
...
1. 选择 Capacitor v8 作为 PDA App 原生运行时（native runtime）。
2. Android PDA 是首批唯一必须达到的原生目标（native target）。
3. 硬件扫描优先通过厂商 intent/SDK 桥接；官方 Barcode Scanner 用作摄像头兜底（fallback）。
4. 离线事实源使用 SQLite/outbox，不把 LocalStorage/IndexedDB 作为业务事实存储。
5. PDA 只访问 BusinessGateway 移动门面（mobile facade）。
6. 设备桥接（bridge）首批限于 App 本地（app-local），出现复用需求后抽取 package。
...
6. 离线执行白名单与授权时长。
7. 是否需要独立的移动设备登记服务（mobile device registry），或先由 IAM/BusinessGateway 承载设备登记。
