...
2. PDA 只访问 PlatformGateway mobile facade，不直连业务服务、数据库、对象存储或消息队列。
...
flowchart LR
  ...
  Gateway["PlatformGateway<br/>Mobile Facade"]
  IAM["IAM"]
  WMS["WMS"]
  INV["Inventory"]
  MES["MES"]
...
5. `frontend/packages/api-client` 继续承载 Gateway OpenAPI 生成；PDA 只消费稳定导出。
...
| key | 说明 |
| --- | --- |
| `gatewayBaseUrl` | PlatformGateway mobile facade 地址。 |
| `organizationId` | 默认组织。 |
| `environmentId` | 默认环境。 |
...
PDA 只访问 PlatformGateway 的 mobile facade：

```text
PDA -> PlatformGateway /api/mobile/v1/*
PlatformGateway -> IAM / WMS / Inventory / MES / Quality / Maintenance / FileStorage
```
...
新增权限码必须先进入 authorization matrix，再进入 IAM seed、Gateway enforcement 和测试。
...
| API client | `@nerv-iip/api-client` | Gateway OpenAPI 生成。 |
...
User action
  ...
  -> Gateway returns normalized result
...
3. Gateway mobile facade 使用 `/api/mobile/v1` 路径版本。
...
| API contract | OpenAPI generated tests | DTO 与 Gateway facade 对齐。 |
...
1. 冻结 PRD 和架构文档。
2. 确认首批设备型号和扫描模式。
3. 确认 mobile facade 是否进入 PlatformGateway。
4. 确认离线动作白名单。
...
1. 登录、上下文、设备注册、诊断页。
2. 任务中心和离线状态。
3. Gateway mobile bootstrap/tasks/sync API。
...
5. PDA 只访问 PlatformGateway mobile facade。
...
6. 离线执行白名单与授权时长。
7. 是否需要 mobile device registry 独立服务，或先由 IAM/PlatformGateway 承载设备登记。
