...
- The `exposed` rows are summarised by count here; the full 354-row registry with
...
The `exposed` rows (281) — each with its verified facade `gatewayOperationIds` — are
