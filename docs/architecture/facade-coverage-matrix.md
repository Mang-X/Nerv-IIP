...
- The `exposed` rows are summarised by count here; the full 344-row registry with
...
The `exposed` rows (277) — each with its verified facade `gatewayOperationIds` — are
