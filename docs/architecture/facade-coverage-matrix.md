...
- The `exposed` rows are summarised by count here; the full 372-row registry with
...
The `exposed` rows (300) — each with its verified facade `gatewayOperationIds` — are
