...
- The `exposed` rows are summarised by count here; the full 353-row registry with
...
The `exposed` rows (280) — each with its verified facade `gatewayOperationIds` — are
