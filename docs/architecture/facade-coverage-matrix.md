...
service endpoint _and_ a Gateway facade (OpenAPI snapshot → api-client codegen →
...
- **`deferred`** — the service endpoint exists but no Gateway facade is delivered
  yet. This is a _tracked_ gap that follows a frontend menu phase or a named
...
- The `exposed` rows are summarised by count here; the full 409-row registry with
...
The `exposed` rows (338) — each with its verified facade `gatewayOperationIds` — are
