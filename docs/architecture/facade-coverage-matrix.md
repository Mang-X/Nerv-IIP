...
- The `exposed` rows are summarised by count here; the full 343-row registry with
...
The `exposed` rows (275) — each with its verified facade `gatewayOperationIds` — are
