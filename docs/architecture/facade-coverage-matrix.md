...
- The `exposed` rows are summarised by count here; the full 386-row registry with
...
The `exposed` rows (315) — each with its verified facade `gatewayOperationIds` — are
