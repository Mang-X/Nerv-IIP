...
8. P2 企业身份入口仅在 Development 环境保留配置驱动的外部 OIDC callback stub 和已有 IAM User/Membership 映射，不把 IAM 升级为完整 OAuth2/OIDC 授权码服务器；非 Development 环境不得使用请求体明文身份、callback secret 或静态 MFA code 换发用户会话。后续若需要标准授权服务器、生产 IdP 联邦登录、consent 页面或第三方应用市场，再评估 OpenIddict 或等价方案。
...
新 token 必须继承同一 `tokenFamilyId` 并记录 `previousSessionId`，用于表达 refresh lineage。已被 rotation 消费的旧 refresh token 再次提交时，IAM 必须识别为 replay/reuse，记录安全告警日志，并把同一 token family 中尚未撤销的 session 级联撤销，防止被窃旧 token 与合法新 token 并行存活。
...
Access token 采用 JWT Bearer，并使用 RS256 非对称签名。IAM 只在签发侧持有 private key；Gateway、业务服务和其它下游验签方只能使用 IAM JWKS 中的 public key，通过 `kid` 选择匹配密钥。生产环境必须显式配置 `Iam:Jwt:SigningKeys`（含 `Kid` 与 `PrivateKeyPem`），Gateway 必须配置 JWKS 公钥 JSON 或等价公开 key 来源；不得把 IAM 签发私钥扩散到 Gateway 或业务服务。JWT 中只放稳定且短期可接受的信息：
...
IAM 暴露 `GET /api/iam/v1/auth/jwks` 返回当前可验签的 public JWK set。密钥轮换顺序为：先把新 public key 加入 Gateway/下游 `Iam:Jwt:JwksJson` 并完成发布，再把 IAM signing key 切为 active 开始签发新 token；旧 public key 必须在 JWKS/validation set 中保留到其签发的 access token 全部过期后再移除。下游只按 JWKS 与 `kid` 验证，不配置 HS256 共享密钥。
...
1. MFA、短信码、邮箱码或 WebAuthn 是登录因子，不是 session 认证码。P2 的 MFA challenge hook 只在 Development OIDC callback stub 后换发会话前使用，并把验证时间写入 UserSession；非 Development 环境在接入 per-user key、限速和生产级一次性校验前禁用该 hook。
...
4. 首批不实现 WebAuthn 或完整企业 IdP 联邦登录；P2 只提供 Development-only OIDC callback stub、SSO session binding 和 Development-only MFA challenge hook。
