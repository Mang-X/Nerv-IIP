...
8. 后续若需要标准 OAuth2/OIDC、consent 页面、第三方应用市场或联合身份，再评估 OpenIddict 或等价方案，不在第一阶段硬上。
...
1. MFA、短信码、邮箱码或 WebAuthn 是登录因子，不是 session 认证码。
...
3. 首批不实现复杂 ABAC 规则引擎。
4. 首批不实现 SSO、MFA、WebAuthn 或企业 IdP 联邦登录。
