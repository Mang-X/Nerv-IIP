...
- 该 manifest 的根 `project` 是 BusinessGateway restore manifest 的唯一 owner/root：必须恰好出现一次、必须是字符串，并精确指向 `backend/gateway/BusinessGateway/src/Nerv.IIP.BusinessGateway.Web/Nerv.IIP.BusinessGateway.Web.csproj`；根 `toolchain.sdk` 是受管 CI .NET SDK 版本的唯一 owner，`.github/workflows/ci.yml` 只是消费者，不另立版本常量。`scripts/verify-ci-dotnet-sdk-authority.ps1` 从 manifest 单向读取精确三段版本，并在普通 YAML 解析折叠 mapping key 之前通过 Psych AST 要求每个受管 `actions/setup-dotnet` step 恰好包含一个 `dotnet-version` key，再结构化核对全部受管 step，并把当前进程真实 `dotnet --version` 与同一值比较；manifest owner/root 或 SDK 缺失、重复、类型/路径错误，workflow key 缺失、重复、浮动、任一侧漂移及真实进程版本不一致均失败关闭。mutation 同时证明：manifest 与 workflow 同步升级时测试期望由变更后的 manifest 驱动，旧期望会失败；重复 YAML key 即使折叠后的末值正确也不能通过。此门禁只证明受管 CI 配置和执行该门禁的进程工具链一致，hosted job 的最终运行身份仍须由各 job 日志与 MAN-661 evidence authority 独立证明。
