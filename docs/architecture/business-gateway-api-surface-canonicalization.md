...
- 该 manifest 的根 `project` 是 BusinessGateway restore manifest 的唯一 owner/root：必须恰好出现一次、必须是字符串，并精确指向 `backend/gateway/BusinessGateway/src/Nerv.IIP.BusinessGateway.Web/Nerv.IIP.BusinessGateway.Web.csproj`；根 `toolchain.sdk` 是受管 CI .NET SDK 版本的唯一 owner，`.github/workflows/ci.yml` 只是消费者，不另立版本常量。`scripts/verify-ci-dotnet-sdk-authority.ps1` 的生产合同只有一个结果入口：调用方只提供 workflow 路径、manifest 路径和当前进程真实 SDK，不能注入已解析 workflow、预计算 finding、替代 SDK 或选择旁路模式；CLI 也消费这一结果对象输出 findings 与 exit code。入口从 manifest 单向读取精确三段版本，在普通 YAML 解析折叠 mapping key 之前通过 Psych AST 要求每个受管 `actions/setup-dotnet` step 恰好包含一个 `dotnet-version` key，再结构化核对全部受管 step，并把真实 `dotnet --version` 与同一值比较。解析证据只能按 resolved source path 与内容 SHA-256 的组合 identity 缓存；同一路径内容改变必须重新解析，解析期间内容改变必须失败。manifest owner/root 或 SDK 缺失、重复、类型/路径错误，workflow key 缺失、重复、浮动、任一侧漂移及真实进程版本不一致均失败关闭。独立 oracle 与 mutation 必须精确证明：manifest 与 workflow 同步升级至不同版本时真实生产入口零 finding，任何额外硬编码旧版本 finding 都被杀死；同一路径 workflow 漂移不能复用旧证据；重复 YAML key 即使折叠后的末值正确也不能通过。此门禁只证明受管 CI 配置和执行该门禁的进程工具链一致，hosted job 的最终运行身份仍须由各 job 日志与 MAN-661 evidence authority 独立证明。
