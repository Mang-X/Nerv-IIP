...
ERP health 可以早于启动后 seed transaction 的提交而变为可用；因此 acceptance 在任何 mutation 前，必须等待 ERP 已提交的 `SO-DEMO-001` header（version `1`）及 line `10`。每个 mutation 只执行一次，并同时满足 HTTP 和 `ResponseData` 业务契约；HTTP 200 但业务 envelope 失败、缺失或 data 非预期都会立即失败，绝不盲目重试。source readiness timeout、POST business rejection、Redis state 与 DemandPlanning convergence 保持为不同诊断阶段。后续应以受跟踪的设计使 application readiness 代表必需 startup seed/UoW work 已完成；#1140 有意不改变服务 lifecycle 或 seed semantics。
