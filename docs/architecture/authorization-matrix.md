...
| `business.maintenance.work-orders.read` | `user` / `external-client` / `internal-service` | environment + resource | 查看维修工单、故障、停机、维修结果与生命周期审计；MAN-631 显式 self/team（本人/班组）队列每次实时校验主体作业范围，#207 Gateway equipment facade（设备门面）仍未额外强制该权限。 |
