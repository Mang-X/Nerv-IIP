...
后续 Notification、Knowledge、AI Integration 与 Observability 基线文档创建后，应反向链接本文档，并在对应服务实现时标明权限初始数据与强制校验状态。
...
2. permission code：稳定的权限码。
3. organization/environment scope：组织与环境范围。
4. resource/capability scope：具体资源或能力范围。

端点只声明需要的权限码与上下文；业务不变式仍由领域守卫或应用服务校验。
...
| `connector-host` | Connector Host 机器身份。 | AppHub 注册/心跳/状态同步，Ops 任务领取与结果回传。 | IAM ConnectorHostCredential、organization/environment、capability 范围。 |
| `external-client` | 外部系统、平台应用或受控第三方客户端。 | Platform SDK、公开 API、Webhook 回调、后续 OAuth/OIDC 客户端凭据。 | IAM ExternalClient、AuthorizationGrant、organization/environment、resource/capability 范围。 |
| `internal-service` | 平台内部服务到服务调用主体。 | Gateway 到 IAM 授权检查，服务间后台任务或事件处理后的回查。 | 平台部署身份、服务账号或后续工作负载身份；不得绕过 IAM 边界事实。 |

当前 access token（访问令牌）基线已覆盖 `user`、`external-client`、`connector-host` 的 token claim（令牌声明）形态；`internal-service` 已作为服务间认证入口落地在 Ops、FileStorage、Notification 的非健康检查内部 API 上。服务间调用必须携带 `Authorization: Bearer <InternalService:BearerToken>`，Development 环境使用本地默认令牌，非 Development 环境必须显式配置 `InternalService:BearerToken`。该 Bearer 令牌只证明调用方属于平台内部服务，不替代最终用户、Connector Host 或外部客户端的 IAM 授权事实。
...
| resource | 单个或一组资源边界。 | 应用实例、运维任务、文件、知识源、通知模板。 | 权限码只表达动作类别；资源所有权、可见性和状态需单独校验。 |
| capability | 能力级边界。 | Connector Host 上报能力、AI 工具执行、外部客户端可调用能力。 | 必须和权限码同时匹配，不能只靠能力声明授权执行。 |

## 主体与范围矩阵

| principalType | organization | environment | resource | capability | 说明 |
| --- | --- | --- | --- | --- | --- |
| `user` | 必须 | 常规必须 | 按入口需要 | 按入口需要 | 控制台和管理后台的默认主体；角色权限只在组织/环境成员关系内生效。 |
...
| --- | --- | --- | --- | --- | --- |
| `iam.users.read` | IAM | `user` | organization | 已写入初始数据；IAM 管理端点已检查；Gateway 控制台门面已强制校验 | 查看用户。 |
| `iam.users.manage` | IAM | `user` | organization | 已写入初始数据；IAM 管理端点已检查；Gateway 控制台门面已强制校验 | 创建、编辑、禁用、重置用户。 |
| `iam.roles.read` | IAM | `user` | organization | 已写入初始数据；IAM 管理端点已检查；Gateway 控制台门面已强制校验 | 查看角色与权限。 |
| `iam.roles.manage` | IAM | `user` | organization | 已写入初始数据；IAM 管理端点已检查；Gateway 控制台门面已强制校验 | 创建角色、调整角色权限。 |
| `iam.sessions.read` | IAM | `user` | organization | 已写入初始数据；IAM 管理端点已检查；Gateway 控制台门面已强制校验 | 查看会话。 |
| `iam.sessions.revoke` | IAM | `user` | organization | 已写入初始数据；IAM 管理端点已检查；Gateway 控制台门面已强制校验 | 撤销会话。 |
| `connectors.registrations.write` | Connectors/AppHub | `connector-host` | environment + capability | 已写入初始数据 | Connector Host 注册或更新应用实例事实。 |
| `connectors.heartbeats.write` | Connectors/AppHub | `connector-host` | environment + resource + capability | 已写入初始数据 | Connector Host 上报心跳。 |
| `connectors.state-snapshots.write` | Connectors/AppHub | `connector-host` | environment + resource + capability | 已写入初始数据 | Connector Host 上报实例状态快照。 |
| `apphub.instances.read` | AppHub | `user` / `external-client` / `internal-service` | environment + resource | 已写入初始数据；Gateway 已强制校验 | 查看应用实例列表与详情。 |
| `files.upload` | File Storage | `user` / `connector-host` / `external-client` | environment + resource | 已写入初始数据 | 创建上传会话并完成文件上传。 |
| `files.read` | File Storage | `user` / `connector-host` / `external-client` / `internal-service` | environment + resource | 已写入初始数据 | 查看文件元数据。 |
| `files.download-grants.create` | File Storage | `user` / `external-client` / `internal-service` | environment + resource | 已写入初始数据 | 创建短期下载授权。 |
| `files.archive` | File Storage | `user` / `internal-service` | environment + resource | 已写入初始数据 | 归档文件。 |
| `ops.tasks.create` | Ops | `user` / `external-client` / `internal-service` | environment + resource + capability | 已写入初始数据；Gateway 已强制校验；ExternalClient 的 client_credentials 授予已可校验 | 创建运维任务。 |
| `ops.tasks.read` | Ops | `user` / `connector-host` / `external-client` / `internal-service` | environment + resource | 已写入初始数据；Gateway 已强制校验 | 查看运维任务。 |
| `ops.results.write` | Ops | `connector-host` | environment + resource + capability | 已写入初始数据 | Connector Host 回传动作结果。 |
| `ops.audit.read` | Ops | `user` / `internal-service` | organization / environment + resource | 已写入初始数据 | 查看审计记录。 |
| `observability.logs.read` | Observability | `user` / `internal-service` | organization / environment + resource | 已写入初始数据；Gateway 控制台门面已强制校验 | 查询集中平台日志；Gateway 不暴露 VictoriaLogs URL 或 LogsQL。 |

## 控制台 IAM 管理门面权限映射

PlatformGateway 的控制台 IAM 管理门面在转发 IAM 管理请求前，会用当前 bearer 令牌调用 IAM current-principal/authorization 路径，校验组织、环境和权限码。浏览器只访问 Gateway，不直连 IAM。
...
PlatformGateway 的控制台可观测性门面在查询 VictoriaLogs 前，会用当前 bearer 令牌调用 IAM current-principal/authorization 路径，校验组织、环境和权限码。浏览器只访问 Gateway，不直连 VictoriaLogs、Collector 或 Aspire Dashboard。
...
| `ai.tools.register` | `user` / `external-client` / `internal-service` | organization / environment + capability | 注册 MCP 服务器（MCP Server）、技能（Skill）或平台工具。 |
...
| `observability.retention.manage` | `user` / `internal-service` | organization / environment | 管理日志保留期、清理任务和归档策略。 |
...
业务平台权限码用于 ADR 0012 与 ADR 0014 定义的关键链路领域扩展。BusinessMasterData、BusinessProductEngineering、BusinessInventory、BusinessQuality、BusinessMES、BusinessDemandPlanning、BarcodeLabel、BusinessApproval、WMS、BusinessIndustrialTelemetry、BusinessMaintenance 和 BusinessScheduling / APS lite 权限已随对应 MVP、#206 或 #207 进入 IAM 初始数据或服务授权基线。BusinessScheduling 当前已完成服务端端点权限元数据、IAM 初始数据和 BusinessGateway 门面的最终用户权限强制校验。#207 BusinessGateway 设备运行事实门面当前对总览、设备和可用性接口强制 `business.iiot.telemetry.read`，对报警接口强制 `business.iiot.alarms.read`；`business.maintenance.work-orders.read` 与 `business.maintenance.plans.read` 属于 Maintenance 下游领域目录和相关只读权限，当前设备门面合并维护窗口时通过内部服务令牌调用 Maintenance，并未在 Gateway 端点上额外强制这两个 Maintenance 权限。#2696 BusinessGateway MES 停机读面（`GET /api/business-console/v1/mes/downtime-events`）同样按此逐案口径：它以内部服务令牌取 Maintenance 停机原因目录，把**整份目录**随响应作为 `reasonCatalog` 回给读面（用于原因取名与按原因筛选），端点仍只强制 `business.mes.downtime.read`，不额外强制 `business.maintenance.work-orders.read`。该目录是受控词表（原因码 + 名称），不含维修工单、故障或停机执行事实。其他业务域权限在实现对应服务时，也必须按本表进入 IAM 初始数据、端点鉴权、OpenAPI 测试和权限测试。
...
| `business.scheduling.plans.read` | `user` / `external-client` / `internal-service` | environment + resource | 查看 APS lite 排程方案、资源负载、冲突、不可排原因和 Gantt DTO；当前 IAM 初始数据、服务端端点权限元数据与 BusinessGateway 门面强制校验已实现。 |
| `business.scheduling.plans.manage` | `user` / `internal-service` | environment + resource | 预览和生成 APS lite 排程方案；当前 IAM 初始数据、服务端端点权限元数据与 BusinessGateway 门面强制校验已实现。 |
| `business.scheduling.plans.release` | `user` / `internal-service` | environment + resource | 发布已生成的 APS lite 排程方案供 MES 等下游消费；当前 IAM 初始数据、服务端端点权限元数据与 BusinessGateway 门面强制校验已实现。 |
...
| `business.iiot.tags.manage` | `user` / `external-client` | environment + resource | 管理设备 tag（标签）映射、采集点和单位。 |
| `business.iiot.alarm-rules.manage` | `user` / `external-client` | environment + resource | 管理设备报警规则阈值和启停状态；BusinessGateway telemetry alarm-rules（遥测报警规则）写面强制该权限。 |
| `business.iiot.telemetry.read` | `user` / `external-client` / `internal-service` | environment + resource | 查看设备状态快照、时序摘要和 OEE 输入数据；#207 BusinessGateway equipment overview/device/availability facade（设备总览/详情/可用性门面）已强制该权限。 |
| `business.iiot.telemetry.write` | `user` / `connector-host` / `external-client` | environment + resource + capability | 写入受控采集样本或状态摘要；#472 BusinessGateway manual seed facade（手工样本注入门面）强制该权限；不得表达 PLC/DCS 控制授权。 |
| `business.iiot.alarms.read` | `user` / `external-client` / `internal-service` | environment + resource | 查看工业报警、状态和处理进度；#207 BusinessGateway equipment alarms facade（设备报警门面）已强制该权限。 |
| `business.iiot.alarms.write` | `user` / `connector-host` / `external-client` | environment + resource + capability | 写入报警产生、清除和状态变化事实；#472 BusinessGateway manual alarm facade（手工报警门面）强制该权限。 |
| `business.iiot.device-control.read` | `user` / `external-client` / `internal-service` | environment + resource | 查看设备控制命令下发结果、历史与控制通道绑定；MAN-438/#792 BusinessGateway device-control command get/list 与 binding list facade（设备控制命令查询/列表与绑定列表门面）强制该权限。 |
| `business.iiot.device-control.write` | `user` / `external-client` | environment + resource + capability | 下发审批门禁的设备控制命令（写值/启停/参数下发）；MAN-438/#792 BusinessGateway device-control command create facade（设备控制命令创建门面）强制该权限；不表达直连 PLC/DCS/SCADA 控制授权。 |
| `business.iiot.device-control.manage` | `user` / `external-client` | environment + resource | 维护设备控制通道绑定（设备 → 连接器主机/实例路由目标）；MAN-438/#792 BusinessGateway device-control binding create-or-update/disable facade（设备控制绑定创建或更新/停用门面）强制该权限，与命令下发 `.write` 分离以防止有下发权限的角色篡改路由。 |
| `business.maintenance.work-orders.read` | `user` / `external-client` / `internal-service` | environment + resource | 查看维修工单、故障、停机、维修结果与生命周期审计；MAN-631 显式 self/team（本人/班组）队列每次实时校验主体作业范围，#207 Gateway equipment facade（设备门面）与 #2696 Gateway MES downtime read facade（停机读面）仍未额外强制该权限。 |
| `business.maintenance.work-orders.manage` | `user` / `external-client` / `internal-service` | environment + resource | 创建、分派、接单、开始、暂停/待料、恢复、完成、验证、关闭或取消维修工单；MAN-631 Gateway 写面从认证结果注入 actor（操作人）并实时校验 self/team（本人/班组）范围。 |
| `business.maintenance.plans.read` | `user` / `external-client` / `internal-service` | environment + resource | 查看保养计划、点检计划和执行记录；#207 availability（可用性）合并维护窗口时由下游 Maintenance/domain catalog（维护领域目录）使用，当前 Gateway equipment facade（设备门面）未额外强制该权限。 |
| `business.maintenance.plans.manage` | `user` / `external-client` | environment + resource | 创建、调整和关闭保养计划、点检计划。 |

## 落地要求

1. 新增权限码必须先更新本文档，再进入 IAM 初始数据、迁移、端点授权检查和测试。
2. 对外 API、Gateway 门面、SDK 和服务间调用不得各自发明未登记权限码。
...
5. Notification、Knowledge、AI Integration、Observability 和 Mobile Platform 首次实现时，应在对应服务文档中链接本文档，并标明实际初始数据状态。
