...
- **图例与图面一致**(#1274 铁律):「缺料待备」既然出现在甘特卡片与 tooltip 上,
  `deriveLegendSemantics` 就必须同步推导出 `status.materialRisk`,图例随之出现;方案全齐套时图例里不出现该项。
...
2. ~~**资源产能日历可视化**~~:已由 MAN-693 / #1261 关掉——`SchedulePlanContract` 增补
   ...
   遗留:块的泳道粒度是**工作中心**(资源板默认按工作中心铺泳道),单台设备维护会落在该工作中心那一行;
