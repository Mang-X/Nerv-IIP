...
- 报警自动建单按组织/环境的本地配置选择“只建单”或“建单并占用”，仅需区别报警时使用精确 AlarmCode；未匹配配置默认只建普通工单，不登记设备不可用。占用必须使用同范围当前停机原因目录的精确码，目录失效会使消费失败，修正后按既有消息治理重试；不回退普通建单。诊断与来源保持，重复报警不重复建单。配置修改后重启 Maintenance 生效，键名及样例见[运行配置](../../runbooks/maintenance-alarm-policy.md)。
