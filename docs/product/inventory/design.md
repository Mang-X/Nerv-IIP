...
**库位 / 批次 / 序列号**选择器走网关可搜目录 `GET /api/business-console/v1/directories/{location|batch|serial}`
（#3832）：输入关键字由服务端在全部数据里查找，列表滚到底再取下一页，候选上百条时只渲染可视区附近的行。
库位目录列启用状态的库位主数据；批次 / 序列号目录列有在库数量的台账编码。
可见范围是用户授权工厂的并集（组织级授权不收窄；self 等非工厂范围不给出工厂，也不让工厂授权失效）；
一个工厂都没授权时选择器显示「当前角色无权查看库位」。
表单里的库位选择器可就地「新增库位」（与「库存管理 ▸ 库位」维护页同一个表单），筛选区不带新增。
收货批次可能是新批次号，入库单行的批次仍可自由录入，目录只给建议。

> **后端补齐 §4 的读面后，`useInventorySiteStockOverview` 应当整体删除**，页面直接换成真实读面。
> 它是补偿层，不是长期架构。
...
**当前实现**：`GET /api/inventory/v1/locations` 与 `POST` upsert 均由网关代理
（`listBusinessConsoleInventoryLocations` / `createOrUpdateBusinessConsoleInventoryLocation`），
业务前端「库存管理 ▸ 库位」维护页列出全部库位（含停用），可新建、编辑类型/工厂/上级库位/状态。
开箱数据由 Inventory 产品基线种子补齐 `loc-raw-01 / loc-semi-01 / loc-fg-01 / loc-line-01`
（`loc-line-01` 为线边库位）。

**当前限制**：读面只支持 `keyword`（按库位编码）与分页；下方草案里的 `displayName`、仓库/库区、
容量等字段库存侧没有存储。各页库位选择器走可搜目录的 `location` 类型（同一张库位表，只列启用库位）。

下方为该读面的原始草案，字段口径以上文「当前实现」为准：
...
**当前实现**：选择器用的批次 / 序列号目录已由网关可搜目录 `directories/{batch|serial}` 提供
（关键字、分页、按授权工厂并集收窄，只列有在库数量的台账编码；见 §3）。下方独立读面草案里的
生产日期、有效期、质量状态、可用量等字段仍未提供。
