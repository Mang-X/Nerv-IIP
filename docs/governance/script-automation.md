...
| 工作流里 `if: false` 的 step 点名、或在 PR 上从不触发的工作流（如 nightly）里点名 | 判定「`if:` 在本次事件下是否为真」「哪个工作流会在 PR 上触发」会把选取器变成 GitHub 表达式求值器与事件模型，是本仓已判定永不收敛的那条路（#3176 / PR #3214）。与已关掉的「run 体内 shell 注释」相比，这两种写法在 diff 里一眼就是错的、**不可抵赖** |
| 改 runner 自己的 CI step、或在 runner 里插 `exit 0` | 自指缴械面：任何护栏都能被改护栏本身缴械 |
| 伪造父测试并在其源码加一行引用 | 要挡住就得证明父测试真的执行了子测试，成本远超收益 |

**run 体内的 shell 注释是关掉了的**：读 `run` 时先滤掉注释行。它是上面这些缺口里唯一「一行、且看起来像无辜注释」的那条，可抵赖性最高，所以值得关。
