...
**覆盖边界（声明多少就只断言多少）**：门禁校验的是种类闭集、字段必填/禁填矩阵、目标存在性、父子引用真实性、票号**形态**与 Requirement 的**出处**。它**不**校验「该 issue 存在或仍开着」（需联网，且该 job 无 token），也**不**校验「豁免理由为真」——一条 `excluded` 若引用目标文件确实声明过、但与真实红因无关的依赖，只能靠人工复审拦下。不要把这两条读成已被机器守住。
