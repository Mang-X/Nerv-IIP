...
- ⚠️ 这条留存**不是零留存**：字母表内的 token 是按原文留下的。因此字母表的准入范围本身就是隐私边界，必须可枚举、可复核。当前准入：小位数计数、`hh:mm:ss` 时长、异常类型名与 `Assert.*` 成员名、闭合英文诊断词表、`键=小位数值`。当前**明示拒绝**：GUID 等主体标识、超过位数上界的数字串（手机号/证件号/卡号/单号/金额）、非类型形状的点分串（人名、主机名）、标识符内的长数字串、引号内的数字串。
- 裸数字既是诊断读数也是业务标识，是同一个 token 类，无法靠模式区分，只能靠量级上界区分。因此「读数需要 5 位以上量级」这一类诊断能力是被**主动放弃**的，不是遗漏。放弃项必须写下来，不能靠实现沉默地表达。
- 异常类型名按原文留存，判据**不依赖仓库可见性**：类型名是**编译期源码标识符**，不可能承载运行期数据，因此它的暴露面**恒等于源码本身的暴露面** —— 无论仓库公开还是私有，留存它都不增加任何新的暴露。（凡是「因为现在是公开仓库所以无所谓」这类依赖某个设置的理由，都会在设置改变时失效，而那时没人会回来重新论证，所以不采用。）
- ⚠️ 上一条**只对「除了标识符什么都不是」的标识符成立**。唯一的例外是敏感串被拼进标识符自身的拼写（如把证件号写进异常类型名），这一类由「标识符内不得出现 5 位以上连续数字」这条守卫排除。**这两条是配合关系，缺一不闭合**：删掉数字守卫会直接证伪类型名留存所依赖的前提，并重新打开这条走私通道。修改任一条必须同时复核另一条。
