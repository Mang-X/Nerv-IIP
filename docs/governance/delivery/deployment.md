...
4. 服务间地址继续使用服务当前声明的配置键；不得在 PoC、Compose、安装包或 Production profile 静默 fallback 到 localhost。AppHost 侧由 `scripts/verify-apphost-base-url-injection.ps1` 强制：消费方代码要的每个下游基址都必须由 AppHost 注入，且必须指向该下游资源本会话的端点——只校验「注没注」挡不住「注了但指向另一套栈」（#3313）。需求集的推导方式、判据、豁免登记表的位置与登记纪律以该脚本及其 library 为准，本文不复述。
