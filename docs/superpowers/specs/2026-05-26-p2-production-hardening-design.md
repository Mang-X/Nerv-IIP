...
2026-05-26 收口：Ops 已在 `operation_tasks` 上落地轻量 approval gate 字段。`operation_templates.RequiresApproval=true` 的任务创建后进入 `approval-pending`，通过 `/api/ops/v1/operation-tasks/{operationTaskId}/approval/approve|reject` 决策；通过后回到 `queued` 并允许 Connector Host claim，拒绝后进入 `rejected`。Ops 发布 `OperationApprovalRequested`、`OperationApprovalApproved`、`OperationApprovalRejected` 和既有 `OperationTaskCompleted`/`OperationTaskFailed` 事件，Notification 真实消费者把审批待办、审批结果和执行结果转成 Notification intent。
