...
4. Count tasks freeze the target ledger while open and capture `ExpectedLedgerVersion`. Confirmation checks the current ledger version before calculating variance. Version drift requires recount instead of posting a stale variance.
5. Quality inspection result events are consumed through `Nerv.IIP.Contracts.Quality`. Passed inspections transfer quantity from `quality` to `unrestricted`; rejected inspections transfer from `quality` to `blocked`. Because the current Quality event payload does not carry location/UOM/lot/owner, this integration only auto-releases when Inventory can resolve a single matching `quality` ledger for the SKU and quantity. Ambiguous releases fail visibly instead of guessing.
...
- open count tasks freeze movements and stale ledger versions require recount;
