...
| -------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -----------: | ----------------------------------------------------------------------------------------- |
| Threshold proximity  | For each enabled alarm rule, use the newest raw `LastValue` only while it is at most ten minutes old. A high/low threshold is near or breached when the signed safe-side distance is at most 20% of `max(abs(threshold), 1)`. Select the closest/highest-severity result. |           15 | `no-current-sample` when no enabled rule has a current sample inside the realtime window. |
...
| Sustained exceedance | Per enabled rule, evaluate raw samples from the trailing 24 hours with the rule's exact `>`, `>=`, `<`, or `<=` comparison semantics. At least six samples spanning at least 30 minutes are required. Trigger when at least 80% breach the rule threshold.                |           20 | `history-accumulating` until the sample-count/span requirement is met.                    |
...
- Freshness uses the newest consumed raw-sample, device-state, or alarm lifecycle timestamp. It is `fresh` through two minutes, `delayed` through ten minutes, `stale` afterward, and `unavailable` when no source fact exists. A stale raw sample remains freshness evidence but no longer acts as the current value for threshold-proximity scoring.
