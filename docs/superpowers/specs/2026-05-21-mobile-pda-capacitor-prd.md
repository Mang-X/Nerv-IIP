...
4. 复用现有 Gateway facade 模式、IAM、OpenAPI codegen、业务权限矩阵和平台观测能力；业务移动聚合入口落在 BusinessGateway。
...
| ID           | 需求                                                                       | 优先级 | 验收                                                                    |
| ------------ | -------------------------------------------------------------------------- | ------ | ----------------------------------------------------------------------- |
| PDA-AUTH-001 | App 必须通过 BusinessGateway 登录，不直连 IAM。                            | P0     | 抓包确认所有浏览器请求只访问 BusinessGateway 公开域名或其反向代理域名。 |
...
| ID           | 需求                                                         | 优先级 | 验收                                                                                                                                                                                  |
| ------------ | ------------------------------------------------------------ | ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| PDA-TASK-001 | 任务中心展示本人可执行任务、最近任务和异常任务。             | P0     | 首页“我的任务”仅在服务端按当前主体派工过滤时使用个人语义；WMS、质检、维修和 MES 范围列表必须显示组织/业务环境范围、来源、计数、最近成功响应时间和诚实空态，不把范围待办伪称个人任务。 |
...
| PDA-WMS-007 | 五类作业按当前主体的服务端授权范围查询，并区分本人、作业池与站点作业队列。 | P0     | 客户端选择不能扩大服务端授权；越权范围必须 fail closed。 |
| PDA-WMS-008 | 上架与拣货按开始、累计进度、异常、完成四段执行。        | P0     | 终态不可操作；短拣须说明差异；版本冲突须刷新后重试。 |

MAN-629 首版只交付在线业务闭环并保留离线扩展边界：五类队列均提供范围、状态、关键字及适用的站点、库位或批次筛选，上架/拣货支持下拉刷新与触底加载；任务版本与持久幂等回执由服务端维护。PDA 不缓存可执行性，也不从角色名称、库位前缀或页面默认值推断派工范围。PDA-WMS-005 的离线草稿/同步仍属后续实现，不作为本切片已交付能力。
...
具体 OpenAPI 以后续业务服务实现为准，PDA 需要 BusinessGateway mobile facade 提供以下聚合能力：
...
BusinessGateway facade 只做认证、授权、聚合、DTO 映射和错误归一化；领域不变式仍由 WMS、Inventory、MES、Quality、Maintenance 等服务判断。BusinessGateway 使用 `/api/mobile/v1/**` 路由组，OpenAPI 快照进入 `frontend/packages/api-client/openapi/business-gateway-mobile.v1.json`，生成到 `@nerv-iip/api-client` 的 mobile 稳定导出；operationId 使用 `getMobileBootstrap`、`listMobileTasks`、`getMobileSyncDelta`、`batchMobileOperations`、`interpretMobileScan`、`registerMobileDevice`、`uploadMobileDiagnostics`。
...
5. BusinessGateway mobile facade 的 OpenAPI 进入 api-client 生成链路。
...
4. BusinessGateway 首批工程创建、本地端口、反向代理和 OpenAPI 导出脚本的具体实施顺序。
