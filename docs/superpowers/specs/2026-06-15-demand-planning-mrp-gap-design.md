...
The calculator normalizes inputs into daily buckets. For each bucket and SKU/UOM/site, gross demand is aggregated before netting. Net requirement is:

`gross demand + safety stock - available - scheduled receipts due on or before the required date`
