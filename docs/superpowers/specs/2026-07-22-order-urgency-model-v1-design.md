...
4. `Slack` 小于一个班次为 `highRisk`；
5. `CR <= 1.2` 或非阻塞风险为 `attention`；其余为 `normal`。
...
- 计划创建和业务优先级变化时在同一命令 UoW 内捕获新快照；问题变化/计划失效先持久化权威失效事实，再由 15 分钟 worker 生成 fail-closed stale 快照；
- 后台扫描器周期重算未完成主题，使时间自然流逝能够升级/降级；
- list/detail GET 只读取数据库端 latest-per-order，不触发写入；后台刷新是独立显式持久化边界；
...
- 所有快照 append-only 保留为审计证据；删除/归档策略须由后续合规保留期决定，V1 不擅自清除证据。
