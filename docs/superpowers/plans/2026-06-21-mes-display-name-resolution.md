# MES 列表显示名前端解析（前端阶段 1 · F2）实施计划

> **供代理执行者使用：**必须使用子技能 superpowers:subagent-driven-development，逐项实施本计划。步骤使用复选框（`- [ ]`）语法跟踪。

**目标：**MES 列表页把工作中心码（`WC-ASSY`）、SKU/物料码（`SKU-…`）解析成主数据真名显示，用前端 master-data 兜底 facade 当前为 null 的 `*Name`（#461 残留）。

**架构：**新建 `useMesDisplayNames` composable，从 `useBusinessSkus` + `useBusinessMasterDataResources('work-center')` 建 code→displayName 映射，提供 `resolveSku/resolveWorkCenter`。各 MES 页列 accessor 改为 `r.<facadeName> ?? resolve(r.<code> ?? r.<id>) ?? '无'`——**与后端 #461 前向兼容**：后端一旦回填 `*Name`，accessor 自动优先用之，本兜底极少命中。

**技术栈：**Vue 3 `<script setup>` + TS、`@pinia/colada`（查询缓存，重复加载不重复请求）、`@nerv-iip/ui` DataTable accessor。
...
## Task 1：构建 useMesDisplayNames composable

**文件：**
- 创建：`frontend/apps/business-console/src/composables/mes/useMesDisplayNames.ts`
...
- [ ] **步骤 2：对 composable 执行 typecheck**

运行：`cd frontend/apps/business-console && pnpm typecheck 2>&1 | tail -8`
预期：0 错（确认 `useBusinessSkus().skus` / `useBusinessMasterDataResources('work-center').resources` 字段名正确）。
...
- [ ] **步骤 1：operation-tasks.vue** — 工作中心列 accessor 改为：
  `accessor: (r) => r.workCenterName ?? resolveWorkCenter(r.workCenterCode ?? r.workCenterId) ?? '无'`
- [ ] **步骤 2：wip.vue** — 同上工作中心列 accessor。
- [ ] **步骤 3：work-orders/index.vue** — SKU 副行 `r.skuCode ?? r.skuId` 改为 `resolveSku(r.skuCode ?? r.skuId) ?? '无'`；工序行若显示工作中心，接入 `resolveWorkCenter`。
- [ ] **步骤 4：receipts.vue** — 成品列 accessor 接入 `resolveSku(r.skuCode ?? r.skuId)`。
- [ ] **步骤 5：dispatch.vue** — 工作中心列 accessor 接入 `resolveWorkCenter`。
- [ ] **步骤 6：materials.vue** — 物料列使用 `resolveSku(r.materialCode ?? r.materialId)`（物料码即 SKU 码；解析不到时自动回退原码，不报错）。
- [ ] **步骤 7：统一门禁**：`pnpm typecheck 2>&1 | tail -8`（0 错）+ `pnpm build 2>&1 | tail -5`（成功）。
- [ ] **步骤 8：提交**
...
- [ ] **步骤 1：E2E** — `pnpm exec playwright test --project=desktop`（全部通过；mock 无 master-data 时解析回退原值，断言不变）。
- [ ] **步骤 2：实机走查** — 确保栈在运行（`curl 127.0.0.1:5125` = 200，否则运行 `.\nerv.ps1 dev` 并等待就绪，再登录 admin），在 Chrome 查看 `/mes/operation-tasks`、`/mes/work-orders`：工作中心/物料**显示真名或回退码**（master-data 有 displayName 则显示名称，否则显示编码），且不破坏其他功能。截图留证。
- [ ] **步骤 3：推送** — `git push`。
...
- **规格覆盖**：composable（T1）+ 6 页 accessor（T2）+ 门禁/E2E/实机/推送（T3）。✓
