# PDA 设备运维（CMMS：报修 / 点检 / 报警查看）实施计划（Plan 4）

> **供代理执行者使用：**必须使用子技能 superpowers:subagent-driven-development；所有步骤均使用复选框（`- [ ]`）跟踪。

**目标：**在 PDA 地基上交付设备运维一线三件套——**故障报修**（创建维修工单）、**点检**（记录点检结果）、**报警查看**（只读 + 一键报修）——点亮首页对应入口。Maintenance/Telemetry 门面已就绪，**无后端阻塞**。

**架构：**消费 BusinessGateway 既有 Maintenance + Telemetry/Equipment 门面（由 `@nerv-iip/api-client` 生成的客户端）；数据封装进新建 PDA `useBusinessMaintenance` + `useBusinessEquipmentAlarms`（维修无 PC 实现可镜像，按 api-client 类型构建；报警镜像 business-console `useBusinessEquipment`）；org/env 取登录主体 `useAuthStore().principal`，`openedBy`/`inspector` 取 `principal.loginName`；**Maintenance 端点无服务端幂等**→创建操作使用 **UI 防重复提交**（等待期禁用 + 成功后离场/重置）；报修/点检多步流程使用 `defineStepFlow`；文案为中文（无 i18n）。
...
- **请求体**（均含 `organizationId`/`environmentId`，由 composable 注入）：
  - 报修创建：`{ deviceAssetId, priority, openedBy, sourceAlarmId?, assetUnavailableReason? }`（priority≤40、deviceAssetId≤100、openedBy≤100、assetUnavailableReason≤200、sourceAlarmId≤100）。**无 idempotencyKey**。
  - 点检记录：`{ inspector, result, planId? | workOrderId?（至少一个）, inspectedAtUtc? }`（inspector≤100、result≤100）。**无 idempotencyKey**。
  - 列表查询：`{ organizationId, environmentId, skip(≥0), take }`（work-orders/inspections/plans take 1-200；alarms 含 `deviceAssetId?`/`status?`，take 1-500）。
- **org/env**：`useAuthStore().principal.organizationId/environmentId`；空则不发查询、空态。`openedBy`/`inspector` = `principal.loginName`。
- **无服务端幂等** → 所有创建操作：提交时以 `disabled` 防双击 + `pending` 期间禁用 + 成功后清空/离场，避免重复工单/点检。
...
## 任务 1：文档 + 点亮设备字典 + 设备 StepFlow/标签（business-core）
**文件：**文档 × 2；`pdaTaskKinds.{ts,test.ts}`；新建 `sop/equipmentFlows.ts` + 测试、`labels/equipmentLabels.ts` + 测试；`src/index.ts`

- [ ] **步骤 1：文档**：模块文档分期标注“设备运维 报修/点检/报警查看 已建（Plan 4）（门面就绪、无后端阻塞）”；导航图设备 PDA 状态同步 + 更新校验日期。
- [ ] **步骤 2：字典（TDD）**：`pdaTaskKinds.test.ts` 断言 `equipment.repair`/`equipment.inspect`/`equipment.alarms` 三者 `routeReady===true`；**新增** `{ id:'equipment.alarms', label:'查看报警', group:'equipment', route:'/equipment/alarms', routeReady:true }`，并把 repair/inspect 改为 true。运行先失败再通过。（mes.*/wms.* 不动——本分支它们仍为 false。）
- [ ] **步骤 3：设备标签（TDD）**：`labels/equipmentLabels.ts`（纯 TS）导出 `alarmSeverityLabel`（critical→严重/blocked→阻塞/warning→预警/info→信息，回退为未知）、`equipmentStateLabel`（running→运行中/idle→空闲/down→停机/faulted→故障/offline→离线/ready→就绪/stopped→停止，回退为未知状态）、`maintenancePriorityLabel`（high→高/medium→中/low→低，回退为原值或未知）、`maintenanceWorkOrderStatusLabel`、`inspectionResultLabel`（pass→通过/fail→不通过）。镜像 business-console `useBusinessEquipment` 的现有映射，确保代码→中文一致。测试覆盖代码→标签 + 回退值。从 `src/index.ts` 导出。
- [ ] **步骤 4：设备 StepFlow（TDD）**：`sop/equipmentFlows.ts`：
```typescript
...
```
  编写测试（仿 `defineStepFlow.test.ts`），从 `src/index.ts` 导出流程 + 上下文类型。运行先失败再通过。
- [ ] **步骤 5：门禁 + 提交**：`pnpm -C frontend --filter @nerv-iip/business-core typecheck && ... test` 通过；提交消息为 `feat(business-core): equipment flows + labels + light up PDA equipment wall`。

## 任务 2：PDA 设备运维数据封装（可组合函数）
**文件：**新建 `business-pda/src/composables/useBusinessMaintenance.ts` + 测试、`useBusinessEquipmentAlarms.ts` + 测试

- [ ] **步骤 1：可组合函数测试（先红，模拟 api-client + colada，仿 business-console useBusinessEquipment.test 风格 / 与 MES 分支同法）**：断言 (a) 主体无 org/env 时列表查询的 `enabled:false`；(b) `createWorkOrder({deviceAssetId,priority,assetUnavailableReason?,sourceAlarmId?})` 调用创建变更，请求体含注入的 `organizationId/environmentId/openedBy`（=principal.loginName）+ 入参，且**调用方不能覆盖** org/env/openedBy（注入后置 + Omit 收窄）；(c) `recordInspection({planId,result})` 同理注入 org/env/inspector；(d) 报警 `useBusinessEquipmentAlarms` 在有范围时启用查询并暴露 `alarms/pending/error/refresh`。
- [ ] **步骤 2：实现**（org/env 取 `useAuthStore().principal`；范围为空时不发请求）：
...
- [ ] **步骤 3：运行通过 + 提交**：提交消息为 `feat(business-pda): maintenance + equipment-alarms data composables`。

## 任务 3：故障报修页 `/equipment/repair`
**文件：**`src/pages/equipment/repair.vue` + 测试
- [ ] **步骤 1：测试（先红）**：模拟可组合函数 + vue-router（含 `useRoute` 返回 `query:{deviceAssetId?,sourceAlarmId?}`）。断言：渲染 AppShellMobile + 近期维修工单 ListRow；新建走 `repairOrderFlow`（选/扫/带入设备→填优先级+故障描述→提交）；提交调 `createWorkOrder({deviceAssetId,priority,assetUnavailableReason,sourceAlarmId?})`（不含 org/env/openedBy）；**等待期禁用提交**（防重）；成功后显示 Result（“报修已提交”，操作为继续/返回）。若路由带 `deviceAssetId`/`sourceAlarmId`（来自报警页）则预填。
- [ ] **步骤 2：实现**：`definePage({meta:{requiresAuth:true,title:'故障报修'}})`。设备来源：路由查询预填 > ScanBar 扫码（设 deviceAssetId）> 手工输入。优先级用全屏/抽屉选择（high/medium/low → 中文）。故障描述 = assetUnavailableReason。提交按钮 `:disabled="!valid || createPending"`。
- [ ] **步骤 3：运行通过 + 提交**：提交消息为 `feat(business-pda): equipment repair (maintenance work order) page`。

## 任务 4：点检页 `/equipment/inspect`
**文件：**`src/pages/equipment/inspect.vue` + 测试
- [ ] **步骤 1：测试（先红）**：`inspectionFlow` 驱动：选保养计划（`plans` 列表）→ 选结果（通过/不通过）→ 记录。断言提交调用 `recordInspection({planId,result})`（不含注入字段）；等待期禁用以防重；成功后显示 Result。
- [ ] **步骤 2：实现**：`title:'点检'`。先列 `plans` 选择（ScanBar 可扫设备/计划号过滤）→ BottomSheet 内选结果（通过/不通过）+ 可选备注 → 提交。
- [ ] **步骤 3：运行通过 + 提交**：提交消息为 `feat(business-pda): equipment inspection page`。

## 任务 5：报警查看页 `/equipment/alarms`（只读 + 去报修）
**文件：**`src/pages/equipment/alarms.vue` + 测试
- [ ] **步骤 1：测试（先红）**：渲染报警 ListRow（设备/报警码/级别中文/时间）；ScanBar 扫设备码→`filters.deviceAssetId`；行内“去报修”→`router.push('/equipment/repair', { query:{ deviceAssetId, sourceAlarmId: alarmEventId }})`；空态/加载/错误区分。只读，无写操作。
- [ ] **步骤 2：实现**：`title:'查看报警'`。severity 走 `alarmSeverityLabel` 中文；alarmEventId 若为 GUID 则仅作键、不外显（显示报警码/设备/时间）；“去报修”尾部按钮带 deviceAssetId + sourceAlarmId。
- [ ] **步骤 3：运行通过 + 提交**：提交消息为 `feat(business-pda): equipment alarms view + jump-to-repair`。
...
## 任务 7：e2e（报修/点检/报警，网关模拟）
**文件：**`e2e/equipment.spec.ts`（+ 扩展 `e2e/fixtures.ts`）
- [ ] **步骤 1**：扩展夹具，模拟 `GET maintenance/work-orders|inspections|plans`、`POST maintenance/work-orders`、`POST maintenance/inspections`、`GET equipment/alarms`（使用真实形状的数据行）。规格覆盖：报修（选设备→填→提交→Result）、点检（选计划→结果→Result）、报警（列表渲染→去报修带 deviceAssetId→URL `/equipment/repair?deviceAssetId=...`）、首页点“报修”→`/equipment/repair`。seedStoredSession 中的主体含 org/env + loginName。
- [ ] **步骤 2**：在真实 Chromium 上运行 e2e，提交消息为 `test(business-pda): e2e for equipment repair/inspect/alarms`。

## 任务 8：验收 + PR
- [ ] **步骤 1：门禁全部通过**：business-core 的类型检查/测试；business-pda 的类型检查/测试/构建；`... exec playwright test --list` + 真实浏览器 e2e；`pnpm -C frontend typecheck`（工作区无回归）。
- [ ] **步骤 2：推送 + 创建 PR**（基分支为 main，标题 `feat(pda): 设备运维（报修/点检/报警查看）`；正文列出范围、无后端阻塞、组件复用、无服务端幂等→UI 防重、与 MES/WMS 分支并行关系）。
