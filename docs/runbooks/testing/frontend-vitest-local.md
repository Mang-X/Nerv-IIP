...
- 失败可能出现在三个**互相独立的横幅**下（见下一节），只 grep `FAIL` 或只看 `Failed Tests` 会漏掉其中两类。
...
vitest 把失败按来源打在三个独立横幅下，先看横幅再看正文，不要凭印象判「这条像不像环境问题」：
...
能就地定性的只有第一格的前半。「失败信息里含 `Test timed out`」这条单独**零鉴别力**：一个真正挂死的用例（例如 `await` 一个永不 settle 的 promise）产出的正文、堆栈与行号标记和争用超时完全一致，只有重跑能分开两者。

#2951 票面把「worker 启动类错误」与超时并列为争用信号；本次取证只复现出超时那一支，未复现出 worker 启动失败。这里不给它单独的定性规则，只保证它落在后两个横幅里时不会被误判成真红。

### 2. 只改「本机负载」这一个变量，重跑失败文件

```
pnpm -C frontend/<apps|packages>/<name> exec vitest run <失败文件…>
```

**空载下跑，不加任何并发 flag**：保持包内并行拓扑与出问题那次一致，改变的只有「本机有没有别的 run 在抢 CPU」。

- 全绿 → **资源争用假红**。
- 仍红 → **真红**。

这不是「重跑碰绿」：它换的是运行条件，且只换一个，因此绿/红各自都能归因。同一条命令在同样的负载下原样重跑直到变绿，不构成任何归因。

不要在这一步加 `--no-file-parallelism --maxWorkers=1`：那会同时改掉负载和包内并行拓扑两个变量，于是「同包两个文件抢同一固定端口/临时路径导致并行下崩溃」这类**必须修的真红**会随串行重跑一起转绿，被误判成争用。
