...
上表**不是穷举**：`Unhandled Errors` 段内还会按错误来源打出子横幅，且横幅集合随 vitest 版本变动。所以承重的是最后那条兜底行而不是名单——**只有第一行的前半可以就地定性，其余一律进第 2 步**，新增或未见过的横幅不会掉进空隙。完全打不出横幅的失败（进程被 SIGKILL、配置求值失败、没找到测试文件）没有失败清单，由第 0 步接住。
...
**空载下跑，不加任何并发 flag**：不主动关闭包内并行，让共享端口/临时路径这类并行下才暴露的真红仍有机会复现；改变的是「本机有没有别的 run 在抢 CPU」。

但收窄文件本身会缩小并行宽度（并发 fork 数被待跑文件数封顶），所以这一步不是严格的单变量对照。失败只剩一两个文件时，转绿可能来自并行宽度下降而不是负载——此时连同同包相邻文件一起重跑再定性。

- 全绿 → **资源争用假红**。
- 仍红 → **真红**。

这不是「重跑碰绿」：换掉的主要运行条件是本机负载，绿/红各自都能归因。同一条命令在同样的负载下原样重跑直到变绿，不构成任何归因。
