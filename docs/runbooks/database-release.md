...
`AddBarcodeSerialAllocation` 为 `barcode.label_print_items` 回填批次的组织/环境/规则归属，安装规则 scope 内非空 `serial_number` 唯一索引，并新增持久号段计数器及其到 `barcode_rules` 的 `RESTRICT` 外键。发布前仍须执行第 2 节的备份、版本冻结与失败停止条件：

1. migration 若发现同一 `organization_id/environment_id/barcode_rule_id` 内存在重复历史序列，会以 SQLSTATE `23000` fail-closed；消息正文逐组列出 `organization / environment / barcode_rule_id / serial_number: item_id@label_print_batch_id`。事务回滚后原行、序列值及 migration history 均保持不变。
