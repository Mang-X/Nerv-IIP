...
> 本文分两篇。
> **上篇（PR-B）**：后端快速门禁的四个分片，应该各自还原并构建自己的项目集合，
> 还是把构建提到一个构建作业里执行一次，再把产物分发给各分片？
> **结论：保持“每个分片精确构建自己的项目集合”，不采用一次构建/产物复用。**
> **下篇（PR-C）**：connector-hosts、OpenAPI 漂移、ERP 三个专项作业又在构建什么，
> 其中有多少是它们不需要的？**结论：三个都已是精确构建，构建命令一律不改。**
> 两篇的结论都来自托管运行器实测（PR-B 四次：热两次冷两次；PR-C 两次 A/B 探针 +
> 三次原位观测），不是推断。数字全部可回溯到文中列出的运行 ID 与它们日志里的
> `MAN669PROBE` / `MAN669PRC` 原始行。
>
> **本文是这些实测数字与论证的唯一权威。** `ci.yml` 注释、`scripts/verify-*.ps1` 的
> 文档字符串/注释、契约测试注释、`implementation-readiness.md` 一律**只写结论并指回本文**，
> 不复述数字、运行 ID 与推导过程——复述过的事实在 MAN-664 重测时必然只改一处、剩下三处
...
还原并构建（62.7–180.9 s）更贵**。一次构建把这段成本从“四片并行各付一份”改成
“全流水线串行付一份”，因此即使产物传输完全免费，关键路径也会变长。实测传输并不免费：
产物原始 3.03 GB、经 tar.zst 压缩后 1.01 GB，打包 15.5–16.0 s、上传 6–14 s、下载 11–57 s、解包 5.2–6.1 s。

## 度量方式

四次 CI 运行，每次同时执行：

1. **现状**：四个真实分片作业各自执行 `./scripts/run-backend-test-shard.ps1`，即
   `dotnet test <自己的 .slnf> --configuration Release`（一次 MSBuild 调用完成还原、构建与测试）。
2. **方案 3 的显式拆分探针**：`dotnet restore <.slnf>` → `dotnet build <.slnf> -c Release --no-restore`
   → 再运行同一个受治理的分片执行器，得到还原、构建、测试三段耗时。
   注意第三段仍是完整的 `dotnet test`（受治理的执行器不接受 `--no-build`），因此该段
   **含一次增量空转构建**，是纯测试时间的上界。
3. **方案 2 的一次构建探针**：`dotnet restore backend/Nerv.IIP.sln` →
   `dotnet build backend/Nerv.IIP.sln -c Release --no-restore` → 量 `bin`/`obj` 体积 →
   tar.zst 打包 → 上传产物。
4. **方案 2 的消费端探针**：另一个作业下载该产物、解包，再运行 `business-core-a` 分片。

> **口径声明（必须先读）**：第 4 条的作业/步骤名称里带“no-build”，但它调用的
> `scripts/run-backend-test-shard.ps1` 只拼 `dotnet test <slnf> --configuration Release …`，
> **没有传 `--no-build`，也没有 `--no-restore`**。所以消费端运行的是一次完整的还原、构建与测试，
> 只是起点已有解包出来的产物。**票面方案 ② 的字面定义（“测试分片下载后 `--no-build --no-restore`”）
> 因此没有被直接实测**，第 4 条测到的是它的一个上界。下文“裁决”一节按最有利于一次构建的
> 口径把这部分重复构建扣掉后重算，结论不变。

探针作业是临时的，采集完即从 `ci.yml` 删除；它们从不是必需检查。

| 代号 | 运行 ID | 提交 | NuGet 缓存 |
|---|---|---|---|
...
跑完即还原。合并进 main 的 `ci.yml` 中缓存键与 PR-B 之前**逐字节相同**。

**样本不同质，如实登记**：H1 跑在 `5003a4d5c`，此时 `Nerv.IIP.Contracts.Mes` 还在 `bin/Debug`
产出（见本文末尾“PR-B 实际落地的改动”）；H2/C1/C2 在修复之后。也就是说“现状”这四个样本
跨了一次构建行为变更。该项目只是一个无依赖的契约程序集，量级上不足以解释任何一条结论，
但四个样本不是同一份构建图，引用时应知道。

## 现状：四片各自构建（秒）

`job` 表示作业墙钟时间，`step` 表示 `Test … shard` 步骤耗时。

| 分片 | H1 作业/步骤 | H2 作业/步骤 | C1 作业/步骤 | C2 作业/步骤 |
|---|---|---|---|---|
...
| **关键路径（最大作业）** | **281** | **309** | **297** | **300** |

## 每个分片的还原 / 构建 / 测试拆分（秒）

| 分片 | 阶段 | H1 | H2 | C1 | C2 |
|---|---|---|---|---|---|
| BusinessGateway | 还原 | 6.9 | 8.5 | 13.4 | 28.6 |
| | 构建 | 96.2 | 82.8 | 91.9 | 95.2 |
| | 测试（含增量空转） | 190.5 | 154.2 | 183.5 | 169.6 |
| Platform | 还原 | 18.6 | 17.0 | 57.4 | 21.0 |
| | 构建 | 136.2 | 133.8 | 123.5 | 122.4 |
| | 测试（含增量空转） | 102.9 | 114.0 | 103.9 | 99.8 |
| Business Core A | 还原 | 16.4 | 22.8 | 20.0 | 16.8 |
| | 构建 | 125.2 | 141.5 | 123.5 | 121.2 |
| | 测试（含增量空转） | 172.8 | 177.9 | 165.1 | 148.7 |
| Business Core B | 还原 | 11.5 | 12.1 | 17.4 | 13.3 |
| | 构建 | 52.4 | 50.6 | 52.4 | 53.5 |
| | 测试（含增量空转） | 148.6 | 153.9 | 158.3 | 150.4 |

各片自己的还原与构建（16 个读数）：**62.7 – 180.9 s**。
逐次运行的四片合计（运行器时间，非关键路径）**463.5 / 469.0 / 499.6 / 471.9 s**
——这就是“重复还原/构建”的总量。但四片是并行的，**关键路径上只有 max() 算数**，
逐次运行的最大值为 **154.8 / 164.3 / 180.9 / 143.4 s**，即 **143.4–180.9 s**。
这就是一次构建最多能优化的目标。
...
四个数都远小于整解决方案的 3.03 GB——**这就是“没有分片为运行少数项目重建整个解决方案”的实测证据**，
不是靠读 `ci.yml` 断言。该性质在机器层面由 `scripts/verify-backend-test-shards.ps1` 中
**PR-B 之前就存在**的“解决方案筛选器的项目集必须与清单逐项相等”保证（一个列出全部 163
...
## 方案 ①：单运行器一次构建后在同一作业执行多个测试命令（排除，不需另测）

这条不需要单独探针，用已有数据即可关掉：单个作业意味着四片的测试**串行**。
用整解决方案还原与构建加上四片的测试段（后者已是上界）：

| 运行 | 四片测试段之和 | + 整解还原与构建 | = 单作业下界 | 对比现状关键路径 |
|---|---:|---:|---:|---|
...
即使把测试段里的增量空转全额扣掉（每片按最坏 ~60 s 计，共 240 s），仍在 555–599 s，
...
| | H1 | H2 | C1 | C2 |
|---|---|---|---|---|
| 整解决方案还原与构建 | 224.0 | 195.7 | 202.8 | 233.2 |
...
| 解包 | 5.2 | 6.1 | 5.6 | 5.6 |
| 分片运行 | 231.2 | 172.2 | 277.5 | 269.0 |
| **消费作业墙钟** | **260** | **231** | **345** | **347** |

**不盲目上传整个 `bin`/`obj`**：上传的是单个 tar.zst（`compression-level: 0`，
因为 zstd 已经压过），而不是把三万多个文件交给产物服务自行压缩成 zip；这已经是一次构建
能拿到的最好形态。

**消费端的重复构建有多贵**：把消费端的分片运行时间与同一次运行中同一片
（`business-core-a`）拆分探针的测试段相比——后者是“自己刚构建完再运行一次 `dotnet test`”，
两者的差就是消费端白付的构建：

| | H1 | H2 | C1 | C2 |
|---|---:|---:|---:|---:|
| 消费端分片运行 | 231.2 | 172.2 | 277.5 | 269.0 |
| 拆分探针测试段 | 172.8 | 177.9 | 165.1 | 148.7 |
...
关键路径对比（一次构建的两段是**串行**的：消费作业通过 `needs` 依赖构建作业；实测中消费作业
在构建作业完成后 3–5 s 启动）。
...
**修正口径**（按票面 ② 的字面定义重算，取最有利于一次构建的假设）：
消费作业 = 30 s 作业固定开销 + 下载 + 解包 + **最慢那一片的测试段**。
其中 30 s 取自现状四片作业的实测固定开销 26–46 s 的下界（检出代码/配置 dotnet/缓存/
解析证据/采集/上传证据，真正的分片作业一样要付）；
测试段本身还含一次增量空转，所以这已经是一次构建的**上界性能**：
...
**四次实测、两种口径一致：一次构建更慢 59%~108%，差距远大于抖动。**
票面写明“只有实测更快才采用”“不强制采用某一种产物/缓存设计”，
因此 PR-B 的交付是**不采用**，并把上面这组数据固化在本文。

**为什么必然更慢（结构性原因，不是调参问题）**：整解决方案构建本身
（195.7–233.2 s）就比**任何**一片自己的还原与构建（62.7–180.9 s）更贵。
一次构建把这段成本从“四片并行各付一份”改成“全流水线串行付一份”，
所以即使产物传输完全免费、`--no-build` 完全生效，关键路径也会变长。

同时不采用的还有：分片内部拆成 `restore` + `build --no-restore` + `dotnet test` 三步
（实测更慢，见上）。既然没有采用任何跨作业的产物复用，也就没有“用别的 SHA/配置的产物运行测试”
这条风险面——每片仍然在自己的作业内、从自己这次检出的源码构建。
**“防陈旧产物验证”因此不适用**，不做成一条恒真断言摆着。PR-B 落地的是**同一族的另一个
真实缺陷**：配置一致性，见下文。

### 维护成本（票面裁决轴之一）

现状的维护面是**一处**：`scripts/backend-test-shards.json` 决定每片运行哪些项目，
`.slnf` 与它逐项对齐、由门禁强制。一次构建会新增：一个构建作业（及其超时预算与
层级分类）、一份 1 GB 级产物的命名/保留/清理策略、`needs` 依赖导致的**串行失败面**
（构建作业失败则四片全部不运行，且四片的 MAN-661 证据包一个都产不出来）、
以及产物与 SHA/配置一致性的校验及其自测。这些成本换来的是一个实测更慢的流水线——
即使耗时打平也不值得，何况慢 59% 以上。

## 冷缓存与热缓存对比

`actions/cache` 恢复步骤：热缓存 4–7 s（命中），冷缓存 0–1 s（未命中，什么都没恢复）。
...
**冷热差落在运行器抖动之内**：Business Core B 的四个读数是 194 / 173 / 141 / 201 s，
...
1. **`backend/common/Contracts/Nerv.IIP.Contracts.Mes` 加入 `backend/Nerv.IIP.sln`。**
   ...
   `ProjectReference` 传递引用。MSBuild 通过解决方案的配置映射解析项目配置，
   不在映射里的项目回落到自身默认配置，于是**四条分片的 `--configuration Release`
   构建都把它产出到 `bin/Debug`**：Release 测试程序集链接的是 Debug 依赖，而构建输出里
   没有任何东西会失败。运行 `31136085020` 的四条分片日志各有一行
   `-> …/Nerv.IIP.Contracts.Mes/bin/Debug/net10.0/Nerv.IIP.Contracts.Mes.dll`；
   修复后运行 `31141964632` 的四条分片日志中，`bin/Debug` 出现次数全部为 0。
2. **`scripts/verify-backend-test-shards.ps1` 把“必须是解决方案成员”从 `*.Tests.csproj`
   扩到 `backend/**` 下的每个 `csproj`**，并在失败信息里写明 Release/Debug 后果。
   自测位于 `scripts/tests/backend-test-shards.Tests.ps1`：创建一个**非测试**项目
   （`backend/common/Nerv.IIP.TemporarySolutionMembership`），它对旧的 Tests-only 规则不可见，
   因此把新检查削弱掉该用例立刻红。
3. **拒绝把分片的 `solutionFilter` 指成整个解决方案**，同一份自测里用临时清单覆盖。
   注意这只是补窄口：一个“列出全部项目的 `.slnf`”早已被 PR-B 之前就存在的
   「筛选器与清单逐项相等」检查挡住。

## 复评触发条件（可执行）

本文的结论建立在一个可量化的余量上：**整解决方案还原与构建减去最慢那一片自己的
还原与构建**。四次实测分别是 **69.2 / 31.4 / 21.9 / 89.8 s**（H1/H2/C1/C2）。
C1 只剩 **21.9 s**——余量本来就不厚。

出现下列任一情况时，本文结论**必须重测，不得沿用**：

1. **最慢一片的还原与构建 ≥ 整解决方案还原与构建的 90%**（即余量 < 约 20 s）。
   MAN-664 的重新配平、或把某一片继续拆细，都会把最慢片的构建闭包推向整解；
   一旦推到这个线，一次构建的串行成本就不再天然大于它省下的量。
   量法：把本文方案 3 的拆分探针照抄回 `ci.yml` 运行一次即可，不需要一次构建探针。
2. **运行器规格变化**（例如换更大规格的托管运行器或自托管运行器）。
   它同时改变构建与测试两侧，本文的比值不可直接沿用。
3. **产物传输成本出现数量级改善**（例如 `upload/download-artifact` 大版本变更、
...
# 三个专项作业的构建盘点（MAN-669 PR-C）

> **当前态（#2085）：**`ERP Sales Order Demand Acceptance` 独立作业已退出；其 verifier 继续由
> acceptance shadow runtime 复用。下述三作业数据与逐作业分析保留为 MAN-669 当时的历史测量，
> 不表示当前 workflow 仍存在 ERP 专项作业，也不作为重新引入该作业的依据。

> 上半篇回答“四片要不要共享构建”。本节回答票面范围第 7 条**前半句**：
> **connector-hosts、OpenAPI 漂移和 ERP 专项这三个作业各自在构建什么，其中有多少是它们
> 其实不需要的。**结论：**三个作业都已经是精确构建，没有任何一个为了运行少数项目去构建整个
> 解决方案，“收敛到精确项目集合”这条收益假设的实际值为 0。**盘点过程中发现并修复了一个
> 与 PR-B 同族的真实缺陷（ConnectorHost 解决方案的 Release/Debug 配置泄漏）。
>
> **范围 7 的后半句“跨业务验收统一后由场景执行器精确构建需要的服务”没有被本文实现，
...
> 在 #1240 落地之前不存在“统一的跨业务验收”，也就不存在可以承担精确构建职责的场景执行器。
> 本文对 ERP 作业的裁决（不改构建命令）正是在“它仍然是一个单链路特例、自己硬编码五个项目”
> 这个前提下做出的；#1240 落地时应当连同下面的复评触发条件一起重看。

## 口径：跨作业的“重复”与作业内的“重复”不是一回事

三个专项作业与四个快速分片是**并行**的独立运行器。它们的依赖闭包确实大量重叠，但
**并行作业之间的重复构建只消耗运行器分钟数，不消耗关键路径墙钟时间**；要把它变成收益，唯一途径是
跨作业复用产物，而 PR-B 已用四次实测否掉了这条路（更慢 59%~108%，见上半篇）。
**PR-C 因此只把“同一个作业内部的重复”当作可优化空间**，跨作业的交集只做登记。

三个作业都不在关键路径上，这一点先摆在前面（三次实测，作业墙钟，秒）：

**口径与本文上半篇一致：关键路径 = 该次运行里最大的作业，不是最慢的分片。**
这不是措辞问题——运行 `31145563813` 的最大作业是 `Frontend Typecheck and Build`（305 s），
比该次运行最慢的分片（287 s）还长。按“最慢分片”算会低估余量，并让下面的复评触发条件偏早触发。

| | 运行 `31143773140` | 运行 `31138913408` | 运行 `31145563813` |
|---|---:|---:|---:|
| **关键路径（最大作业）** | **298**（Business Core A） | **307**（BusinessGateway） | **305**（Frontend） |
| 参考：最慢分片 | 298 | 307 | 287 |
| ERP Sales Order Demand Acceptance | 232 | 271 | 229 |
| OpenAPI/api-client Drift | 117 | 142 | 95 |
| Connector Host Tests | 102 | 117 | 118 |

即：**把这三个作业的构建时间全部清零，CI 的墙钟一秒都不会缩短**——它们分别比关键路径短
36–76 s、165–210 s、187–196 s。任何改动只能以运行器分钟数、可维护性或正确性为理由，
不能以“提速 CI”为理由。
...
五次**串行**、单进程（`-m:1`）、不复用 MSBuild 节点（`-nr:false`）、不带 `--configuration`
（因此全程 Debug，与后续 `dotnet run --no-build` / `dotnet test --no-build` 一致）。
**没有 `dotnet build backend/Nerv.IIP.sln`，也没有任何一次整解构建。**

**耗时拆分**（`Invoke-DotNet` 的 `durationMs` 原始行，两次运行）：

| 项目 | 运行 `31143773140` | 运行 `31138913408` | 运行 `31145563813` |
|---|---:|---:|---:|
...
| `Verify ERP …` 步骤 | 210 | 249 | 207 |
| **构建占比** | **51%** | **52%** | **51%** |

另一半（约 100–120 s）是 compose 起 PostgreSQL+Redis、建一次性数据库、起三个真服务并等
健康检查、运行完整验收剧本、两次 `--no-build` 探针测试以及清理。这部分是这个作业的**本体**，
与构建策略无关。

**先记住这个抖动量**：同一份工作在三次运行里是 107.6 / 128.6 / 106.4 s，跨度 **22.2 s**。
下面探针测出来的收益要放在这个尺度上读。

**与快速分片的交集**：`DemandPlanning.Web.Tests` 同时属于快速分片 `business-core-b`
（那边是 Release）；`FullChain.Tests` 属于重型通道 `full-chain`，不在任何快速分片里；
三个 `.Web` 服务项目在 `business-gateway` / `business-core-b` 两片的依赖闭包内。
按上面的口径，这些都是并行作业之间的交集，不构成可优化空间。

**作业内的可优化空间**：五次独立 MSBuild 调用对同一个重叠闭包各求值一次。这正是票面背景段
“会再次串行构建……”所指的内容。收益用探针实测，见下节，**不靠估算**。
...
要么是被测程序集的依赖。**没有多余项目，也没有作业内重复调用，无可优化空间。**

耗时：作业墙钟 102 / 117 s，`Test connector host solution` 步骤 72 / 75 s；
该通道的已提交基线 TRX 为 55.2 s，即还原与构建约 17–20 s。
...
配置映射解析项目配置，不在映射里的项目回落到自身默认配置，于是
**`--configuration Release` 的构建把这三个产出到 `bin/Debug`**：Release 测试程序集链接
Debug 依赖，构建输出里没有任何东西会失败。

两次独立运行各有三行原始日志（运行 `31143773140`、作业 `92758931855`；
运行 `31138913408`、作业 `92744407293`）：
...
耗时：作业墙钟 117 / 142 / 95 s，`Verify OpenAPI/api-client drift` 步骤 92 / 118 / 76 s；
其中 `export-gateway-openapi.ps1`（两次构建 + 起两个网关 + 抓两份 swagger）65.5 s（运行
`31143773140`）与 44.2 s（运行 `31145563813`），`pnpm install` 6.9 / 5.5 s、`generate:api`
4.5 / 3.7 s——原始 `Invoke-*` `durationMs` 行。**导出脚本本身占该步骤的绝大部分，而构建又占
导出脚本的绝大部分**（探针实测两次构建单独就要 48.4 / 57.9 s，见下节；它跨越了原位观测的两个
读数，说明这里的运行器抖动大于任何可比结构差异）。

**与快速分片的交集**：两个网关的 `.Web` 项目在 `business-gateway` / `platform` 两片的
依赖闭包内（那边是 Release）。同样是并行作业之间的交集。

**作业内的可优化空间**：两次独立 MSBuild 调用共享 `backend/common/**` 闭包。
与 ERP 同类，但只有两次而不是五次，量级更小。用探针实测，见下节。

## 探针：量化“串行多次调用”的代价

四个临时探针作业，各自独占一个全新运行器、使用同一个提交与同一个 NuGet 缓存键，
运行完即从 `ci.yml` 删除；它们从不是必需检查。样本取自
运行 `31145563813`（S1）与运行 `31145875145`（S2）。
`openapi-consolidated` 只有 S2 一个样本，因为它是在 S1 之后才加进来的——如实登记。

### ERP：5 次串行调用与 1 次合并调用

`serial` 臂逐字复刻脚本现状（5 次 `dotnet build <csproj> -m:1 -nr:false`）；
`consolidated` 臂用一个只含这 5 个项目的临时 `.slnf` 执行一次 `dotnet build`。
...
重新拷贝一遍——这就是这个作业内真正的重复项。

**两臂产出不同**：`split` 全进 `-o` 目录，`bin/Debug` 与 `bin/Release` 都是 0 个 dll；
`consolidated` 落回各自 `bin/Debug`（219 个 dll）。因此 OpenAPI 这一侧**不能直接替换**：
...
**1. “收敛到精确项目集合”的收益 = 0。**三个作业没有一个在构建它不需要的东西，
也没有一个执行 `dotnet build backend/Nerv.IIP.sln`。票面验收里“没有分片为运行少数项目
重新执行整解构建”这一条，对这三个作业同样成立，而且是从脚本源码与运行日志两侧核过的。

**2. “消除作业内串行重复调用”的收益是真的，但买不到任何东西。**

| | 实测收益 | 该作业距关键路径（最大作业） | CI 墙钟收益 |
|---|---|---|---|
| ERP | 16.4–31.3 s | 低 36–76 s | **0** |
| OpenAPI | 2.1–11.6 s | 低 165–210 s | **0** |
| Connector Host | 无重复可消 | 低 187–196 s | **0** |

ERP 的 16.4–31.3 s 还要放在它自己的抖动尺度上读：同一份串行构建在三次真实运行里是
107.6 / 128.6 / 106.4 s（跨度 22.2 s），探针的两次是 133.7 / 125.1 s。
...
- ERP：项目清单会从“脚本里一个硬编码数组”变成“脚本 + `.slnf` 两处必须一致”。
  ...
  而 `--no-build` 恰恰是这个作业全程依赖的前提。要安全采用就得再加一条门禁 + 自测，
  换来 0 s 关键路径。
- ERP 还有一个未测变量：`consolidated` 臂**同时**丢掉了 `-m:1`（单进程）与 `-nr:false`
  （不复用节点）。这两个开关是 MAN-517 引入时就带着的。因此实测的 16.4–31.3 s
  里有多少来自“少求值四次依赖图”、多少来自“放开 MSBuild 并行”，**本次没有分离**。
  后者在这个作业里不是免费的：同一台运行器上还并存着 Docker、PostgreSQL、Redis
  和三个真服务进程。**保留 `-m:1 -nr:false` 的合并构建这一臂没有被测量**，如实登记。
- OpenAPI：不能直接替换（见上），要改导出脚本的启动路径，收益 2.1–11.6 s。
- Connector Host：无事可做。

**因此 PR-C 不改这三个作业的任何构建命令。**这与 PR-B 的裁决同源：
...
1. **`Sdk.Ops`、`Contracts.Ops`、`Contracts.IntegrationEvents` 加入
   ...
   运行 `31143773140` / `31138913408` 的 Connector Host 作业日志各有三行
   `-> …/bin/Debug/net10.0/…`；修复后运行 `31145563813`（作业 `92764223035`）
   该作业日志里 `bin/Debug` 出现次数为 **0**，三个项目全部落在 `bin/Release`。
...
2. **新增 `scripts/verify-solution-configuration-membership.ps1`**。
   ...
   **解决方案是扫描发现的，不是列出来的**——否则将来第三个 `.sln` 会静默不受检，
   ...
   并把真实发现全埋在后面）。**无允许列表、无责任 Issue 逃生口**，理由与 PR-B 的目录规则相同。

   失败时把每条发现输出到标准输出再 `exit 1`（与 `check-script-governance.ps1` 同形），
...
3. **新增 `scripts/tests/solution-configuration-membership.Tests.ps1`**：8 步，全部是**行为断言**
   （运行验证器、查看其真实输出），**没有一条源码文本断言**。
   真仓库必须通过，且必须从验证器自己的输出里看到**两个解决方案都被报告**——
   ...
   **`Release` 映射到 `Debug` 必红**、映射完整必绿、glob 必须展开（`**` 跨目录、叶子模式不误伤、
   字面 glob 文本不得被当成路径），以及**未登记的 `.sln` 放进树里也必须被发现并检查**。
...
5. **`ci.yml`**：上面两个脚本接入必需检查 `Script Governance`。
   该作业是 B 级，不要求 `sum(step) < job`；步骤预算合计 33m → 43m，
   最大单步 5m 仍小于 10m 的作业预算，作业预算不变（本机实测 1–5 s）。

## 复评触发条件（可执行）

本节结论建立在“这三个作业都远低于关键路径”上。出现下列任一情况时**必须重测，不得沿用**：

1. **任一作业的墙钟接近或超过该次运行的最大作业。**当前余量 36–210 s。ERP 最薄（36 s），
   它又是唯一依赖 Docker 镜像拉取的作业，镜像变大或验收剧本变长都会吃掉这段余量。
   一旦 ERP 成为关键路径，上面实测的 16.4–31.3 s 立刻变成真收益，应当采用。
   注意用**最大作业**判定而非“最慢分片”：`Frontend Typecheck and Build` 已经在
   运行 `31145563813` 上超过了最慢分片。
...
4. **`export-gateway-openapi.ps1` 改掉 `-o` 输出布局**（例如为别的原因改成默认 `bin/`），
   那时 OpenAPI 的合并构建就能直接替换，2.1–11.6 s 可以顺手拿走；同时这也会把当前
   “调用次数”与“输出布局”两个变量分开，让那个区间第一次变得可解释。
5. **#1240 落地、跨业务验收统一。**范围 7 后半句的场景执行器那时才有落点，
   ERP 作业也不再是硬编码五个项目的单链路特例，本节对它的裁决前提随之失效。

## 遗留 / 下一步

- 关键路径当前 298–307 s，通常是最慢那一片，但**已经不总是**：运行 `31145563813` 上是
  `Frontend Typecheck and Build`（305 s）。后端侧继续压缩要靠**配平**（MAN-664，必须用配平后的
  实测重定权重）和**单片内部并行**（MAN-663 已在 BusinessGateway 上做过），不是靠构建策略；
  再往下压之前，先确认压的是当次真正的最大作业。三个专项作业都在关键路径之下，
  改它们不影响这个数字。
- **配平的耗时来源自 #1507 起是 `pwsh scripts/report-backend-test-shard-balance.ps1`**：它聚合最近
  5 次成功 `main` push 运行的 TRX 证据产物（同一次运行内同程序集先求和，跨运行取中位数），
  缓存超过 24 小时自行刷新，因此不再需要“先刷新基线再读数”这一步。缓存无法获取时回落
  到已提交快照，再回落到估值，**全程只生成报告，永不变红**。MAN-664 重测时直接运行它
...
裁决本身（不采用一次构建、三个专项作业不改构建命令）**未被触碰**。

1. **叙述收敛（两份走查各自提出）。**#1494：“实测数字同时写入 ci.yml 注释、验证脚本注释、
   implementation-readiness、strategy 文档四处；前三处可只留结论 + 指针”；#1495：“‘漏一行即
   复现’论证在验证器文档字符串、测试注释、治理文档、readiness 四处近逐字重复；建议保留
   strategy 文档一处权威”。已按本文开头的单一权威声明收敛：`ci.yml` 的构建策略注释块、
   `verify-backend-test-shards.ps1` 的整解拒绝注释、`verify-solution-configuration-membership.ps1`
   的文档字符串、`solution-configuration-membership.Tests.ps1` 的步骤 4 注释、
   `implementation-readiness.md` 的 PR-B/PR-C 段落全部改为结论 + 指回本文。
   **`ci.yml` 顶部超时策略块里的 A/B 级算术依据（`3+5+8+3+10+5+5 = 39 < 45` 及其权重推导）
...
3. **两个检查器的失败输出统一成标准输出 + `exit 1`。**#1494 微瑕 2：“整 sln 回退用例只在异常
   Message 上 `Contains(...)`，折行截断会假红”；判断题 2 同源。

   **本条是“折行”与“退出码”两段论证的唯一权威**，两个验证器、两个契约测试、
   ...
   **为什么不用 `throw`。** PowerShell 的错误格式化器把抛出的消息**按终端宽度硬折行**，
   ...
   `Contains(...)` 匹配都变成**终端宽度的函数**——宽运行器上绿、窄运行器上红——只能退而
   匹配短到不会跨行的片段。改成把每条发现 `Write-Host` 到标准输出再 `exit 1`（与
   `check-script-governance.ps1` 同形）之后，子进程标准输出是原样文本，断言可以匹配整句。
   契约测试仍然折叠捕获文本的空白，那只是让“验证器在哪里断行”不进入契约，与本问题无关。

   **因此调用方必须查退出码，而且不能与别的脚本共用一个 `run:` 块。** `exit` 在被点斜杠调用的
   `.ps1` 里**不会**中止调用方脚本；调用方继续往下运行，而后续脚本里任何原生命令调用都会覆写
   `$LASTEXITCODE`。实测确认「前一个脚本 `exit 1` + 后一个脚本成功」的组合**整体退出 0**——
   失败被完全吞掉。`ci.yml` 里 `Backend Test Shard Governance` 作业原本正是把检查器与其契约
   测试写在同一个 `run:` 块里，已拆成两个步骤（各 5m，步骤预算合计仍是 18m，作业 15m 不变）。

   > **这个洞在改造前不存在，拆分步骤是防止引入而不是修复既有。**改成 `exit 1` 之前该检查器
   > 用的是 `throw`，在 GitHub `pwsh` shell 的 `$ErrorActionPreference = 'Stop'` 下会**中止整个
   > `run:` 块**——同一组合实测整体 `EXIT=1`，第二个脚本根本没被执行。也就是说 `Backend Test
   > Shard Governance`（及经 `needs` 传导的必需检查 `Backend Tests`）**历史上从未失效放行**。
   > 会失效放行的只有“已改成 `exit 1` + 步骤仍不拆”这一种组合，而它从未进入 main。
   >
   > 这条“用 `exit 1` 报错的脚本必须独占一个 `run:` 步骤”目前**只有约定、没有门禁**，已立案
   ...
   `backend-test-shards.Tests.ps1` 的断言统一走 `Invoke-ShardValidator`（判退出码 + 完整标准输出），
   删掉了原先“从命令日志目录读取 stdout.log/stderr.log 补全被折断文本”的兜底，断言从短片段
   升级成整句。
...
4. **解决方案发现跳过 git 忽略的路径**（[#1496](https://github.com/Mang-X/Nerv-IIP/issues/1496)，
   ...
   一台开发机，该数字只反映当时本机有几个代理工作副本，明天就不是 10**），其中 8 个来自
   `/worktrees/` 下的工作副本；过滤后为当前工作树中的 2 个。
   ...
   本来就不下钻，检查器眼里该目录下的 `.sln` 数量是 0，补它一条都命中不了。

   CI 是干净检出，不受影响；发现语义不变——Git 跟踪的一切仍然被发现，包括没人登记过的
   第三个 `.sln`。空扫描仍然失败（不得空洞通过），契约测试用「同一棵树 + 有/无忽略
   规则」的成对断言证明该过滤削弱即红。
