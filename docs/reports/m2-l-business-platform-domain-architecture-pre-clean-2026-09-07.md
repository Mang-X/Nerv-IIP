...
| BarcodeLabel | `backend/services/Business/BarcodeLabel` | `barcode` | 条码规则、标签模板、标签打印批次、扫码记录、GS1 序列化解析字段、EPCIS 最小追溯事件 | 库存余额、业务单据状态 | MasterData、Inventory、File Storage |
...
1. [`ZplV1LabelCompiler`](../../backend/services/Business/BarcodeLabel/src/Nerv.IIP.Business.BarcodeLabel.Domain/Printing/ZplV1LabelCompiler.cs) 严格解析模板与变量 schema，并确定性生成 Code 128、GS1-128、QR、Data Matrix 和 GS1 Data Matrix 的 UTF-8 ZPL。Code 128 / GS1-128 数据上下文按 Zebra `^BC` invoked-mode 合同把业务 `>` 编码为 `>0`，因此业务值中的 `>8`、`>;`、`>:` 不会变成 FNC1 或子集切换；renderer 自有的 start/FNC1 invocation 不经过该编码器，QR、Data Matrix、GS1 Data Matrix 和文本上下文也保持各自既有边界。这个兼容扩展不改变既有合法输入的输出字节，仍属于 `zpl-v1`。
...
`LabelPrintBatch.RecordPrinted` 与 `LabelPrintBatch.ReprintItem` 保留物理状态迁移的领域入口，但生产 dispatch/reprint 链当前不调用它们；生产装配中只有 Development-only world-history seed 会生成 `printed` / `reprinted` 演示事实。当前 `LabelPrinterOptions` 在宿主启动时验证：`simulated` 只允许 Development/Testing，其他环境必须显式选择 `zpl-tcp`，并提供至少一条启用且完整的逻辑 printer-id 路由。每条路由固定 host、port、连接/写入超时、DPI、`zpl` 语言和支持码制；重复 id、非法端点/超时/DPI/码制或没有启用路由都会失败关闭。dispatch/reprint 使用请求中的 `printerId` 精确选择路由，并在连接前核对冻结模板的 DPI 与码制；这些规则仍没有生产物理确认来源、真实打印机/扫码枪验收或现场重打审计闭环。
...
前端导航的规范、当前 route-ready 范围、能力目录和角色导航口径以 `docs/architecture/frontend-navigation-map.md` 为准。服务边界不是用户操作边界，Business Console 页面可以按角色任务聚合多个服务事实。当前 BusinessGateway 已覆盖 MasterData、ProductEngineering、Inventory、Quality、DemandPlanning、MES PC 工作台、Scheduling/APS lite、ERP Procurement/Sales/Finance 读写子集、WMS 收货/出库/上架/拣货/盘点/WCS 读面、BarcodeLabel 分页读面、BusinessApproval 审批中心、IndustrialTelemetry/Equipment runtime facts 和 Maintenance 工单/计划/点检/备件/availability-windows/reliability facade。后端服务或 facade 已存在不自动等于前端菜单已完整交付；新增可见入口仍必须跟随 `frontend-navigation-map.md` 的菜单分期、角色任务、权限和 OpenAPI/codegen 流程。
...
| BarcodeLabel | `barcode.BarcodeScanAccepted`；受支持的 `inventory.receipt`、`inventory.issue`、`inventory.adjustment` 扫码会同时发布 `inventory.InventoryMovementRequested` | Inventory、后续 MES/WMS/Quality scan consumers | BarcodeLabel 解释 GS1 扫码和保留 EPCIS 追溯事实；库存过账仍由 Inventory 消费既有 movement-requested 契约完成。 |
...
Wave 3 ERP 已落地并接入 solution/AppHost/IAM/schema catalog/verify scripts，代码事实覆盖 Procurement、Sales 和 Finance MVP。Wave 4 的 #77 P0 收口入口是 `docs/superpowers/specs/2026-05-23-business-full-chain-acceptance-design.md` 和 `docs/superpowers/plans/2026-05-23-business-full-chain-acceptance.md`；2026-05-27 起，后续主线转向 #188 到 #207 的 MES operational foundation reset，其中 #206 APS lite 已形成 BusinessScheduling 当前基线，#207 继续负责设备 IIoT 联动深化。#142 对象存储、PDA/mobile 专用 API 和离线能力、事件可靠性 hardening 与部署发布深化继续作为并行专题推进。上述 specs/plans 是历史 handoff 与验收记录，当前状态源仍以 `docs/architecture/implementation-readiness.md`、schema catalog、frontend navigation map 和 integration-event matrix 为准。
