...
本目录维护**当前可查事实的人工索引与解释层**：Schema 目录、受控码表、契约/页面矩阵、事件消费矩阵、权限导航、术语和技术资料链接。Reference 不拥有运行时实现，也不承担项目状态、历史审计或长期工程规则。
...
| [`security/authorization-catalog.md`](security/authorization-catalog.md) | IAM `NervIipSeedPermissions` / permission catalog / seed、PlatformGateway/BusinessGateway endpoint authorization | 权限/默认角色/operation 映射变化时先改 producer 与测试，再同步导航 | IAM、Gateway、服务授权与权限评审 | 人工索引，不是权限 registry |
