...
| MES | `AndonCallEscalatedIntegrationEvent` | MES：未认领呼叫到期后的单次升级，事实与 CAP outbox 同事务；canonical topic 由部署环境构造，稳定业务幂等键为 `andon-call-escalated:{CallId}` | Notification 站内通知消费由 [#3657](https://github.com/Mang-X/Nerv-IIP/issues/3657) 承接；当前未接消费方 | `needs-business-consumer` |
