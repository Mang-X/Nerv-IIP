...
- FileStorage 最小骨架需要包含文件元数据、上传会话、上传指令、下载授权、Upload Provider 抽象、FilePurposePolicy 和对象存储适配边界；文件存储基线以 docs/architecture/file-storage-baseline.md 为准，但不要求首批实现复杂网盘、预览或转码能力。
