...
public sealed class ListeningThreadOffloadTests
{
    ...
    /// <summary>
    /// 验收 4：专用线程**不泄漏**——取消后那个阻塞调用必须<b>返回</b>，线程才能被释放。
    ///
    /// <para>⚠️ <b>断的是「委托有没有返回」，不是「<see cref="Thread"/> 对象有没有被回收」。</b>
    /// 先前那版用 <c>dedicated.Join(10s)</c>，实测在<b>整程序集</b>跑法下抖动率约 39%（18 跑 7 红，每次恰好 10 s 超时），
    /// 而单独 <c>--filter</c> 跑 6/6 全绿 —— 复审自建探针查清：此时<b>委托早已退出</b>（<c>finally</c> 已执行）、
    /// Task 已 <c>Canceled</c>，但那个 Background <see cref="Thread"/> 对象 60 秒后仍 <c>IsAlive</c>。
    /// ⇒ <c>Join</c> 断的是<b>运行时回收线程的时机</b>，那是不可确定性观测量，<b>不是本票的不变量</b>；
    /// 而且它会把变异矩阵污染到无法区分「真杀」与「抖动」。</para>
    ///
    /// <para>真正需要、且可确定性观测的不变量是：<b>阻塞调用返回了</b>（委托解栈）。委托一返回，
    /// <c>LongRunning</c> 的专用线程就没有工作可做、由运行时回收；回收时机不归本票管。</para>
    /// </summary>
    [Fact]
    public async Task Cancellation_makes_the_blocking_call_return_so_the_dedicated_thread_is_released()
    {
        var inner = new BlockingConsumerClient();
        var client = new DecoratedConsumerClient(inner);
        using var cancellation = new CancellationTokenSource();
    ...
    /// <b>这条路径可达，依据是对 <c>DotNetCore.CAP</c> 10.0.1 的 <c>ConsumerRegister</c> 反编译实读</b>（逐条核过，非推断）：
    /// ① <c>StartAsync</c> 里 <c>_cts = CancellationTokenSource.CreateLinkedTokenSource(stoppingToken)</c>
    /// ——它<b>与宿主停机 token 联动</b>；② <c>_cts.Token.Register(Dispose)</c>，而 <c>Dispose()</c> → <c>Pulse()</c> → <c>_cts.Cancel()</c>；
    /// ③ 传给消费者的确实是它自己的 token：<c>await client.ListeningAsync(_pollingDelay, _cts.Token)</c>；
    /// ④ 该调用<b>之前</b>还有 <c>await CreateAsync(...)</c> 与 <c>await SubscribeAsync(...)</c> 两次<b>真实 Redis 往返</b>。
    /// ⇒ 停机若落在③之前的那两次 await 期间，<c>ListeningAsync</c> 拿到的就是一个<b>已取消</b>的 token。</para>
    ///
    ...
    private sealed class BlockingConsumerClient : StubConsumerClient
    {
        ...
        /// <summary>阻塞调用<b>退出</b>的边沿。<c>finally</c> 跑到就说明委托已经解栈——这是可确定性观测的。</summary>
