...
public sealed class BusinessPartnerEntityTypeConfiguration : IEntityTypeConfiguration<BusinessPartner>
{
    public void Configure(EntityTypeBuilder<BusinessPartner> builder)
    {
        ...
        builder.Property(x => x.Code).HasColumnName("code").IsRequired().HasMaxLength(100).HasComment("Business unique partner code within the partner type.");
        builder.Property(x => x.PartnerType).HasColumnName("partner_type").IsRequired().HasMaxLength(80).HasComment("Business partner type such as supplier, customer or carrier.");
        ...
        builder.HasIndex(x => new { x.OrganizationId, x.EnvironmentId, x.PartnerType, x.Code }).IsUnique();
