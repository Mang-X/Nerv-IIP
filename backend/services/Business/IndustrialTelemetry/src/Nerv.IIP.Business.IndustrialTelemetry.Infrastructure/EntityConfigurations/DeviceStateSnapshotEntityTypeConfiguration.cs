...
public sealed class DeviceStateSnapshotEntityTypeConfiguration : IEntityTypeConfiguration<DeviceStateSnapshot>
{
    public void Configure(EntityTypeBuilder<DeviceStateSnapshot> builder)
    {
        ...
        builder.Property(x => x.OccurredAtUnixTimeMilliseconds).HasColumnName("occurred_at_unix_time_milliseconds").HasComment("Observed UTC time represented as Unix time milliseconds for provider-neutral current-state ordering.");
        ...
        builder.Property(x => x.RecordedAtUnixTimeMilliseconds).HasColumnName("recorded_at_unix_time_milliseconds").HasComment("Recorded UTC time represented as Unix time milliseconds for provider-neutral current-state ordering.");
        ...
        builder.HasIndex(x => new { x.OrganizationId, x.EnvironmentId, x.DeviceAssetId, x.OccurredAtUnixTimeMilliseconds, x.RecordedAtUnixTimeMilliseconds, x.SourceSequence })
