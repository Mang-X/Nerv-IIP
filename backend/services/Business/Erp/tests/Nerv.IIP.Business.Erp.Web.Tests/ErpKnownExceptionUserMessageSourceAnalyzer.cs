...
internal sealed record ErpKnownExceptionSourceDocument(string Path, string Text);

internal sealed record ErpKnownExceptionSite(
    string Path,
    string TypeName,
    string MethodName,
    int DirectKnownExceptionCount,
    ErpKnownExceptionSiteKind Kind,
    string Reason)
{
    public string Key => $"{Path}|{TypeName}|{MethodName}";
}

internal enum ErpKnownExceptionSiteKind
{
    Target,
    Excluded,
}

internal static class ErpKnownExceptionUserMessageSourceAnalyzer
{
    ...
    public static IReadOnlyList<ErpKnownExceptionSite> Discover(
        IReadOnlyCollection<ErpKnownExceptionSourceDocument> documents)
    {
        if (documents.Count == 0)
        {
            throw new InvalidOperationException("Erp KnownException 源集合不能为空。");
        }
        ...
        var syntaxTrees = sourceTrees
            .Append(CSharpSyntaxTree.ParseText(
                ...
                path: "__ErpKnownExceptionGlobalUsings.g.cs"))
            .ToArray();
        var compilation = CSharpCompilation.Create(
            "ErpKnownExceptionMessageArchitecture",
            syntaxTrees,
            CreateMetadataReferences());
        var discovered = new List<ErpKnownExceptionSite>();
        ...
        foreach (var syntaxTree in sourceTrees)
        {
            ...
            foreach (var method in syntaxTree.GetRoot().DescendantNodes().OfType<MethodDeclarationSyntax>())
            {
                ...
                discovered.Add(new ErpKnownExceptionSite(
                    NormalizePath(syntaxTree.FilePath),
                    typeName,
                    MethodName(method),
                    directKnownExceptionCount,
                    ErpKnownExceptionSiteKind.Excluded,
                    "discovered Erp KnownException site"));
            }
        }
    ...
    public static IReadOnlyList<string> AnalyzeUserMessages(
        IReadOnlyCollection<ErpKnownExceptionSourceDocument> documents,
        IReadOnlyCollection<ErpKnownExceptionSite> excludedSites)
    {
        if (documents.Count == 0)
        {
            throw new InvalidOperationException("Erp KnownException 源集合不能为空。");
        ...
        var syntaxTrees = sourceTrees
            ...
            .ToArray();
        var compilation = CSharpCompilation.Create(
            "ErpKnownExceptionUserMessageArchitecture",
            syntaxTrees,
            CreateMetadataReferences());
        ...
        foreach (var syntaxTree in sourceTrees)
        {
            ...
            foreach (var creation in root.DescendantNodes().OfType<BaseObjectCreationExpressionSyntax>())
            {
                ...
                var methodName = method is null ? "<global>" : MethodName(method);
                var typeName = method?.Ancestors()
                    .OfType<TypeDeclarationSyntax>()
                    .Select(type => type.Identifier.ValueText)
                    .FirstOrDefault()
                    ?? "<global>";
                var siteKey = $"{NormalizePath(syntaxTree.FilePath)}|{typeName}|{methodName}";
