...
public sealed class WorkCenterCostRateApplicationTests
{
    ...
    public async Task Labor_cost_uses_highest_active_revision_at_report_occurrence_time()
    {
        ...
        await new ProductionReportRecordedIntegrationEventHandlerForAccumulateLaborCost(
            db, new InMemoryIntegrationEventDeadLetterStore(), db, TestWorkOrderCostMutationLock.Instance, ErpTestCoding.For(db)).HandleAsync(Report("event-active", July1), CancellationToken.None);
        await db.SaveChangesAsync();
    ...
    public async Task Configure_then_replay_of_the_exact_event_succeeds()
    {
        ...
        var consumer = new ProductionReportRecordedIntegrationEventHandlerForAccumulateLaborCost(db, deadLetters, db, TestWorkOrderCostMutationLock.Instance, ErpTestCoding.For(db));
