...
namespace Nerv.IIP.Business.Erp.Web.Application.Queries;
...
public sealed record OffsetPage
{
    ...
    public static OffsetPage From(int skip, int take) =>
        new(Math.Max(0, skip), Math.Min(take <= 0 ? DefaultTake : take, MaxTake));
}
...
public static class ListQueryValidationExtensions
{
        ...
        Expression<Func<T, string>> environmentId)
    {
        validator.RuleFor(organizationId)
            .NotEmpty()
            .WithMessage("组织标识不能为空。");
        validator.RuleFor(environmentId)
            .NotEmpty()
            .WithMessage("环境标识不能为空。");
    }

    public static void AddPageRules<T>(
        this AbstractValidator<T> validator,
        Expression<Func<T, int>> skip,
        Expression<Func<T, int>> take)
    {
        validator.RuleFor(skip).GreaterThanOrEqualTo(0);
        validator.RuleFor(take).InclusiveBetween(1, OffsetPage.MaxTake);
    }
}
