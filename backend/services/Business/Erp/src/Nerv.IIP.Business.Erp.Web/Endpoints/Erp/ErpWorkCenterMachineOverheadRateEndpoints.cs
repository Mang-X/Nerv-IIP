    ...
    : ErpEndpoint<ConfigureWorkCenterMachineOverheadRateRequest, ResponseData<ConfigureWorkCenterMachineOverheadRateResponse>>
{
    ...
    public override async Task HandleAsync(ConfigureWorkCenterMachineOverheadRateRequest req, CancellationToken ct)
    {
        using var causationScope = eventContext.BeginScope(ErpCommandCausationIds.ForHttpCommand(
            "configure-work-center-machine-overhead-rate",
            req.OrganizationId,
            req.EnvironmentId,
            req.WorkCenterId,
            req.AccountingPeriodCode,
            req.Applicability,
            req.FixedOverheadBudget,
            req.VariableOverheadBudget,
            req.NormalCapacityMachineHours,
            req.CurrencyCode,
            req.Reason));
        var actor = eventContext.GetContext().Actor;
        var id = await sender.Send(new ConfigureWorkCenterMachineOverheadRateCommand(
            req.OrganizationId, req.EnvironmentId, req.WorkCenterId, req.AccountingPeriodCode,
            req.Applicability, req.FixedOverheadBudget, req.VariableOverheadBudget,
            req.NormalCapacityMachineHours, req.CurrencyCode, actor, req.Reason,
            timeProvider.GetUtcNow()), ct);
        await Send.OkAsync(new ConfigureWorkCenterMachineOverheadRateResponse(id).AsResponseData(), cancellation: ct);
    }
}

public sealed class ListWorkCenterMachineOverheadRatesEndpoint(ISender sender)
...
public sealed class GetCurrentWorkCenterMachineOverheadRateEndpoint(ISender sender)
