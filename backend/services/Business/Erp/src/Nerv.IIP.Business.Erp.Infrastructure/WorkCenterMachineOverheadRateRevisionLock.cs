...
public interface IWorkCenterMachineOverheadRateRevisionLock
{
    long GetLockKey(
...
public sealed class PostgreSqlWorkCenterMachineOverheadRateRevisionLock(ApplicationDbContext dbContext)
    : IWorkCenterMachineOverheadRateRevisionLock
{
        ...
        string workCenterId)
    {
        ...
        AppendScopePart(canonicalScope, "machine-overhead-rate");
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!string.Equals(dbContext.Database.ProviderName, "Npgsql.EntityFrameworkCore.PostgreSQL", StringComparison.Ordinal))
            throw new NotSupportedException("Machine-overhead rate revision allocation requires PostgreSQL advisory locks.");

        var currentTransaction = dbContext.Database.CurrentTransaction
            ?? throw new InvalidOperationException("Machine-overhead rate revision allocation requires a current EF transaction.");
