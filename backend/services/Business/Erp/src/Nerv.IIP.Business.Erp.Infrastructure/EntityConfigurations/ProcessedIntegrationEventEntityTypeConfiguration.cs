...
public sealed class ProcessedIntegrationEventEntityTypeConfiguration : IEntityTypeConfiguration<ProcessedIntegrationEvent>
{
    ...
    {
        builder.ToTable("processed_integration_events", table => table.HasComment("Integration events already processed by BusinessERP for idempotent consumption."));
        builder.HasKey(x => x.Id);
        builder.Property(x => x.Id).UseGuidVersion7ValueGenerator().HasComment("Processed integration event identifier.");
        builder.Property(x => x.ConsumerName).IsRequired().HasMaxLength(256).HasComment("BusinessERP integration event consumer name.");
        ...
        builder.Property(x => x.IdempotencyKey).IsRequired().HasMaxLength(512).HasComment("Deterministic BusinessERP idempotency key unique within a consumer.");
        builder.Property(x => x.ProcessedAtUtc).HasComment("UTC time when BusinessERP processed the event.");
