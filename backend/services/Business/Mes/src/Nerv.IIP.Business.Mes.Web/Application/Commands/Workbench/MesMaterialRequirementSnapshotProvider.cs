...
public sealed class HttpMesProductEngineeringMaterialRequirementSnapshotProvider(
    ...
    IMesMaterialAvailabilityReader materialAvailabilityReader,
    ...
    : IMesMaterialRequirementSnapshotProvider
{
    ...
    public HttpMesProductEngineeringMaterialRequirementSnapshotProvider(
        ...
        : this(
            ...
            new HttpMesMaterialAvailabilityReader(inventoryClient, null, inventoryOptions, internalTokenProvider),
    ...
    public HttpMesProductEngineeringMaterialRequirementSnapshotProvider(
        ...
        MesMaterialRequirementInventoryOptions? inventoryOptions,
        IInternalServiceTokenProvider? internalTokenProvider = null,
        ILogger<HttpMesProductEngineeringMaterialRequirementSnapshotProvider>? logger = null,
        IMemoryCache? uomConversionCache = null)
        : this(
            productEngineeringClient,
            new HttpMesMaterialAvailabilityReader(
                inventoryClient,
                masterDataClient,
                inventoryOptions,
                internalTokenProvider,
                uomConversionCache: uomConversionCache),
            internalTokenProvider,
            logger)
    {
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        var lines = await Task.WhenAll(requiredLines.Select(async line =>
        {
            var candidateMaterialIds = new[] { line.MaterialId }
                ...
                .ToArray();
            var availability = await materialAvailabilityReader.ReadAsync(
                new MesMaterialAvailabilityReadRequest(
                    ...
                    line.UomCode,
                    null,
                    DateOnly.FromDateTime(request.CapturedAtUtc.UtcDateTime)),
                cancellationToken);
            if (!availability.SourceAvailable)
            {
                throw new KnownException(
                    availability.FailureMessage
                    ?? "MATERIAL_REQUIREMENT_SOURCE_UNAVAILABLE: Inventory or MasterData material availability source is unavailable.");
            }

            if (availability.AvailableQuantity <= 0m)
            {
                logger?.LogWarning(
                    "MES material availability returned zero availability for materials {MaterialIds} required UOM {UomCode}.",
                    string.Join(',', candidateMaterialIds),
                    line.UomCode);
            }
