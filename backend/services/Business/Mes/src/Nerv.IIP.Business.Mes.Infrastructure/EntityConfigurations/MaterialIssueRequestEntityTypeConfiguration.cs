...
public sealed class MaterialIssueRequestEntityTypeConfiguration : IEntityTypeConfiguration<MaterialIssueRequest>
{
    public void Configure(EntityTypeBuilder<MaterialIssueRequest> builder)
    {
        ...
        builder.Property(x => x.PendingIssueLegPosted).HasColumnName("pending_issue_leg_posted").IsRequired().HasDefaultValue(false).HasComment("Whether the warehouse outbound leg of the in-flight line-side receipt has been posted by Inventory.");
        builder.Property(x => x.PendingIssueLegCount).HasColumnName("pending_issue_leg_count").IsRequired().HasDefaultValue(0).HasComment("Number of warehouse outbound posting details expected for the in-flight line-side receipt.");
        builder.Property(x => x.PendingIssueLegPostedIndexesJson).HasColumnName("pending_issue_leg_posted_indexes_json").HasColumnType("text").IsRequired().HasDefaultValue("[]").IsConcurrencyToken().HasComment("JSON array of warehouse outbound detail indexes already acknowledged by Inventory.");
        builder.Property(x => x.ReceiptUsesActualIssueValue).HasColumnName("receipt_uses_actual_issue_value").IsRequired().HasDefaultValue(false).HasComment("Whether this receipt waits for all actual warehouse issue values before requesting inbound; false preserves legacy in-flight transfers.");
        builder.Property(x => x.PendingReceiptIntentSent).HasColumnName("pending_receipt_intent_sent").IsRequired().HasDefaultValue(false).IsConcurrencyToken().HasComment("Whether the current receipt attempt has emitted its inbound intent in the same transaction as the outbox.");
        builder.Property(x => x.PendingReceiptLegPosted).HasColumnName("pending_receipt_leg_posted").IsRequired().HasDefaultValue(false).HasComment("Whether the line-side inbound leg of the in-flight line-side receipt has been posted by Inventory.");
        builder.Property(x => x.SourceSiteCode).HasColumnName("source_site_code").HasMaxLength(100).HasComment("Inventory site code the material is actually issued from, resolved from real stock holdings instead of a hardcoded namespace.");
        ...
        builder.Property(x => x.SourceAllocationsJson).HasColumnName("source_allocations_json").HasColumnType("text").IsRequired().HasDefaultValue("[]").HasComment("JSON array of actual source location, lot and quantity allocations used to emit multiple Inventory posting details.");
