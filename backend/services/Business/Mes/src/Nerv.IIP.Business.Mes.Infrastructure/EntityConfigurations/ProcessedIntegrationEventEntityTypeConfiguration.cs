...
public sealed class ProcessedIntegrationEventEntityTypeConfiguration : IEntityTypeConfiguration<ProcessedIntegrationEvent>
{
    ...
    {
        builder.ToTable("processed_integration_events", table => table.HasComment("Integration events already processed by BusinessMES for idempotent consumption."));
        ...
        builder.Property(x => x.DedupeKey).IsRequired().HasMaxLength(512).HasComment("BusinessMES dedupe key associated with the processed event.");
