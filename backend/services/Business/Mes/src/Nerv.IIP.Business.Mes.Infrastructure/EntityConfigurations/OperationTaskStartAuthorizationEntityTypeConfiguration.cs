    ...
    : IEntityTypeConfiguration<OperationTaskStartAuthorization>
{
    public void Configure(EntityTypeBuilder<OperationTaskStartAuthorization> builder)
    {
        ...
        builder.Property(x => x.ApprovalChainId).HasColumnName("approval_chain_id").IsRequired().HasMaxLength(150).HasComment("BusinessApproval chain whose approved decision authorizes this start.");
        ...
        builder.Property(x => x.AuthorizedBy).HasColumnName("authorized_by").IsRequired().HasMaxLength(200).HasComment("Canonical principal from the approved BusinessApproval decision.");
        ...
        builder.HasOne<OperationTask>()
            .WithMany()
            .HasPrincipalKey(x => new { x.OrganizationId, x.EnvironmentId, x.OperationTaskIdValue, x.WorkOrderId })
            .HasForeignKey(x => new { x.OrganizationId, x.EnvironmentId, x.OperationTaskId, x.WorkOrderId })
