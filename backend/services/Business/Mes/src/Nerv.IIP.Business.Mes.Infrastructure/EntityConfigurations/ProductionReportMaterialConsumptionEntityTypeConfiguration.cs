...
public sealed class ProductionReportMaterialConsumptionEntityTypeConfiguration : IEntityTypeConfiguration<ProductionReportMaterialConsumption>
{
    public void Configure(EntityTypeBuilder<ProductionReportMaterialConsumption> builder)
    {
        ...
        builder.Property(x => x.ConsumedQuantity).HasColumnName("consumed_quantity").HasPrecision(18, 6).IsRequired().HasComment("Consumed material quantity for this lot; reversal reports store a negative quantity to request Inventory line-side replenishment.");
        ...
        builder.Property(x => x.SiteCode).HasColumnName("site_code").HasMaxLength(100).HasComment("Inventory site code the material was consumed from, copied from the supplying material issue request line-side target.");
        builder.Property(x => x.LocationCode).HasColumnName("location_code").HasMaxLength(100).HasComment("Inventory location code the material was consumed from, copied from the supplying material issue request line-side target.");
        builder.Property(x => x.OwnerType).HasColumnName("owner_type").HasMaxLength(50).IsRequired().HasDefaultValue("production").HasComment("Frozen inventory owner type; historical consumption facts retain production ownership.");
        builder.Property(x => x.OwnerId).HasColumnName("owner_id").HasMaxLength(100).HasComment("Frozen inventory owner id; null for company and legacy production ownership.");
