...
public sealed class MesEndpointContractTests
{
    ...
    public async Task Production_report_rejects_non_completion_report_for_operation_outside_work_order()
    {
        ...
        var exception = await Assert.ThrowsAsync<KnownException>(() => new RecordProductionReportCommandHandler(dbContext, new NullMesOeeDimensionSnapshotProvider()).Handle(
    ...
    public async Task Mes_public_production_queries_return_reports_receipt_requests_and_capacity_impacts()
    {
        ...
        await dbContext.SaveChangesAsync(CancellationToken.None);
        var reportResult = await new RecordProductionReportCommandHandler(dbContext, new NullMesOeeDimensionSnapshotProvider()).Handle(
