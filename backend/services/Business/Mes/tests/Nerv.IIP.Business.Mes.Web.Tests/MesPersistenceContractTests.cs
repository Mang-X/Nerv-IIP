...
public sealed class MesPersistenceContractTests
{
    ...
    public async Task Operation_lifecycle_rejects_complete_before_start_and_pause_after_completion()
    {
        ...
        using var scope = services.CreateScope();
        var dbContext = scope.ServiceProvider.GetRequiredService<ApplicationDbContext>();
        var workOrder = ReleasedForReporting(WorkOrder.Create(
            "org-001",
            "env-dev",
            "WO-LIFE-001",
            "FG-FSA",
    ...
    public async Task Production_report_that_completes_operation_persists_operation_completion()
    {
        ...
        {
            var dbContext = scope.ServiceProvider.GetRequiredService<ApplicationDbContext>();
            dbContext.WorkOrders.Add(ReleasedForReporting(WorkOrder.Create("org-001", "env-dev", "WO-REPORT-COMPLETE-001", "FG-FSA", "PV-FSA-1", 10m, 20, now.AddHours(8))));
            dbContext.OperationTasks.Add(OperationTask.Create(
                "org-001",
                "env-dev",
    ...
    public async Task Completed_production_report_persists_registered_participant_labor_allocations()
    {
        ...
        {
            var dbContext = scope.ServiceProvider.GetRequiredService<ApplicationDbContext>();
            dbContext.WorkOrders.Add(ReleasedForReporting(WorkOrder.Create("org-001", "env-dev", "WO-COLLAB-001", "FG-001", "PV-001", 10m, 10, now.AddHours(8))));
            dbContext.OperationTasks.Add(OperationTask.Create(
    ...
    public async Task Stage_production_report_does_not_allocate_labor_and_legacy_assignee_falls_back_to_full_share_on_completion()
    {
        ...
        using var scope = services.CreateScope();
        var dbContext = scope.ServiceProvider.GetRequiredService<ApplicationDbContext>();
        dbContext.WorkOrders.Add(ReleasedForReporting(WorkOrder.Create("org-001", "env-dev", "WO-LEGACY-001", "FG-001", "PV-001", 10m, 10, now.AddHours(8))));
        var task = OperationTask.Create(
            "org-001", "env-dev", "WO-LEGACY-001", "OP-LEGACY-10",
            OperationTaskLifecycleStatus.InProgress, 10, "WC-001", [], now,
            TimeSpan.FromMinutes(40), now, null);
    ...
    public async Task Batch_traceability_reads_durable_material_consumption_facts()
    {
        ...
        {
            var dbContext = scope.ServiceProvider.GetRequiredService<ApplicationDbContext>();
            dbContext.WorkOrders.Add(ReleasedForReporting(WorkOrder.Create("org-001", "env-dev", "WO-BATCH-001", "FG-FSA", "PV-FSA-1", 10m, 20, now.AddHours(8))));
            dbContext.OperationTasks.Add(OperationTask.Create(
                "org-001",
                "env-dev",
                "WO-BATCH-001",
    ...
    [InlineData(true)]
    [InlineData(false)]
    public async Task Consumed_material_lot_traceability_returns_operator_and_device_on_both_read_faces(bool viaMaterialLotReadFace)
    {
        var services = CreateServices($"{nameof(Consumed_material_lot_traceability_returns_operator_and_device_on_both_read_faces)}-{viaMaterialLotReadFace}");
        var now = DateTimeOffset.Parse("2026-05-30T08:00:00Z");
        ...
        using (var scope = services.CreateScope())
        {
            ...
            await dbContext.SaveChangesAsync();
            var request = await dbContext.MaterialIssueRequests.SingleAsync();
            request.ConfirmAndPostLineSideReceipt(MaterialSupplyTestFixtures.Locations, now.AddMinutes(10), materialLotId: "LOT-CONSUME-A");
            await dbContext.SaveChangesAsync();
            ...
            dbContext.ProductionReports.Add(ProductionReport.Record(
                ...
                false,
                now.AddMinutes(30),
                oeeProjection: new ProductionReportOeeProjection("WC-FILL", "DEV-CNC-02", "PCS", null),
                reportedBy: "user-emp-020"));
        ...
        using var recreatedScope = services.CreateScope();
        var dbContextForQuery = recreatedScope.ServiceProvider.GetRequiredService<ApplicationDbContext>();
        var traceability = viaMaterialLotReadFace
            ? await new GetMaterialLotTraceabilityQueryHandler(dbContextForQuery)
                .Handle(new GetMaterialLotTraceabilityQuery("org-001", "env-dev", "LOT-CONSUME-A"), CancellationToken.None)
    ...
    public async Task Production_report_can_reference_consumed_material_lots_for_traceability()
    {
        ...
        {
            var dbContext = scope.ServiceProvider.GetRequiredService<ApplicationDbContext>();
            dbContext.WorkOrders.Add(ReleasedForReporting(WorkOrder.Create("org-001", "env-dev", "WO-TRACE-001", "FG-FSA", "PV-FSA-1", 10m, 20, now.AddHours(8))));
            dbContext.OperationTasks.Add(OperationTask.Create(
                "org-001",
                "env-dev",
                "WO-TRACE-001",
    ...
    public async Task Production_report_material_consumption_requires_received_line_side_request_reference()
    {
        ...
        using var scope = services.CreateScope();
        var dbContext = scope.ServiceProvider.GetRequiredService<ApplicationDbContext>();
        dbContext.WorkOrders.Add(ReleasedForReporting(WorkOrder.Create("org-001", "env-dev", "WO-CONSUME-001", "FG-FSA", "PV-FSA-1", 10m, 20, now.AddHours(8))));
        dbContext.OperationTasks.Add(OperationTask.Create(
            "org-001",
            "env-dev",
            "WO-CONSUME-001",
    ...
    public async Task Production_report_rejects_cumulative_consumption_that_exceeds_received_line_side_quantity()
    {
        ...
        using var scope = services.CreateScope();
        var dbContext = scope.ServiceProvider.GetRequiredService<ApplicationDbContext>();
        dbContext.WorkOrders.Add(ReleasedForReporting(WorkOrder.Create("org-001", "env-dev", "WO-CUM-001", "FG-FSA", "PV-FSA-1", 10m, 20, now.AddHours(8))));
        dbContext.OperationTasks.Add(OperationTask.Create(
            "org-001",
            "env-dev",
            "WO-CUM-001",
    ...
    public async Task Production_report_rejects_line_side_request_from_another_work_order_or_operation()
    {
        ...
        using var scope = services.CreateScope();
        var dbContext = scope.ServiceProvider.GetRequiredService<ApplicationDbContext>();
        dbContext.WorkOrders.Add(ReleasedForReporting(WorkOrder.Create("org-001", "env-dev", "WO-REPORT-001", "FG-FSA", "PV-FSA-1", 10m, 20, now.AddHours(8))));
        dbContext.OperationTasks.Add(OperationTask.Create(
            "org-001",
            "env-dev",
            "WO-REPORT-001",
    ...
    public async Task Production_report_rejects_operation_task_from_another_work_order()
    {
        ...
        using var scope = services.CreateScope();
        var dbContext = scope.ServiceProvider.GetRequiredService<ApplicationDbContext>();
        dbContext.WorkOrders.Add(ReleasedForReporting(WorkOrder.Create("org-001", "env-dev", "WO-REPORT-A", "FG-FSA", "PV-FSA-1", 10m, 20, now.AddHours(8))));
        dbContext.WorkOrders.Add(ReleasedForReporting(WorkOrder.Create("org-001", "env-dev", "WO-REPORT-B", "FG-FSA", "PV-FSA-1", 10m, 20, now.AddHours(8))));
        dbContext.OperationTasks.Add(OperationTask.Create(
            "org-001",
            "env-dev",
            "WO-REPORT-B",
    ...
    public async Task Production_report_idempotency_fingerprint_includes_consumed_material_lots()
    {
        ...
        using var scope = services.CreateScope();
        var dbContext = scope.ServiceProvider.GetRequiredService<ApplicationDbContext>();
        dbContext.WorkOrders.Add(ReleasedForReporting(WorkOrder.Create("org-001", "env-dev", "WO-IDEMP-001", "FG-FSA", "PV-FSA-1", 10m, 20, now.AddHours(8))));
        dbContext.OperationTasks.Add(OperationTask.Create(
            "org-001",
            "env-dev",
            "WO-IDEMP-001",
    ...
    public async Task Production_report_rejects_duplicate_consumed_material_lot_for_same_report()
    {
        ...
        using var scope = services.CreateScope();
        var dbContext = scope.ServiceProvider.GetRequiredService<ApplicationDbContext>();
        dbContext.WorkOrders.Add(ReleasedForReporting(WorkOrder.Create("org-001", "env-dev", "WO-DUP-001", "FG-FSA", "PV-FSA-1", 10m, 20, now.AddHours(8))));
        dbContext.OperationTasks.Add(OperationTask.Create(
            "org-001",
            "env-dev",
            "WO-DUP-001",
    ...
    /// </summary>
    private static WorkOrder ReleasedForReporting(WorkOrder workOrder)
    {
        workOrder.MarkReleased();
