...
public sealed class MesKnownExceptionMessageArchitectureTests
{
    ...
    [
        ...
        Excluded("backend/services/Business/Mes/src/Nerv.IIP.Business.Mes.Web/Application/Quality/MesFirstArticleGate.cs", "UnconfiguredMesFirstArticleGate", "EnsureBatchReportAllowedAsync", 1, "稳定错误码 FIRST_ARTICLE_SOURCE_UNAVAILABLE"),
    ...
    public void Static_target_and_exclusion_ledger_matches_the_current_MES_source()
    {
        ...
        Assert.Equal(68, discovered.Count);
        Assert.Equal(162, discovered.Sum(site => site.DirectKnownExceptionCount));
        Assert.Equal(158, documents.Sum(document => CountOccurrences(document.Text, "new KnownException")));
        ...
        Assert.Equal(17, ExpectedLedger.Where(site => site.Kind == MesKnownExceptionSiteKind.Target).Sum(site => site.DirectKnownExceptionCount));
    }
