...
internal static class MesMaterialRequirementEfSourceFixtures
{
    public static TheoryData<string, string> EquivalentEfReadBypasses => new()
    {
        ...
        { "FromSql", "var rows = dbContext.Set<MaterialRequirement>().FromSql($\"SELECT * FROM mes.material_requirements\");" },
        { "FromSqlInterpolated", "var rows = dbContext.Set<MaterialRequirement>().FromSqlInterpolated($\"SELECT * FROM mes.material_requirements\");" },
        ...
        { "generic FindAsync no-cancellation overload", "var row = dbContext.FindAsync<MaterialRequirement>([new object()]);" },
        ...
        { "Type FindAsync no-cancellation overload", "var row = dbContext.FindAsync(typeof(MaterialRequirement), [new object()]);" },
    ...
    public static TheoryData<string, string> KnownNonTargetEfReadShapes => new()
    {
        ...
        { "FindAsync OtherEntity no-cancellation overload", "var row = dbContext.FindAsync<OtherEntity>([new object()]);" },
        ...
        { "runtime Type FindAsync OtherEntity", "var row = dbContext.FindAsync(typeof(OtherEntity), [new object()], CancellationToken.None);" },
        { "runtime Type FindAsync OtherEntity no-cancellation overload", "var row = dbContext.FindAsync(typeof(OtherEntity), [new object()]);" },
