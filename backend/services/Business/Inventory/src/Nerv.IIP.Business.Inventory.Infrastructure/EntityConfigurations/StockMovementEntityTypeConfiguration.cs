...
public sealed class StockMovementEntityTypeConfiguration : IEntityTypeConfiguration<StockMovement>
{
    public void Configure(EntityTypeBuilder<StockMovement> builder)
    {
        ...
        builder.Property(x => x.RequestedUnitCost).HasColumnName("requested_unit_cost").HasPrecision(18, 6).HasComment("Caller-supplied unit cost exactly as posted, never overwritten by valuation; null means the caller left costing to the ledger. Idempotency payload comparison uses this column instead of unit_cost.");
