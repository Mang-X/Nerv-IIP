...
public sealed class InventoryReservationExpirationTests
{
    ...
    [Fact]
    public async Task Expired_reservation_releases_the_ledger_and_raises_an_expiration_event()
    {
        await using var dbContext = CreateContext();
        var ledger = CreateLedger();
        ...
        Assert.Contains(reservation.GetDomainEvents(), x => x is StockReservationExpiredDomainEvent);
    }
    ...
    private static ApplicationDbContext CreateContext()
    {
        var options = new DbContextOptionsBuilder<ApplicationDbContext>()
            ...
            .Options;
        return new ApplicationDbContext(options, new ReservationExpiryNoopMediator());
    }
