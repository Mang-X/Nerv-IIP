...
/// <summary>
/// #3805：产品基线 seed（<c>Inventory:Seed:Enabled</c>）默认开启，不再依赖
/// <c>Persistence:AutoMigrate</c> 或部署方显式配置；显式 <c>false</c> 仍可关闭。
/// </summary>
public sealed class InventoryProductBaselineSeedDefaultTests
{
    [Fact]
    public async Task Product_baseline_seed_runs_when_the_switch_is_not_set()
    {
        ...
        Assert.True(
            await dbContext.StockLocations.AnyAsync(),
            "Inventory:Seed:Enabled 未设置时，产品基线 seed 应默认运行。");
    }
    ...
    public async Task Product_baseline_seed_does_not_run_when_explicitly_disabled()
    {
        ...
        Assert.False(
            await dbContext.StockLocations.AnyAsync(),
            "Inventory:Seed:Enabled=false 时，产品基线 seed 不应运行。");
    }
    ...
    private static WebApplicationFactory<Program> CreateFactory(string databaseName, bool? seedEnabled)
    {
        return new WebApplicationFactory<Program>()
            .WithWebHostBuilder(builder =>
            {
                ...
                {
                    builder.UseSetting("Inventory:Seed:Enabled", seedEnabled.Value ? "true" : "false");
                }
