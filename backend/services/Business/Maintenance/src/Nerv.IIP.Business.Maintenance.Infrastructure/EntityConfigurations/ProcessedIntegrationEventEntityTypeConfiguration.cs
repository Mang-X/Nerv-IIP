...
public sealed class ProcessedIntegrationEventEntityTypeConfiguration : IEntityTypeConfiguration<ProcessedIntegrationEvent>
{
    ...
    {
        builder.ToTable("processed_integration_events", table => table.HasComment("Integration events already processed by BusinessMaintenance for idempotent consumption."));
        builder.HasKey(x => x.Id);
        builder.Property(x => x.Id).UseGuidVersion7ValueGenerator().HasComment("Processed integration event identifier.");
        builder.Property(x => x.ConsumerName).IsRequired().HasMaxLength(256).HasComment("BusinessMaintenance integration event consumer name.");
        builder.Property(x => x.EventId).IsRequired().HasMaxLength(256).HasComment("Source integration event identifier unique within a consumer.");
        ...
        builder.Property(x => x.DedupeKey).IsRequired().HasMaxLength(512).HasComment("BusinessMaintenance dedupe key associated with the processed event.");
        builder.Property(x => x.ProcessedAtUtc).HasComment("UTC time when BusinessMaintenance processed the event.");
