...
public sealed class MaintenanceIntegrationEventTests
{
    ...
    public void Asset_unavailable_converter_matches_common_contract_shape()
    {
        ...
        workOrder.MarkAssetUnavailable(fromUtc, "DT-OVER-TEMP");

        var integrationEvent = new AssetUnavailableIntegrationEventConverter().Convert(new AssetUnavailableDomainEvent(workOrder, "DT-OVER-TEMP", fromUtc));
        ...
        Assert.Equal("DT-OVER-TEMP", integrationEvent.Payload.Reason);
    ...
    public void Asset_restored_and_local_work_order_events_use_required_event_types()
    {
        ...
        workOrder.MarkAssetUnavailable(DateTimeOffset.UtcNow, "DT-PM");
