...
/// - <c>LeaderDemo:History:Scale</c>：缩放比例，<c>1.0</c> = 全量约 3200 单，<c>0.1</c> = 约 320 单快速验证。
/// - <c>LeaderDemo:History:AsOfDate</c>：历史截止日（<c>yyyy-MM-dd</c>），缺省取当前 UTC 日期。
///   AppHost 会把同一个值同时发给 ERP / MES / Quality / Approval——各服务若跨零点分别启动而各自取「今天」，
///   审批单引用的 <c>PO-2026-*</c> / <c>NCR-2026-*</c> 号段会与源服务错开一周，跨库配对随之断裂。
///
/// 与 ERP / MES / Quality 侧按同一字面量重复声明。
/// </summary>
