...
public sealed record WmsSourceDocument(string Path, string Text);

public sealed record WmsKnownExceptionSite(
    string Path,
    string TypeName,
    string MethodName,
    int DirectKnownExceptionCount,
    WmsKnownExceptionSiteKind Kind,
    string Reason)
{
    public string Key => $"{Path}|{TypeName}|{MethodName}";
}

public sealed record WmsExcludedSite(string Path, string TypeName, string MethodName, string Reason);

public enum WmsKnownExceptionSiteKind
{
    Target,
    Excluded,
}

public static class WmsUserMessageSourceAnalyzer
{
    ...
    public static IReadOnlyList<WmsKnownExceptionSite> Discover(
        IReadOnlyCollection<WmsSourceDocument> documents)
    {
        if (documents.Count == 0)
        {
            throw new InvalidOperationException("Wms KnownException 源集合不能为空。");
        }

        var sourceTrees = ParseTrees(documents);
        var compilation = CreateCompilation(sourceTrees);
        var discovered = new List<WmsKnownExceptionSite>();
        ...
        foreach (var syntaxTree in sourceTrees)
        {
            ...
            foreach (var method in syntaxTree.GetRoot().DescendantNodes().OfType<MethodDeclarationSyntax>())
            {
                ...
                discovered.Add(new WmsKnownExceptionSite(
                    NormalizePath(syntaxTree.FilePath),
                    typeName,
                    method.Identifier.ValueText,
                    directKnownExceptionCount,
                    WmsKnownExceptionSiteKind.Excluded,
                    "discovered Wms KnownException site"));
            }
        }
    ...
    public static IReadOnlyList<string> Analyze(
        IReadOnlyCollection<WmsSourceDocument> documents,
        IReadOnlyCollection<WmsExcludedSite> excludedSites)
    {
        if (documents.Count == 0)
        {
            throw new InvalidOperationException("Wms 用户消息源集合不能为空。");
        }
    ...
    private static SyntaxTree[] ParseTrees(IReadOnlyCollection<WmsSourceDocument> documents) =>
        documents
    ...
    private static CSharpCompilation CreateCompilation(IReadOnlyCollection<SyntaxTree> sourceTrees)
    {
        var syntaxTrees = sourceTrees
            .Append(CSharpSyntaxTree.ParseText(
                ...
                path: "__WmsGlobalUsings.g.cs"))
            .ToArray();
        return CSharpCompilation.Create(
            "WmsUserMessageArchitecture",
            syntaxTrees,
