...
public sealed class ProcessedIntegrationEventEntityTypeConfiguration : IEntityTypeConfiguration<ProcessedIntegrationEvent>
{
    ...
    {
        builder.ToTable("processed_integration_events", table => table.HasComment("Integration events already processed by BusinessWMS for idempotent consumption."));
        builder.HasKey(x => x.Id);
        builder.Property(x => x.Id).UseGuidVersion7ValueGenerator().HasComment("Processed integration event identifier.");
        builder.Property(x => x.ConsumerName).IsRequired().HasMaxLength(256).HasComment("BusinessWMS integration event consumer name.");
        ...
        builder.Property(x => x.IdempotencyKey).IsRequired().HasMaxLength(512).HasComment("Deterministic BusinessWMS idempotency key unique within a consumer.");
        builder.Property(x => x.ProcessedAtUtc).HasComment("UTC time when BusinessWMS processed the event.");
