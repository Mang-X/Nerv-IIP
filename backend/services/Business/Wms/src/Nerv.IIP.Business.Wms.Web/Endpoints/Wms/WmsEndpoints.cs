...
public sealed record ListReceivingQualityGatesRequest(string? OrganizationId, string? EnvironmentId, int Skip = 0, int Take = 100, string? GateStatus = null, string? Keyword = null, bool IncludeNotRequired = false);
...
public sealed class ListReceivingQualityGatesEndpoint(ISender sender) : WmsEndpoint<ListReceivingQualityGatesRequest, ResponseData<ListReceivingQualityGatesResponse>>
{
    ...
    {
        var response = await sender.Send(new ListReceivingQualityGatesQuery(req.OrganizationId, req.EnvironmentId, req.Skip, req.Take, req.GateStatus, req.Keyword, req.IncludeNotRequired), ct);
        await Send.OkAsync(response.AsResponseData(), cancellation: ct);
    }
}
