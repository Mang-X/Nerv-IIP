...
public sealed class CreatePickingTaskCommandHandler(ApplicationDbContext dbContext)
    : ICommandHandler<CreatePickingTaskCommand, WarehouseTaskId>
{
    public async Task<WarehouseTaskId> Handle(CreatePickingTaskCommand request, CancellationToken cancellationToken)
    {
        ...
        var task = outbound.CreatePickingTask(request.TaskNo, request.LineNo, request.FromLocationCode, request.ToLocationCode, request.Quantity);
        dbContext.WarehouseTasks.Add(task);
        return task.Id;
    }
}
...
public sealed record CompleteWcsTaskCommand(string ExternalTaskId, string CompletionPayloadJson) : ICommand;
    ...
    : ICommandHandler<CompleteWcsTaskCommand>
{
    ...
    {
        var task = await dbContext.WcsTasks.SingleOrDefaultAsync(x => x.ExternalTaskId == request.ExternalTaskId, cancellationToken)
            ?? throw new KnownException($"WCS task was not found: {request.ExternalTaskId}");
        task.Complete(request.CompletionPayloadJson);
    }
}

public sealed record FailWcsTaskCommand(string ExternalTaskId, string FailureCode, string FailureMessage) : ICommand;
