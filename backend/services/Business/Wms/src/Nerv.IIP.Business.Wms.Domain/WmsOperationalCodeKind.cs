...
///
/// **这条守卫的可达面要说精确：它是「构造期可达」，不是生产数据流上可达。** 已声明的三类单号
/// 都满足它，所以正常运行时永远不触发；真正会撞上它的是「声明一个新种类、而它最窄的承载列
/// 放不下回落形态」——那发生在静态初始化期。当前另一条可达路径是
/// <see cref="CreateForContractProbe"/> 这个只给契约测试用的探针。
///
