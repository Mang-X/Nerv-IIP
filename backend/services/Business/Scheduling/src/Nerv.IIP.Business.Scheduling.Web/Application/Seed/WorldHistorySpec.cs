...
///
/// 这是 ERP / MES / DemandPlanning / Scheduling 之间唯一的约定：各侧用同一 <c>(asOfDate, scale)</c>
/// 调用 <see cref="BuildOrderPlans"/>，必须逐字段得到同一张订单计划表，
/// 于是 <c>SO-2026-#####</c> 与 <c>WO-2026-#####</c> 天然配对，无需跨服务查询或外键
/// （排产库不允许跨库查 MES）。各侧按同一字面量重复声明本类型，各有黄金向量测试防止漂移。
///
...
public static class WorldHistorySpec
{
    ...
    /// <summary>本次生成覆盖的总订单数（跨 ERP/MES/计划/排产必须一致）。</summary>
    ...
    /// <summary>
    /// 生成全量订单计划表。ERP 按它写销售侧全链，MES 按它写工单侧全链，排产按它选可排工单；
    /// 各侧不通信、不查库，仅靠本函数的确定性达成一致。
    /// </summary>
...
/// <summary>单张历史订单的跨服务计划（各侧必须逐字段一致）。</summary>
