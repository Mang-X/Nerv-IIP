...
/// <summary>
/// L1 背景历史引擎的开关与参数（与 ERP / MES / Quality / DemandPlanning 同一字面量重复声明）。
///
/// - <c>LeaderDemo:History:Enabled</c>：总开关（AppHost 在 leader-demo profile 下默认开）。
/// - <c>LeaderDemo:History:Scale</c>：缩放比例，<c>1.0</c> = 全量，<c>0.1</c> = 快速验证。
/// - <c>LeaderDemo:History:AsOfDate</c>：历史截止日（<c>yyyy-MM-dd</c>），缺省取当前 UTC 日期。
///   AppHost 会把同一个值同时发给 ERP / MES / DemandPlanning / Scheduling——若各自取「今天」，
///   订单计划表会错开一周，排产方案里的 <c>WO-2026-*</c> 与 MES 工单的配对随之断裂。
/// </summary>
