...
///
/// 关键性质：**按流键取流**。每张单据用自己的业务编码作为流键（如 <c>SP-2026-0042</c>），
/// 因此同一份产出的内容与生成顺序、缩放比例、其他单据的存在与否都无关——
/// <c>LeaderDemo:History:Scale=0.1</c> 的快速验证跑出来的第 42 个方案，与全量跑出来的第 42 个方案逐字段相同。
///
/// ERP / MES / Quality / DemandPlanning / Scheduling 各侧按同一字面量重复声明本类型，
/// 各有黄金向量测试防止漂移（与 L0 <c>WorldBibleSpec</c> 的跨服务一致性策略相同）。
/// </summary>
