...
public sealed record SchedulingSourceDocument(string Path, string Text);

public sealed record SchedulingKnownExceptionSite(
    string Path,
    string TypeName,
    string MethodName,
    int DirectKnownExceptionCount,
    SchedulingKnownExceptionSiteKind Kind,
    string Reason)
{
    public string Key => $"{Path}|{TypeName}|{MethodName}";
}

public sealed record SchedulingExcludedSite(string Path, string TypeName, string MethodName, string Reason);

public enum SchedulingKnownExceptionSiteKind
{
    Target,
    Excluded,
}

public static class SchedulingUserMessageSourceAnalyzer
{
    ...
    public static IReadOnlyList<SchedulingKnownExceptionSite> Discover(
        IReadOnlyCollection<SchedulingSourceDocument> documents)
    {
        if (documents.Count == 0)
        {
            throw new InvalidOperationException("Scheduling KnownException 源集合不能为空。");
        }

        var sourceTrees = ParseTrees(documents);
        var compilation = CreateCompilation(sourceTrees);
        var discovered = new List<SchedulingKnownExceptionSite>();
        ...
        foreach (var syntaxTree in sourceTrees)
        {
            ...
            foreach (var method in syntaxTree.GetRoot().DescendantNodes().OfType<MethodDeclarationSyntax>())
            {
                ...
                discovered.Add(new SchedulingKnownExceptionSite(
                    NormalizePath(syntaxTree.FilePath),
                    typeName,
                    method.Identifier.ValueText,
                    directKnownExceptionCount,
                    SchedulingKnownExceptionSiteKind.Excluded,
                    "discovered Scheduling KnownException site"));
            }
        }
    ...
    public static IReadOnlyList<string> Analyze(
        IReadOnlyCollection<SchedulingSourceDocument> documents,
        IReadOnlyCollection<SchedulingExcludedSite> excludedSites)
    {
        if (documents.Count == 0)
        {
            throw new InvalidOperationException("Scheduling 用户消息源集合不能为空。");
        }
    ...
    private static SyntaxTree[] ParseTrees(IReadOnlyCollection<SchedulingSourceDocument> documents) =>
        documents
    ...
    private static CSharpCompilation CreateCompilation(IReadOnlyCollection<SyntaxTree> sourceTrees)
    {
        var syntaxTrees = sourceTrees
            .Append(CSharpSyntaxTree.ParseText(
                ...
                path: "__SchedulingGlobalUsings.g.cs"))
            .ToArray();
        return CSharpCompilation.Create(
            "SchedulingUserMessageArchitecture",
            syntaxTrees,
