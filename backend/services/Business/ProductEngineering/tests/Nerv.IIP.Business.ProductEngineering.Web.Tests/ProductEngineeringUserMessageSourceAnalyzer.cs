...
public sealed record ProductEngineeringSourceDocument(string Path, string Text);

public sealed record ProductEngineeringExcludedQuerySite(string Path, string TypeName, string MethodName, string Reason);

public static class ProductEngineeringUserMessageSourceAnalyzer
{
    ...
    public static IReadOnlyList<string> Analyze(
        ...
        IReadOnlyCollection<ProductEngineeringExcludedQuerySite> excludedSites)
    {
        if (documents.Count == 0)
        {
            throw new InvalidOperationException("ProductEngineering 查询用户消息源集合不能为空。");
        }
        ...
        var compilation = CSharpCompilation.Create(
            "ProductEngineeringUserMessageArchitecture",
            syntaxTrees,
