...
/// <summary>
/// 《工厂世界观设定集》§1 工作制 + §7 历史节奏的工作日历（ProductEngineering 侧）。
///
/// - 上线日 2026-01-05（周一），电子历史自此开始，与 L0 <see cref="WorldBibleSpec.V1EffectiveDate"/> 同值；
/// - 周日停产保养（设定集 §1），周一至周六为工作日；
/// - 春节 2026-02-09–02-22 低谷：工程侧同样放缓（评审开不起来，变更单与文档改版都少）；
/// - 早班 08:00–16:00 / 中班 16:00–24:00（Asia/Shanghai，UTC+8），
///   即 UTC 的 00:00–08:00 与 08:00–16:00——同一 UTC 自然日内，跨时区不会把动作甩到班次外。
///
/// 与 ERP / MES / Quality / Approval 侧按同一字面量重复声明，各侧各有黄金向量测试防止漂移。
/// </summary>
public static class WorldHistoryCalendar
{
    /// <summary>平台上线日（设定集 §1），与 L0 <c>WorldBibleSpec.V1EffectiveDate</c> 同值。</summary>
    ...
    /// <summary>春节周产出系数，设定集 §7 的「低谷」。</summary>
