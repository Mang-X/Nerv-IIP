...
public sealed class ZplTcpLabelPrinterTests
{
    ...
    public async Task Failure_before_the_first_byte_reports_failed()
    {
        ...
        var result = await CreateScriptedPrinter(factory).PrintAsync(
            "printer-01",
            CompileDocuments(1),
    ...
    public async Task Failure_after_a_partial_write_reports_delivery_unknown_without_retrying()
    {
        var connection = new ScriptedConnection(
            buffer => Math.Min(3, buffer.Length),
            _ => throw new IOException("scripted disconnect"));
        var factory = new ScriptedConnectionFactory { Connection = connection };
    ...
    public async Task Caller_cancellation_before_the_first_byte_propagates_with_a_failed_attempt_for_persistence()
    {
        ...
        Assert.Equal("failed", exception.AttemptResult.Status);
        Assert.Equal("TCP 传输在首字节写入前失败。", exception.AttemptResult.FailureReason);
        Assert.Equal(1, factory.ConnectCalls);
    }
    ...
    public async Task Caller_cancellation_after_the_first_byte_reports_delivery_unknown_without_retrying()
    {
        ...
        Assert.Equal("delivery-unknown", exception.AttemptResult.Status);
        Assert.Equal(1, factory.ConnectCalls);
        Assert.Equal(2, connection.SendCalls);
    }
    ...
    private static IReadOnlyCollection<CompiledLabelDocument> CompileDocuments(int count)
    {
        ...
        var items = Enumerable.Range(1, count)
            .Select(sequence => new LabelCompilationItem(
                "{}",
                new PlainLabelBarcodePayload(PlainLabelBarcodeType.Code128, $"LABEL-{sequence:000}"),
                sequence,
                "ASN-001"))
            .ToArray();
        return ZplV1LabelCompiler.CompileBatch(template, schema, items);
    }

    private static async Task<byte[]> ReceiveUntilEofAsync(
    ...
    private sealed class ScriptedConnectionFactory : IZplTcpConnectionFactory
    {
        ...
        public Task<IZplTcpConnection> ConnectAsync(
            string host,
            int port,
            TimeSpan timeout,
            CancellationToken cancellationToken)
    ...
    private sealed class BudgetAwareScriptedConnection(TimerRegistrationObservingTimeProvider clock)
        : IZplTcpConnection
    {
        ...
        {
            SendCalls++;
            clock.Advance(SendCalls == 1 ? TimeSpan.FromMilliseconds(800) : TimeSpan.FromMilliseconds(300));
            cancellationToken.ThrowIfCancellationRequested();
    ...
    private sealed class ScriptedConnection(params Func<ReadOnlyMemory<byte>, int>[] steps)
        : IZplTcpConnection
    {
        ...
        public ValueTask<int> SendAsync(ReadOnlyMemory<byte> payload, CancellationToken cancellationToken)
        {
            ...
            return ValueTask.FromResult(
                remainingSteps.Count == 0 ? payload.Length : remainingSteps.Dequeue()(payload));
        }
