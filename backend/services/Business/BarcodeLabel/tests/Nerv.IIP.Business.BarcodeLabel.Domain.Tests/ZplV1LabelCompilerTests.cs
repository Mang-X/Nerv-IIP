...
public sealed class ZplV1LabelCompilerTests
{
    ...
    [InlineData(300)]
    [InlineData(600)]
    public void Template_parser_accepts_approved_dpi(int dpi)
    {
    ...
    public void Compile_emits_the_approved_zpl_v1_barcode_fragments(
        ...
        string dataFragment)
    {
        var document = Compile(barcodeType, item);
        var zpl = Encoding.UTF8.GetString(document.Payload.Span);
        ...
        string expectedDataFragment)
    {
        ...
        var document = Compile(barcodeType, Gs1Item(gs1));
        var zpl = Encoding.UTF8.GetString(document.Payload.Span);
    ...
    {
        var code128 = Encoding.UTF8.GetString(Compile("code128", PlainItem("{\"skuCode\":\"SKU-001\"}", "MAT-0001")).Payload.Span);
        var dataMatrix = Encoding.UTF8.GetString(Compile("datamatrix", PlainItem("{\"skuCode\":\"SKU-001\"}", "DM-SN0001")).Payload.Span);
    ...
    {
        var item = PlainItem("{\"skuCode\":\"SKU-001\"}", "MAT-0001");
    ...
    [Fact]
    public void Template_parser_accepts_the_inclusive_numeric_boundaries()
    {
        var template = Template()
            .Replace("\"x\":10", "\"x\":0", StringComparison.Ordinal)
            .Replace("\"y\":20", "\"y\":32767", StringComparison.Ordinal)
            .Replace("\"fontHeight\":30", "\"fontHeight\":32767", StringComparison.Ordinal)
            .Replace("\"fontWidth\":30", "\"fontWidth\":1", StringComparison.Ordinal)
            .Replace("\"widthDots\":812", "\"widthDots\":32767", StringComparison.Ordinal)
            .Replace("\"heightDots\":406", "\"heightDots\":1", StringComparison.Ordinal);

        var parsed = LabelTemplateDocument.Parse(template);

        Assert.Equal(203, parsed.Media.Dpi);
        Assert.Equal(2, parsed.Fields.Length);
    }

    [Fact]
    public void Template_parser_rejects_more_than_64_fields()
    {
        var textFields = string.Join(',', Enumerable.Range(0, 64).Select(index =>
            $$"""{"kind":"text","x":{{index}},"y":0,"fontHeight":1,"fontWidth":1,"variable":"skuCode"}"""));
        var template = $$"""
            {"format":"nerv-iip.label-template","version":1,"media":{"dpi":203,"widthDots":812,"heightDots":406},"fields":[{{textFields}},{{BarcodeField}}]}
            """;
    ...
    [Fact]
    public void Template_parser_accepts_exactly_64_fields()
    {
        var textFields = string.Join(',', Enumerable.Range(0, 63).Select(index =>
            $$"""{"kind":"text","x":{{index}},"y":0,"fontHeight":1,"fontWidth":1,"variable":"skuCode"}"""));
        var template = $$"""
            {"format":"nerv-iip.label-template","version":1,"media":{"dpi":203,"widthDots":812,"heightDots":406},"fields":[{{textFields}},{{BarcodeField}}]}
            """;

        Assert.Equal(64, LabelTemplateDocument.Parse(template).Fields.Length);
    }
    ...
    public void Binder_rejects_missing_invalid_extra_duplicate_or_reserved_business_values(string valuesJson)
    {
        ...
        Assert.Throws<ArgumentException>(() => LabelTemplateBinder.BindBatch(template, schema, [PlainItem(valuesJson, "MAT-0001")]));
    }
    ...
    public void Binder_enforces_explicit_max_length_and_allows_missing_optional_values()
    {
        ...
        Assert.Throws<ArgumentException>(() => LabelTemplateBinder.BindBatch(template, requiredSchema, [PlainItem("{\"skuCode\":\"LONG\"}", "MAT-0001")]));
        var bound = LabelTemplateBinder.BindBatch(template, optionalSchema, [PlainItem("{}", "MAT-0001")]);
    ...
    {
        var template = LabelTemplateDocument.Parse(Template());
        var schema = LabelVariableSchema.Parse(Schema(maxLength: 200));

        Assert.Throws<ArgumentException>(() => LabelTemplateBinder.BindBatch(template, schema, [PlainItem(valuesJson, labelValue)]));
    }

    [Fact]
    public void Compiler_validates_the_complete_batch_before_returning_documents()
    {
        var template = LabelTemplateDocument.Parse(Template());
        var schema = LabelVariableSchema.Parse(Schema());
        ...
        Assert.Throws<ArgumentException>(() => ZplV1LabelCompiler.CompileBatch(
            ...
            schema,
            "code128",
            [PlainItem("{\"skuCode\":\"SKU-001\"}", "MAT-0001"), PlainItem("{}", "MAT-0002")]));
    }
    ...
    {
        var template = LabelTemplateDocument.Parse(Template());
        var schema = LabelVariableSchema.Parse(Schema(maxLength: 262144));
        var largeValue = new string('A', 262144);

        Assert.Throws<ArgumentException>(() => ZplV1LabelCompiler.CompileBatch(
            template,
            schema,
            "code128",
            [PlainItem($$"""{"skuCode":"{{largeValue}}"}""", "MAT-0001")]));
    }
    ...
    public void Compiler_rejects_an_unknown_barcode_type()
    {
        ...
        Assert.Throws<ArgumentException>(() => ZplV1LabelCompiler.CompileBatch(
            ...
            schema,
            "pdf417",
            [PlainItem("{\"skuCode\":\"SKU-001\"}", "MAT-0001")]));
    }
    ...
    [Fact]
    public void Compiler_preserves_parentheses_that_are_part_of_a_gs1_value()
    {
        var gs1 = new Gs1BarcodeValue("09501101530003", "LOT(A)", "789012", null);

        var zpl = Encoding.UTF8.GetString(Compile("gs1-128", Gs1Item(gs1)).Payload.Span);

        Assert.Contains("10LOT(A)>821789012", zpl, StringComparison.Ordinal);
    ...
    public static TheoryData<string, LabelCompilationItem, string, string> GoldenBarcodeCases()
    {
        ...
        {
            { "code128", PlainItem("{\"skuCode\":\"SKU-001\"}", "MAT-0001"), "^BCN,100,Y,N,N,A", "^FD>:MAT-0001^FS" },
            { "gs1-128", Gs1Item(gs1), "^BCN,100,Y,N,N,A", "^FD>;>8010950110153000310123456>821789012^FS" },
            { "qr", PlainItem("{\"skuCode\":\"SKU-001\"}", "https://example.invalid/items/SN0001"), "^BQ,2,6,Q,7", "^FDQA,https://example.invalid/items/SN0001^FS" },
            { "datamatrix", PlainItem("{\"skuCode\":\"SKU-001\"}", "DM-SN0001"), "^BXN,6,200", "^FDDM-SN0001^FS" },
    ...
    private static LabelCompilationItem PlainItem(string valuesJson, string labelValue) =>
        new(valuesJson, new LabelReservedVariables(labelValue, null, 1, "DOC-001"));
    ...
    private static string Schema(string? variable = null, int? maxLength = null)
    {
        variable ??= $$"""{"name":"skuCode","label":"物料编码","type":"string"{{(maxLength is null ? string.Empty : $",\"maxLength\":{maxLength}")}}}""";
        return $$"""{"version":1,"variables":[{{variable}}]}""";
    }

    private static string Template(int dpi = 203, string textVariable = "skuCode") => $$"""
        {"format":"nerv-iip.label-template","version":1,"media":{"dpi":{{dpi}},"widthDots":812,"heightDots":406},"fields":[{"kind":"text","x":10,"y":20,"fontHeight":30,"fontWidth":30,"variable":"{{textVariable}}"},{{BarcodeField}}]}
        """;
