...
namespace Nerv.IIP.Business.BarcodeLabel.Infrastructure.Migrations
{
    ...
    [Migration("20260905031436_RejectBlankLabelPrintBatchReplaySnapshots")]
    partial class RejectBlankLabelPrintBatchReplaySnapshots
    {
        /// <inheritdoc />
                    ...
                    b.ToTable("label_print_batches", "barcode", t =>
                        {
                            ...
                            t.HasCheckConstraint("ck_label_print_batches_replay_snapshot_complete", "(template_file_id_snapshot IS NULL AND template_asset_sha256 IS NULL AND variable_schema_json_snapshot IS NULL AND barcode_type_snapshot IS NULL AND renderer_contract_version IS NULL) OR (template_file_id_snapshot IS NOT NULL AND trim(template_file_id_snapshot) <> '' AND template_asset_sha256 IS NOT NULL AND trim(template_asset_sha256) <> '' AND variable_schema_json_snapshot IS NOT NULL AND trim(variable_schema_json_snapshot) <> '' AND barcode_type_snapshot IS NOT NULL AND trim(barcode_type_snapshot) <> '' AND renderer_contract_version IS NOT NULL AND trim(renderer_contract_version) <> '')");
                        });
                });
