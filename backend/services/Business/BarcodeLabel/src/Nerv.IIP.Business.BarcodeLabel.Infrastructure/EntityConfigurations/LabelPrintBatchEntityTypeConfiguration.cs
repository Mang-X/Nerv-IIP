...
public sealed class LabelPrintBatchEntityTypeConfiguration : IEntityTypeConfiguration<LabelPrintBatch>
{
    ...
    public void Configure(EntityTypeBuilder<LabelPrintBatch> builder)
    {
        ...
        builder.Property(x => x.ReportIntentFingerprint).HasColumnName("report_intent_fingerprint").HasColumnType("text").HasComment("Opaque canonical MES report-intent fingerprint supplied by the authorized caller; null only for rows created before fingerprint persistence.");
        ...
        builder.Property(x => x.ProductionReportId).HasColumnName("production_report_id").HasMaxLength(150).HasComment("MES production report id that activated this reserved print batch.");
        builder.Property(x => x.ProductionReportNo).HasColumnName("production_report_no").HasMaxLength(150).HasComment("MES production report number that activated this reserved print batch.");
