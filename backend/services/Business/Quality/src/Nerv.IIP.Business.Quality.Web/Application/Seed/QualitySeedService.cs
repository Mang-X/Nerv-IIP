...
{
    public const string LeaderDemoPlanCode = "IP-DEMO-OP-001";
    ...
    public async Task SeedAsync(string organizationId, string environmentId, CancellationToken cancellationToken = default)
    {
        ...
        var plan = await dbContext.InspectionPlans
            ...
            .SingleOrDefaultAsync(x => x.OrganizationId == organizationId && x.EnvironmentId == environmentId && x.PlanCode == LeaderDemoPlanCode, cancellationToken);
        if (plan is null)
        {
            plan = InspectionPlan.Create(
                organizationId, environmentId, LeaderDemoPlanCode, "operation", "SKU-DEMO-001", null, "WC-CNC-DEMO", null, null);
        ...
        else
        {
            ...
            {
                throw new InvalidOperationException($"Reserved leader-demo quality plan '{LeaderDemoPlanCode}' exists with incompatible tenant facts; the seed will not overwrite it.");
            }
        }
