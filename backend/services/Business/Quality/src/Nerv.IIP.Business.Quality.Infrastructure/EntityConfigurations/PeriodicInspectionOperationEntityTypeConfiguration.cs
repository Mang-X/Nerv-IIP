    ...
    : IEntityTypeConfiguration<PeriodicInspectionOperation>
{
    public void Configure(EntityTypeBuilder<PeriodicInspectionOperation> builder)
    {
        ...
        builder.Property(x => x.SkuCode).HasColumnName("sku_code").HasMaxLength(100).HasComment("SKU snapshot, composite by source: the work-order release event SKU for directly delivered facts; for legacy work orders backfilled by the release-projection backfill it carries the reconstructed SKU, or - when the operation already had authoritative completion facts that disagreed - completion_sku_code. Null until release facts arrive.");
    ...
    : IEntityTypeConfiguration<PeriodicInspectionRuntimeContext>
{
    public void Configure(EntityTypeBuilder<PeriodicInspectionRuntimeContext> builder)
    {
        ...
        builder.Property(x => x.SkuCode).HasColumnName("sku_code").IsRequired().HasMaxLength(100).HasComment("SKU snapshot frozen from the release facts; carries the same composite meaning as periodic_inspection_operations.sku_code - release event SKU for directly delivered facts, completion_sku_code when a backfilled reconstruction yielded to authoritative completion facts.");
