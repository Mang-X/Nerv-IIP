    ...
    : IIntegrationEventHandler<MeasuringDeviceCalibrationDueIntegrationEvent>, ICapSubscribe
{
    ...
    private async Task HandleValidAsync(MeasuringDeviceCalibrationDueIntegrationEvent integrationEvent, CancellationToken cancellationToken)
    {
        ...
        var request = new SubmitNotificationIntentRequest(integrationEvent.SourceService, integrationEvent.EventType, integrationEvent.EventId,
            ...
            $"Measuring device {payload.DeviceCode} is {payload.CalibrationState}; calibration due at {payload.CalibrationDueAtUtc:O}.", recipients);
        await sender.Send(new SubmitNotificationIntentCommand(integrationEvent.OrganizationId, integrationEvent.EnvironmentId,
            request, NotificationSummary.Render(request.Summary, summaryBudget), timeProvider.GetUtcNow()), cancellationToken);
    }
}
