    ...
    : IQueryHandler<ListApplicationInstancesQuery, InstanceListResponse>
{
    public async Task<InstanceListResponse> Handle(ListApplicationInstancesQuery request, CancellationToken cancellationToken)
    {
        ...
        if (db is null)
        {
            return services.GetRequiredService<IAppHubStateStore>().QueryInstances(request.Query.ToDomainCriteria()).ToContract();
        }
        ...
        var rows = instances
            ...
            .ApplyInstanceListSort(query.ToDomainCriteria())
            .ToList();
    ...
    private static InstanceListItemFact ToListProjection(ApplicationInstance instance, List<AppHubApplication> applications, List<ManagedNode> nodes)
    {
        ...
        return new InstanceListItemFact(
