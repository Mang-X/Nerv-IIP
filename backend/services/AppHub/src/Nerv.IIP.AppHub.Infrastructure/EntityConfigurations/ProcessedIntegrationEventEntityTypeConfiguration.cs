...
public sealed class ProcessedIntegrationEventEntityTypeConfiguration : IEntityTypeConfiguration<ProcessedIntegrationEvent>
{
    ...
    {
        builder.ToTable("processed_integration_events", table => table.HasComment("Integration events already processed by AppHub for idempotent consumption."));
        builder.HasKey(x => x.Id);
        builder.Property(x => x.Id).UseGuidVersion7ValueGenerator().HasComment("Processed integration event identifier.");
        builder.Property(x => x.ConsumerName).IsRequired().HasMaxLength(256).HasComment("AppHub integration event consumer name.");
        ...
        builder.Property(x => x.DedupeKey).IsRequired().HasMaxLength(512).HasComment("AppHub dedupe key associated with the processed event.");
        builder.Property(x => x.ProcessedAtUtc).HasComment("UTC time when AppHub processed the event.");
