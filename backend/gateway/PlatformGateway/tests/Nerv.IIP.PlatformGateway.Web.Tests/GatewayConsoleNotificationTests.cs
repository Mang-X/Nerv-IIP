...
public sealed class GatewayConsoleNotificationTests
{
    ...
    public async Task List_messages_forwards_route_query_headers_and_bearer()
    {
        ...
        Assert.Equal("/api/notifications/v1/messages?recipientRef=user%3Auser-admin&status=unread", notification.LastRequest!.RequestUri);
    ...
    public async Task List_tasks_forwards_query_to_notification_service()
    {
        ...
        Assert.Single(body.Items);
        Assert.Equal("/api/notifications/v1/tasks?recipientRef=user%3Auser-admin&status=open", notification.LastRequest!.RequestUri);
    }
    ...
    public async Task Mark_single_read_forwards_message_id()
    {
        ...
        Assert.Equal("/api/notifications/v1/messages/msg-001/read?recipientRef=user%3Auser-admin", notification.LastRequest!.RequestUri);
    }
