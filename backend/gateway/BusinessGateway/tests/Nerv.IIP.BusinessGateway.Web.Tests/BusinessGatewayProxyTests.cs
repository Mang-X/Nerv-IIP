...
public sealed class BusinessGatewayProxyTests
{
    ...
    public async Task Inventory_expiry_alerts_forwards_query_context_with_internal_service_token()
    {
        ...
        Assert.Equal(HttpStatusCode.OK, response.StatusCode);
        Assert.Equal("internal-test-token", inventory.LastInternalToken);
        Assert.Equal(1, inventory.ExpiryAlertsCallCount);
        Assert.Equal("S1", inventory.LastExpiryAlertsRequest!.SiteCode);
        Assert.Equal("SKU-001", inventory.LastExpiryAlertsRequest.SkuCode);
    ...
    public async Task Quality_inspection_tasks_list_and_create_record_from_task_forward_context_and_internal_service_token()
    {
        ...
        Assert.Equal("user-admin", quality.LastCreateInspectionRecordFromTaskRequest.InspectorUserId);
        ...
        Assert.Equal("DIM-1", resultLine.CharacteristicCode);
        Assert.Equal("10.2", resultLine.ObservedValue);
        Assert.Equal("pass", resultLine.Result);

        using var listDocument = JsonDocument.Parse(await list.Content.ReadAsStringAsync());
        var firstItem = listDocument.RootElement.GetProperty("data").GetProperty("items")[0];
        Assert.Equal("inspection-task-001", firstItem.GetProperty("inspectionTaskId").GetString());
    ...
    [Fact]
    public async Task Quality_inspection_task_list_resolves_self_scope_and_forwards_server_filters()
    {
        var auth = FakeBusinessGatewayAuthorizationClient.Allowed(
        ...
        Assert.Equal("self", quality.LastInspectionTaskListRequest!.ScopeKind);
        Assert.Equal("user-admin", quality.LastInspectionTaskListRequest.PrincipalId);
        Assert.Empty(quality.LastInspectionTaskListRequest.AuthorizedTeamIds!);
        Assert.Equal("production", quality.LastInspectionTaskListRequest.SourceType);
        Assert.Equal("mes", quality.LastInspectionTaskListRequest.SourceService);
        Assert.Equal("WO-100", quality.LastInspectionTaskListRequest.Keyword);
        Assert.True(quality.LastInspectionTaskListRequest.Overdue);
        Assert.Equal(5, quality.LastInspectionTaskListRequest.Skip);
        Assert.Equal(25, quality.LastInspectionTaskListRequest.Take);
    }

    [Fact]
    public async Task Quality_inspection_task_claim_uses_authenticated_principal_and_authorized_team_scope()
    {
        var auth = FakeBusinessGatewayAuthorizationClient.Allowed(
            scopeGrants:
            [
        ...
        Assert.Equal(HttpStatusCode.OK, response.StatusCode);
        Assert.Equal("task-001", quality.LastClaimInspectionTaskId);
        Assert.Equal("user-admin", quality.LastClaimInspectionTaskRequest!.ActorPrincipalId);
        Assert.Equal(["TEAM-QA"], quality.LastClaimInspectionTaskRequest.AuthorizedTeamIds);
        Assert.Equal("org-001", quality.LastClaimInspectionTaskRequest.OrganizationId);
        Assert.Equal("env-dev", quality.LastClaimInspectionTaskRequest.EnvironmentId);
    }
    ...
    public async Task Mes_actual_hours_facades_forward_distinct_upstream_values_without_nested_carriers()
    {
        ...
        {
            "/api/business/v1/mes/operation-tasks" => JsonResponse(HttpStatusCode.OK, new
            {
                items = new[]
                {
                    new
            ...
            "/api/business/v1/mes/production-reports" => JsonResponse(HttpStatusCode.OK, new
            {
                items = new[]
                {
                    new
                    {
                        productionReportId = "report-actual",
                        reportNo = "PRPT-ACTUAL",
                        workOrderId = "WO-ACTUAL",
                        operationTaskId = "OP-ACTUAL",
                        goodQuantity = 1m,
                        scrapQuantity = 0m,
            ...
            _ => throw new InvalidOperationException($"Unexpected MES request: {request.RequestUri.PathAndQuery}"),
        });
        var auth = FakeBusinessGatewayAuthorizationClient.Allowed(
            scopeGrants:
            [
                new AuthorizationScopeGrant(
                    ...
                    "organization",
                    "org-001",
                    [BusinessGatewayPermissions.MesOperationsRead, BusinessGatewayPermissions.MesReportingRead],
                    OrganizationWide: true),
            ]);
        await using var lease = LeaseHost(auth, services =>
        {
            services.AddSingleton<IHttpMessageHandlerBuilderFilter>(
                new NamedPrimaryHandlerFilter("IBusinessMesClient", handler));
            services.RemoveAll<IBusinessMasterDataClient>();
            services.AddSingleton<IBusinessMasterDataClient>(new RecordingMasterDataClient
            {
                PrincipalWorkContext = PrincipalWorkContext(
        ...
        var operationsResponse = await client.GetAsync(
            "/api/business-console/v1/mes/operation-tasks?organizationId=org-001&environmentId=env-dev");
        var reportsResponse = await client.GetAsync(
    ...
    [Fact]
    public async Task Mes_production_report_uses_authoritative_sku_policy_and_barcode_serials()
    {
        var auth = FakeBusinessGatewayAuthorizationClient.Allowed(
            scopeGrants:
            [
        ...
        var mes = new RecordingMesClient
        {
            ...
            [
                new BusinessConsoleMesOperationTaskRow(
                    "OP-SELF", "WO-SELF", "InProgress", 10, "WC-A", null, null,
                    "user-admin", "当前操作员", null, null, "Ready"),
            ],
        };
        var masterData = new RecordingMasterDataClient
        {
            PrincipalWorkContext = PrincipalWorkContext(
                new BusinessMasterDataWorkContextCandidateScope(
                    "team", "TEAM-MC-A", "机加早班组", "active-membership", [])),
            ResourceDetailResponse = new BusinessConsoleMasterDataResourceDetail(
                "sku", "SKU-001", "Demo SKU", true, "v1", "org-001", "env-dev",
                SerialTrackingPolicy: "on-production",
                DefaultBarcodeRuleCode: "FG"),
        };
        var barcode = new RecordingBarcodeLabelClient
        {
            PrintBatchResponse = new BusinessConsoleBarcodePrintBatchResponse(
                new BusinessConsoleBarcodePrintBatchDetail(
                    ...
                    [
                        ...
                        new BusinessConsoleBarcodePrintItemDetail(1, "label-1", null, "reserved", null, "SN-001", null, null, null),
                    ])),
        };
        await using var lease = LeaseHost(auth, services =>
        {
        ...
        var response = await client.PostAsJsonAsync(
            ...
            new
            {
                ...
                serialTrackingPolicy = "none",
                serialNumbers = new[] { "CLIENT-SERIAL" },
            });

        Assert.Equal(HttpStatusCode.OK, response.StatusCode);
        Assert.NotNull(barcode.LastPrintBatchRequest);
        Assert.Equal(2, barcode.LastPrintBatchRequest.RequestedQuantity);
        Assert.Equal("rule-001", barcode.LastPrintBatchRequest.BarcodeRuleId);
        Assert.Equal("template-001", barcode.LastPrintBatchRequest.LabelTemplateId);
        Assert.Equal("report-serial-001", barcode.LastPrintBatchRequest.IdempotencyKey);
        Assert.NotNull(barcode.LastActivateRequest);
        Assert.Equal("report-001", barcode.LastActivateRequest.Body.ProductionReportId);
        Assert.Equal("PR-001", barcode.LastActivateRequest.Body.ProductionReportNo);
        Assert.NotNull(mes.LastRecordProductionReportRequest);
        Assert.Equal("on-production", mes.LastRecordProductionReportRequest.SerialTrackingPolicy);
        Assert.Null(mes.LastRecordProductionReportRequest.SerialNo);
        Assert.Equal(["SN-001", "SN-002"], mes.LastRecordProductionReportRequest.SerialNumbers);
    }

    [Theory]
    [InlineData("none")]
    [InlineData("on-receipt")]
    [InlineData("on-shipment")]
    public async Task Mes_production_report_for_non_production_policies_does_not_reserve_labels(string policy)
    {
        ...
        var response = await client.PostAsJsonAsync(
            "/api/business-console/v1/mes/production-reports",
            ProductionReportBody(serialTrackingPolicy: "on-production", serialNumbers: ["CLIENT-SERIAL"]));

        Assert.Equal(HttpStatusCode.OK, response.StatusCode);
        Assert.Equal(0, barcode.CreatePrintBatchCallCount);
        Assert.NotNull(mes.LastRecordProductionReportRequest);
        Assert.Equal(policy, mes.LastRecordProductionReportRequest.SerialTrackingPolicy);
        Assert.Null(mes.LastRecordProductionReportRequest.SerialNo);
    ...
    public async Task Mes_production_report_rejects_non_authoritative_serial_policy_values(string policy)
    {
        var auth = AllowedOrganizationScope(BusinessGatewayPermissions.MesReportingWrite);
        var mes = new RecordingMesClient();
        var masterData = new RecordingMasterDataClient
        ...
        var response = await client.PostAsJsonAsync(
            "/api/business-console/v1/mes/production-reports",
            ProductionReportBody());

        Assert.Equal(HttpStatusCode.BadRequest, response.StatusCode);
        using var document = JsonDocument.Parse(await response.Content.ReadAsStringAsync());
        Assert.Equal("production-serial-policy-invalid", document.RootElement.GetProperty("message").GetString());
        Assert.Equal(0, mes.RecordProductionReportCallCount);
    }

    [Fact]
    public async Task Mes_rejection_leaves_reserved_print_batch_unactivated()
    {
        var auth = AllowedOrganizationScope(BusinessGatewayPermissions.MesReportingWrite);
        var mes = new RecordingMesClient
        {
            RecordProductionReportFailure = BusinessServiceProxyException.FromSafeDownstreamMessage(
    ...
    [Theory]
    [InlineData("good-quantity")]
    [InlineData("operation-task")]
    [InlineData("scrap-quantity")]
    [InlineData("rework-quantity")]
    [InlineData("completes-operation")]
    [InlineData("reported-at")]
    [InlineData("material-id")]
    [InlineData("material-lot-id")]
    [InlineData("consumed-quantity")]
    [InlineData("material-issue-request")]
    [InlineData("scrap-reason")]
    [InlineData("defect-record")]
    [InlineData("produced-lot")]
    [InlineData("label-template")]
    public async Task Each_complete_report_intent_field_change_conflicts_before_mes_replay(string changedField)
    {
        var auth = AllowedOrganizationScope(BusinessGatewayPermissions.MesReportingWrite);
        var mes = new RecordingMesClient();
        mes.RecordProductionReportFailuresBeforeCommit.Enqueue(
        ...
        var client = lease.CreateClient();
        BusinessGatewayTestHost.Authenticated(client);
        var request = ProductionReportBody(labelTemplateId: "template-001") with
        {
            ConsumedMaterialLots =
            [
                new BusinessConsoleConsumedMaterialLotInput("MAT-B", "LOT-B", 2m, "ISSUE-B"),
                new BusinessConsoleConsumedMaterialLotInput("MAT-A", "LOT-A", 1m, "ISSUE-A"),
            ],
        };
        ...
        var changedRequest = changedField switch
        {
            ...
            "completes-operation" => request with { CompletesOperation = true },
            "reported-at" => request with { ReportedAtUtc = request.ReportedAtUtc.AddMinutes(1) },
            "material-id" => request with
            {
                ConsumedMaterialLots =
                [
                    new BusinessConsoleConsumedMaterialLotInput("MAT-C", "LOT-B", 2m, "ISSUE-B"),
                    new BusinessConsoleConsumedMaterialLotInput("MAT-A", "LOT-A", 1m, "ISSUE-A"),
                ],
            },
            "material-lot-id" => request with
            {
                ConsumedMaterialLots =
                [
                    new BusinessConsoleConsumedMaterialLotInput("MAT-B", "LOT-C", 2m, "ISSUE-B"),
                    new BusinessConsoleConsumedMaterialLotInput("MAT-A", "LOT-A", 1m, "ISSUE-A"),
                ],
            },
            "consumed-quantity" => request with
            {
                ConsumedMaterialLots =
                [
                    new BusinessConsoleConsumedMaterialLotInput("MAT-B", "LOT-B", 3m, "ISSUE-B"),
                    new BusinessConsoleConsumedMaterialLotInput("MAT-A", "LOT-A", 1m, "ISSUE-A"),
                ],
            },
            "material-issue-request" => request with
            {
                ConsumedMaterialLots =
                [
                    new BusinessConsoleConsumedMaterialLotInput("MAT-B", "LOT-B", 2m, "ISSUE-C"),
                    new BusinessConsoleConsumedMaterialLotInput("MAT-A", "LOT-A", 1m, "ISSUE-A"),
                ],
            },
            "scrap-reason" => request with { ScrapReasonCode = "SCRAP-001" },
            "defect-record" => request with { DefectRecordNo = "DEFECT-001" },
            "produced-lot" => request with { ProducedLotNo = "LOT-FG-002" },
            "label-template" => request with { LabelTemplateId = "template-002" },
    ...
    [Fact]
    public async Task Consumed_material_lot_order_does_not_change_the_complete_report_intent()
    {
        var auth = AllowedOrganizationScope(BusinessGatewayPermissions.MesReportingWrite);
        var mes = new RecordingMesClient();
        mes.RecordProductionReportFailuresBeforeCommit.Enqueue(
        ...
        var client = lease.CreateClient();
        BusinessGatewayTestHost.Authenticated(client);
        var firstLot = new BusinessConsoleConsumedMaterialLotInput("MAT-A", "LOT-A", 1m, "ISSUE-A");
        var secondLot = new BusinessConsoleConsumedMaterialLotInput("MAT-B", "LOT-B", 2m, "ISSUE-B");
        var request = ProductionReportBody(labelTemplateId: "template-001") with
        {
            ConsumedMaterialLots = [secondLot, firstLot],
        };
        ...
        var replay = await client.PostAsJsonAsync(
            "/api/business-console/v1/mes/production-reports",
            request with { ConsumedMaterialLots = [firstLot, secondLot] });
    ...
    [Fact]
    public async Task Clearing_the_template_after_mes_rejection_conflicts_with_the_reserved_report_intent()
    {
        var auth = AllowedOrganizationScope(BusinessGatewayPermissions.MesReportingWrite);
        var mes = new RecordingMesClient();
        mes.RecordProductionReportFailuresBeforeCommit.Enqueue(
        ...
        var replay = await client.PostAsJsonAsync(
            "/api/business-console/v1/mes/production-reports",
            request with { LabelTemplateId = null });
    ...
    [Fact]
    public async Task Activation_response_unknown_is_recovered_by_reading_the_same_batch()
    {
        var auth = AllowedOrganizationScope(BusinessGatewayPermissions.MesReportingWrite);
        var mes = new RecordingMesClient();
        var masterData = new RecordingMasterDataClient
        ...
        barcode.PrintBatchResponses.Enqueue(ProductionPrintBatch("reserved"));
        barcode.PrintBatchResponses.Enqueue(ProductionPrintBatch(
            "ready-to-print", "report-001", "PR-001"));
        await using var lease = LeaseHost(auth, services =>
        {
            services.RemoveAll<IBusinessMesClient>();
            services.AddSingleton<IBusinessMesClient>(mes);
        ...
        var data = document.RootElement.GetProperty("data");
        Assert.Equal("print-batch-001", data.GetProperty("printBatchId").GetString());
        Assert.Equal("ready-to-print", data.GetProperty("printStatus").GetString());
        Assert.False(data.GetProperty("printingPreparationPending").GetBoolean());
        Assert.Equal(1, barcode.ActivatePrintBatchCallCount);
        Assert.Equal(2, barcode.GetPrintBatchCallCount);
    }

    [Fact]
    public async Task Activation_not_observed_returns_the_successful_report_with_printing_pending()
    {
        var auth = AllowedOrganizationScope(BusinessGatewayPermissions.MesReportingWrite);
        var mes = new RecordingMesClient();
        var masterData = new RecordingMasterDataClient
        ...
        using var document = JsonDocument.Parse(body);
        var data = document.RootElement.GetProperty("data");
        Assert.Equal("report-001", data.GetProperty("productionReportId").GetString());
        Assert.Equal("reserved", data.GetProperty("printStatus").GetString());
        Assert.True(data.GetProperty("printingPreparationPending").GetBoolean());
    }
    ...
    public async Task Same_report_intent_recovers_the_committed_mes_report_after_master_data_changes()
    {
        ...
        var mes = new RecordingMesClient
        {
            CommittedProductionReportBeforeResponseFailure = new BusinessConsoleRecordProductionReportResponse(
                "report-committed-001",
                "PR-COMMITTED-001",
                ["SN-001"]),
            ProductionReportResponseFailureAfterCommit = BusinessServiceProxyException.FromSafeDownstreamMessage(
                HttpStatusCode.BadGateway,
                "downstream-request-failed"),
        };
        var masterData = new RecordingMasterDataClient
        ...
        masterData.ResourceDetailResponse = new BusinessConsoleMasterDataResourceDetail(
            "sku", "SKU-001", "Demo SKU", false, "v2", "org-001", "env-dev",
            SerialTrackingPolicy: "none",
            DefaultBarcodeRuleCode: "FG-CHANGED");
        barcode.PrintBatchListResponse = new BusinessConsoleBarcodePrintBatchListResponse(
        ...
        Assert.Equal("report-committed-001", replayData.GetProperty("productionReportId").GetString());
        Assert.Equal("PR-COMMITTED-001", replayData.GetProperty("reportNo").GetString());
        Assert.Equal("print-batch-001", replayData.GetProperty("printBatchId").GetString());
        Assert.Equal("[\"SN-001\"]", replayData.GetProperty("serialNumbers").GetRawText());
        Assert.Equal(1, barcode.CreatePrintBatchCallCount);
        Assert.Equal("report-serial-001", Assert.Single(barcode.PrintBatchRequests).IdempotencyKey);
        Assert.Single(masterData.DetailRequests);
        Assert.Equal(2, mes.RecordProductionReportCallCount);
    ...
    [Fact]
    public async Task On_production_report_requires_a_template_before_reserving_serials()
    {
        var auth = AllowedOrganizationScope(BusinessGatewayPermissions.MesReportingWrite);
        var mes = new RecordingMesClient();
        var masterData = new RecordingMasterDataClient
    ...
    private static BusinessConsoleBarcodePrintBatchResponse ProductionPrintBatch(
        string status,
        string? productionReportId = null,
    ...
    [Fact]
    public async Task Mes_production_report_rejects_a_work_order_that_does_not_own_the_authorized_operation_task()
    {
        var auth = AllowedOrganizationScope(BusinessGatewayPermissions.MesReportingWrite);
        var mes = new RecordingMesClient
        {
            ...
            [
                new BusinessConsoleMesOperationTaskRow(
                    "OP-A", "WO-A", "InProgress", 10, "WC-A", null, null,
                    "user-admin", "当前操作员", null, null, "Ready"),
            ],
        };
        var masterData = new RecordingMasterDataClient
        ...
        var response = await client.PostAsJsonAsync(
            "/api/business-console/v1/mes/production-reports",
            ProductionReportBody(labelTemplateId: "template-001") with
            {
                WorkOrderId = "WO-B",
    ...
    public async Task Mes_work_order_hold_forwards_context_and_internal_service_token()
    {
        ...
        var response = await client.PostAsJsonAsync(
            ...
            new { reason = "line-blocked", changedAtUtc = (DateTimeOffset?)null });

        Assert.Equal(HttpStatusCode.OK, response.StatusCode);
        Assert.Equal("internal-test-token", mes.LastInternalToken);
        Assert.Equal(1, mes.HoldWorkOrderCallCount);
        Assert.NotNull(mes.LastHoldWorkOrderRequest);
        Assert.Equal("WO-001", mes.LastHoldWorkOrderRequest!.WorkOrderId);
        Assert.Equal("org-001", mes.LastHoldWorkOrderRequest.OrganizationId);
        Assert.Equal("env-dev", mes.LastHoldWorkOrderRequest.EnvironmentId);
        Assert.Equal("line-blocked", mes.LastHoldWorkOrderRequest.Reason);
        using var document = JsonDocument.Parse(await response.Content.ReadAsStringAsync());
    ...
    public async Task Mes_work_order_cancel_forwards_context_and_internal_service_token()
    {
        ...
        var response = await client.PostAsJsonAsync(
            ...
            new { reason = "order-scrapped", changedAtUtc = (DateTimeOffset?)null });

        Assert.Equal(HttpStatusCode.OK, response.StatusCode);
        Assert.Equal("internal-test-token", mes.LastInternalToken);
        Assert.Equal(1, mes.CancelWorkOrderCallCount);
        Assert.NotNull(mes.LastCancelWorkOrderRequest);
        Assert.Equal("WO-002", mes.LastCancelWorkOrderRequest!.WorkOrderId);
    ...
    public async Task Mes_quality_hold_force_release_injects_principal_actor_and_uses_internal_service_token()
    {
        ...
        Assert.Equal("QH-001", mes.LastForceReleaseQualityHoldRequest!.SourceDocumentId);
    ...
    public async Task Mes_production_report_reverse_injects_principal_actor_and_ignores_spoofed_actor_fields()
    {
        ...
        Assert.Equal(HttpStatusCode.OK, response.StatusCode);
        Assert.Equal("internal-test-token", mes.LastInternalToken);
        Assert.Equal(1, mes.ReverseProductionReportCallCount);
        Assert.NotNull(mes.LastReverseProductionReportRequest);
        Assert.Equal("PR-001", mes.LastReverseProductionReportRequest!.ReportNo);
        Assert.Equal("org-001", mes.LastReverseProductionReportRequest.OrganizationId);
    ...
    public async Task Mes_finished_goods_receipt_inventory_posting_retry_forwards_context_and_maps_response()
    {
        ...
        var response = await client.PostAsJsonAsync(
            ...
            new { idempotencyKey = "retry-001" });

        Assert.Equal(HttpStatusCode.OK, response.StatusCode);
        Assert.Equal("internal-test-token", mes.LastInternalToken);
        Assert.Equal(1, mes.RetryFinishedGoodsReceiptInventoryPostingCallCount);
        Assert.NotNull(mes.LastRetryFinishedGoodsReceiptInventoryPostingRequest);
        Assert.Equal("FGR-001", mes.LastRetryFinishedGoodsReceiptInventoryPostingRequest!.RequestNo);
        Assert.Equal("org-001", mes.LastRetryFinishedGoodsReceiptInventoryPostingRequest.OrganizationId);
        Assert.Equal("env-dev", mes.LastRetryFinishedGoodsReceiptInventoryPostingRequest.EnvironmentId);
        Assert.Equal("retry-001", mes.LastRetryFinishedGoodsReceiptInventoryPostingRequest.IdempotencyKey);
        using var document = JsonDocument.Parse(await response.Content.ReadAsStringAsync());
        var data = document.RootElement.GetProperty("data");
        Assert.Equal("FG-REQ-001", data.GetProperty("finishedGoodsReceiptRequestId").GetString());
    ...
    public async Task Device_control_command_facade_injects_principal_requester_and_uses_internal_service_token()
    {
        ...
        var response = await client.PostAsJsonAsync("/api/business-console/v1/telemetry/device-control-commands", new
        {
            ...
            value = "120",
            reason = "ramp spindle to setpoint",
            idempotencyKey = "idem-device-control-001",
            correlationId = "corr-device-control-001",
            // A caller-supplied requester must never be trusted; the gateway overrides it with the principal.
            requestedBy = "user:attacker-should-be-ignored",
        });

        Assert.Equal(HttpStatusCode.OK, response.StatusCode);
        // Facade forwards the internal service token, never the caller bearer.
        Assert.Equal("internal-test-token", industrialTelemetry.LastInternalToken);
        // The requester is bound to the authenticated principal; the caller-supplied value is ignored.
        Assert.Equal("user-admin", industrialTelemetry.LastDeviceControlRequestedBy);
        ...
        Assert.Equal("120", forwarded.Value);
        Assert.Equal("ramp spindle to setpoint", forwarded.Reason);
        Assert.Equal("idem-device-control-001", forwarded.IdempotencyKey);
        Assert.Equal("corr-device-control-001", forwarded.CorrelationId);

        using var document = JsonDocument.Parse(await response.Content.ReadAsStringAsync());
        var data = document.RootElement.GetProperty("data");
        Assert.Equal("op-task-001", data.GetProperty("operationTaskId").GetString());
        Assert.Equal("pending-approval", data.GetProperty("status").GetString());
        Assert.Equal("user-admin", data.GetProperty("approval").GetProperty("requestedBy").GetString());
    }
    ...
    [Fact]
    public async Task Inventory_expiry_alerts_http_client_builds_downstream_expiry_alerts_query()
    {
        var handler = new RecordingHandler(_ => JsonResponse(HttpStatusCode.OK, new
        {
            data = new
...
#pragma warning restore CS0618
    ...
    public async Task Quality_http_client_maps_inspection_task_list_to_console_items()
    {
        var handler = new RecordingHandler(_ => JsonResponse(HttpStatusCode.OK, new
        {
            data = new
            {
                ...
                items = new[]
                {
                    ...
                    {
                        inspectionTaskId = "inspection-task-001",
                        inspectionPlanId = "inspection-plan-001",
                        sourceType = "receiving",
                        sourceService = "wms",
                        sourceDocumentId = "GRN-001",
                        sourceDocumentLineId = "GRN-001-1",
                        skuCode = "SKU-RM-1000",
                        quantity = 12,
                        uomCode = "EA",
                        batchNo = "LOT-001",
                        serialNo = (string?)null,
                        status = "pending",
                        dueAtUtc = "2026-07-09T08:00:00Z",
                        createdAtUtc = "2026-07-07T08:00:00Z",
        ...
        Assert.Equal(1, response.Total);
        var item = Assert.Single(response.Items);
        Assert.Equal("inspection-task-001", item.InspectionTaskId);
        Assert.Equal("inspection-plan-001", item.InspectionPlanId);
        Assert.Equal("receiving", item.SourceType);
        Assert.Equal("wms", item.SourceService);
        Assert.Equal("GRN-001", item.SourceDocumentId);
        Assert.Equal("GRN-001-1", item.SourceDocumentLineId);
        Assert.Equal("SKU-RM-1000", item.SkuCode);
        Assert.Equal(12m, item.Quantity);
        Assert.Equal("EA", item.UomCode);
        Assert.Equal("LOT-001", item.BatchNo);
        Assert.Equal("pending", item.Status);
        Assert.Equal(DateTimeOffset.Parse("2026-07-09T08:00:00Z", CultureInfo.InvariantCulture), item.DueAtUtc);
        Assert.Null(item.InspectionRecordId);
        var request = handler.Requests.Single();
        Assert.Equal(HttpMethod.Get, request.Method);
        Assert.Equal("/api/business/v1/quality/inspection-tasks?organizationId=org-001&environmentId=env-dev&status=pending&skuCode=SKU-RM-1000&skip=1&take=50&inspectionTaskId=0199aa00-0000-7000-8000-000000000004&scopeKind=organization&principalId=user-admin", request.RequestUri!.PathAndQuery);
        Assert.Equal("internal-token-001", request.Headers.Authorization!.Parameter);
    }

    [Fact]
    public async Task Quality_http_client_maps_create_inspection_record_from_task_to_downstream_request_shape()
    {
        string? requestBody = null;
        var handler = new RecordingHandler(request =>
        {
        ...
        using var document = JsonDocument.Parse(requestBody);
        var root = document.RootElement;
        Assert.Equal("inspection-task-001", root.GetProperty("inspectionTaskId").GetString());
        Assert.Equal("inspector-007", root.GetProperty("inspectorUserId").GetString());
        Assert.Equal("within tolerance", root.GetProperty("dispositionReason").GetString());
        Assert.Equal("file-cert-001", root.GetProperty("dispositionAttachmentFileIds")[0].GetString());
        var line = root.GetProperty("resultLines")[0];
        Assert.Equal("DIM-1", line.GetProperty("characteristicCode").GetString());
        Assert.Equal("10.2", line.GetProperty("observedValue").GetString());
        Assert.Equal("mm", line.GetProperty("unitCode").GetString());
...
internal sealed class RecordingInventoryClient : IBusinessInventoryClient
{
    ...
    public int ExpiryAlertsCallCount { get; private set; }

    public BusinessConsoleInventoryExpiryAlertsRequest? LastExpiryAlertsRequest { get; private set; }
    ...
    public Task<BusinessConsoleInventoryExpiryAlertsResponse> ListExpiryAlertsAsync(
        ...
        CancellationToken cancellationToken)
    {
        ExpiryAlertsCallCount++;
        LastInternalToken = internalBearerToken;
        LastExpiryAlertsRequest = request;
        return Task.FromResult(new BusinessConsoleInventoryExpiryAlertsResponse(
        [
            new BusinessConsoleInventoryExpiryAlertLineResponse(
                request.SkuCode ?? "SKU-EXP",
                "EA",
                request.SiteCode,
                request.LocationCode ?? "L1",
                "LOT-1",
                null,
                "available",
                "owned",
...
internal sealed class RecordingQualityClient : IBusinessQualityClient, IBusinessQualityScrapReasonCodeClient
{
    ...
    public int InspectionTaskListCallCount { get; private set; }

    public BusinessQualityInspectionTaskListRequest? LastInspectionTaskListRequest { get; private set; }

    public string? LastGetInspectionTaskId { get; private set; }

    public BusinessQualityInspectionTaskDetailRequest? LastGetInspectionTaskRequest { get; private set; }

    public string? LastAssignInspectionTaskId { get; private set; }

    public BusinessQualityAssignInspectionTaskRequest? LastAssignInspectionTaskRequest { get; private set; }

    public string? LastClaimInspectionTaskId { get; private set; }

    public BusinessQualityClaimInspectionTaskRequest? LastClaimInspectionTaskRequest { get; private set; }
    ...
    public Task<BusinessConsoleQualityInspectionTaskListResponse> ListInspectionTasksAsync(
        string internalBearerToken,
        BusinessQualityInspectionTaskListRequest request,
        CancellationToken cancellationToken)
    {
        InspectionTaskListCallCount++;
        LastInternalToken = internalBearerToken;
        LastInspectionTaskListRequest = request;
        return Task.FromResult(new BusinessConsoleQualityInspectionTaskListResponse(
            [
                new BusinessConsoleQualityInspectionTaskItem(
                    ...
                    null,
                    "pending",
                    DateTimeOffset.Parse("2026-07-09T08:00:00Z"),
                    DateTimeOffset.Parse("2026-07-07T08:00:00Z"),
                    null),
            ],
            1));
    ...
    public Task<BusinessConsoleQualityInspectionTaskDetailResponse> GetInspectionTaskAsync(
        string internalBearerToken,
    ...
    {
        LastInternalToken = internalBearerToken;
        LastAssignInspectionTaskId = inspectionTaskId;
        LastAssignInspectionTaskRequest = request;
        return Task.FromResult(new BusinessConsoleQualityInspectionTaskAssignmentResponse(
            inspectionTaskId,
            "pending",
    ...
    {
        LastInternalToken = internalBearerToken;
        LastClaimInspectionTaskId = inspectionTaskId;
        LastClaimInspectionTaskRequest = request;
        return Task.FromResult(new BusinessConsoleQualityInspectionTaskAssignmentResponse(
            inspectionTaskId,
            "in-progress",
            request.ActorPrincipalId,
            request.AuthorizedTeamIds.FirstOrDefault(),
            request.ExpectedVersion + 1,
            DateTimeOffset.Parse("2026-07-30T08:00:00Z")));
    }
...
internal sealed class RecordingBarcodeLabelClient : IBusinessBarcodeLabelClient, IBusinessBarcodeResolverClient
{
    ...
    public List<BusinessConsoleCreateBarcodePrintBatchRequest> PrintBatchRequests { get; } = [];

    public BusinessConsoleBarcodePrintBatchResponse? PrintBatchResponse { get; init; }

    public Queue<BusinessConsoleBarcodePrintBatchResponse> PrintBatchResponses { get; } = new();
    ...
    public int CreatePrintBatchCallCount { get; private set; }
        ...
        CancellationToken cancellationToken)
    {
        ...
        PrintBatchRequests.Add(request);
        CreatePrintBatchCallCount++;
        ...
        CancellationToken cancellationToken)
    {
        ...
        return Task.FromResult(PrintBatchListResponse ?? new BusinessConsoleBarcodePrintBatchListResponse(
        [
...
internal sealed class RecordingIndustrialTelemetryClient : IBusinessIndustrialTelemetryClient
{
    ...
    public BusinessConsoleTelemetryDeviceControlCommandRequest? LastDeviceControlRequest { get; private set; }
    ...
    {
        LastInternalToken = internalBearerToken;
        LastDeviceControlRequest = request;
        LastDeviceControlRequestedBy = requestedBy;
...
internal sealed class RecordingMesClient : IBusinessMesClient
{
    ...
    public List<BusinessConsoleRecordProductionReportRequest> RecordProductionReportRequests { get; } = [];
    ...
    public int HoldWorkOrderCallCount { get; private set; }

    public BusinessConsoleMesWorkOrderReasonRequest? LastHoldWorkOrderRequest { get; private set; }

    public int CancelWorkOrderCallCount { get; private set; }
    ...
    {
        LastInternalToken = internalBearerToken;
        HoldWorkOrderCallCount++;
        LastHoldWorkOrderRequest = request;
        return Task.FromResult(new BusinessConsoleAcceptedResponse(true));
    }
    ...
    {
        LastInternalToken = internalBearerToken;
        RetryFinishedGoodsReceiptInventoryPostingCallCount++;
        LastRetryFinishedGoodsReceiptInventoryPostingRequest = request;
        return Task.FromResult(new BusinessConsoleMesCreateReceiptResponse("FG-REQ-001", requestNo));
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (CommittedProductionReportBeforeResponseFailure is not null)
        {
            ...
            {
                CommittedProductionReportCount++;
                throw ProductionReportResponseFailureAfterCommit
                    ?? throw new InvalidOperationException("The committed-response failure must be configured.");
            }

            return Task.FromResult(CommittedProductionReportBeforeResponseFailure);
        }
        if (RecordProductionReportFailure is not null)
        {
            throw RecordProductionReportFailure;
        }
        return Task.FromResult(new BusinessConsoleRecordProductionReportResponse(
            "report-001",
            "PR-001",
