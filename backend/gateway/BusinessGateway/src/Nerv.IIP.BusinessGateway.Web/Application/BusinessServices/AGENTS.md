...
- 当前 legacy 清单覆盖 `BusinessServiceClients.cs` 与 `BusinessConsoleWmsClient.cs` 中尚未迁移的受管声明；#2163 已将 `IBusinessInventoryClient`、`HttpBusinessInventoryClient` 与 `BusinessGatewayInventoryForwardedPermissionOptions` 迁至 `Capabilities/Inventory/BusinessInventoryClient.cs`，#2164 已将 `IBusinessQualityClient` 与 `HttpBusinessQualityClient` 迁至 `Capabilities/Quality/BusinessQualityClient.cs`，这些符号不再属于 legacy 基线。其余 capability client 的物理迁移由独立子项（#2160–#2174）各自交付，不得将这些文件迁移混入 B；公开 API 形状冻结属于后续 C 子项。
